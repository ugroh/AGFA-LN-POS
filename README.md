#### AGFA-LN-POS: Neuauflage das Buches LN1184 (Stand: 2024/01/31)
...
Ist ein solches Kapitel vollständig fertig und bereinigt, so wird es in `author-final/part-a/chap-a1.tex` kopiert und final bearbeitet. Das Verzeichnis `author-final` ist analog aufgebaut, wobei aber auf eine nochmalige Verfeinerung verzichtet wurde. 
...
wobei `n` die Seitennummer des PDFs ist, das man umgewandelt hat. Dann kann man relativ einfach das neue PDF mit dem Original vergleichen und Fehler finden.
...
* Bitte momentan die Literaturzitate belassen und diese mit `[ ]` einklammern. In `author-test/bib` findet sich die Literaturdatenbank `ln-references-bib`, die noch überarbeitet werden muss.
