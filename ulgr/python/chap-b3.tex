...
The spectral properties indicated above were first proved by [Perron (1907)] and [Frobenius (1909)] for positive square matrices, \ie for positive operators on the Banach lattice $\C^{n}$.
Later these results were extended to the infinite dimensional setting; important contributions are due to Jentzsch, Karlin, Krein, Krasnoselski'i, Lotz, Rota, Rutman, Schaefer and others (see Chapt.V of [Schaefer (1974)]).
...
$C_{0}(\left[0,\infty\right[)$ both are topologically (but not isometrically) isomorphic to $C(\left[0,1\right])$ (see [Semadeni (1971), Sec.21.5]), one obtains from (i) and (ii) (non-positive) semigroups on $C(\left[0,1\right])$ whose generators have empty spectrum.
\end{enumerate}
\end{examples}
%% --
...
\end{proof}
%% --
We use Prop.1.5 to prove an analogue of the famous Krein-Rutman result.
For the sake of completeness we include the proof of this classical result, which states that the spectral radius of a positive operator $T$ on $C(K)$ (or more generally on an order unit space) is an eigenvalue of the adjoint $T'$ (see the Corollary of Thm.2.6 in the appendix of [Schaefer (1966)]).
%% --
...
\index{Spectral Theory!Boundary Spectrum}
%% --
...
$\alpha + \im\beta \in M$, $\alpha,\beta \in \R$ implies that $\alpha + ik\beta \in M$ for every $k \in \Z$.
	...
	Every subset of $\R$ is cyclic. On the other hand, if $M$ is cyclic and $M \not\subset \R$, then $M$ has to be unbounded.
...
\begin{examples}\label{ex:b3-2.6}
	\index{Examples!Boundary Spectrum}
%	\index{Boundary Spectrum!Examples}
\begin{enumerate}[(i)]
\item
...
The assumptions of Cor.2.10 do not imply that $s(A)$ is dominant, the rotation semigroup (A-III,Ex.5.6) is a counterexample.
%% --
...
\begin{example}\label{ex:b3-2.13}
	\index{Examples!Non-cyclic Spectrum}
%	\index{Spectrum!Non-cyclic Example}
...
\begin{example}\label{ex:b3-2.14}
	\index{Examples!Schrödinger Operator}
%	\index{Operator!Schrödinger}
...
\begin{remarks}\label{rem:b3-2.15}
	\index{Remarks!Resolvent Poles}
	\index{Poles!First Order}
...
%
	...
	\begin{equation}\label{eq:b3-2.31}
		...
		\end{split}
	\end{equation}
%	If $T \in L(C(K))$, $K$ compact, is positive, then it is weakly compact if and only if 
%	its biadjoint $T''$ maps the bounded Borel functions into $C(K)$ (see B-IV,Prop.2.4).
...
(d) $\implies$ (e): Given $0 < f \in E$, $\lambda > \omega(A)$, $y \in X$, there exists $t_{0} \geq 0$ such that $\{x : f(x) > 0\} \cap \supp T(t_{0})'\delta_{y} \neq \emptyset$.
...
(f) $\implies$ (a): At first we recall that a closed linear subspace which is invariant for $R(\lambda_0,A)  (\lambda_0 \in \rho(A))$, is invariant for $R(\lambda ,A)$
...
%
	...
	If $f > 0$, then $g > 0$, hence [Protter-Weinberger (1967), Chap.I, Thm.3] implies that $g$ is strictly positive.
...
\end{proof}
%% --
The group $P\sigma(A) \cap \im\R$ need not be discrete. For example, the semigroup described in Ex.2.6(d) satisfies the assumptions of Thm.3.6 if $\frac{\alpha}{\beta}$ is irrational. In this case $P\sigma(A) = \im\alpha \Z + \im\beta \Z$ is a dense subgroup of $\im \R$. Actually one can show that for every subgroup $H$ of $\im \R$ there is an irreducible semigroup on $C(G), G \coloneqq (H_d)\,{\tilde{ }}$, such that 
$P\sigma(A) = H)$. Here $(H_d)\,{\tilde{ }}$ denotes the dual of the abelian group $H$ equipped with the discrete topology. For details see [Greiner (1982), p.62]. 
...
\begin{examples}\label{ex:b3-3.10}
	...
	For example this is true if $\partial\Omega$ is $C^{\infty}$ and $a_{ij} \in C^{\infty}(\overline{\Omega})$ (cf Thm.4.8.3 of [Fattorini (1983)]).
	We will show that $P\sigma(A)\cap(s(A) + \im\R) = \{s(A)\}$.
	...
	Assuming that $\lambda > \lambda_{0}$ then $h$ is positive, even strictly positive by the maximum principle [Protter-Weinberger (1967), Chap.2, Thm.6].
...
To prove this result one uses the proof of the analogous result for a single operator (cf. [Schaefer (1968), Thm.7]) as a guideline.
%% --
...
For details see [Derndinger-Nagel (1979)]. 
...
%% -- page 36  / 198
%%
A global flow exists if $F$ is globally Lipschitz continuous or uniformly bounded. For $\{x \in \R^n : (x|F(x)) > 0\}$ bounded in $\R^n$, a global semiflow always exists (cf. [Deimling (1977), Sec.5.2]).
%% --
	\item 
...
The half-plane $\{\lambda \in \C : \Re(\lambda) > \overline{c}(h,\phi)\}$ is always a subset of the resolvent set (see Prop.4.8(a) below).
...
For more details and the proof of the following result we refer to [Arendt-Greiner (1984)].
%% --
...
Instead we refer to [Schaefer (1974)] and the notes and references given there.
...
\item
Theorem~\ref{thm:b3-1.1} was stated by [Karlin (1959)], but a complete proof is given in [Derndinger (1980)].
Proposition 1.5 is taken from [Greiner (1982)] and Theorem 1.6 is (implicitely) contained in [Derndinger-Nagel (1979)].
A generalization to (non-lattice) ordered Banach spaces can be found in Sec.2.4 of [Batty-Robinson (1984)].

\item
Lemma 2.3 dates back to Rota (see [Schaefer (1974)]).
Our approach follows [Greiner (1981)].
The notion \enquote{(imaginary) additively cyclic} was introduced by [Derndinger (1980)] (and {Schaefer (1980)} respectively).
Derndinger proves some cyclicity results for the boundary spectrum.
A result similar to Proposition 2.7 is given in Sec.7.4 of [Davies (1980)].
...
Moreover, \enquote{ergodicity} and \enquote{unique ergodicity} are closely related to irreducibility (see[ Cornfeld-Fomin-Sinai (1982)] or [Krengel (1985)]).
Irreducible semigroups are discussed to some extent in [Davies (1980)].
E.g. he proves a special case of Theorem 3.6.
Proposition 3.3 will be generalized in C-III,Prop.3.3.
Assertion (a) of Proposition 3.5 was proven by [Schaefer (1983)] while Theorem 3.6 is taken from [Greiner (1982)].
Elliptic operators (more general than Example 3.10(b)) as generators on spaces of continuous functions, were investigated by many people, \eg [Bony-Courrège-Priouret (1968)], [Kuhn (1985)], [Roth (1976) \& (1978)] and [Stewart (1974)].

\item
Theorem 4.1 is due to [Derndinger (1984)].
The spectrum of semigroups of Markov lattice homomorphisms is investigated by [Derndinger-Nagel (1979)].
In particular they prove Theorem 4.4 for Markov semigroups.
Earlier results are due to [Scarpellini (1974)].
...
For more details we refer to [Chicone-Swanson (1981)] and [Sacker-Sell (1978)].
E.g., the \enquote{annular hull theorem} is a special case of Theorem 4.11(b).
The general result 4.11 was proven by [Arendt-Greiner (1984)].
