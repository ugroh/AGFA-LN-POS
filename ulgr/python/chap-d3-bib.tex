...
The approprite hypothesis on the semigroup implying the desired results seems to be the concept of irreducibility.
...
We refer to this as the \emph{polar decomposition} of $\phi$.
 %\citet[Theorem III.4.2]{takesaki:1979}.

In addition, $|\phi|$ is uniquely determined by the following two conditions.
%(\citet[Proposition III.4.6]{takesaki:1979})%\citet[Proposition III.4.6]{takesaki:1979}:
%% -- 
\begin{align*}
	\|\phi\| &= \| |\phi| \|, \\
...
Recall that for every positive map $T \in \L{M}$ there exists a state $\phi$ on $M$ such that $T'\phi = r(T)\phi$, where $r(T)$ denotes the spectral radius of $T$. (\citet[Theorem 2.1]{groh:1981}) %\citet[Theorem 2.1]{groh:1981})
...
\]
%% -- 
Since $b(v^{*},v^{*}) = 0$ we obtain $b(x,v^{*}) = 0$ for all $x \in M$ (Lemma 1.1.i), hence $T(xv) = S(x)v$.
...
\]
%% --
(\citet[Theorem 1.5.2]{pedersen:1979}). %\citet[Theorem 1.5.2]{pedersen:1979}.
...
Take $\SS$ and $T$ as in Lemma 1.2 (ii).
...
In particular, if $Ms(|\phi^{*}|)$ is $\SS$-invariant, then
%% --
\[
...
Let $T_{|}$ (resp. $S_{|}$) be the restriction of $T$ to $Ms(|\phi|)$ (\resp of $\SS$ to $Ms(|\phi^{*}|)$).
...
\item 
$\operatorname{Fix}(\TT)$ is invariant under the involution of $M_{*}$.
If $\psi \in \Fix{\TT}$ is selfadjoint, then the positive part $\psi^{+}$ and the negative part $\psi^{-}$ of $\psi$ are elements of $\Fix{\TT}$.
\end{enumerate}

\end{corollary}
%% --
The proof follows immediately from D-I, Corollary 2.2 and the fact that $\Kern{A} = \Fix{\lambda R(\lambda,A)}$ for all $\lambda \in \C$ with $\Re\,\lambda > 0$.

If $\TT$ is the semigroup of translations on $L^1(\R)$ and $A'$ the generator 
of the adjoint weak*-semigroup, then $P\sigma(A) \cap \im\R = \emptyset$, while $P_{\sigma}(A') \cap \im\R = \im\R$.
...
Since $\SS$ is a contraction, its adjoint $S'$ maps the dual unit ball $E'_{1}$ into itself.
...
Since $\RR$ and $\R\RR'$ are pseudo-resolvents, the assertion is proved.
\end{proof}
%% --
...
%% --
\[
P\sigma(A) \cap \im\R = P_{\sigma}(A') \cap \im\R
\]
%% --
\end{proposition}
...
Hence from Lemma~\ref{lem:d3-1.2}\,(i) it follows
%% --
\[
...
Recall that a semigroup $\SS$ of positive operators on an ordered Banach space $(E,E_{+})$ is called \emph{irreducible} if no closed face of $E_{+}$, different from $\{0\}$ and $E_{+}$, is invariant under $\SS$.
...
In the context of \WA-algebras $M$ we call a semigroup $\SS$ of positive maps on $M$ \emph{weak*-irreducible}, if no $\sigma(M,M_{*})$-closed face of $M_{+}$ is $\SS$-invariant.

Since the norm closed faces of $M_{*}$ and the $\sigma(M,M_{*})$-closed faces of $M$ are related by formation of polars with respect to the dual system $\langle M,M_{*} \rangle$ (see \citet[Theorem 3.6.11 and Theorem 3.10.7.]{pedersen:1979}) a semigroup $\SS$ is (norm) irreducible on $M_{*}$ if and only if its adjoint semigroup is weak*-irreducible.
%% --
...
\item 
$P_{\sigma}(A') \cap \im\R = P\sigma(A) \cap \im\R$ for the generator $A'$ of the adjoint semigroup, and every $\gamma \in P_{\sigma}(A') \cap \im\R$ is simple.
\end{enumerate}
\end{theorem}
%% --
\begin{proof}
Since $P\sigma(A) \cap \im\R \neq \emptyset$ there exists $\psi \in \Fix{\TT}_{+}$ of norm one (Corollary 1.5).
...
Since $\Fix{\TT}$ is positively generated (Corollary 1.5), $\Fix{\TT} = \C.\phi$ for some faithful normal state $\phi$.
...
\item 
If the group $H$ is generated by a single element, \ie $H = \im\gamma\mathbb{Z}$ for some $\gamma \in \R$ then the family $\{u_{\gamma}^{k} \colon k \in \mathbb{Z}\}$ is a complete family of eigenvectors pertaining to the eigenvalues in $H$, where $u_{\gamma} \in M$ is unitary such that $A'u_{\gamma} = \im\gamma u_{\gamma}$.
...
\item 
$G \coloneqq (T_{0})^{-} \subseteq L_{s}(N_{*})$ is a compact, Abelian group.

\item 
$\Id|N_{*}\in\{T_{0}(t) \colon t>s\}^{-} \subseteq L_{s}(N_{*})$ for all $0<s \in \R$.
\end{enumerate}
\end{proposition}
%% --
\begin{proof}
...
Since $T_{0}(t)\psi_{i} = e^{\eta_{i}t}\psi_{i}$ $(1 \leq i \leq n)$ for some $\eta_{i} \in H$, it follows from a theorem of Kronekker (see, \citet[Satz 6.1., p.77]{jacobs:1976}) that there exists $s < t$ such that
%% --
\[
...
\item 
The compact group $G$ and the discret group $P\sigma(A) \cap \im\R$ are dual in the sense of locally compact Abelian groups.
...
\item If $\TT$ is the irreducible semigroup of Schwarz type on the predual of $B(H)$ given in \citet{evans:1977} then $P\sigma(A) \cap \im\R = \emptyset$.
\end{enumerate}
\end{remark}
...
In particular the fixed space $N$ of the adjoint pseudo-resolvent $\R\RR'$ is a \WA-subalgebra of $M$ with $\1 \in N$ (by Lemma 1.1.b).
...
Sine the map $\mu R(\mu)'$, $\mu \in \R_{+}$ is an identity preserving Schwarz map, we obtain from Lemma 1.1.b that for all $x \in N$ and $y \in M$,
%% --
\[
	\mu R(\mu)'(xy) = x(\mu \RR'(\mu)y) .
\]
%% --
In particular, $R_{j}$, \resp $L_{j}$ are invariant under $\R\RR'$, respectively, $\RR$.
...
\]
%% --
The closed left ideal $Mp_{j}$ is $S$-invariant, therefor it follows
%% --
\[
...
\]
%% --
Consequently, the holomorphic functions $(\mu \mapsto \mu R(\mu)'(x u_{\alpha})u_{\alpha}^{*})$ and $(\mu \mapsto \mu R(\mu+\im\alpha)'x)$ coincide on $\R_{+}$ from which we conclude
%% --
\[
...
Since the map $(y \mapsto y u_{\alpha})$ is a continuous bijection from $M(u_{\alpha}u_{\alpha}^{*})$ onto $Mp_{j}$ and its inverse is the map $(y \mapsto y u_{\alpha}^{*})$, we can deduce that
%% --
\begin{align*}
...
\end{proof}
%% --
Before going on let us recall the basic facts of the \emph{ultrapower} $\hat{E}$ of a Banach space $E$ with respect to some free ultrafilter $\mathfrak{U}$ on $\mathbb{N}$ (compare A-I,3.6).
...
Recall that the approximative point spectrum $A_{\sigma}(T)$ is equal to the point spectrum $P_{\sigma}(\hat{T})$ (see, e.g., \citet[Chapter V, §1]{schaefer:1974}).
...
$\dim \Fix((\lambda-\im\alpha)R(\lambda)) = \dim \Fix((\lambda-\im\alpha)\hat{R}(\lambda))$ for all $\lambda \in D$.
...
\item
Let $M$ be a \WA-algebra with predual $M_{*}$, $\mathfrak{U}$ a free ultrafilter on $\mathbb{N}$ and $\hat{M}$ (resp. $(M_{*})^{\wedge}$) the ultrapower of $M$ (resp. $M_{*}$) with respect to $\mathfrak{U}$.
Then it is easy to see that $c_{\mathfrak{U}}(M)$ is a two sided ideal in $\ell^{\infty}(M)$ hence $\hat{M}$ is a \CA-algebra, but in general not a \WA-algebra.
...
Hence there exists a central projection $z \in \hat{M}''$ such that $J((M_{*})^{\wedge}) = \hat{M}''z$ \citet[Proposition 2.2]{groh:1984}.
...
\item
The Perron-Frobenius theory for a single positive operator on a non-commutative operator algebra is worked out in \citet{albeveriohöeghkrohn:1978} and \citet{groh:1981}.
...
\item
Theorem 2.5 has its roots in the Niiro-Sawashima Theorem for a single irreducible positive operator on a Banach lattice (see \citet[V.5.4]{schaefer:1974}).
