%%Stand 2025-02-26_16:12 
%%%KGK p-A2-1; LNMp-25 A-II (Chapter 2)
%%
...
\end{theorem}
%% –
\begin{proof} \ref{thm:a2-1.1.1} (\lnm i) $\implies$ \ref{thm:a2-1.1.2} (\lnm ii)
Assume that \ref{thm:a2-1.1.1} (\lnm i) %% (i)
...
Then by Theorem~\ref{thm:a2-1.1} (\lnm 1.1.), $A_{1}$ is a generator.
...
condition \ref{cor:a2-1.2-2} (\lnm ii) in Corollary \ref{cor:a2-1.2} (\lnm 1.2). does not hold, since for each $ \lambda \in \C $:
%% --
\[
...
Since $ B $ is unbounded, condition \ref{cor:a2-1.2-4} (\lnm iv) of \ref{cor:a2-1.2} (\lnm 1.2.) is clearly violated.
\end{example}
%% --
...
\end{theorem}
%% –
In general it is not easy to give an estimate for the powers of the resolvent which enables one to apply Theorem \ref{thm:a2-1.7}(\lnm 1.7).
...
\end{corollary}
%% –
The difficult part in the proof of Theorem \ref{thm:a2-1.7} (\lnm 1.7) is to show that \ref{cor:a2-1.8.2} (\lnm ii) implies \ref{cor:a2-1.8.1} (\lnm i) .
...
(\lnm ii) of Theorem  \ref{thm:a2-1.7} (\lnm 1.2). (see \citet{davies:1980}, [Goldstein (1985b)] or [Pazy [1982]).
...
\end{remark*}
%% --
As an application of Theorem \ref{thm:a2-1.12} (\lnm 1.12.)we prove the following.
%% --
...
%%KGK \ref{a1-2.4}.
Thus $A^{2}$ generates a bounded holomorphic semigroup by corollary \ref{cor:a2-1.13} (\lnm 1.13).
...
Since $\|R(\lambda,A)\| \to \infty$ when $\lambda$ approaches $\sigma(A)$ (cf. Lemma  \ref{lem:a2-1.21} (\lnm 1.21) below), 
...
\end{proof}
%% --
Compared with the Hille-Yosida theorem, Theorem \ref{thm:a2-1.14} (\lnm 1.14) gives a very simple criterion for an operator to be the generator of a (holomorphic) semigroup.
...
The remaining estimate for a sector around the imaginary axis is given by the proof of Theorem \ref{thm:a2-1.14}  (\lnm 1.14) 
and shows that $A-w$ generates a holomorphic semigroup.
The reverse implication is clear.
\end{proof}

We now prove the following extension of corollary \ref{cor:a2-1.13} (\lnm 1.13.)
...
In Theorem  \ref{thm:a2-1.15} (\lnm 1.15)
...
For the proofs of these two theorems we refer to \citet[Chap.3, Theorem 4.7 and 4.8]{pazy:1983}.
...
\begin{proof}[Proof of Theorem  \ref{thm:a2-1.20} (\lnm Thm.1.20)]
It is enough to show the following.
%%KGK ersetzt: alpha durch a
Let $a > \omega(A)$.
...
Eventually norm continuous semigroups are of particular interest in spectral theory (cf. A-III,Thm.6.6).
...
The other follows from Proposition \ref{prop:a2-1.24} (\lnm 1.24).
\end{proof}
%% --
...
Thus the property proved in Theorem~\ref{thm:a2-1.20} (\lnm 1.20) characterizes generators of eventually norm continuous semigroups in the case that the semigroup consists of multiplication operators.
%%\end{enumerate}
\begin{proof}
...
%% of Theorem~\ref{thm:a2-1.14}.
Moreover, it follows from (\eqref{eq:a2-1.12}) (\lnm ) that $R(\lambda,A+B)$ is compact whenever $R(\lambda,A)$  is compact.
Consequently by Theorem~\ref{thm:a2-1.25} (\lnm 1.25) and the assertion proved above, $(S(t))_{t \geq 0}$ is compact whenever $(T(t))_{t \geq 0}$ is compact.
...
Using  (\eqref{eq:a2-1.9}) (\lnm 1.9) we obtain $\|S(t+h) - S(t)\|$ 
  fehlt?? :
\begin{align*}
...
Using Theorem~\ref{thm:a2-1.29} (\lnm 1.29) we now prove a perturbation result due to [Desch-Schappacher(1984)].
...
Then $A + C$ is the generator of a strongly continuous semigroup by Theorem \ref{thm:a2-1.29} (\lnm 1.29).
...
Keeping the hypotheses and notations of Theorem \ref{thm:a2-1.31} (\lnm 1.31) denote by $(S(t))_{t\geq 0}$ the semigroup generated by $A+B$.
...
%% --
\begin{proof}
This follows from Theorem \ref{thm:a2-1.30} (\lnm 1.30) since 
...
So by Theorem \ref{thm:a2-1.31} (\lnm 1.31) the operator $A+B$ with domain $D(A)$ is a generator.
...
Then \eqref{eq:a2-2.5} (\lnm 2.5) can be written in the form
...
Then it follows from Corollary \ref{cor:a2-2.5} (\lnm 2.5) that $A$ is p-dissipative.
Hence by \eqref{eq:a2-2.8} (\lnm 2.8) $p(\lambda R(\lambda,A)g) \leq p(g)$ for all $g \in E$, $\lambda > \omega(A)$.
Hence $\lambda R(\lambda,A)$ is p-contractive for $\lambda > \omega(A)$.
This implies that $T(t)$ is p-contractive by the formula \eqref{eq:a2-1.3} (\lnm 1.3) :
%% –
\[
...
Hence $p(g) + p(-g) \leq 0$ which implies $g = 0$ by \eqref{eq:a2-2.11} (\lnm 2.11).
\end{proof}
...
It follows from \eqref{eq:a2-2.14} (\lnm 2.14)
...
(ref{thm:a2-2.11.1}) (\lnm i) implies (\ref{thm:a2-2.11.2}) (\lnm ii) by Theorem \ref{thm:a2-2.6}  (\lnm 2.6) .

Now suppose that \ref{thm:a2-2.11.2} (\lnm ii) holds.
Then it follows from Lemma \ref{lem:a2-2.10} (\lnm 2.10) 
...
In the case when $p = N$, Theorem \ref{thm:a2-2.11} (\lnm 2.11)  
...
Before that let us formulate Theorem \ref{thm:a2-2.11} (\lnm 2.11) 
...
Lemma \ref{lem:a2-2.10} (\lnm 2.10) 
...
If in Lemma \ref{lem:a2-3.1} (\lnm 3.1) 
...
The condition $s < \infty$ in Lemma \ref{lem:a2-3.1} (\lnm 3.1) 
...
Then Lemma \ref{lem:a2-3.2} (\lnm 3.2) implies that $T'$ and $T$ are strongly continuous.
Hence by the uniform boundedness principle, $\lim \sup_{t \to 0} \|(T(t) - Id)\|$ is finite.
By Lemma \ref{lem:a2-3.1} (\lnm 3.1) it suffices to show that $\lim_{t \to 0} \|(T(t) - Id)^2\| = 0$.
...
Since $T'$ and $T''$ are strongly continuous, Lemma \ref{lem:a2-3.2} (\lnm 3.2) implies that $((T(t_{n}) - Id)x_{n})$ and $((T(t_{n}) - Id)'x_{n}')$ converge weakly to zero.
Since $E$ has the Dunford-Pettis property, $\lim \|(T(t_{n}) - Id)^2\| = 0$.
Consequently, $\lim_{t \to 0} \|(T(t) - Id)^2\| = 0$.
\end{proof}

An immediate consequence of Theorem \ref{thm:a2-3.3} (\lnm 3.3) and Theorem \ref{thm:a2-3.4} (\lnm 3.4) is the following.
...
\begin{proof}
By Theorem \ref{thm:a2-3.5} (\lnm 3.5) it suffices to show that the space listed above are Grothendieck spaces with the Dunford-Pettis property.
\begin{enumerate}[(i)]
\item 
...
\ref{thm:a2-3.6-2} (\lnm 2) and \ref{thm:a2-3.6-3} (\lnm 3) are $\sigma$-order complete AM-spaces with unit and therefore by 
proof of \ref{thm:a2-3.6-1} (\lnm 1) Grothendieck spaces with the Dunford-Pettis property.
%%--
 
\item
%%--5) 
%%KGK alternativ: \ref{thm:a2-3.6-4} and \ref{thm:a2-3.6-5}
...
It follows from Theorem \ref{thm:a2-3.6} (\lnm 3.6) that on $L^{\infty}$ the infinitesimal generator of a strongly continuous semigroup is necessarily bounded.
...
One implication of Theorem \ref{thm:a2-1.1} (\lnm 1.1) is proved in [Krein (1971) (Thm. 2.11)].
...
Our Theorem \ref{thm:a2-1.14} (\lnm 1.14) deviates from the usual presentation since the condition on the resolvent is merely required on a half-plane.

Differentiable semigroups are treated in detail in the book of \citet{pazy:1983} who discovered Theorem \ref{thm:a2-1.17} (\lnm 1.17) and \ref{thm:a2-1.18} (\lnm 1.18).
The spectral property of eventually norm continuous semigroups given in Theorem \ref{thm:a2-1.20} (\lnm 1.20) is contained in [Hille-Phillips (1957) (Thm.16.4.2)] with a proof depending on Gelfand theory.
...
Theorem \ref{thm:a2-1.29} (\lnm 1.29) on the perturbation by bounded operators is due to \citet{phillips:1953} who also investigated permanence of smoothness properties by this kind of perturbation.
...
A complete information on the general theory can be obtained by consulting the books of \citet{davies:1980}, [Goldstein (1985a)] and \citet{pazy:1983}.
...
Related previous results were obtained by \citet{calvert:1971}, \citet{hasegawa:1966}, \citet{sato:1968}, \citet{bénilanpicard:1979} and \citet{picard:1972}, where the two last consider non-linear semigroups.
A further investigation of half-norms can be found in \citet{battyrobinson:1983} who consider ordered Banach spaces other than Banach lattices in great detail.
...
That every strongly continuous semigroup on $L^{\infty}$ is uniformly continuous was first shown by \citet[(1984), (1985)]{lotz:1982}.
The proof of Lemma \ref{lem:a2-3.1} (\lnm 3.1) was communicated to the author by T. Coulhon, who independently obtained a particular case (\citet{coulhon:1984}).
