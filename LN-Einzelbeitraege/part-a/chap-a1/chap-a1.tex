...
%% -- 
We consider a one-parameter semigroup $(T(t))_{t \geq 0}$ on a Banach space $E$ and observe that the domain $\R_{+}$ and range $\LE$ of the (semigroup) homomorphism $\tau \colon t \mapsto T(t)$ are topological semigroups for the natural topology on $\R_{+}$ and any one of the standard operator topologies on $\LE$.
...
$ \lVert T(t) \rVert \leq M \cdot \mathrm{e}^{ \omega t} $ for every $  t \geq 0 $.
...
%% --
The exponential estimate from Remark for the growth of $ \lVert T(t) \rVert $ can be used to define an important characteristic of the semigroup.


\begin{definition}\label{def:a1-1.3}
By the growth bound (or type) of the semigroup $ T(t)_{t\geq0} $ we understand the number $ \omega_{0} $,
%% --
\begin{align*}
	\omega_{0} & \coloneq \inf \{ \omega \in \R   \colon \text{there exists $ M \in \R_{+} $   
 		such that $\|T(t)\| \leq M \cdot  \mathrm{e}^{wt}$  for  $t \geq 0$} \} \\
...
%Hence, $\frac{\sinh(tD){D} = \frac{\sin(t|D|}{|D|}$ and $\cosh(tD}=\cos(|D|t)}$ 
...
\begin{align*}
    ...
    ( \mathrm{e}^{\lambda t}-T(t))f &= (\lambda-A) \int_{0}^{t}  \mathrm{e}^{\lambda(t-s)}T(s)f \ds \quad \text{for } f \in E, %\tag{3.1}
\end{align*}
%% --
and
%% --
\begin{align*}
    ...
    \notag \\
    ( \mathrm{e}^{\lambda t} - T(t))f &= \int_{0}^{t}  \mathrm{e}^{\lambda(t-s)}T(s)(\lambda-A)f \ds \quad \text{for } f \in D(A). %\tag{3.2}
\end{align*}
%% --
%% --
