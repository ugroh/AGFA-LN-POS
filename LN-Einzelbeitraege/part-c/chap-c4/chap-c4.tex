%% Stand 2025-05-07 ulgr: Formeln überarbeitet und korrigiert
%% Stand 2025-05-06 gugr - C-IV überarbeitet insbes. Rainers Korrekturen von C-IV-2 eingearbeitet
...
%%\item 
\ref{thm:c4-1.1-2} We identify $E_{h}$ according to the \emph{Kakutani-Krein Theorem} with a space $C(K)$, $K$ compact.
...
%there does not exist a non-zero function $h \in L^{p}(\R^{n})$ with $\Delta h = 0$.
...
%% --
\[ 
\text{$\omega_{1}(A) < 0 $ if and only if 
$E_{+} \subset \Image{A(D(A)_{-})}$.}
\]
\end{proposition}
%% --
\begin{proof}
...
\marginpar{GG:der Verweis A-IV, Thm.~1.14 passt nicht}
...
and $h \otimes x \in E$ is defined by $(h \otimes x)(s) \coloneqq h(s) \cdot x$ for $h \in L^{1}[-1,0]$, $x \in F$ and $s \in [-1,0]$.
Clearly $\|\epsilon_{\lambda}\| = 1/\lambda \cdot (1 - e^{-\lambda}) \to 0$ as $\lambda \to \infty$ and we have
%% --
\[
...
\]
and $\|R(\lambda,A_0)\| \leq 1/\lambda$ for $\lambda > 0$ we obtain for $\lambda \geq \|\Phi\|$
%% --
\begin{align*}
...
\end{equation}
%% --
\marginpar{GG: Ich verstehe (3.4) nicht!}
...
\begin{enumerate}[\upshape (i), wide, labelsep=1em] 
    \item 
    %\ref{lem:c4-3.5-1}
    $\rho(A_{0}) = \C$ follows directly from $(T_{0}(t))_{t \geq 0}$ being nilpotent (see A-III, Proposition 1.1). For $g \in E$ we obtain $R(\lambda,A_{0})g = f$ where $f$ is a solution of $\lambda f - f' = g$.
...
Therefore there must exist $\mu' > \lambda$ such that $s(\Phi_{\mu'}) = 1$. 
\end{enumerate} 
...
Furthermore define $E = L^{1}([-1,0],F)$ as usual and let $\{K(s) \colon s \in [-1,0]\}$ be a family of positive, irreducible, weakly compact operators on $F$ which is bounded.
...
If we define $K \in \L{F}$  as follows 
%% --
\[
