...
\begin{enumerate}[$\bullet$, wide, labelindent=1.0em]
\item 
...
\end{enumerate}
%% --
While all semigroups in these texts are assumed to be strongly continuous, in many natural situations semigroups appear -- under various names -- that are continuous only for some weaker topology. 
...
We refer the reader to \mycite{zbMATH07458830} for an overview of Grothendieck spaces. 
For the Dunford-Pettis property, the classical survey by Diestel \citeEN{zbMATH03911038} remains a valuable resource, although many of the open problems posed therein have since been resolved. 
Another useful reference is \mycite{zbMATH00769497}.
%% --
%% --
...
\mycite[Chapter 5]{zbMATH05842872} while  in \mycite{zbMATH05080033} provides a 
...
\item
Two books appeared which are specialized on the asymptotic behaviour of semigroups, namely the book by \mycite{zbMATH00921898} and the one by \mycite{zbMATH05625330}, which has weak stability as a main subject.

\item
Results on countable boundary spectrum have interesting consequences for positive semigroups. 
Here we mention a basic result on stability (the ABLV-Theorem). 
...
Then the semigroup  $(T(t))_{t\geq 0}$ is stable (\ie converges strongly to $0$ as $t \rightarrow 0$) if and only if there is no point spectrum on the imaginary line. 
...
The condition that the spectrum be countable is not necessary for stability. 
However results by \mycite{zbMATH00410037} and \mycite{zbMATH0493176} characterize countability in terms of \enquote{super stability} (\ie stability of all the semigroups induced on an ultra power of the underlying Banach space).

\item
There are more general versions of the ABLV-Theorem which imply properties on the asymptotic behavior of the semigroup, we refer to \mycite[Theorem~2.21]{zbMATH01354832} and \mycite[Theorem~5.5.5]{zbMATH05842872}. 
