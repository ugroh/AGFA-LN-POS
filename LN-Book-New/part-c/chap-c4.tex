...
Now, using the combining results of Part-I, Chapter~\ref{chap:a3} and Chapter~\ref{chap:a4},  
...
In fact, combining 
Part III, Theorems~\ref{thm:c3-1.1} \ref{thm:c3-1.1}  with 
...
Theorem~\ref{thm:a4-1.3} \textbf{da fehlt ein Eq ???} we deduce that $\|T(s)R(0,A)\| \leq M\eu^{-\epsilon s}$ for $s \geq 0$ and suitable constants $M \geq 1$, $\epsilon > 0$.
...
%% --
\begin{proof}
By Part II, Theorem~\ref{thm:b4-2.10} the set $\{\lambda \in \sigma(A) \colon \Re(\lambda) = 0\}$ is finite and by Theorem 2.10 of Part-3, Chapter 3  even imaginary additively cyclic.
