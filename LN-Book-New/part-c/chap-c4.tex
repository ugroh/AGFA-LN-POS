...
\[
C(t)f \coloneqq  \frac{1}{t}\int_{0}^{t} T(s)f \ds \text{ for } f \in E
\]
...
and on $E_1$ the operator $A_1$ which is the restriction of\/ $A$ to $D(A_1) \coloneqq D(A^{2})\subseteq E_1$.
By 
...
%% --
%Setze $b_{ij}(\lambda) \coloneqq \int_{0}^{r} \beta_{ij}(a)\,\eu^{-\lambda a} \diff{a}$.
...
%
Take $F \coloneqq L^{1}(\Omega)$ where $\Omega \subseteq \R^{2}$ is bounded and take
%
\[
	 E \coloneqq L^{1}([0,r], F) = L^{1}([0,r] \times \Omega)
\]
%
for some $r \in \R_{+}$.
...
As in Example~\ref{ex:c4-3.13} the first equation in~\myeqpageref{eq:c4-3.14} corresponds to the aging process.
