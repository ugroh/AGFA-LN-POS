...
\newline
%% --
In Section~1 of Part-II, Chapter~\ref{chap:b4} we have seen that the growth bound of a positive semigroup on spaces $C_{0}(X)$ coincides with the spectral bound of the generator $A$, which is --- for positive semigroups --- an element of the spectrum of\/ $A$.
Now, using the results of Part-I, Chapter 3,  Part-I, Chapter 4,  Part-II, Section~\ref{sec:b4-1} and Chapter~\ref{chap:c4},  it can be shown that this is valid for positive semigroups on AM- , AL- and Hilbert spaces.
...
\item 
%\ref{thm:c4-1.1-1} 
For AM-spaces the proof given in Section 1 of Part-II, Chapter~\ref{chap:b4} works (cf.\ Part-II, Remark~\ref{rem:b4-1.5}.).
Since for positive semigroups we always have $\|R(\lambda,A)\| \leq \|R(\Re(\lambda), A)\|$ ($\Re(\lambda) > s(A)$) (see   Corollar~\ref{cor:c3-1.3}), the assertion for $L^{2}$-spaces follows from Part-I, Corollary~\ref{cor:a3-7.11}.
...
Thus $\rho \in R\sigma(T(\tau))$ and hence $s(A) \geq \omega_{0}(A)$ by Part-I, Theorem~\ref{thm:a3-6.2}.
...
\end{theorem}
%% --
Recalling the definition of\/ $\omega_{1}(A)$ (see Part-I, Definition~\ref{def:a4-1.1}) and the fact that $s(A)$ is always an element of\/ $\sigma(A)$, we can reformulate the statement of Theorem~\ref{thm:c4-1.3} as follows.
%% --
...
%% --
\begin{proof}
\ref{thm:c4-1.5-1} $\Rightarrow$ \ref{thm:c4-1.5-2} was proven in Part-I, Theorem~\ref{thm:a4-1.12} in a more general setting.
...
Thus irreducibility of the semigroup (see Part-I, Chapter 1, 2.8 and Example~\ref{ex:c3-3.4}\,(i)) implies that $\dim \Kern\Delta \leq 1$.
...
Hence, by Part-I, Remark~\ref{rem:a4-1.17}(a), $\Image{A} = \{f \in L^{2}(\R_{+}) \colon \int_{0}^{\infty} f(x) \, \dx \text{ exists}\}$ and we see that $(\Image{A})_{+}$ is a total subset of\/ $L^{2}(\R_{+})$.
...
Using the weak version of Part-I, Theorem~\ref{thm:a4-1.14}, 
...
Using the general results presented in Section 2 of Part-II, Chapter~\ref{chap:b4} and the spectral theoretical result of Chapter~\ref{chap:c3} we obtain the following.
...
%% --
\begin{proof}
By Theorem 2.10 of Part-II, Chapter~\ref{chap:b4} the set $\{\lambda \in \sigma(A) \colon \Re(\lambda) = 0\}$ is finite and by Theorem 2.10 of Part-3, Chapter 3  even imaginary additively cyclic.
Thus it contains only the value $s(A) = 0$.
Then by Part-II, Chapter~\ref{chap:b4}, (2.5) we have
%% --
...
Moreover, Eq.~Eq.~\myeqpageref{eq:c4-2.3} implies 
%
\[
...
Moreover, in view of Part-I, Proposition~\ref{prop:a2-5.2} and Corollary~5.3(ii) 
...
By a solution of Eq.~\myeqpageref{eq:c4-re} with initial function $g \in E$ we understand a function $u \in L^{1}_{\mathrm{loc}}([-1,\infty),F)$ which satisfies equation Eq.~\myeqpageref{eq:c4-re}.
The problem Eq.~\myeqpageref{eq:c4-re} is called \emph{well-posed} if for each $g \in E$ there exists exactly one solution.
%% --
\begin{remarks*}
\begin{enumerate}[\upshape (i), wide, labelindent=.5em] 
...
\end{align*}
%% --
(where $B$ is the generator of a bounded semigroup on $F$) seems to be in better analogy to the retarded Cauchy problem of Part-II, Section~\ref{sec:b4-3} and can be reduced to an equation of the type Eq.~\myeqpageref{eq:c4-re}.
...
The choice of \enquote{$L^{1}$-functions} instead of \enquote{C-functions} (as in the case of (RCP)) enforces the solutions of Eq.~\myeqpageref{eq:c4-re} to yield a strongly continuous semigroup of operators (on the space $E$ of initial functions) as in Part-II, Section~\ref{sec:b4-3}.
\end{enumerate}
\end{remarks*}
%% --
\index{differential operator}
In order to solve Eq.~\myeqpageref{eq:c4-re} we consider on $E = L^{1}([-1,0],F)$ the differential operator $A \coloneq  \frac{\diff{}}{\dx}$ with domain
%% --
...
\end{proof}
%% --
The following corollary corresponds to Part-II, Corollary~\ref{cor:b4-3.2} and assures the well-posed\-ness of Eq.~\myeqpageref{eq:c4-re}.
...
\end{equation}
%% --
is the unique solution of Eq.~\myeqpageref{eq:c4-re}, in particular Eq.~\myeqpageref{eq:c4-re} is well-posed.
...
Let $w$ be a solution of Eq.~\myeqpageref{eq:c4-re} with initial function $w_{0} = 0$.
Then
\begin{align*}
...
\end{proof}
%% --
Now we turn to the aspect of positivity in Eq.~\myeqpageref{eq:c4-re}. We assume $F$ to be a Banach lattice and let $E$ inherit the canonical ordering from $F$ making it a Banach lattice again. Additionally, let $\Phi$ be positive.
...
The semigroup generated by $A_{0}$ is positive (use Eq.~\myeqpageref{eq:c4-re}), hence $R(\lambda,A_{0}) \geq 0$.
...
If\/ $\Phi \in \L{E,F}$ is a positive operator, then the solution semigroup corresponding to Eq.~\myeqpageref{eq:c4-re} is positive.
\end{proposition}
%% --
...
\end{proof}
%% --
The previous results will now be used to characterize the spectral bound of\/ $A$ and hence the stability of the solutions of Eq.~\myeqpageref{eq:c4-re}.

\begin{theorem}\label{thm:c4-3.7}
\index{stability}
\index{spectral bound $s(A)$}
Let $A \coloneqq \frac{\diff{}}{\dt}, D(A) \coloneqq \{f \in AC([-1,0],F) \colon f' \in L^{1}([-1,0],F)$ and $f(0) = \Phi(f)\}$ be the generator of the solution semigroup on $E \coloneqq L^{1}([-1,0],F)$ corresponding to Eq.~\myeqpageref{eq:c4-re}. 
...
However, in order to determine the long term behavior of the solutions of Eq.~\myeqpageref{eq:c4-re}, it is often enough to know the spectral bound $s(A)$. 
...
Then Eq.~\myeqpageref{eq:c4-re} has the form
%% --
...
Thus we are in the situation of Example \ref{ex:c4-3.9} and from Formula Eq.~\myeqpageref{eq:c4-3.10} we obtain the equivalence
%% --
\[
...
For the result on AM-spaces we refer to Remark~1.1 of Part-II, Chapter~\ref{chap:b4} and the corresponding notes.
