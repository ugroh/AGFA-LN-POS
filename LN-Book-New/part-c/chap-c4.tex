...
%% --
\begin{proof}
We know that $s(A) \leq \omega_{1}(A) \leq \omega_{0}(A)$ (see Part-I, Chapter 4,  Corollary 1.5) and $s(A) \in \sigma(A)$ (see Part-III, Chapter 3,  Corollary 1.4).
Thus we have to show $s(A) = \omega_{0}(A)$.

\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
\item 
%\ref{thm:c4-1.1-1} 
For AM-spaces the proof given in Section 1 of B-IV works (cf.\ Part-II, Chapter 4,  Remark~1.5.).
...
\end{align*}
%% --
(the last inequality follows from Part-III, Chapter 3,  Theorem~1.2).
...
Since $\omega_{0}(u) = \log r(T_{u}(1))$ (see Part-I, Chapter 3, \,(1.4)), \eqref{eq:c4-1.2} implies that $u \mapsto \omega_0(1/u)$ is a convex function.
By Part-III, Chapter 3,  Theorem~1.1 we have $r(R(k,A_{u})) = (k-s(u))^{-1}$ for $k \in \N$ sufficiently large.
...
If \ref{cor:c4-1.4-2} is satisfied then $R(0,A) \geq 0$, hence $s(A) < 0$ by Part-III, Chapter 3,  Theorem 1.1\,(ii)).
...
%% --
\begin{proof}
If\/ $\omega_{1}(A) < 0$, then $s(A) < 0$ (Part-I, Chapter 4,  Corollary 1.5), hence $A^{-1} = -R(0,A) \leq 0$ by Part-III, Chapter 3,  Theorem 1.1.
...
\]
%% --
Using Part-III, Chapter 3,  Theorem 1.2 we conclude that $R(\alpha,A)f \leq g$ for $\alpha > \max\{0,s(A)\}$.
...
\end{proof}
Next we show that weak uniform stability implies uniform stability provided that $E$ is weakly sequentially complete (see Part-III, Chapter 1,  Section 5) and $(\Image{A})_{+} \coloneqq A(D(A)) \cap E_{+}$ is a total subset of\/ $E$.
...
%% --
\begin{proof}
In view of Part-III, Chapter 3,  Theorem 1.2 we have for $\phi \in E'_{+}$:
%% --
\begin{align*}
...
%%
\footnote{Note, that the assumptions imply that \eqref{eq:c4-1.6} has a unique strong solution, see \textcite[ Theorem~4.2.4]{pazy:1983}.}
\end{proposition}
%% --
\begin{proof}
...
the second term to $R(0,A)f_{0} = -A^{-1}f_{0}$ by Part-III, Chapter 3,  Theorem 1.2.
...
%% --
\begin{proof}
By Theorem 2.10 of B-IV the set $\{\lambda \in \sigma(A) \colon \Re(\lambda) = 0\}$ is finite and by Theorem 2.10 of C-III even imaginary additively cyclic.
...
If\/ $\TT = (T(t))_{t \geq 0}$ is an irreducible semigroup with $s(A) = 0$, then quasi-compactness implies boundedness of\/ $\TT$ (This follows from \ref{rem:c4-2.2-3} and Part-III, Chapter 3,  Proposition 3.5).
...
This also is a consequence of Part-III, Chapter 3,  Proposition~3.5.
...
Indeed, by Part-III, Chapter 3,  Theorem~3.7 we know that $\omega_0(\TT) > -\infty$, while $\omega_{ess}(\TT) = -\infty$.
...
Moreover, Eq.~\eqref{eq:c4-2.3} implies 
%
\[
...
Consequently the semigroup generated by $A$ satisfies all the assumptions of Theorem~\ref{thm:c4-2.1} provided that $\mu$ and $\beta$ satisfy \eqref{eq:c4-2.3} (the boundedness of the semigroup follows from Remark \ref{rem:c4-2.2}\,\ref{rem:c4-2.2-3}).
...
For more general generators of the type \eqref{eq:c4-2.2} we refer to Part-III, Chapter 4,  Section~3.
\end{example}
Clearly, quasi-compactness was essential in the above example as well as in Theorem~\ref{thm:c4-2.1}.
For spaces $C_{0}(X)$ we proved in Part-II, Chapter 4,  Theorem 2.12 that Doeblin's condition is sufficient for quasi-compactness.
...
Since $\sigma(A) \cap \im\R$ is cyclic and bounded (see Part-III, Chapter 3,  Theorem~2.10 and Part-I, Chapter 2,  Theorem 1.20 resp.), we have $\sigma(A) \cap \im\R = \{0\}$.
...
\end{proof}
%% --
Typical examples for which Theorem~\ref{thm:c4-2.6} and Corollary \ref{cor:c4-2.7}  can be applied, occur in the theory of stochastic processes (see also Part-II, Chapter 4,  Example~2.6).
...
Now Part-III, Chapter 3,  Proposition 3.5(i)-(iii) implies that $P$ has the form $P = h \otimes e$ for a strictly positive function $h \in L^{q}(\mu) = (L^{p}(\mu))'$.
\end{proof}
%%% --
...
From Part-III, Chapter 3,  Corollary~3.9 we conclude that  $P\sigma(A_j) \cap \im\R = {0}$\,. 
...
Under the assumption that the spectral bound $s(A) = 0$ is a pole of the resolvent, we can apply Part-III, Chapter 3,  Theorem~3.12.
...
\end{equation*}
%% --
Therefore the assumptions of Part-III, Chapter 3,  Theorem~3.8 are satisfied and formula Part-III, Chapter 3, \,(3.13) implies
%% --
...
Since $0$ is an eigenvalue of\/ $A$, it follows that $\TT$ has a quasi-interior fixed point $h \in E_{+}$ (use Part-III, Chapter 3,  Proposition 3.5(i)).
Hence, $\{T(t)f \colon t \geq 0\}$ is contained in the weakly compact (see Part-III, Chapter 1,  Section 5) order interval $[-h,h]$ whenever $|f| \leq h$.
...
Hence the representation of\/ $\TT_{1}$ as a rotation group follows from Part-III, Chapter 3,  Corollary~3.9.
\end{proof}
%% --
...
Using Part-III, Chapter 3,  Proposition~3.3 one can show that $(S(t))$ is irreducible.
...
By a solution of \eqref{eq:c4-re} with initial function $g \in E$ we understand a function $u \in L^{1}_{\mathrm{loc}}([-1,\infty),F)$ which satisfies equation \eqref{eq:c4-re}.
...
\end{align*}
%% --
(where $B$ is the generator of a bounded semigroup on $F$) seems to be in better analogy to the retarded Cauchy problem of Part-II, Chapter 4,  Section~3 and can be reduced to an equation of the type \eqref{eq:c4-re}.
...
The choice of \enquote{$L^{1}$-functions} instead of \enquote{C-functions} (as in the case of (RCP)) enforces the solutions of \eqref{eq:c4-re} to yield a strongly continuous semigroup of operators (on the space $E$ of initial functions) as in Part-II, Chapter 4,  Section 3.
\end{enumerate}
\end{remarks*}
%% --
...
\end{equation}
%% --
Moreover, if\/ $f \in D(A)$, then the translation property (T) (see Part-II, Chapter 4,  Theorem 3.1) is satisfied.
\end{theorem}
...
On $E_1$ the point evaluation is a continuous mapping and therefore the translation property can be shown as in the proof of Part-II, Chapter 4,  Theorem 3.1.
Hence we obtain
%% --
...
\end{proof}
%% --
The following corollary corresponds to Part-II, Chapter 4,  Corollary~3.2 and assures the well-posed\-ness of \eqref{eq:c4-re}.
...
%% --
\begin{proof}
As in the proof of Part-II, Chapter 4,  Corollary 3.2 we have $u_t = T(t)f$ for $t \geq 0$ since $u_t(s) = u(t+s) = (T(t)f)(s)$ by the definition of\/ $u$ and by formula \eqref{eq:c4-3.3}.
...
It follows that $R(\lambda,A) \geq 0$ which is equivalent to the positivity of\/ $(T(t))_{t \geq 0}$ (see Part-III, Chapter 2,  Proposition~4.1).
...
$s(\Phi_{\lambda})$ is the boundary spectrum $\sigma_b(\Phi_{\lambda})$ (see Part-III, Chapter 3,  Corollary 2.12) of\/ $\Phi_{\lambda}$. Moreover, $s(\Phi_{\lambda})$ is a pole of the resolvent with residue of finite rank. Such spectral sets vary continuously under smooth perturbations of\/ $\Phi_{\lambda}$ (see \textcite[VII.6, Theorem 9]{dunfordschwartz:1958}), thus $\lambda \mapsto s(\Phi_{\lambda})$ is continuous.
\end{proof}
%% --
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em] 
    ...
    Thus $R(\lambda,A_{0})g(t) = \int_{0}^{t} \eu^{\lambda(t-s)}g(s)\, \ds + \eu^{\lambda t} \cdot x$ for some $x \in F$. The condition $f \in D(A_{0})$ now implies $x = 0$ and Formula \eqref{eq:c4-3.7}.
...
The positivity of\/ $(T(t))_{t \geq 0}$ implies $r \in \sigma(A)$ (see Part-III, Chapter 3,  Theorem 1.1.(i)) and by Proposition~\ref{prop:c4-3.6}\,\ref{prop:c4-3.6-1}  this implies $1 \in \sigma(\Phi_r)$, so that $s(\Phi_{r)} \geq 1$. 
Since $r \geq \lambda$, this yields $s(\Phi_{\lambda}) \geq s(\Phi_{r}) \geq 1$.

\item 
%\ref{thm:c4-3.7-2} 
Let $s(\Phi_{\lambda}) = 1$. 
Since $1 \in \sigma(\Phi_{\lambda})$ (see Part-III, Chapter 3,  Theorem 1.1(i)), $\lambda \in \sigma(A)$ by Proposition~\ref{prop:c4-3.6}\,\ref{prop:c4-3.6-3} whence $s(A) \geq \lambda$. 
...
We call the identity\, \enquote{$s(\Phi_{\lambda}) = 1$} a \emph{generalized characteristic equation} (see also the remark following Part-II, Chapter 4,  Theorem~3.7). The usual characteristic equation (see for example  \textcite[p.168ff]{hale:1977} and \textcite[Section~5]{heijmans:1985b}
