...
Since $\omega_{0}(u) = \log r(T_{u}(1))$ (see Eq.~\eqref{eq:a3-1.4} on p.~\pageref{eq:a3-1.4}), Eq.~\eqref{eq:c4-1.2} implies that $u \mapsto \omega_0(1/u)$ is a convex function.
...
For more general generators of the type Eq.~\eqref{eq:c4-2.2} we refer to Section~\ref{sec:c4-3}.
\end{example}
...
\end{equation*}
%% --
Therefore the assumptions of Theorem~\ref{thm:c3-3.8} are satisfied and formula \eqref{eq:c3-3.13} on p.~\pageref{eq:c3-3.13} implies
%% --
...
\end{align*}
%% --
(where $B$ is the generator of a bounded semigroup on $F$) seems to be in better analogy to the retarded Cauchy problem of Part-II, Section~\ref{sec:b4-3} and can be reduced to an equation of the type Eq.~\eqref{eq:c4-re}.
...
The choice of \enquote{$L^{1}$-functions} instead of \enquote{C-functions} (as in the case of (RCP)) enforces the solutions of Eq.~\eqref{eq:c4-re} to yield a strongly continuous semigroup of operators (on the space $E$ of initial functions) as in Part-II, Section~\ref{sec:b4-3}.
\end{enumerate}
\end{remarks*}
%% --
