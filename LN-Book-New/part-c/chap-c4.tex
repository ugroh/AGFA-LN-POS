...
The first section is devoted to the stability of positive semigroups, and we give sufficient and necessary conditions that ensure that the semigroup, and likewise the solutions of the abstract Cauchy problem, converges to zero as $t \to \infty$.
...
\begin{enumerate}[\upshape (i)] 
    ...
    There exist $\tau > 0$ and $h \in E_{+}$ such that $T(\tau)E \subset E_{h}$.
...
%% --
\[ 
\text{$\omega_{1}(A) < 0 $ \quad \text{if and only if} \quad $E_{+} \subset \Image{A(D(A)_{-})}$.}
\]
\end{proposition}
%% --
\begin{proof}
