...
%% --
In Chapter B-III we showed that positive semigroups on spaces $C_{0}(X)$ possess several interesting spectral properties.
In this chapter we will extend many of these results to the more general setting of Banach lattices, 
improving some considerably and give the complete proof of Theorem~4.1 from Chapter B-III. 
...
Furthermore, Corollary~1.3 and Proposition~1.5 of B-III are true in the setting of Banach lattices and their proofs can be extended to the general case.
...
\end{theorem}
%% --
The fact that $s(A)$ is always an eigenvalue of the adjoint (\cf B-III Theorem~1.6) is characteristic for spaces $C(K)$, $K$ compact, as can be seen by considering the Laplacian on $L^p(\R^n)$, where $1 < p < \infty$, or on $C_{0}(\R^n)$ (see Part-II, Chapter 3,  Example~1.7).
...
\index{spectrum!boundary|(} 
%% --
In Chapter B-III we have seen that under suitable assumptions the boundary spectrum $\sigma_{b}(A)$, consisting of all spectral values with maximal real part is a cyclic set (\cf Part-II, Chapter 3,  Definition~2.5).
...
(see Section~5 of C-II). As a consequence of Part-III, Chapter 2,  Corollary~5.8 we have the following.
...
Retarded equations as dicussed in Example 3.4\,(iii) will be discussed in more detail in Section 3 of C-IV. Example 3.4\,(iv) is a one-dimensional version of the linear transport equation. 
