...
\begin{proposition}\label{prop:c3-2.11}
	...
	\begin{equation}\label{eq:c3-2.22}
		I_{-1} \coloneqq \{0\} \subseteq I_{0} \subsetneq I_{1} \subsetneq ... \subsetneq I_{k} \coloneqq E
	\end{equation}
...
%% --
\begin{proof}
We recall that a closed subspace $I \subseteq E$ is invariant for a semigroup generated by $C$ if and only if\/ $C(D(C)\cap I) \subseteq I$ and the restriction $C_{|I}$ of\/ $C$ with domain $D_{|I} \coloneqq D(C)\cap I$ generates a semigroup on $I$ (see Part-I, Section~\ref{subsec:a1-3.3}).
...
\begin{proof}
    ...
    We define $j \colon C(\Gamma) \to E$ by $j(f) \coloneqq f\circ h \in C(K) \cong E_{h} \subseteq E$, then $j$ is injective.
...
If\/ $s(A)$ is a pole of finite algebraic multiplicity, then there exists a finite chain of\/ $\TT$-invariant ideals $I_{-1} \coloneqq  \{0\} \subseteq I_{0}\subseteq \ldots \subseteq I_{N} \coloneqq  E$ $(N\in\N)$ such that the following is true.
%% --
...
$-\mu$ yields $\sigma(A_{|J_{\mu}}) \subseteq \{\lambda \in \C  \colon \Re(\lambda) > \mu\}$. Now the assertion follows
...
Given a Borel set $C \subseteq X$ we write $C(t) \coloneqq  \phi_t^{-1}(C)$.
