...
\begin{proof}
    ...
    Moreover, 
    \[
    \langle h^{[n]},\phi\rangle = \langle i(e_{n}),\phi\rangle = \int_{0}^{2\pi} e_{n}(\eu^{it})\dt  =2\pi\cdot\delta_{n0}\  \text{ for all } n \in \Z\,,
    \] 
    hence $\int_{0}^{2\pi} f(\eu^{it})\dt   = \langle i(f),\phi\rangle$ for all $f \in C(\Gamma)$.
...
\begin{equation}\label{eq:c3-4.6}
	\begin{split} 
		...
		f_t(z) &\coloneqq f(z \cdot \eu^{it}) \quad (z \in \Gamma) \quad \text{and}\\
		g_t(x) &\coloneqq g(x+t) + \frac{1}{2\pi}\int_{0}^{2\pi} f(z \cdot \eu^{is}) \ds  \cdot \int_{x}^{x+t} k(u) \du \,.
	\end{split}
\end{equation}
%% --
