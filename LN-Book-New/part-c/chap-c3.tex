...
\begin{equation}\label{eq:c3-2.23}
    \begin{split}
        L_{p,0}f &\coloneqq \sum_{i,j=1}^{n} a_{ij} f_{ij}'' 
                  + \sum_{i=1}^{n} b_{i} f_{i}' + c\,f, 
                  \quad \text{with domain} \\
        D(L_{p,0}) &\coloneqq \{f \in C^{2}(\bar{\Omega}) \colon 
...
\begin{proof}
    ...
    It follows that $(E,\phi) \cong L^{1}(\Gamma)$, and we define $j$ to be the canonical mapping from $E$ into $(E,\phi) \cong L^{1}(\Gamma)$ (see Section~\ref{sec:c1-4}).
...
\begin{proof}
    ...
    \]
    %% --
	Now, considering the AL-space $(E,\psi)$ (see Section~\ref{sec:c1-4}) the first inequality of Eq.~\eqref{eq:c3-4.1} implies that $(T(t))_{t \geq 0}$ induces a strongly continuous semigroup $(T_{1}(t))_{t \geq 0}$ on $(E,\psi)$.
	That is we have
	%% --
