...
%% --
In Part-II, Chapter 3we showed that positive semigroups on spaces $C_{0}(X)$ possess several interesting spectral properties.
In this chapter we will extend many of these results to the more general setting of Banach lattices, 
improving some considerably and give the complete proof of Theorem~4.1 from Part-2, Chapter 3. 
...
The proof given in Part-II, Chapter 3,  Theorem~1.1 for spaces $C_{0}(X)$ works in the general case too.
Another proof is given below (\cf Corollary~\ref{cor:c3-1.4}).
Furthermore, Corollary~1.3 and Proposition~1.5 of Part-II, Chapter 3are true in the setting of Banach lattices and their proofs can be extended to the general case.
...
\end{theorem}
%% --
The fact that $s(A)$ is always an eigenvalue of the adjoint (\cf Part-II, Chapter 3Theorem~1.6) is characteristic for spaces $C(K)$, $K$ compact, as can be seen by considering the Laplacian on $L^p(\R^n)$, where $1 < p < \infty$, or on $C_{0}(\R^n)$ (see Part-II, Chapter 3,  Example~1.7).
Another result which cannot be extended to arbitrary Banach lattices is that spectral bound and growth bound%
\index{growth bound} 
coincide (\cf Part-II, Chapter 4,  Theorem~1.4); an example is given in Part-I, Chapter 3,  Example~1.3.
...
By Datko's Theorem (Part-I, Chapter 4,  Theorem~1.11) the function $t \mapsto \eu^{-\lambda t}T(t)f$ cannot be Bochner integrable for all $f \in E$ in the case $\Re(\lambda) \leq \omega_0(A)$.
%% --
...
\begin{proof}
	...
	Then, by Part-I, Chapter 1,  Proposition~1.11,
	%% --
...
Thus, by Part-I, Chapter 1, \,Proposition~1.11 we have  $R(\lambda,A)f = \int_{0}^{\infty} \eu^{-\lambda s} T(s)f \ds $\,. 
...
\end{proof}
%% --
The spectrum of a positive semigroup may be empty (see Part-II, Chapter 3,  Example~1.2(i)) and the spectrum of a general group may be empty as well (see \textcite[Section~23.16]{hillephillips:1957}).
...
\index{spectrum!boundary|(} 
%% --
In Part-II, Chapter 3we have seen that under suitable assumptions the boundary spectrum $\sigma_{b}(A)$, consisting of all spectral values with maximal real part is a cyclic set (\cf Part-II, Chapter 3,  Definition~2.5).
...
We first want to extend some of the notions used in Part-II, Chapter 3to the more general setting considered here.
...
\end{definition}
%% --
Obviously, for $E = C_{0}(X)$ this amounts to the same as Part-II, Chapter 3,  Definition~2.2.
...
\end{equation}
%% --
Next we show that Part-II, Chapter 3,  Theorem~2.4 is true for arbitrary Banach lattices.
For definition and simple properties of the sign operator $S_{h}$ see Part-III, Chapter 1,  Section~8.
...
\begin{proof}
	...
	Then the assumption Eq.~\eqref{eq:c3-2.6} implies that $T(t)|h| = |h|$ and $T(t)h = \eu^{\im\beta t}h$ for $t \geq 0$ (see Part-I, Chapter 3,  Corollary~6.4).
	In particular, the principal ideal $E_{|h|}$ is invariant under every operator $T(t)$.
	By the Kakutani-Krein Theorem (Part-III, Chapter 1,  Section~4) we can identify $E_{|h|}$ with a space $C(K)$, $K$ compact.
	...
	From Part-II, Chapter 3,  Theorem~2.4(i) we conclude $\overline{T}(t)\overline{h}^{[n]} = \eu^{\im\beta  t}\overline{h}^{[n]}$ for all $t \geq 0$, $n \in \Z$.
	Translating this back to $T(t)$ and $E$ this means precisely $T(t)h^{[n]} = \eu^{\im n\beta}h^{[n]}$, hence $h^{[n]} \in D(A)$ and $Ah^{[n]} = \im  n\beta h^{[n]}$.
	
	Moreover, by Part-II, Chapter 3,  Theorem~2.4(i) we have $\eu^{\im\beta  t}\overline{T}(t) = S_{h}^{-1}\overline{T}(t)S_{h}$.
...
\end{proof}
%% --
In the proof above we could not apply assertion (ii) of Part-II, Chapter 3,  Theorem~2.4 because the semigroup $(\overline{T}(t))$ on $E_{|h|} \cong C(K)$ need not be strongly continuous with respect to the sup-norm.
...
\index{pseudo-resolvent}
	\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
		...
		\item 
		Suppose $A$ is a densely defined linear operator on $G$ with $\rho(A) \neq \emptyset$ and let $G_{\F}$ be an $\F$-product of\/ $G$ (\cf Part-I, Chapter 1,  Section~3.7).
		...
		If\/ $A$ is unbounded, then $0 \in A\sigma(R(\lambda,A))$ hence $0 \in P\sigma(R_{\F}(\lambda,A))$ (\cf Part-I, Chapter 3,  Section~4.2).
...
\]
%% --
Applying Part-II, Chapter 3,  Lemma~2.3 we obtain $\tilde{R}(\alpha) = S_{h}^{-1}\tilde{R}(\alpha + \im\beta)S_{h}$ for every $\alpha > 0$ and using the uniqueness theorem for holomorphic functions we obtain
%% --
...
\begin{enumerate}[\upshape (i), wide, labelindent=0.5em]
    ...
    A slight modification of Part-I, Chapter 3,  Example 1.3 shows that there are semigroups with slowly growing resolvent and $s(A) < \omega_0(A)$.
\end{enumerate}
\end{remarks*}
%% --
\begin{proof} 
...
Given $\im\beta \in \sigma(A)$ $(\beta \in \R )$, then $\im\beta \in A\sigma(A)$ (Part-I, Chapter 3,  Proposition~2.2) and $(\lambda-\im\beta)^{-1} \in A\sigma(R(\lambda,A))$ (Part-I, Chapter 3,  Proposition~2.5).
...
\end{proof}
%% --
To prove cyclicity of the boundary spectrum in case $s(A)$ is a pole (of arbitrary order), one applies Part-II, Chapter 3,  Lemma~2.8 to reduce the problem to the case of first order poles. Actually, Part-II, Chapter 3,  Lemma~2.8 is true for arbitrary Banach lattices and the proof given in Part-II, Chapter 3works in the general case as well. For completeness we recall this result. 
...
\end{proposition}
%% --
Combining Theorem~\ref{thm:c3-2.10} and Proposition~\ref{prop:c3-2.11} one obtains the following generalization of Part-II, Chapter 3,  Theorem~2.9.
...
%% --
\begin{proof}
Considering the sequence of ideals as described in Proposition~\ref{prop:c3-2.11} and the corresponding generators $A_{n}$ $(0 \leq n \leq k)$, then we have by Part-I, Chapter 3,  Proposition~4.2 that $\sigma_{b}(A) = \cup_{n=1}^{k} \sigma_{b}(A_{n})$.
...
In case that $\sigma_{b}(A)$ is cyclic one has the alternative (\cf Part-II, Chapter 3, \,(2.19))
%% --
\[
\text{Either $\sigma_{b}(A) = \{s(A)\}$ or else $\sigma_{b}(A)$ is an unbounded set.}
\]
%% -- 
Obviously one can use this fact to prove the existence of a dominant spectral value (\cf the explanation preceding Part-II, Chapter 3,  Corollary~2.11).
...
%% --
\begin{proof}
The boundary spectrum $\sigma_{b}(A)$ is cyclic (Theorem~\ref{thm:c3-2.10} and Corollary~\ref{cor:c3-2.12} resp.) and compact (Part-I, Chapter 2,  Theorem~1.20).
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]	
	...
	\item 
    \index{quasi-interior point|(}
	If\/ $s(A)$ is a pole, then the criteria stated in Part-II, Chapter 3,  Remark~2.15(i) for $s(A)$ to be a first order pole are valid in the setting of arbitrary Banach lattices as well.
...
However, some of the (equivalent) assertions stated in Part-II, Chapter 3,  Definition~3.1 do not make sense here, others need a slightly different formulation.
%% --
...
\end{definition}
%% --
Equivalence of the conditions above is obtained by a slight modification of the arguments given in Part-II, Chapter 3,  Definition~3.1.,
...
This is the second statement of the following theorem (see also Part-II, Chapter 3,  Remark~3.2).
...
%% --
The next result can be used to check irreducibility for a semigroup whose generator is a bounded perturbation of a known semigroup. It is a generalization (and an extension to Banach lattices) of Part-II, Chapter 3,  Proposition~3.3. 

%% --
\begin{proposition}\label{prop:c3-3.3}
	\index{semigroup!bounded perturbation}
	Suppose that $A$ is the generator of a positive semigroup $\TT$, further assume that $K$ is a bounded positive operator and $M$ is a bounded real multiplier (\cf Part-III, Chapter 1,  Section~8).
...
Note that each closed ideal is invariant for $M$, \cf Part-III, Chapter 1,  Section~8.
...
\begin{examples}\label{ex:c3-3.4}
	...
	Then the statements made in Part-II, Chapter 3,  Example~3.4(c) are true.
	The same is true for Part-II, Chapter 3,  Example~3.4(iii) and (v) (second order differential operator) when the corresponding $L^{p}$-spaces are considered.
	...
	is a core%
    \index{core} 
    for its generator $A$ (\cf Part-I, Chapter 1,  Corollary~1.34).
	We have
	%% --
...
In the following proposition we list some properties which are consequences of irreducibility. This extends Proposition~3.5 of Part-II, Chapter 2 to the setting of Banach lattices. The first assertion of the latter proposition is no longer true in the general setting (see Example~\ref{ex:c3-3.6} and Theorem~\ref{thm:c3-3.7}). 
...
%% --	
\begin{proof}
    To prove (i), (ii) and (iv) one can proceed as in the case $C_{0}(X)$ (see Part-II, Chapter 3,  Proposition~3.5).
...
%% --
\begin{proof}
	(i) is proved in Part-II, Chapter 3,  Proposition~3.5(i).
	
	Assertion (ii)-(f) will be proved utilizing the analogous results for a single operator.
	In view of Part-I, Chapter 3,  Proposition~2.5 we have to show that $r(R(\lambda,A)) > 0$ for some $\lambda \in \rho(A)$.
...
Concerning the eigenvalues of an irreducible semigroup which are contained in $\sigma_{b}(A)$ all statements established for spaces $C_{0}(X)$ in Part-II, Chapter 3,  Theorem~3.6 are true in the setting of Banach lattices.
...
%% --
\begin{proof}
    By Theorem~\ref{thm:c3-3.8} and Part-I, Chapter 3,  Theorem~5.4 we have $R\sigma(A) = P\sigma(A) = \sigma(A) = \im\alpha\Z$ with $\alpha \coloneqq \frac{2\pi}{n\tau}$ for suitable $n \in \N$.
    ...
    Then $h$ is a unimodular function onto $\Gamma$ (use the argument given in the proof of Part-II, Chapter 3,  Theorem~3.6(iii)).
    
    We define $i \colon C(\Gamma) \to E$ by $i(f) \coloneqq f\circ h \in C(K) \cong E_{h} \subset E$, then $i$ is injective.
    For the monomials $e_{n}(z) \coloneqq z^{n}$ $(n \in \Z)$ we have $i(e_{n}) = h^{[n]}$ thus $i$ has dense image in $E$ (by Part-I, Chapter 3,  Theorem~5.4).
    Moreover, 
    \[
    ...
    It follows that $(E,\phi) \cong L^{1}(\Gamma)$, and we define $j$ to be the canonical mapping from $E$ into $(E,\phi) \cong L^{1}(\Gamma)$ (see Part-III, Chapter 1,  Section~4).
...
\end{proof}
%% --
In view of Part-I, Chapter 3,  Proposition~4.2 the result above has consequences for semigroups which can be reduced (by considering restrictions to invariant ideals or quotients) to semigroups which satisfy the hypothesis of Theorem~\ref{thm:c3-3.12}. 
...
\end{equation}
%% --
The following theorem is an immediate consequence of Eq.~\eqref{eq:c3-3.19}, Theorem~\ref{thm:c3-3.12} and Part-I, Chapter 3,  Proposition~4.2.
...
The meaning of\/ $r_{ess}(T(t))$ and $\omega_{ess}(\TT)$ and its properties are explained in Part-I, Chapter 3,  Example~3.7.
\end{example}
...
\begin{proof}
	...
	In view of Part-I, Chapter 3,  Corollary~6.5 the set $\{\lambda \in \sigma(A) \colon \Re(\lambda) > s(A)-\epsilon\}$ is finite and contains only poles of\/ $R(\cdot,A)$. 
...
\begin{proof}
	...
	In addition $\{\lambda \in \sigma(A) \colon \Re(\lambda) \geq -1\}$ is compact since $T$ is eventually norm-continuous (see Part-I, Chapter 2,  Theorem~1.20). 
	It follows that $s(A)$ is strictly dominant.
	By Part-I, Chapter 3,  Theorem~6.6 we have $s(A) = \omega_0(\TT)$.
	%$\square$
\end{proof}
...
It follows from Proposition~\ref{prop:c3-3.3} that $(S(t))$ is irreducible and we have $S(t) = T(t) + \int_{0}^{t} T(t-s)KS(s)\ds$ (see Part-I, Chapter 2, \,(1.9)). 
...
\begin{proof}
    ...
    \]
    %% --
	Now, considering the AL-space $(E,\psi)$ (see Part-III, Chapter 1,  Section~4) the first inequality of Eq.~\eqref{eq:c3-4.1} implies that $(T(t))_{t \geq 0}$ induces a strongly continuous semigroup $(T_{1}(t))_{t \geq 0}$ on $(E,\psi)$.
	That is we have
	%% --
...
% 	\]
	...
	Since the toplogical boundary of the spectrum is always contained in the approximate point spectrum (see Part-I, Chapter 3,  Proposition~2.2) and 
    \index{approximate point spectrum $A\sigma$}
    $R\sigma(T(t))\backslash\{0\} = \exp(tR\sigma(A))$ (see Part-I, Chapter 3,  Theorem~6.3), precisely one of the following two cases occurs.
	\begin{enumerate}[\upshape (A)]
		\item
...
\begin{proof}
	...
	We use the notation of Part-I, Chapter 1,  Section~3.7. 
	...
	It follows that $P\sigma(A_{\mathcal{F}})$ is cyclic, hence $A\sigma(A)$ is cyclic by Part-I, Chapter 3,  Section~4.2.
...
\begin{proof}
	...
	Now we assume that $E$ is weakly sequentially complete, which is equivalent to (\cf Section~5 of Part-III, Chapter 1)
	%% --
...
Then by Part-I, Chapter 3,  Theorem~6.3 there exists
...
\index{spectral decomposition} 
 of the semigroup (\cf Part-I, Chapter 3,  Definition~3.1) (One simply performs a spectral decomposition of a single operator $T(t)$). 
...
from Part-I, Chapter 3,  Proposition~4.2.
%$\square$
\end{proof}
...
Thus the assertion follows from Part-I, Chapter 3,  Section~4.1.
\end{proof}

We conclude by describing a general situation of lattice semigroups. 
In Section 4 of Part-II, Chapter 3we constructed semigroups of lattice
...
\index{core} 
(by Part-I, Chapter 1,  Proposition~1.9).
\end{enumerate}
...
All these properties were proven in Example~3.15 of Part-II, Chapter 2.
