...
\index{growth bound} 
coincide (\cf B-IV, Theorem~1.4); an example is given in A-III, Example~1.3.
...
By Datko's Theorem (A-IV, Theorem~1.11) the function $t \mapsto \eu^{-\lambda t}T(t)f$ cannot be Bochner integrable for all $f \in E$ in the case $\Re(\lambda) \leq \omega_0(A)$.
%% --
...
Thus, by A-I,\,Proposition~1.11 we have  $R(\lambda,A)f = \int_{0}^{\infty} \eu^{-\lambda s} T(s)f \ds $\,. 
...
\end{proof}
%% --
The spectrum of a positive semigroup may be empty (see B-III, Example~1.2(i)) and the spectrum of a general group may be empty as well (see \textcite[Section~23.16]{hillephillips:1957}).
...
For definition and simple properties of the sign operator $S_{h}$ see C-I, Section~8.
...
\begin{proof}
	...
	Then the assumption \eqref{eq:c3-2.6} implies that $T(t)|h| = |h|$ and $T(t)h = \eu^{\im\beta t}h$ for $t \geq 0$ (see A-III, Corollary~6.4).
	...
	From B-III, Theorem~2.4(i) we conclude $\overline{T}(t)\overline{h}^{[n]} = \eu^{\im\beta  t}\overline{h}^{[n]}$ for all $t \geq 0$, $n \in \Z$.
	Translating this back to $T(t)$ and $E$ this means precisely $T(t)h^{[n]} = \eu^{\im n\beta}h^{[n]}$, hence $h^{[n]} \in D(A)$ and $Ah^{[n]} = \im  n\beta h^{[n]}$.
	
	Moreover, by B-III, Theorem~2.4(i) we have $\eu^{\im\beta  t}\overline{T}(t) = S_{h}^{-1}\overline{T}(t)S_{h}$.
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]	
	...
	\item 
    \index{quasi-interior point|(}
	If\/ $s(A)$ is a pole, then the criteria stated in B-III, Remark~2.15(i) for $s(A)$ to be a first order pole are valid in the setting of arbitrary Banach lattices as well.
...
\begin{examples}\label{ex:c3-3.4}
	...
	The same is true for B-III, Example~3.4(iii) and (v) (second order differential operator) when the corresponding $L^{p}$-spaces are considered.
	...
	is a core%
    \index{core} 
    for its generator $A$ (\cf A-I, Corollary~1.34).
	We have
	%% --
...
%% --
\begin{proof}
	(i) is proved in B-III, Proposition~3.5(i).
	...
	Moreover, from A-I,\,(1.3) we deduce
	%% --
	\begin{align*}
...
\end{theorem}
%% -- 
One can apply the theorem in order to prove that the rotation semigroup on $\Gamma$ (\cf A-I,\,2.5) is the only positive periodic semigroup%
...
\begin{proof}
	...
	\end{equation}
	%% --
	In fact, A-I,\,(1.3) implies $(\eu^{rt} - T(r))R(r,A) \geq 0$ hence 
    \[
...
% 	\]
	...
	Since the toplogical boundary of the spectrum is always contained in the approximate point spectrum (see A-III, Proposition~2.2) and 
    \index{approximate point spectrum $A\sigma$}
    $R\sigma(T(t))\backslash\{0\} = \exp(tR\sigma(A))$ (see A-III, Theorem~6.3), precisely one of the following two cases occurs.
	\begin{enumerate}[\upshape (A)]
		\item
...
\index{spectral decomposition} 
 of the semigroup (\cf A-III, Definition~3.1) (One simply performs a spectral decomposition of a single operator $T(t)$). 
...
(see Section~5 of C-II). As a consequence of C-II, Corollary~5.8 we have the following.
...
$\int_{0}^{t} \eu^{-\mu s}T(s)h \ds = (\Id - \eu^{-\mu t}T(t))R(\mu,A)h$ (see A-I,\,(3.2)).
%
