...
%
	...
	\begin{equation}\label{eq:c3-3.2}
		A_{g}f \coloneqq f', \quad 
		D(A_{g}) \coloneqq \mleft\{f \in W^{1}[-1,0] \colon f(0) = \int_{-1}^{0} f(x)g(x)\dx \mright\}
	\end{equation}
	%% --
...
\begin{equation}\label{eq:c3-3.10}
	m_{t}(z,w) \coloneqq \exp\mleft(-\int_{0}^{t} m(z\cdot \eu^{\im\alpha s},w\cdot \eu^{\im\beta s})\ds \mright) .
\end{equation}
%% --
...
\]
%% --
where the kernel $k$ is given by $k(x,v,x',v') \coloneqq  \kappa(x,v,v')r_{\lambda}(x,x',v')$ (\cf Eq.~\eqref{eq:c3-3.5} and Eq.~\myeqpageref{eq:c3-3.7}). 
