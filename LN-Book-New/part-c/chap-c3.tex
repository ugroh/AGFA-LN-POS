...
%% --
	...
	It follows from \eqref{eq:c3-1.3} at the net $(\int_{0}^t \eu^{-\mu s}T(s) f \ds )_{t\geq 0}$ converges weakly to $R(\mu,A)f$ for $t \to \infty$.
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
	...
	For details we refer to Part-III, Chapter 4,  Theorem~1.1.
\end{enumerate}
\end{remarks}
...
\begin{proof}
	...
	Then the assumption \eqref{eq:c3-2.6} implies that $T(t)|h| = |h|$ and $T(t)h = \eu^{\im\beta t}h$ for $t \geq 0$ (see Part-I, Chapter 3,  Corollary~6.4).
...
Applying Pringsheim's Theorem (see Theorem~2.1 in the appendix of \textcite{schaefer:1966}) to the expansion given in \eqref{eq:c3-2.11} one can conclude that $R$ has an extension to the half-plane $\{z \in \C  \colon \Re  z > 0\}$.
...
\end{equation}
%% --
The following theorem is an immediate consequence of \eqref{eq:c3-3.19}, Theorem~\ref{thm:c3-3.12} and Part-I, Chapter 3,  Proposition~4.2.
...
\begin{proof}
    ...
    \]
    %% --
	Now, considering the AL-space $(E,\psi)$ (see Part-III, Chapter 1,  Section~4) the first inequality of \eqref{eq:c3-4.1} implies that $(T(t))_{t \geq 0}$ induces a strongly continuous semigroup $(T_{1}(t))_{t \geq 0}$ on $(E,\psi)$.
	That is we have
	%% --
...
% 	\]
	...
	Indeed, assuming $A_{1}^*x = \lambda x$ then $T_{1}(t)'x = \eu^{\lambda t}x$, hence  $T(t)'q_{\psi}'(x) = \eu^{\lambda t}q_{\psi}'(x)$ (by \eqref{eq:c3-4.2}) or equivalently, $A^*(q_{\psi}'(x)) = q_{\psi}'(x)$. 
	...
	The third inequality in \eqref{eq:c3-4.1} implies that $\phi$ can be considered as an element of\/ $(E,\psi)'$ and $T(t)'\phi = \eu^{\im\alpha t}\phi$ $(t \geq 0)$ implies $T_{1}(t)'\phi = \eu^{\im\alpha t}\phi$. 
	Furthermore, we have
...
(see Section~5 of Part-3, Chapter 2 ). As a consequence of Part-III, Chapter 2,  Corollary~5.8 we have the following.
...
and growth bound%
\index{growth bound} 
coincide (see Part-III, Chapter 4,  Theorem~1.1).
