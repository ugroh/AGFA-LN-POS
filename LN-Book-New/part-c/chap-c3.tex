...
Thus in Eq.~\myeqpageref{eq:c3-2.15}~(a) the essential fact is boundedness near $s(A)$. 
    ...
    %
\[
	\{\frac{1}{t} \int_{0}^{t} \exp(-\tau s(A))T(\tau) \diff{\tau} \colon 0 \leq t \leq t_{0} \}
\]
%
...
%
	...
	%% --
    \begin{align*}
    \alpha & \coloneqq \sup\left\{x \in [0,1] \colon \int_{0}^{x} \1_{X}(s)\ds  = 0\right\}, \\
	 \beta & \coloneqq \inf\left\{x \in [0,1] \colon \int_{x}^{1} \1_{X}(s)\ds  = 0 \right\}.
    \end{align*}
    %% .. 
...
\begin{proof}
    ...
    We define $j \colon C(\Gamma) \to E$ by $j(f) \coloneqq f\circ h \in C(K) \cong E_{h} \subset E$, then $j$ is injective.
    For the monomials $e_{n}(z) \coloneqq z^{n}$ $(n \in \Z)$ we have $j(e_{n}) = h^{[n]}$ thus $j$ has dense image in $E$ (by Part-I, Theorem~\ref{thm:a3-5.4}).
    Moreover, 
    \[
...
\begin{split}
		&P_{\mathcal{U}} = \hat{\phi}\otimes\hat{u} \text{ where } \hat{u} \coloneqq (u,u,u,\ldots)+c_{\mathcal{U}}(E) \in E_{\mathcal{U}} \ \text{ and } \hat{\phi} \in (E_{\mathcal{U}})'
		\\
	...
	%
\[
	M \coloneqq \ker(1 - \lambda R(\lambda+\im\beta,A)_{\mathcal{U}})
\]
%
...
We denote this ideal $J_{k}$ and define $J\coloneqq  J_{1}\oplus J_{2}\oplus \ldots \oplus J_{m}$.
The ideal $J$ is closed, $\TT$-invariant and we have $PE = \ker A \subset J$ (see \textcite[III Theorem~1.2]{schaefer:1974}). 
...
If\/ $s(A)$ is a pole of finite algebraic multiplicity, then there exists a finite chain of\/ $\TT$-invariant ideals $I_{-1} \coloneqq  \{0\} \subset I_{0}\subset \ldots \subset I_{N} \coloneqq  E$ $(N\in\N)$ such that the following is true.
%% --
...
%
\[
		\sigma_{b}(A) = s(A) + \cup_{k=1}^{N}\im\alpha_{k}\Z \text{ with $\alpha_{k} \in \R$.}
\]
%
...
$B \coloneqq A + K$ satisfies the assumptions of Theorem~\ref{thm:c3-3.14}.%
\item
...
\begin{split}
    ...
    R_{m}(t) &\coloneqq \textit{\small $\int_{0}^{t}\int_{0}^{t_{1}} \ldots \int_{0}^{t_{m-1}} T(t-t_{1})KT(t_{1}-t_{2})K \ldots T(t_{m-1}-t_m)KS(t_{m})\dt _{m}\ldots \dt _{1}$}\,.
\end{split}
\end{equation}
%% --
...
Thus the integrand appearing in the the representation Eq.~\myeqpageref{eq:c3-3.21} of\/ $\langle R_{m}(t)f,\phi\rangle$ is non-zero at $t_{1}=t_{2}= \ldots =t_{m-1}=t$, $t_{m}= t$.
...
\[
    	\|T(s)f\| \geq M^{-1}\|T(t)f\|
\]
for  $0 \leq s \leq t$, $f \in E$ and $ M \coloneqq \sup_{ 0 \leq s \leq t} \|T(s)\|$.
