...
By Proposition~\ref{prop:c3-2.6}\,(i) and Part-I, Section~\ref{subsec:a3-4.5} there exists $h_{1} \in E_{1}$, $h_{1} \neq 0$ such that
%% --
...
%% --
\begin{proof}
We recall that a closed subspace $I \subset E$ is invariant for a semigroup generated by $C$ if and only if\/ $C(D(C)\cap I) \subset I$ and the restriction $C_{|I}$ of\/ $C$ with domain $D_{|I} \coloneqq D(C)\cap I$ generates a semigroup on $I$ (see Part-I, Section~\ref{subsec:a1-3.3}).
...
\begin{proof}
	...
	Assertion (ii)-(v) will be proved utilizing the analogous results for a single operator.
	In view of Part-I, Proposition~\ref{prop:a3-2.5} we have to show that $r(R(\lambda,A)) > 0$ for some $\lambda \in \rho(A)$.
	Moreover, from Eq.~\myeqpageref{eq:a1-1.3} we deduce
	%% --
	\begin{align*}
...
\begin{proof}
	...
	\end{equation}
	%% --
	In fact, Eq.~\myeqpageref{eq:a1-1.3} implies $(\eu^{rt} - T(r))R(r,A) \geq 0$ hence 
    \[
...
Retarded equations as dicussed in Example~\ref{ex:c3-3.4}\,(iii) will be discussed in more detail in 
