...
\[
	f_{n} \coloneqq T(t_{n})f 
	\quad \text{and} \quad 
	g_{n} \coloneqq f - \sum_{k=n}^{\infty}(f-f_{k})^{+}.
\]
%
...
\begin{examples}\label{ex:c3-3.4}
	...
	Then $(T(t))$ is irreducible if and only if for any two measurable sets $M$ and $N$ with $0 < \mu(M) < \infty$, $0 < \mu(N) < \infty$ and $\mu(M\cap N) = 0$ there exist $t_{0} > 0$ such that 
	%
\[
...
\begin{proof}
	...
	Given an ultrafilter $\mathcal{U}$ on $\N$ which is finer than the Frechet filter, then $\lim_{\mathcal{U}}\phi(f_{n})$ exists for every bounded sequence $(f_{n})$ in $E$.
