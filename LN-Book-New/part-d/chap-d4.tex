...
%% --
\begin{proof}
Consider $\Omega \coloneqq P\sigma(A) \cap \im\R$ which is not empty by assumptions.
...
Let $\phi$ be the faithful normal state generating $\Fix{T}$ and let $\UG$ be a family of unitary eigenvectors of\/ $A'$ pertaining to the eigenvalues in $\Omega$ (see Remark~\ref{rem:d3-1.12}).
...
%% --
\[
(T_{\tau}(t)f)(\xi) = f\left( \exp\left(\frac{2\pi \im \, t }{\tau} \right) \cdot \xi \right) \quad (f \in L^{1}(H,\dm), \xi \in H).
\]
%% --
%
%\[
...
It is called \emph{partially periodic}, if there exists a projection $ Q \in L(M_{*}) $  reducing $ \TT $  such that $ Q(M_{*}) \cong L^{1}(H,\dm) $  and $ \TT_{| \Image{Q}} $  is conjugate to a periodic semigroup on $ L^{1}(H,\dm) $.
...
\item
The restriction $ T_{0} $  of\/ $ \TT $  to $ \Image{Q} $  is irreducible and conjugate to a rotation semigroup of period $ \tau = \frac{2\pi}{\gamma} $  on $ H $.
...
For $ \tau \coloneqq \frac{2\pi}{\gamma} $, we obtain $ T(\tau)u_{\gamma}^{k} = u_{\gamma}^{k} $, hence $ T|_{\Image{Q}} $  is periodic.
From the Halmos-von Neumann theorem (see \textcite[Theorem III.7.11]{schaefer:1974})
it follows that $ \TT|_{\Image{Q}} $  is conjugate to the rotation semigroup of period $ \tau $  on $ L^{1}(H,m) $.
\end{proof}
%% --
