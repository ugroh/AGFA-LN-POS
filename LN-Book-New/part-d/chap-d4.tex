...
While the growth bound $\omega_{0}$ characterizes uniform exponentially stability of the semigroup there are other (and weaker) stability concepts (\cf Part-I, Section~\ref{sec:a4-1}).
\end{remark}
%% --
...
\item
The maps $T(t)$ converge strongly to $P$ (see Propositions~\ref{prop:d4-3.7}, \ref{thm:d4-3.8} \&. \ref{cor:d4-3.9}).
...
Since $T(t)|\phi| = |\phi|$ for all $\phi \in \Fix{T}$ (Corollary~\ref{cor:d3-1.5}), we obtain $T(t)'p_{r} \geq p_{r}$ (see Section~\ref{sec:d1-3}\,(3)).
...
%% --
\begin{proof}
Consider $\Gamma \coloneqq P\sigma(A) \cap \im\R$ which is not empty by assumptions.
From Proposition 3.1 it follows that $\TT$ is relatively compact in the weak operator topology.
Let $K$ be the semigroup kernel of\/ $\overline{\TT}{w} \subset \mathcal{L}{(M_{*})}$ and $Q$ the unit of\/ $K$.
Recall that $Q\psi_{\gamma} = \psi_{\gamma}$ for all $\psi_{\gamma} \in M_{*}$ such that $A\psi_{\gamma} = \gamma\psi_{\gamma}$ $(\gamma \in \Gamma)$.
...
If\/ $\gamma \in \Gamma$ and $\psi_{\gamma} \in M_{*}$ such that $A\psi_{\gamma} = \gamma\psi_{\gamma}$, then for all $x \in M$ we obtain
%% --
\[
\langle \psi_{\gamma} ,x \rangle = \langle Q\psi_{\gamma},x \rangle = \lim_{\mathfrak{U}} \langle T(t)\psi_{\gamma},x \rangle = 
(\lim_{\mathfrak{U}} \eu^{\gamma}) \langle \psi_{\gamma}, x \rangle,
\]
%% --
hence $\lim_{\mathfrak{U}} \eu^{\gamma t} = 1$.
From this it follows that for all $\psi \in M_{*}$ we have
%% --
\[
\langle\psi,Q'(u_{n})\rangle = \lim_{\mathfrak{U}} \langle\psi,T(t)'u_{\gamma}\rangle 
= (\lim_{\mathfrak{U}} \eu^{\gamma t})\langle\psi,u_{\gamma}\rangle 
 \langle\psi,u_{\gamma}\rangle.
\]
%% --
Hence $N \subseteq Q'(M)$.

For $\alpha$ in the dual group of\/ $K$ and $x \in M$ we define $x_{\alpha}$ by
%% --
\[
\psi(x_{\alpha}) \coloneqq \int_{K} 
	\langle S\psi ,x \rangle \, \overline{\langle S,\gamma \rangle}  \dm(S) \quad (\psi \in M_{*}^{+}).
\]
%% --
Then $x_{\alpha} \in M$ and $T(t)'x_{\alpha} = \langle QT(t),\alpha\rangle x_{\alpha}$.
Therefore $x_{\alpha} \in N$.
...
%% --
\[
\int_{K} \langle S\psi,x \rangle \, \overline{\langle S,\gamma \rangle} \dm (S) = 0, \gamma \in K.
\]
%% --
...
\item
Let us add a few words concerning the structure of\/ $N$: If\/ $\TT$ is irreducible and $K$ is the semigroup kernel of\/ $\TT^{-} \subseteq L_{w}(M_{*})$, then $(S \mapsto S'): K \to \mathcal{L}((N,\sigma(N,N_{*})))$ is a representation of the compact, Abelian group $K$ as group of *-automorphism such that the fixed space is one dimensional.
...
\]
%
Tand let $ \{p_{i}: i \in A\} $  be an increasing net of projections of finite rank in $ \BH $  with strong limit $\1$.
...
%% --
\[
(T_{\tau}(t)f)(\xi) = f\left( \exp\left(\frac{2\pi \im \, t }{\tau} \right) \cdot \xi \right) \quad (f \in L^{1}(\Gamma,\dm), \xi \in \Gamma).
\]
%% --
%
%\[
%(T_{\tau}(t)f)(\xi) = f\!\left(\xi\exp\!\left(\frac{2\pi\im}{\tau}t\right)\right)
%\quad (f \in L^{1}(\Gamma,\dm),\ \xi \in \Gamma).
%\]
%
...
%% --
Since $ \TT $  is periodic of period $ \tau $, it follows that 0 is a pole of the resolvent of its generator $ B $  with residuum $ P = \1 \otimes \1 $  and %
\[
...
Using the result in 
Part-I, Corollary~\ref{cor:a3-7.11} it follows that
%% --
\[
...
Since $ \widehat{M}_{1}^{+} $  is $ \sigma(\widehat{M},\widehat{M}') $-dense in $ (\widehat{M}'')_{1}^{+} $  (Kaplansky's Density Theorem, see \textcite[1.9.1]{sakai:1971} and \textcite[1.8.9 and 1.8.12]{sakai:1971}), there exists for all $ n \in \N $  a net $ (z_{n,\gamma}) $  in $ \widehat{M}_{1}^{+} $  such that
%% --
\[
\sigma(\widehat{M}'',\widehat{M}')\text{-}\lim_{\gamma} \widehat{z}_{n,\gamma} = s(\widehat{\psi}_{n}).
\]
%% --
From \textcite[1.7.8]{sakai:1971} and the above considerations, we obtain that the net $ (p\widehat{z}_{n,\gamma}\widehat{p}) $  converges to $ s(\widehat{\psi}_{n}) $  in the $ \sigma(\widehat{M}'',\widehat{M}') $-topology.
Therefore we may assume $ \widehat{z}_{n,\gamma} \in (\widehat{M}_{p}')_{1}^{+} $.
...
For all $ n \in \N $  choose $ \widehat{z}_{n} \in (\widehat{M}_{p}')_{1}^{+} $  such that
%% --
\begin{align*}
|\langle\widehat{\phi},s(\widehat{\psi}_{n}) - \widehat{z}_{n}\rangle| &\leq \frac{1}{2}r_{n}, \\
|\langle\widehat{\psi}_{n},s(\widehat{\psi}_{n}) - \widehat{z}_{n}\rangle| &\leq \frac{1}{2}r_{n}.
\end{align*}
%% --
Hence $ \widehat{\phi}(\widehat{z}_{n}) \leq r_{n} $  and $ \widehat{\psi}_{n}(\widehat{z}_{n}) \geq \frac{1}{2} $  for all $ n \in \N $.
For every $ n \in \N $  let $ (z_{n,k}) \in \widehat{z}_{n} $  be a representing sequence in $ (M_{p})_{1}^{+} = p(M_{1}^{+})p $  (note that $ {M}_{\widehat{p}} = \widehat{M_{p}} $ ) and fix $ \mu \in \R_{+} $.
Since 
%% --
\[
\text{$\mu R(\mu)'\widehat{\psi}_{n} = \widehat{\psi}_{n}$, 
$\widehat{\phi}(\widehat{z}_{n}) \leq r_{n}$ and 
$\widehat{\psi}_{n}(\widehat{z}_{n}) \geq \frac{1}{2}$, }
\]
%% --
...
\item
Theorem~\ref{thm:d4-2.2} generalizes the Liapunov stability theorem from the matrix algebra $ M_{n}(\C) $  to arbitrary \WA-algebras.
