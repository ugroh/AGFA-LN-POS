...
Using \citet[Theorem III.4.2 and Theorem II.2.6]{takesaki:1979}, we conclude as in the implication from (c) to (e) that for all $\xi,\eta \in H$ the integral
%% --
\[
...
Thus $\TT$ is strongly ergodic (\citet[Chap. 2, Theorem 1.4]{krengel:1985}).
\end{proof}
%% --
...
\]
%% --
contains a unique minimal ideal $\mathcal{K}$, called the kernel of\/ $\mathcal{S}$, which is a compact Abelian group (\citet{deleeuw:1961}, \citet{junghenn:1971} \& \citet[§ 2.4]{krengel:1985}).
...
From the Halmos-von Neumann theorem (see \citet[Theorem III.7.11]{schaefer:1974})
...
Therefore condition \ref{item:d4-4.3-iip} implies that $ \lim_{n} z_{n} = 0 $  in the $ s^{*}(M_{p},(M_{p})_{*}) $-topology (\citet[Proposition III.5.4]{takesaki:1979}).
Since 
%
\[
...
Therefore the fixed space of\/ $ \TT' $  separates the points of\/ $ \Fix{\TT''} $, hence $ \TT' $  is strongly ergodic on $ M $  (\citet[Chap.2, Theorem~1.4]{krengel:1985}).
...
Finally, the characterization of convergence towards a periodic semigroup through spectral properties of the generator---Theorem~\ref{thm:d4-3.11}---is due to \citet{nagel:1984} in the commutative case, \ie in $ L^{1}(\mu) $ (see also C-IV, Theorem~2.14).
%\medskip
