...
\item
The equality $s(A) = \omega_{0}$ still holds for positive semigroups on commutative \CA-algebras without unit (see Part-II, Chapter 4,  Remark~1.2.b).
\end{enumerate}
\end{remark}
%% --
...
\]
%% --
which follows as in Part-III, Chapter 3,  Section 1 or \textcite[Theorem 3]{greinervoigtwolff:1981}.
...
From this the assumption follows by Part-I, Chapter 4,  Theorem~1.11.
\end{proof}
%% --
...
For the discussion of the commutative situation we refer to Part-II, Chapter 4,  Section 1.
\end{remark}
...
Since $T(t)|\phi| = |\phi|$ for all $\phi \in \Fix{T}$ (Part-IV, Chapter 3,  Corollary~1.5), we obtain $T(t)'p_{r} \geq p_{r}$ (see Part-IV, Chapter 1, Sec.3.(c)).
...
Since $s(|\phi|)$ and $s(|\phi^{*}|)$ are majorized by $p_{r}$ (Part-IV, Chapter 3,  Proposition~1.4), it follows that $\phi$ and $\phi^{*}$ are in $p_{r}M_{*}p_{r}$.
...
%% --
are Schwarz maps.
We omit the simple calculation (compare Part-IV, Chapter 1,  Theorem~2.1).
%% --
...
Since $\Fix{\TT} \neq \{0\}$, there exists a faithful normal state $\phi$ on $M$ such that $\Fix{\TT} = \C \,\phi$ (Part-IV, Chapter 3,  Theorem~1.10.).
...
\end{proof}
%% --
The next theorem is an extension of Part-IV, Chapter 3,  Theorem~1.10 and shows the usefulness of the theory of semitopological semigroups.
...
Thus $N$ is a \WA-subalgebra of\/ $M$, \textcite[Corollary 1.7.9.]{sakai:1971}, and $\TT_{0}(t)' \coloneqq T(t)'_{|N}$ is multiplicative (for this see Part-IV, Chapter 3,  Lemma 1.1).
...
Let $\phi$ be the faithful normal state generating $\Fix{T}$ and let $\UG$ be a family of unitary eigenvectors of\/ $A'$ pertaining to the eigenvalues in $H$ (see Part-IV, Chapter 3,  Remark 1.11).
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
\item
Since $QM_{*} = N_{*}$ and $Q'M = N$, where $N_{*}$ is as in Part-IV, Chapter 3,  Proposition 1.12, it follows from general duality theory that $(N_{*})' = N$.

\item
If\/ $\psi \in N_{*}$, then $|\psi| \in N_{*}$.
To see this, note that $Q\psi = \psi$ and $Q$ is an identity preserving Schwarz map.
Then the assertion follows from Part-IV, Chapter 3,  Proposition 1.4.
...
But if\/ $ 0 \neq \im\alpha \in P\sigma(A) \cap \im\R $  then $ \im\alpha\Z \subset P\sigma(A) $  by Part-IV, Chapter 3, \,Theorem~1.10, whence $ N $  must be infinite dimensional.
...
%% --
\begin{proof}
By Part-IV, Chapter 3,  Theorem~1.11 and Part-IV, Chapter 3,  Theorem~2.5 it follows that
%% --
\[
...
If\/ $ H_{0} \coloneqq \Kern{Q} \subseteq H_{\phi} $,  then it is easy to see that $ H_{0} $  is invariant under the extension to $ H_{\phi} $  and for the multiplication maps we defined in Part-IV, Chapter 3,  Remark 1.3.
...
Let $ \RR_{1} $  be the induced pseudo-resolvent on $ pM_{*}p $  (Part-IV, Chapter 1,  Section 3.3).
...
Consequently it follows from Part-IV, Chapter 3,  Lemma 1.1.(ii) and the $ \sigma(M_{p},(M_{p})_{*}) $-continuity of\/ $ \mu R_{1}(\mu)' $  that $ \Fix{R_{1}'} $  is a \WA-subalgebra of\/ $ M_{p} $  and by Part-IV, Chapter 3,  Lemma 1.5, $ \dim \Fix{\RR} \leq \dim \Fix{R_{1}'} $.
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
\item
If\/ $ \dim \Fix{\RR} \geq 2 $  then the Jordan decomposition of self adjoint linear functionals implies that at least two states in $ \Fix{\RR} $  have orthogonal support (compare Part-IV, Chapter 3,  Theorem 1.10.(i)).

\item
If\/ $ \RR $  is a pseudo-resolvent with values in a \WA-algebra such that $ \Fix{\RR'} $  is contained in $ M_{*} $,  then by Part-IV, Chapter 3,  Lemma 1.2, there exists a sequence of normal states in $ \Fix{\RR'} $  with orthogonal supports in $ M $.
\end{enumerate}
\end{remark}
%% --
...
(see Part-IV, Chapter 3,  Remark 2.4.(ii)), there exists a sequence of states $ (\hat{\psi}_{n}) $  in $ \Fix{\RR}^{\wedge} $  such that the corresponding support projections are mutually orthogonal in $ \widehat{M} $  (Lemma~\ref{lem:d4-4.1}).
...
\index{pseudo-resolvent|)}
%% --
The next corollary is an easy application of\/ Theorem~\ref{thm:d4-4.4} and of Part-IV, Chapter 3,  Proposition 2.3.
%% --
...
It follows from Part-IV, Chapter 3,  Proposition 2.3 that 0 is a pole of the resolvent of\/ $ R(\cdot,A) $.
...
We refer to \textcite{battyrobinson:1984} for this more general setting and to Part-II, Chapter 4,  Section 1 for the analogous results on $ C_{0}(X) $.
...
Finally, the characterization of convergence towards a periodic semigroup through spectral properties of the generator---Theorem~\ref{thm:d4-3.11}---is due to \textcite{nagel:1984} in the commutative case, \ie in $ L^{1}(\mu) $ (see also Part-III, Chapter 4,  Theorem~2.14).
%\medskip
