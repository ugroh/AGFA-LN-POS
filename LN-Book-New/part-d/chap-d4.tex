...
Then for $t > 0$ either $r(T(t)) = 0$ (A-III, Proposition~1.1) or $T(t)'\phi = 0$, in particular $\phi(T(t)\1) = 0$.
...
\item
The equality $s(A) = \omega_{0}$ still holds for positive semigroups on commutative \CA-algebras without unit (see B-IV, Remark~1.2.b).
\end{enumerate}
\end{remark}
%% --
...
\]
%% --
which follows as in C-III, Section 1 or \textcite[Theorem 3]{greinervoigtwolff:1981}.
...
From this the assumption follows by A-IV, Theorem~1.11.
\end{proof}
%% --
...
A simple modification of A-III, Example 1.4 (take $c_{0}$ instead of\/ $\ell^2$) shows that Theorem 1.3 is no longer true for non-positive semigroups (for details see \textcite[Beispiel 2.5]{grohneubrander:1981}).
...
Since $T(t)|\phi| = |\phi|$ for all $\phi \in \Fix{T}$ (D-III, Corollary~1.5), we obtain $T(t)'p_{r} \geq p_{r}$ (see D-I,Sec.3.(c)).
...
Since $s(|\phi|)$ and $s(|\phi^{*}|)$ are majorized by $p_{r}$ (D-III, Proposition~1.4), it follows that $\phi$ and $\phi^{*}$ are in $p_{r}M_{*}p_{r}$.
...
%% --
are Schwarz maps.
We omit the simple calculation (compare D-I, Theorem~2.1).
%% --
...
Since $\Fix{\TT} \neq \{0\}$, there exists a faithful normal state $\phi$ on $M$ such that $\Fix{\TT} = \C \,\phi$ (D-III, Theorem~1.10.).
...
\end{proof}
%% --
The next theorem is an extension of D-III, Theorem~1.10 and shows the usefulness of the theory of semitopological semigroups.
...
Thus $N$ is a \WA-subalgebra of\/ $M$, \textcite[Corollary 1.7.9.]{sakai:1971}, and $\TT_{0}(t)' \coloneqq T(t)'_{|N}$ is multiplicative (for this see D-III, Lemma 1.1).
...
But if\/ $ 0 \neq \im\alpha \in P\sigma(A) \cap \im\R $  then $ \im\alpha\Z \subset P\sigma(A) $  by D-III,\,Theorem~1.10, whence $ N $  must be infinite dimensional.
...
%% --
\begin{proof}
By D-III, Theorem~1.11 and D-III, Theorem~2.5 it follows that
%% --
\[
...
Using the result in A-III, Corollary~7.11 it follows that
%% --
\[
...
(see D-III, Remark 2.4.(ii)), there exists a sequence of states $ (\hat{\psi}_{n}) $  in $ \Fix{\RR}^{\wedge} $  such that the corresponding support projections are mutually orthogonal in $ \widehat{M} $  (Lemma~\ref{lem:d4-4.1}).
...
We refer to \textcite{battyrobinson:1984} for this more general setting and to B-IV, Section 1 for the analogous results on $ C_{0}(X) $.
...
Finally, the characterization of convergence towards a periodic semigroup through spectral properties of the generator---Theorem~\ref{thm:d4-3.11}---is due to \textcite{nagel:1984} in the commutative case, \ie in $ L^{1}(\mu) $ (see also C-IV, Theorem~2.14).
%\medskip
