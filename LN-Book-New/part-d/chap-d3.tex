...
\chapternopage{Spectral Theory of Positive Semigroups on \texorpdfstring{W$^{*}$-Algebras}{W\textsuperscript{*}-Algebras} and their Preduals}\label{chap:d3}
...
\end{quote}
%% --
Results of this type have been proved in Part-II, Chapter 3for positive semigroups on commutative \CA-algebras, however in the noncommutative case the situation is more complicated.
While \enquote{$s(A) \in \sigma(A)$} still holds (see \textcite{greinervoigtwolff:1981} or the notes of this chapter), the cyclicity of the boundary spectrum $\sigma_{b}(A)$ is true only under additional assumptions on the semigroup (\eg irreducibility, see Section 1 below).
...
However we will show in the example preceding Theorem~\ref{thm:d3-1.11} on p.~\pageref{thm:d3-1.11} below that the boundary spectrum is no longer cyclic.
...
\end{align*}
%% --
(see Part-IV, Chapter 1,  Corollary 2.2).
Since
%% --
\begin{align*}
\|\psi\| &= \| |\psi| \| = |\psi|(\1) = \\
...
\end{align*}
%% -- 
Then by \textcite[Theorem III.4.2.]{takesaki:1979} we obtain that $\phi^{+}$ and $\phi^{-}$ in $ \Fix{\RR} $.
\end{proof}
%% --
...
%% -- d3-8
This follows from Lemma \ref{lem:d3-1.6} on p.~\pageref{lem:d3-1.6} because $\Fix{\lambda R(\lambda+\im\alpha)} = \Kern{\im\alpha-A}$.
%% --
...
If\/ $\psi_{\alpha} \in M_{*}$ is of norm one with polar decomposition $\psi_{\alpha} = u_{\alpha}|\psi_{\alpha}|$ such that $\psi_{\alpha} = (\mu - \im\alpha)R(\mu)\psi_{\alpha}$ then $\mu R(\mu)|\psi_{\alpha}| = |\psi_{\alpha}|$ (Proposition~\ref{prop:d3-1.4}\,\ref{item:d3-1.4-i} on p.~\pageref{prop:d3-1.4}).
...
\]
%% --
fulfill the assumptions of Lemma~\ref{lem:d3-1.2}\,\ref{item:d3-1.2-ii} on p.~\pageref{lem:d3-1.2}.
...
Hence from Lemma~\ref{lem:d3-1.2}\,\ref{item:d3-1.2-i} on p.~\pageref{lem:d3-1.2} it follows that
%% --
\[
...
\end{remark}
%% --
If\/ $\TT$ is a $C_{0}$-semigroup of Markov operators on a commutative \CA-algebra with generator $A$, it has been shown in Part-II, Chapter 3,  that the boundary spectrum $\sigma(A) \cap \im\R$ of its generator is additively cyclic.
...
%% --
\[
u_{t} \coloneqq \begin{pmatrix} 1 & 0 \\ 0 & \eu^{\lambda t} \end{pmatrix} \in M_{2}(\C) .
\]
%% --
...
%Here a face $F$ in $E$ is a subcone of\/ $E_{+}$ such that the conditions $0 \leq x \leq y$, $x \in E$, $y \in F$ imply $x \in F$ (compare Definitions 3.1 in Part-II, Chapter 3and Part-3, Chapter 3 ).
...
Since $\Fix{\TT}$ is positively generated (Corollary~\ref{cor:d3-1.5} on p.~\pageref{cor:d3-1.5}), 
%% --
\[
...
\]
%% --
we obtain by Lemma~\ref{lem:d3-1.2}\,\ref{item:d3-1.2-ii} on p.~\pageref{lem:d3-1.2} that
%% --
\[
...
$\Fix{T} = \C.\phi$ and let $H \coloneqq P\sigma(A) \cap \im\R$.
...
\]
%% --
for some $\eta_{i} \in H$, it follows from a theorem of Kronecker (see, \textcite[Satz 6.1., p.77]{jacobs:1972}) that there exists $s < t$ such that
%% --
\[
|(1,1, ..., 1) - (\eu^{\eta_{1}t}, \eu^{\eta_{2}t}, ..., \eu^{\eta_{n}t})| < \delta ,
\]
%% --
hence
%% --
...
\[
	\Fix{\kappa(t)} = 
    \overline{ \left(\bigcup_{k \in \Z} \Kern{\frac{2\pi ik}{t} - A)} \right) } ,
\]
%% --
however no $T(t) \in \TT$ is irreducible.

\item 
If\/ $\TT$ is the irreducible semigroup of Schwarz type on the predual of\/ $B(H)$ given in \textcite{evans:1977}, then $P\sigma(A) \cap \im\R = \emptyset$.
\end{enumerate}
\end{remark}
...
%% --
\begin{proof}
By Part-IV, Chapter 4,  Remark 3.2.c, we may assume without loss of generality that there exists a faithful family of\/ $\RR$-invariant normal states on $M$.
...
\]
%% --
Since $\lambda R(\lambda)|\psi_{\alpha}| = |\psi_{\alpha}|$ (Proposition~\ref{prop:d3-1.4} on p.~\pageref{prop:d3-1.4}), we obtain
%% --
\[
...
Since $s(|\psi_{\alpha}^{*}|)M$ is $T$-invariant, the assumptions of Lemma~\ref{lem:d3-1.2} on p.~\pageref{lem:d3-1.2} are fulfilled and we obtain for every $x \in M$
%% --
\[
...
\]
%% --
(see Remark~\ref{rem:d3-1.3} on p.~\pageref{rem:d3-1.3}).
...
% -- d3-16
%% --
and the assertion follows from Lemma~\ref{lem:d3-1.6} on p.~\pageref{lem:d3-1.6}.
\end{proof}
%% --
Before going on let us recall the basic facts of the \emph{ultrapower} $\hat{E}$ of a Banach space $E$ with respect to some free ultrafilter $\mathfrak{U}$ on $\N $ (compare Part-I, Chapter 1,  3.7).
...
Recall that the approximative point spectrum $A{\sigma}(T)$ is equal to the point spectrum $P{\sigma}(\hat{T})$ (see, \eg \textcite[Chapter V, §1]{schaefer:1974}).
...
\]
%% --
Since the fixed space of {$(\lambda-\im\alpha)\hat{R}(\lambda)$} is non trivial such a sequence exists.
...
\]
%% --
one part of the corollary is proved. The other follows from Lemma~\ref{lem:d3-2.2} on p.~\pageref{lem:d3-2.2}.
\end{proof}
%% --
...
Hence there exists a central projection $z \in \widehat{M''}$ such that $J((M_{*})^{\wedge}) = \widehat{M''}z$ (\textcite[Proposition 2.2]{groh:1984b}).
...
\]
%% -- 
for all $\alpha \in \R$ (Proposition~\ref{prop:d3-2.1} on p.~\pageref{prop:d3-2.1}).
Therefore the assertion follows from Lemma~\ref{lem:d3-2.2} on p.~\pageref{lem:d3-2.2}.
\end{proof} 
%% --
The consequences of this result for the asymptotic behavior of one-parameter semigroups will be discussed in Part-IV, Chapter 4,  Section 4.
\index{resolvent!pole|)}
\index{pseudo-resolvent|)}
%% --
...
are explained by the example following Remark~\ref{rem:d3-1.9} on p.~\pageref{rem:d3-1.9} (see also \textcite{groh:1982a}) 
Therefore we concentrate on irreducible semigroups.
Our main result Theorem~\ref{thm:d3-1.11} on p.~\pageref{thm:d3-1.11} extends Part-II, Chapter 3,  Theorem~3.6 to the noncommutative setting.

\item
Theorem~\ref{thm:d3-2.5} on p.~\pageref{thm:d3-2.5} has its roots in the Niiro-Sawashima Theorem for a single irreducible positive operator on a Banach lattice (see \textcite[V.5.4]{schaefer:1974}).
