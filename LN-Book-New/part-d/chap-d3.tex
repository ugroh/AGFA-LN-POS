...
%% --
\begin{align*}
|\psi(x)|^2 &= | \dualp{\mu R(\mu+\im\alpha)'x,\psi} |^2 \leq \\
...
\end{align*}
%% --
we obtain $\mu R(\mu)|\psi| = |\psi|$ by the uniqueness theorem \eqref{eq:star-uniqueness} on p.~\pageref{eq:star-uniqueness} for the absolut value.
...
%% --
\[
u_{t} \coloneqq \begin{pmatrix} 1 & 0 \\ 
...
%
\[
	S \coloneqq \lim_{\mathfrak{U}}T(t-s) .
\]
%
Then 
%
...
\]
%% --
Moreover, for a bounded linear operator $T \in \LE$, we denote by $\widehat{T}$ the well defined operator $\widehat{T}\hat{x} \coloneqq (Tx_{n}) + c_{\mathfrak{U}}(E)$, $(x_{n}) \in \hat{x}$.

It is clear by virtue of\/ $(x \mapsto (x, x, ..) + c_{\mathfrak{U}}(E))$ that each $x \in E$ defines an element $\hat{x} \in \widehat{E}$.
...
%% --
\[
E = \Fix{(\lambda - \im\alpha)R(\lambda)} \bigoplus \bigcap_{j=1}^{n} \Kern{\phi_{j}},
\]
%% --
...
For $\hat{x} \in \widehat{M}$ and $\widehat{\phi} \in (M_{*})^{\wedge}$ let $J: (M_{*})^{\wedge} \to \widehat{M}'$ be defined by
%% --
\[
\dualp{x,J(\widehat{\phi})} 
	\coloneqq \lim_{\mathfrak{U}}\phi_{n}(x_{n}) \quad 
