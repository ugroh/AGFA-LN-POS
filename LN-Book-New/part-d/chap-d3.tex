...
\item 
$G \coloneq (\TT_{0})^{-} \subseteq \mathcal{L}_{s}(N_{*})$ is a compact, Abelian group in the strong operator topology.
...
%% --
\[
U(\eta) \coloneq \{\psi \in D(A): A\psi = \eta\psi\}
\]
%% --
...
%
\[
	S \coloneq \lim_{\mathfrak{U}}T(t-s) .
\]
%
Then 
%
...
Since $N$ is finite dimensional, there exist a natural number $n$ and a set $P \coloneq \{p_{1}, .., p_{n}\}$ of minimal, mutually orthogonal projections in $N$ such that $\sum_{k=1}^{n} p_{k} = \1$.
...
%% --
\[
\widehat{\RR} \coloneq (\lambda \mapsto R(\lambda)^{\wedge}): D \to \mathcal{L}(\widehat{E})
\]
%% --
