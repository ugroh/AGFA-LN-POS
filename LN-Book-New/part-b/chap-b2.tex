...
%% --
\chapternopage{Characterization of Positive Semigroups on \texorpdfstring{$C_{0}(X)$}{C_0(X)}}
\label{chap:b2}%
\thispagestyle{empty}
%% --
{\Large
...
\section{Generators of Positive Semigroups on  \texorpdfstring{$C_{0}(X)$}{C_0(X)}} \label{sec:b2-1}%
%% --
...
Then
$
    \lambda_{0} \coloneq \inf\{\lambda \in \R \colon Au \leq \lambda u \text{ for some } 0 \leq  u \in D(A)\} < \infty
$.
...
%% --
%% --
\section{Lattice Semigroups on  \texorpdfstring{$C_{0}(X)$}{C_{0}(X)}}\label{sec:b2-2}%
%%
...
D_{u}\Theta(f) \coloneq \lim_{t \to 0} 1/t(\Theta(f+tu) - \Theta(f)) \text{ exists.}
\end{equation}
%% –
...
%% –
\[
p(x) \coloneq \int_{0}^{\infty} \eu^{-ws}k_{s}(x) \ds 
\quad \text{and} \quad 
h(x) \coloneq w - 1/p(x) \quad (x \in X).
\]
%% –
...
\[
    D_{0}(\delta_{m}) \coloneq \{ f \in D(\delta_{m}) : f \text{ is differentiable on } (a,b)\}
\]
...
Thus the subspace $E_{0} \coloneq \{f \in C_{0}(a,b) : f \text{ vanishes off } (c,d)\}$
...
$C_{c}^{1}(a,b) \coloneq \{ f \in C^{1}(a,b) \colon f \text{ vanishes in a neighbourhood of } a \text{ and } b\}$
