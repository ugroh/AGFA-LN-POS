...
Thus~\eqref{eq:b2-pmp} is not a "metric" condition; that is, it does not imply any norm estimate for the semigroup.
...
In fact, if\/ $B \coloneqq (m/\|m\|_{\infty}) \cdot A$ is the generator of a positive semigroup, then by 
...
$(I - q \cdot B)$ is surjective and note that $1 \in \rho(B)$.
We have 
%%$
%%--
\[
...
%
\[
	 \|(q-1)BR(1,B)\|_{u} = \|(q-1)(R(1,B) - I)\|_{u} < 1 .
\]
%
%% --
...
%
\[
	\|(q-1)(R(1,B) - I)\|_{u} \leq \|M_{(1-q)}\|_{u}(\|R(1,B)\|_{u} + 1) \leq 2\|1 - q\|_{\infty} < 1 .
\]
%
...
\item
We show that $A$ is densely defined.\\
...
\item
$A$ satisfies~\eqref{eq:b2-pmp} (see Definition~\ref{def:b2-1.5}).\\
...
\item
We show that $(\lambda-A)$ is bijective for $\lambda > \alpha + \|\mu\|$.\\
Let $g \in C[-1,0]$.
...
%
\[
	 (\lambda+\alpha)(\beta-\lambda) + (\lambda-\alpha)(\lambda+\beta)\exp(\lambda^{2}) \neq 0 .
\]
%
...
&\quad + \limsup_{t \downarrow 0} \left\|\frac{1}{t} \bigl[\Theta(k(a)+tk'(a)) - \Theta(k(a)) - D_{k'(a)}\Theta(k(a))\bigr]\right\| \\
...
Although we assume that $(T(t))_{t \geq 0}$ is a real semigroup, \ie $T(t)C(K,\R) \subset C(K,\R)$ for all $t \geq 0$.
...
Let $f \in D(A_{\R})$.
For $\epsilon > 0$ let $f_{\epsilon} \coloneqq f + \iu \epsilon u$.
...
Here we identify $BV[-1,0] \subset L^{1}[-1,0]$ with a subspace of\/ $C[-1,0]'$ by setting 
%
\[
...
\]
%
Where for $\phi \in BV[-1,0]$ we denote by $\diff{\phi}$ he linear form on $C[-1,0]$ given by 
%
\[
...
%%--
\[
%|f(x)|' \coloneqq \frac{\diff^{+}}{\dt}\bigg|_{t=x}|f(t)| = \Re[(\widehat{\sign} \bar{f})f'](x)
|f(x)|' \coloneqq \frac{\diff{^{+}}}{\dt} |f(t)| \bigg|_{t=x}
...
%
\[
	\eta(s) \coloneqq T(t-s) ( T(s)f \cdot T(s)g ).
\]
%
Then 
%
...
Let $A$ be a generator of a semigroup $(T(t))_{t \geq 0}$ on $C(K)$ ans suppose that $\1 \in D(A)$.
...
%
\[
	 \frac{\diff{}}{\dt}|_{t = 0}S(t)f = h \cdot f + \delta f .
\]
%
Hence $\delta + h \subset B$.  
...
%
\[
	W \coloneqq \{x \in X \colon |f(x)| > \frac{1}{2}\};
\]
%
...
Given compact subsets $A \subset \R $ and $B \subset X$.
The set $C \coloneqq \phi(A \times B)$ is compact; hence there exists $f \in C_0(X)$ such that $f|_{C} = 1$.
...
p_{t} \coloneqq p^{-1} \cdot (p \circ \phi_{t}) \quad (t \in \R)
\end{equation}
%% --
...
%% –
\[
p(x) \coloneqq \int_{0}^{\infty} \eu^{-ws}k_{s}(x) \ds 
\quad \text{and} \quad 
...
%%--
\[
k_{t}(x) \cdot p(\phi(t,x)) = \int_{0}^{\infty} \eu^{-ws}k_{t+s}(x) \ds 
= \eu^{wt} \int_{t}^{\infty} \eu^{-ws}k_{s}(x) \ds.
\]
%%--
Now fix $x \in X$ and define $f \colon \R \to \R$ by 
%%--
\[
f(t) \coloneqq k_{t}(x)p(\phi(t,x))/p(x) 
 = \mleft( \eu^{wt}\,p(x)^{-1} \mright) \cdot \int_{t}^{\infty} \eu^{-ws}k_{s}(x)  \ds.
\]
%%-
The function $f$ is differentiable and satisfies the following differential equation 
%%--
\[
f'(t) = wf(t)  - k_{t}(t)\,p(x)^{-1}  = h(\phi(t,x))f(t).
\]
%%--
Moreover $f(0) = 1$.
Hence %
...
%% --
Next we show $T_{0}(t)D_{0} \subset D_{0}$ for all $t \geq 0$, which implies that $D_{0}$
...
\]
Setting $M \coloneqq \sup_{x \in \R^{n}} \|DF(x)\| < \infty$ (finite by hypothesis),
we obtain
\[
...
\[
    \nabla(f \circ \phi_{t})(x) = (\nabla f)(\phi_{t}(x)) \cdot D\phi_{t}(x),
\]
hence $\|\nabla(f \circ \phi_{t})(x)\| \leq \eu^{Mt}\|(\nabla f)(\phi_{t}(x))\|$, and so
%%--
\[
    \lim_{\|x\| \to \infty} \|\nabla(f \circ \phi_{t})(x)\|
    \leq \eu^{Mt} \lim_{\|x\| \to \infty} \|(\nabla f)(\phi_{t}(x))\| = 0.
\]
%%--
%% --
...
\[
    D_{0}(\delta_{m}) \coloneqq \{ f \in D(\delta_{m}) : f \text{ is differentiable on } (a,b)\}
\]
...
\[
    g_{n}(x) \coloneqq \int_{c_{n}}^{x} \frac{1}{m(y)}\,\dy \qquad (x \in (a_{n},b_{n})).
\]
...
%
\[
	Bg = V^{-1}\delta_{m}V = V^{-1}(m(g \circ q)') = V^{-1}(g' \circ q) = g'.
\]
%
...
%
\[
	 u(t) \coloneqq T(t)f \in D(B) \subset D(B_{1}) 
\]
%
for all $t \geq 0$ and 
%
...
%
\[
	T(t)D_{0}(\delta_{m}) \subset D_{0}(\delta_{m}),
\]
%
...
$\phi_{t}((a,c)) \subset (a,c)$ for all $t \in \R$.
Similarly, $\phi_{t}((d,b)) \subset (d,b)$ for all $t \in \R$.

Thus the subspace $E_{0} \coloneqq \{f \in C_{0}(a,b) : f \text{ vanishes off } (c,d)\}$
is invariant under $(T(t))_{t \in \R}$.
Denote by $(T_{0}(t))_{t \in \R}$ the restricted group on $E_{0}$ and by $A_{0}$ its generator.
Then
\[
    D(A_{0}) = \{f \in E_{0} \cap D(\delta_{m}) : \delta_{m}f \in E_{0}\}.
\]
...
$C_{c}^{1}(a,b) \coloneqq \{ f \in C^{1}(a,b) \colon f \text{ vanishes in a neighbourhood of } a \text{ and } b\}$ is contained in $D(\delta_{m})$ (this follows from the definition of\/ $\delta_{m}$).
...
Then $\beta_{n} \coloneqq r_{n}^{-1} \circ q_{n}$ is a homeomorphism from $(a_{n},b_{n})$ onto itself satisfying 
%
\[
...
\[
	\psi_{t} \coloneqq \beta^{-1} \circ \phi_{t} \circ \beta \quad (t \in \R)
\]
%
