...
Concerning the characterization of positive semigroups on $C_{0}(X)$ ($X$ locally compact, non-compact) we follow a completely different line and will treat this case in the context of general Banach lattices in Part-III, Chapter 2.
...
We refer to Part-III, Chapter 2 for a discussion of this identity and classical analogs for the Laplacian due to \textcite{kato:1973}.
...
\end{equation}
%% --
(cf. Part-I, Chapter 2,  Proposition 1.10).
...
\]
%% --
which appears in the Hille-Yosida Theorem (Part-I, Chapter 2,  Theorem 1.7) is automatically fulfilled.
...
For that we will make use of the theory of half-norms developed in Part-I, Chapter 2,  Section 2.
...
\end{equation}
%% –
Then $p_{u}$ is a strict half-norm on $C(K)$ (see Part-I, Chapter 2,  Section  2).
Note that
%% –
...
(see Part-I, Chapter 2,  Section 2 for the definition of the subdifferential $\ddp_{u}$).

Note that for $f \in C(K)$ one has $f \geq 0$ if and only if\/ $p_{u}(-f) \leq 0$ (\ie  the half-norm $p_{u}$ induces the given ordering on $C(K)$ (cf. Part-I, Chapter 2,  Remark 2.8)).
...
Since $D(A)$ is dense, it follows from Part-I, Chapter 2,  Theorem 2.7
...
Hence $B$ is closable and the closure $\overline{B}$ of\/ $B$ is $p_{u}$-dissipative as well (by Part-I, Chapter 2 Proposition 2.9).
...
It is somewhat similar to the Lumer-Phillips Theorem (Part-I, Chapter 2,  Theorem 2.13).
...
Thus by Part-I, Chapter 2,  Corollary 2.12 
...
The proof of Theorem~\ref{thm:b2-1.13} yields estimates for the growth bound of a positive semigroup (see Part-I, Chapter 3, \,(1.3)) which we state explicitly in the next corollary.
...
If in addition the semigroup is irreducible (see Part-II, Chapter 3,  Section 3 below), then $u$ is strictly positive and in \eqref{eq:b2-1.8} 
...
However, it is still true that $s(A) = \omega_{0}(A)$ for generators of positive semigroups on $C_{0}(X)$ (see Part-II, Chapter 4,  Theorem 1.4).
\end{remark}
%% --
%% --
...
Part-I, Chapter 1,  3.2, the operator $m \cdot A = \|m\|_{\infty}B$ also generates a positive semigroup).
...
\begin{align*}
	    \|M_{r}\|_{u} &= \sup\{\|r \cdot f\|_{u} \colon \|f\|_{u} \leq 1\} \\
	              ...
	              &\leq \|r\|_{\infty}.
\end{align*}
Since $B$ is $p_{u}$-dissipative we have $\|R(1,B)\|_{u} \leq 1$ (by Part-I, Chapter 2,  Lemma 2.10).
...
Recall from Part-II, Chapter 1,  Section 3 that a linear bounded operator $T$ on $C_{0}(X)$ is positive if and only if
%% –
...
Conversely, the linear extension $T$ of a lattice homomorphism $T_{\R}$ on $C_{0}(X,\R)$ to $C_{0}(X,\C)$ is a lattice homomorphism (see Part-II, Chapter 1,  Section 3).
\end{remark}
%% --
...
We will see in Part-III, Chapter 2,  Section  5 that in a Banach lattice with order continuous norm the above argument is rigorous (for all $f \in D(A)$).
...
We will see in Part-III, Chapter 2 that the inequality $|T(t)f| \leq T(t)|f|$, which holds precisely for positive semigroups, implies the inequality corresponding to \eqref{eq:b2-2.9}
if\/ $\phi \geq 0$.
...
So by Part-II, Chapter 1,  Section 2, there exist $\beta \geq 0$ and $x \in \left[-1,0\right)$ such that $\mu = \beta\delta_{x}$.
Assume that $\beta \neq 0$.
...
A lattice homomorphism $T$ on $C(K)$ is an algebra homomorphism if and only if\/ $T \1~=~\1$ (see Part-II, Chapter 1,  Section 3).
...
Then there exists a continuous mapping $\phi_{t} \colon K \to K$ such that $T(t)f = f \circ \phi_{t}$ for all $f \in C(K)$ (see Part-II, Chapter 1,  Section 3).
...
Next we show $T_{o}(t)D_{0} \subset D_{0}$ for all $t \geq 0$, which implies that $D_{0}$ is a core of\/ $\delta$ by Part-I, Chapter 1,  Theorem 1.9 (or Part-I, Chapter 2,  Corollary 1.34). 
\index{core}
...
It follows from Part-I, Chapter 1,  Theorem 1.7.  (or Part-I, Chapter 2,  Corollary 1.2.)
that $T_{1}(t)f~=~u(t)$.
...
Finally we show that $T(t)D_{0}(\delta_{m}) \subset D_{0}(\delta_{m})$, which implies that $D_{0}(\delta_{m})$ is a core (by Part-I, Chapter 2,  Corollary 1.34.).
Let $t \in \R$.
...
But one can obtain every such generator by a similarity transformation (see Part-I, Chapter 1,  3.1) from some $\delta_{m}$.
%% –
...
\item
Concerning bounded generators of positive semigroups and the positive minimum principle we refer to the corresponding notes in Chapter Part-III, Chapter 2,  Theorem~\ref{thm:b2-1.6} and \ref{thm:b2-1.8} are due to \textcite{arendtchernoffkato:1982}, but we give a more direct proof here.
...
\item 
A characterization of generators of lattice semigroups by Kato's equality is due to  \textcite{nageluhlig:1981} if the underlying space has order continuous norm (see Part-III, Chapter 2,  Section 5), for general Banach spaces and $C_{0}(X)$ in particular the problem has been considered in \textcite{arendt:1982}.
