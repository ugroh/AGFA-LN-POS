...
Part-I,~Eq.~\myeqpageref{eq:a1-3.2}, the operator $m \cdot A = \|m\|_{\infty}B$ also generates a positive semigroup).
...
Eq.~\myeqpageref{eq:b2-1.8} % we have 
\[
...
Note: Eq.~\myeqpageref{eq:b2-3.18} means that $D(A) = \{ f \colon V^{-1}f \in D(\delta) \}$ and $Af = V\delta V^{-1}f + hf$.
%% –
\begin{proof}
...
R(1,A)g(x) &= \int_{0}^{\infty} \eu^{-t}f(\phi(t,x)) \dt - \int_{0}^{\infty} \eu^{-t}m(\phi(t,x))f'(\phi(t,x)) \dt \\
           ...
           \quad \text{(by Eq.~\myeqpageref{eq:b2-3.23})} \\
           &= f(x) 
\end{align*}
%%--
by integrating by parts.
...
But one can obtain every such generator by a similarity transformation (see Part-I,~Eq.~\myeqpageref{eq:a1-3.1}) from some $\delta_{m}$.
%% –
