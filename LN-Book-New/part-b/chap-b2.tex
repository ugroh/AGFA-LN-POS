...
Eq.~\eqref{eq:a1-3.2} on p.~\pageref{eq:a1-3.2}, the operator $m \cdot A = \|m\|_{\infty}B$ also generates a positive semigroup).
...
\]
%% --
Kato's equality does not have as strong consequences as the positive minimum principle (which by Theorem 1.13 would imply that $A_{\max}$ is a generator).
\end{example}
\index{Kato's equality|)} 
...
Next we show $T_{o}(t)D_{0} \subset D_{0}$ for all $t \geq 0$, which implies that $D_{0}$ is a core of\/ $\delta$ by Part-I, Proposition~\ref{prop:a1-1.9} (or Part-I, Corollary~\ref{cor:a2-1.34}). 
\index{core}
...
But one can obtain every such generator by a similarity transformation (see Eq.~\eqref{eq:a1-3.1} on p.~\pageref{eq:a1-3.1}) from some $\delta_{m}$.
%% –
