...
%
\[
	s \coloneqq \inf \{\lambda \in \R \colon \left[\lambda,\infty \right) \subseteq \rho(A)\}.
\]
%

Then $s \leq \omega_{0}(A) < \infty$ and let $0 \leq  u \in C(K)$ and  
%%--
%% $
\[
...
The proof of Theorem~\ref{thm:b2-1.6} shows that for the generator $A$ of a positive semigroup on $C(K)$, $R(\lambda,A)u \gg 0$ whenever $0 \leq  u \in C(K)$ and $[\lambda,\infty) \subseteq \rho(A)$.
...
%% --
\begin{align*}
M & \coloneqq \{\mu \in \rho(B) \cap [\mu_{1},\mu_{2}] \colon [\mu,\mu_{2}] \subseteq \rho(B) 
\intertext{and} 
...
%% --
\begin{proof}
Since $\{u \in C(K) \colon u \gg 0\}$ is open and non-empty and $D(A)$ is dense, there exists $0 \leq  u \in D(A)$.
...
Then
$
    \lambda_{0} \coloneqq \inf\{\lambda \in \R \colon Au \leq \lambda u \text{ for some } 0 \leq  u \in D(A)\} < \infty
$.
...
s(A) = \inf\{\lambda \in \R \colon Au \leq \lambda u \text{ for some } 0 \leq  u \in D(A)\}
\end{equation}
%% –
and
%% –
...
%% --
\begin{proof}
Let $s \coloneqq \inf\{\lambda \in \R \colon [\lambda,\infty) \subseteq \rho(A)\}$.
Clearly, $s \leq s(A)$.
...
So by the proof of Theorem~\ref{thm:b2-1.13} we can assume that there exists $0 \leq  u \in C(K)$ such that $A$ is $p_{u}$-dissipative.
...
Let $0 \leq  u \in D(A) = D(m \cdot A)$ and $Au \leq \lambda u$.
Then 
%
\[
...
\left.\frac{\diff{}}{\dt}
%%\right
\right|_{t = 0}\, |(T(t)f)(x)| = \left\langle \Re\left[\left(\widehat{\sign} \bar{f}\right)(Af)\right],\phi \right\rangle.
...
%% --
\begin{proof}
Denote by $G(A) \coloneqq \{(h,Ah) \colon h \in D(A)\} \subseteq G \times G$ the graph of\/ $A$.
Assume that $(g,f) \notin G(A)$.
...
|f(x)|' \coloneqq \frac{\diff{^{+}}}{\dt}\bigg|_{t=x} |f(t)| 
...
Since $D(A)$ is dense in $C(K)$ there exists $0 \leq  p \in D(A)$.
Let 
%
\[
	h(x) = (Ap)(x) \cdot p(x)^{-1}.
...
Given a compact neighbourhood $V$ of\/ $x_0 = \phi (0,x_0)$, there exists a continuous function $f \colon X \to [0,1]$ satisfying $f(x_0) = 1$ and $\supp f \subseteq V$.
...
By Proposition~\ref{prop:b2-3.9} and Lemma~\ref{lem:b2-3.12} we know that there exist a continuous flow $\phi$, $0 \leq  p \in C^{b}(X)$ and $h \in C^{b}(X)$ such that Eq.~\eqref{eq:b2-3.19} holds.
...
%
\[
	 u(t) \coloneqq T(t)f \in D(B) \subseteq D(B_{1}) 
\]
%
for all $t \geq 0$ and 
%
...
$C_{c}^{1}(a,b) \coloneqq \{ f \in C^{1}(a,b) \colon f \text{ vanishes in a neighbourhood of } a \text{ and } b\}$
