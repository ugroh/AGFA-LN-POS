...
Their generators are similar to a derivation perturbated by a multiplication operator (Section~\ref{sec:b2-3}).
...
Then $s \leq \omega_{0}(A) < \infty$ and let $0~\ll~u~\in~C(K)$.
Then 
%%--
%% $
\[
...
One has $p_{u}(u) = 1$.
Let $x \in K$.
It follows from Eq.~\eqref{eq:b2-1.6} that $\phi_{x} \in \ddp_{u}(u)$.
...
Since $f(x) = 0$ and $p_{u}(-f) = 0$ we have by Eq.~\eqref{eq:b2-1.6}  $\phi_{x} \in \ddp_{u}(-f)$\,.
...
The proof of Theorem~\ref{thm:b2-1.13} yields estimates for the growth bound of a positive semigroup (see Eq.~\eqref{eq:b3-1.3} on p.~\pageref{eq:b3-1.3} ) which we state explicitly in the next corollary.
...
Hence it follows from Eq.~\eqref{eq:b2-1.3} that $\omega_{0}(A) \leq s$.
Consequently, $s = s(A) = \omega_{0}(A)$.

Next we prove Eq.~\eqref{eq:b2-1.9}.
...
This proves the other inequality in Eq.~\eqref{eq:b2-1.8}. 
\end{proof}
...
So the equality is valid in Eq.~\eqref{eq:b2-1.9}  
and the supremum is a maximum.
If in addition the semigroup is irreducible (see Section~\ref{sec:b3-2}), then $u$ is strictly positive and in Eq.~\eqref{eq:b2-1.8} 
the infimum is attained as well.
\index{resolvent!positivity|)}

Conversely, if in Eq.~\eqref{eq:b2-1.8} 
...
The generator $A$ of\/ $(T(t))_{t \geq 0}$ has empty spectrum, so that Eq.~\eqref{eq:b2-1.7} is violated.
...
Part-I,~Eq.~\eqref{eq:a1-3.2} on p.~\pageref{eq:a1-3.2}, the operator $m \cdot A = \|m\|_{\infty}B$ also generates a positive semigroup).
...
There exists $x \in K$ such that $\phi_{x} \in \ddp_{u}(f)$ (by Eq.~\eqref{eq:b2-1.6}). 
...
\end{equation}
%% –
Because of Eq.~\eqref{eq:b2-1.11} we only have to show that $(I - q \cdot B)$ is surjective. 
...
The proof of Eq.~\eqref{eq:b2-1.12} is complete.
\index{dissipative operator|)}

There exists $k \in \N$ such that $\|1 - m^{1/k}\|_{\infty} < 1/2$.
Applying now Eq.~\eqref{eq:b2-1.12} 
...
\end{equation}
%% –
If\/ $\lambda > \alpha + \|\mu\|$, then $\lambda - \alpha - \int_{-1}^{0} \eu^{\lambda x} d\mu(x) \neq 0$ and there exists exactly one $c \in \R$ satisfying Eq.~\eqref{eq:b2-1.13}.
...
\end{equation}
%% –
The operator $T$ is a lattice homomorphism if and only if in Eq.~\eqref{eq:b2-2.1} equality holds; \ie 
%% –
...
\[
\begin{array}{lll}
\lim_{t \downarrow 0} &\|\frac{1}{t} (\Theta(k(a+t)) - \Theta(k(a))) - D_{k'(a)}\Theta(k(a))\| \\
&\leq \limsup_{t \downarrow 0} \|\frac{1}{t} (\Theta(k(a+t)) - \Theta(k(a)+tk'(a)))\| \\
&\quad + \limsup_{t \downarrow 0} \|\frac{1}{t} [\Theta(k(a)+tk'(a)) - \Theta(k(a)) - D_{k'(a)}\Theta(k(a))]\| \\
&\leq \limsup_{t \downarrow 0} L \cdot \|\frac{1}{t} (k(a+t) - k(a)-tk'(a))\| + 0 \\
&= 0.
\end{array}
\]
\phantom{x}
...
%% --
\begin{proof}
If\/ $z = 0$, relation Eq.~\eqref{eq:b2-2.6} is obvious from the definiton.
Let $z = (x_{0} + iy_{0}) \neq 0$.
We identify $\C$ and $\R^2$.
Then $\Theta(x,y) = (x^2 + y^2)^{1/2}$ is differentiable in $z$ and
%% --
\begin{align*}
D_{u}\Theta(z) &= (\Theta(x_{0},y_{0})|u) = \frac{1}{|z|}\, ((x_{0},y_{0})|(u_{1},u_{2}))  \\
               &= \frac{1}{|z|}\, (x_{0}u_{1} + y_{0}u_{2}) \\
               &= \frac{1}{|z|}\, \Re((x_{0}-iy_{0})\cdot(u_{1}+iu_{2})) \\
...
After these preparations we now can show that the lattice property $|T(t)f| = T(t)|f|$ of the semigroup corresponds to the identity Eq.~\eqref{eq:b2-2.9} below for the generator, which we call Kato's equality (cf. Remark \ref{rem:b2-2.7}).
...
The dominated convergence theorem and Eq.~\eqref{eq:b2-2.11} imply that
%% --
\begin{align*}
...
Conversely, assume that Eq.~\eqref{eq:b2-2.9} holds.
Let $t > 0$, $f \in C_{0}(X)$.
...
This implies that $k(0) = k(t)$ which is Eq.~\eqref{eq:b2-2.13}.
...
%% --
\begin{equation*}
\overline{\lim}_{h \downarrow 0}\, \frac{1}{h}\, \|(T(t-(s+h)) - T(t-s))'\phi\| < \infty
\end{equation*}
%% --
by the uniform boundedness principle.
Hence, since $\lim_{h \downarrow 0}\, |T(s+h)f| = |T(s)f|$, Eq.~\eqref{eq:b2-2.14} implies that
%% --
...
&= -\langle |T(s)f|,A'T(t-s)'\phi \rangle + \lim_{t \downarrow 0}  \frac{1}{h} \, \langle (|T(s+h)f| -  |T(s)f| , T(t-s)'\phi \rangle .
\end{align*}
%%--
...
We will see in Part-III, Chapter~\ref{chap:c2} that the inequality $|T(t)f| \leq T(t)|f|$, which holds precisely for positive semigroups, implies the inequality corresponding to Eq.~\eqref{eq:b2-2.9}
if\/ $\phi \geq 0$.
...
Hence by Eq.~\eqref{eq:b2-2.9}, $\langle \Re[(\sign \bar{f})(Af)],\phi \rangle = \langle |f|,A'\phi \rangle$ for all $\phi \in D(A')$.
...
\]
%% --
Kato's equality does not have as strong consequences as the positive minimum principle (which by Theorem~\ref{thm:b2-1.13}
%%1.13
would imply that $A_{\max}$ is a generator).
\end{example}
\index{Kato's equality|)} 
...
We now show Eq.~\eqref{eq:b2-2.16}.
...
\end{equation}
%% --
Moreover, by Theorem~\ref{thm:b2-2.5}, $A$ satisfies Kato's Equality Eq.~\eqref{eq:b2-2.9}.
Since $f(0) = 0$ this implies
%% --
\begin{align*}
|\langle f,\mu \rangle| &= |f'(0)| = \Re[(\widehat{\sign} f)(f')](0) \\
&= \langle \Re[\widehat{(\sign} f)(Af)],\delta_{0} \rangle = \langle |f|,A'\delta_{0} \rangle \quad \text{(by Eq.~\eqref{eq:b2-2.9})} \\
&= \langle |f|,\mu \rangle.
\end{align*}
%% --
...
It follows that Eq.~\eqref{eq:b2-2.18} holds for all $f~\in~C_{0}[-1,0)$.
...
\end{align*}
%%--
This contradicts Eq.~\eqref{eq:b2-2.9}.
...
Because of Eq.~\eqref{eq:b2-3.1} it suffices to show that the restriction $\widetilde{\phi}$ of\/ $\phi$ to $[0,1] \times K$ is continuous.
...
Note that Eq.~\eqref{eq:b2-3.4} implies $\delta\1 = 0$.
...
Thus $f \cdot g \in D(A)$ and Eq.~\eqref{eq:b2-3.4} holds.
...
Eq.~\eqref{eq:b2-1.8} on p.~\pageref{eq:b2-1.8} % we have 
\[
...
%Eq.~\eqref{eq:b2-1.8} on p.~\pageref{eq:b2-1.8} ].
...
It follows from Eq.~\eqref{eq:b2-3.10} and Eq.~\eqref{eq:b2-3.11} that $h_{t}(x) \neq 0$ for all $x \in X$ and 
...
If for every $x \in X$ the mappings $t \mapsto \phi_{t}(x)$ and $t \mapsto h_{t}(x)$ are continuous, then Eq.~\eqref{eq:b2-3.12} defines a strongly continuous group.
\end{proposition}
%%
\begin{proof}
...
The group defined by Eq.~\eqref{eq:b2-3.12} is positive whenever $(h_t)_{t \in \R} \subset C^{b} (X)_+$.
We now show that every positive group on $C_0(X)$ is of the form Eq.~\eqref{eq:b2-3.12}.
%% –
\begin{proposition}\label{prop:b2-3.9}
Let $(T(t))_{t \in \R}$ be a strongly continuous group of positive operators on $C_0(X)$.
Then there exist a continuous flow $\Phi$ on $X$ and a continuous cocycle $(h_t)_{t \in \R}$ of\/ $\Phi$ such that Eq.~\eqref{eq:b2-3.12} holds.
\end{proposition}
%% –
\begin{proof}
...
\end{equation}
%% --
Then $(h_{t})_{t \in \R}$ is a continuous cocycle of\/ $\phi$ (compare Eq.~\eqref{eq:b2-3.6}).
\end{enumerate}
\end{example}

Cocycles as defined by Eq.~\eqref{eq:b2-3.13} are always globally bounded.
In general this is false for cocycles of the second type.
On the other hand, a cocycle described by Eq.~\eqref{eq:b2-3.14} is differentiable with respect to $t$.
This is not satisfied by cocycles of the first type in general.
Thus neither Eq.~\eqref{eq:b2-3.13} nor Eq.~\eqref{eq:b2-3.14} gives a description of arbitrary cocycles.
However every positive cocyle is a product of cocycles of the form Eq.~\eqref{eq:b2-3.13} and Eq.~\eqref{eq:b2-3.14}.
...
Eq.~\eqref{eq:b2-3.16} follows.
Moreover, $k_{-t} = 1/(k_{t} \circ \phi_{-t})$.
Hence $\|T(-t)\| = \sup_{x \in X} 1/k_{t}(x) = [\inf_{x \in X} k_{t}(x)]^{-1}$.
So Eq.~\eqref{eq:b2-3.17} also implies the first inequality in Eq.~\eqref{eq:b2-3.16}.
...
So by Proposition~\ref{prop:b2-3.9}  it is defined via Eq.~\eqref{eq:b2-3.12} by some continuous flow and cocycle.
...
Note: Eq.~\eqref{eq:b2-3.18} means that $D(A) = \{ f \colon V^{-1}f \in D(\delta) \}$ and $Af = V\delta V^{-1}f + hf$.
%% –
\begin{proof}
Assume that $A$ is given by Eq.~\eqref{eq:b2-3.18}.
...
We have to show that Eq.~\eqref{eq:b2-3.18} holds.
...
This proves Eq.~\eqref{eq:b2-3.21}.

Next we show $T_{0}(t)D_{0} \subset D_{0}$ for all $t \geq 0$, which implies that $D_{0}$ is a core of\/ $\delta$ by Part-I, Proposition~\ref{prop:a1-1.9} (or Part-I, Corollary~\ref{cor:a2-1.34}).
\index{core}
...
It is easy to see that Eq.~\eqref{eq:b2-3.22} then defines a continuous flow on $(a,b)$.
...
R(1,A)g(x) &= \int_{0}^{\infty} \eu^{-t}f(\phi(t,x)) \dt - \int_{0}^{\infty} \eu^{-t}m(\phi(t,x))f'(\phi(t,x)) \dt \\
           ...
           \quad \text{(by Eq.~\eqref{eq:b2-3.23})} \\
           &= f(x) 
\end{align*}
%%--
by integrating by parts.
This shows that $f = R(1,A)g \in D(A)$ and that $\delta_{m}$ is the generator of the group $(T(t))_{t \in \R}$.
Finally we show that $T(t)D_{0}(\delta_{m}) \subset D_{0}(\delta_{m})$, which implies that $D_{0}(\delta_{m})$ is a core (by Part-I, Corollary~\ref{cor:a2-1.34}.).
Let $t \in \R$.
...
Thus the space $E_{0} \coloneqq \{f \in C_{0}(a,b) \colon f \text{ vanishes off } (c,d)\}$ is invariant under the group $(T(t))_{t \in \R}$.
...
Then $f$ has a continuously differentiable inverse in some open neighbourhood of\/ $x_{0}$.
...
Then $r_{n}^{-1}(r_{n}(x_{0}) + t) = f(\phi(t,x_{0})) = (T(t)f)(x_{0})$ for all $t$ in some neighborhood of\/ $0$.
Since $f \in D(\delta)$ it follows that the function $t~\mapsto~r_{n}^{-1}(r_{n}(x_{0}) + t)$ is continuously differentiable in some neighborhood of\/ $0$ and so $r_{n}^{-1}$ is continuously differentiable at $r_{n}(x_{0})$.
...
%%-- 
\[
(\delta f)(x_{0}) = \frac{\partial}{\partial t}_{|_{t=0}} f(r_{n}^{-1}(r_{n}(x_{0}) + t)) = f'(x_{0})(r_{n}^{-1})'(r_{n}(x_{0})) = 0.
\]
%%--
...
But one can obtain every such generator by a similarity transformation (see Part-I,~Eq.~\eqref{eq:a1-3.1} on p.~\pageref{eq:a1-3.1}) from some $\delta_{m}$.
%% –
...
\text{ such that Eq.~\eqref{eq:b2-3.25} holds} 
\end{array} \right\}.
\]
%% –
...
\item
Concerning bounded generators of positive semigroups and the positive minimum principle we refer to the corresponding notes in Part-III, Chapter~\ref{chap:c2},  Theorem~\ref{thm:b2-1.6} and \ref{thm:b2-1.8} are due to \textcite{arendtchernoffkato:1982}, but we give a more direct proof here.
