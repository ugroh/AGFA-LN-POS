...
It is shown that the semigroup consists of positive operators if and only if its generator satisfies a positive minimum principle~\eqref{eq:a2-pmp}.
...
%% --
\begin{align*}
(Af)(x) &= \lim_{t \to 0} \frac{1}{t} \mleft( (\eu^{tA}f(x) - f(x)) \mright) \\
&= \lim_{t \to 0}\frac{1}{t} \mleft( (\eu^{tA}f(x) \mright) \geq 0 .
\end{align*}
%% --
...
%% --
\begin{equation*}\label{eq:a2-pmp} \tag{P}
\left.
\begin{split}
...
Then the semigroup is positive if and only if the generator $A$ satisfies the positive minimum principle~\eqref{eq:a2-pmp}.
\end{theorem}
%%
\begin{proof}
The necessity of the condition is proved as 
\enquote{\ref{thm:b2-1.3-1} implies~\ref{thm:b2-1.3-2}} in Theorem~\ref{thm:b2-1.3}.
Assume that~\eqref{eq:a2-pmp} holds.
...
%
\[
	s \coloneqq \inf \{\lambda \in \R \colon \left[\lambda,\infty \right) \subset \rho(A)\}.
\]
%
...
Then~\eqref{eq:a2-pmp} implies that $A(R(\lambda_{0},A)u)(x) \geq 0$.
Hence, 
%%--
\[
...
If\/ $\mu_{1}$ and $\mu_{2} \in \rho(B) \cap \R$ such that $0 \leq R(\mu_{1},B)$, $0 \leq R(\mu_{2},B)$ and $\mu_{1} < \mu_{2}$, then $[\mu_{1},\mu_{2}] \subset \rho(B)$ and
%% –
\[
...
$Au \leq 0$ and $A$ satisfies~\eqref{eq:a2-pmp}.
\end{enumerate}
\end{proposition}
%% –
...
It follows from~\eqref{eq:a2-pmp} that 
%
\[
...
%%
Let $A$ be a densely defined operator on $C(K)$.
If\/ $A$ satisfies~\eqref{eq:a2-pmp} then $A$ is closable and the closure of\/ $A$ satisfies~\eqref{eq:a2-pmp} as well.
\end{corollary}
%% –
...
The operator $B = A - \lambda$ satisfies~\eqref{eq:a2-pmp} as well and $Bu \leq 0$.
...
Then by Proposition~\ref{prop:b2-1.10} $\overline{B}$ satisfies~\eqref{eq:a2-pmp}.
Thus $A$ is closable and its closure $\overline{A} = \overline{B} + \lambda$ satisfies~\eqref{eq:a2-pmp} as well.
\end{proof}
...
If\/ $A$ satisfies~\eqref{eq:a2-pmp} then $A$ is bounded and $A + \|A\|\Id \geq 0$.
\end{corollary}
%% –
...
Hence $A$ is bounded.
Since $A$ satisfies~\eqref{eq:a2-pmp}, it follows from Theorem~\ref{thm:b2-1.3} that $A + \|A\|\Id \geq 0$.
\end{proof}
...
Note that, however, in contrast to the condition of dissipativity, $A + w$ satisfies~\eqref{eq:a2-pmp} for any $w \in \R$ whenever~\eqref{eq:a2-pmp} holds for $A$.
Thus~\eqref{eq:a2-pmp} is not a "metric" condition; that is, it does not imply any norm estimate for the semigroup.
...
Let $A$ be a densely defined operator on $C(K)$ which satisfies~\eqref{eq:a2-pmp}.
Then
\[
...
Then $B$ satisfies~\eqref{eq:a2-pmp} and $Bu \leq 0$ for some strictly positive $u \in D(B) = D(A)$.
...
\begin{align*}
    ...
    &= \inf\left\{\sup_{i} \sum_{j=1}^{n} a_{ij} \frac{u_{j}}{u_{i}} \colon u \gg 0\right\}.
\end{align*}
...
b) $A$ satisfies~\eqref{eq:a2-pmp} (see Definition~\ref{def:b2-1.5}).
...
\begin{proof}
The operator $A$ satisfies~\eqref{eq:a2-pmp}.
