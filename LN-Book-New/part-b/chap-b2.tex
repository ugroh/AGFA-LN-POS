...
The proof of Theorem~\ref{thm:b2-1.13} yields estimates for the growth bound of a positive semigroup (see Eq.~\eqref{eq:b3-1.3} ) which we state explicitly in the next corollary.
...
Part-I,~Eq.~\eqref{eq:a1-3.2}, the operator $m \cdot A = \|m\|_{\infty}B$ also generates a positive semigroup).
...
But one can obtain every such generator by a similarity transformation (see Part-I,~Eq.~\eqref{eq:a1-3.1}) from some $\delta_{m}$.
%% –
