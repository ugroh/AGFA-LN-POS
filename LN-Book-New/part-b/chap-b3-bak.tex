...
\begin{proof}
    ...
    \[
    -\1_{L} \leq Tf - \1_{L} \leq \1_{L}\text{ or } 0 \leq Tf \leq 2\cdot \1_{L}\,. \qedhere
    \]
\end{proof}
%% --
...
Consider %
\[
	D \coloneqq \{z \in \C \colon |z| \leq 1\} = \{r\cdot \eu^{\im\omega} \colon r \in [0,1], \omega \in \R\} 
\]
%
...
%
\[
	\alpha \coloneqq \inf_r \kappa(r) 
	\quad \text{and} \quad 
	\beta \coloneqq \sup_r \kappa(r).
\]
%
...
\begin{tabular}{p{10mm}p{10mm}p{10mm}|c|p{8mm}ccc} % <--: 1st column left, 2nd middle and 3rd right, with vertical lines in between
		...
		$ d > 0 $, & $ f>0 $, &  $ e  \geq 0 $ & 3 & $\langle e_1 \rangle$ &  $\langle e_1, e_2 \rangle$ &  $\langle e_1, e_2, e_3 \rangle$ &  $\C^4$\\
		$ d = 0 $, & $ f > 0 $, & $ e  \geq 0 $ & 2 & $\langle e_1 \rangle$ &  $\langle e_1, e_2, e_3 \rangle$ &  $\C^4$ & \\	
...
\begin{examples}\label{ex:b3-4.3}
	...
	\item 
	Consider on $K_{1} \coloneq [-1,\infty)$ the equivalence relation $\sim$ defined by 
    \[
    \enquote{x \sim y} \quad\text{if and only if}\quad x,\,y \geq 0 \text{ and } x - y \in \Z\,.
    \]
...
\begin{proof}
    ...
    \[
    D \coloneq \{\lambda \in \C \colon |\lambda| < \exp(\underbar{c}(h,\phi)-2\epsilon)\}
    \]
...
\begin{proof}
	...
	Given $x \in K$ then by Lemma~\ref{lem:b3-4.2}\,(b) there exists $y \in K_{\infty}$ such that $\phi(s,x) = \phi(s,y)$.
...
\begin{example}\label{ex:b3-4.7}
	\begin{enumerate}[\upshape (i), wide, labelindent=.25em]
		...
		Consider 
        \[
        K = \{z \in \C \colon 1 \leq |z| \leq 2\} = \{r\cdot \eu^{\mathcal{\im\omega}} \colon \omega\in \R, 1 \leq r \leq 2\}
         \]
         and a continuous function $\kappa: [1,2] \to \R_+$.
...
then 
\[
\hat{h}(x_{0}) + \frac{2\pi}{\tau}\im\Z \subset R{\sigma}(A)\quad\text{where}\quad \hat{h}(x_{0}) \coloneq \frac{1}{\tau}\int_{T}^{T+\tau}h(\phi(s,x_{0}))\ds\,
\]
...
%% --
\[  
	\phi_{nm}(f) \coloneq \frac{1}{n}\int_{0}^{n} \exp(-(h^{\star}+\im\beta )t)\cdot h_{t}(\phi(m,x_{0}))\cdot f(\phi(m+t,x_{0})) \dt, \ f\in C(K).
\]
...
\begin{equation}\label{eq:b3-4.13}
	\limsup_{m\to\infty} \|(\hat{h}+\im\beta -A')\phi_{nm}\| \leq 1/n \text{ for every } n \in \N.
\end{equation}
%% --
...
\item 
    ...
    \]
    therefore $\lambda_{0} = \im\beta \in \rho(A)$.\qedhere
\end{enumerate}
\end{proof}
%% --
