...
To that purpose we recall that the spectral bound $s(A)$ of a generator $A$ is defined as the least upper bound of the real parts of all spectral values (\cf Eq.~\eqref{eq:b3-1.2} on p.~\pageref{eq:b3-1.2} ).
%\pagebreak[1]
%
...
\begin{proof}
	...
	\end{equation}
	%% --
	This and Eq.~\eqref{eq:b3-1.2} implies that $\lambda_{0} - r(R(\lambda_{0},A))^{-1}$ is a real spectral value which is greater than $s(A) - \epsilon$.
	We have derived a contradiction to Eq.~\eqref{eq:b3-1.1} and thus have proved the first statement of the theorem.
	...
	\end{equation}
	%% --
	We may assume $\lambda_{0} > \omega_{0}(A)$ then $R(\lambda_{0},A) \geq 0$, and iterating Eq.~\eqref{eq:b3-1.4} we obtain
	%% --
...
Moreover, by Eq.~\eqref{eq:a3-3.6} on p.~\pageref{eq:a3-3.6}, we have 
%
\[
...
%% --
	...
	\end{equation}
	%% --
	Since $(r - T')$ is $\sigma(M(K),C(K))$-continuous, equation Eq.~\eqref{eq:b3-1.16} implies that every $\sigma(M(K),C(K))$ cluster point of\/ $(\psi_{n})$ is a positive eigenvector, provided that it is non-zero.
...
\end{proof}
%% --
This theorem implies that for positive semigroups on $C(K)$ the growth and spectral bounds coincide (\cf Part-I, Proposition~\ref{prop:a3-4.4}).
...
\begin{proof}
	...
	Then we have $-\1_{K} \leq f - \1_{K} \leq \1_{K}$ hence by Eq.~\eqref{eq:b3-2.2} $\|f - \1_{K} - \im\cdot r\cdot \1_{K}\| \leq \rho_{r}$ for every $r \in \R$.
	From $T\1_{K} = \1_{L}$ and $\|T\| \leq 1$ it follows that $\|Tf - \1_{L} - \im\cdot r\cdot \1_{L}\| \leq \rho_{r}$ for every $r \in \R$.
	Using Eq.~\eqref{eq:b3-2.2} once again, we obtain $-\1_{L} \leq Tf - \1_{L} \leq \1_{L}$ or $0 \leq Tf \leq 2\cdot \1_{L}$.
\end{proof}
%% --
...
\begin{equation}\label{eq:b3-2.9}
	...
	Defining $T_{1} \coloneqq M_{\tilde{h}}^{-1}\tilde{R}M_{\tilde{h}}$ we have by Eq.~\eqref{eq:b3-2.9}
	%% --
	...
	\end{equation}
	%% --
	Hence we have $\|T_{1}\| \leq \|\tilde{T}\| = 1$ (by Eq.~\eqref{eq:b3-2.11}, Eq.~\eqref{eq:b3-2.9}, Eq.~\eqref{eq:b3-2.1}.
	Then it follows from Lemma~\ref{lem:b3-2.1} that $T_{1}$ is a positive operator.
	Thus Eq.~\eqref{eq:b3-2.11} implies that $0 \leq T_{1} \leq \tilde{T}$ and therefore $\|\tilde{T} - T_{1}\| = \|(\tilde{T} - T_{1})\1\| = 0$ (by Eq.~\eqref{eq:b3-2.10}, Eq.~\eqref{eq:b3-2.9}, Eq.~\eqref{eq:b3-2.1}.
\end{proof}
%% --
...
Let $T$ be a positive linear operator on $C_{0}(X)$, let $h \in C_{0}(X)$ and $\lambda \in \C$, $|\lambda| = 1$.
	If\/ $Th = \lambda h$ and $T|h| = |h|$, then we have $Th^{[n]} = \lambda^{n}h^{[n]}$ for every $n \in \Z$ (\cf Eq.~\eqref{eq:b3-2.4}).
...
\begin{proof}
	...
	Defining $\tilde{R} \coloneqq \lambda\tilde{T}$, then $\tilde{T}$, $\tilde{R}$, $\tilde{h}$ satisfy Eq.~\eqref{eq:b3-2.8}, hence we have
	%% --
	...
	\end{equation*}
	%% --
	(see Eq.~\eqref{eq:b3-2.7} and Eq.~\eqref{eq:b3-2.12}, which is precisely $Th^{[n]} = \lambda^{n}h^{[n]}$ for all $n \in \Z$.
	
	If\/ $h$ does not have zeros, then $\overline{E_{h}} = E$, hence $T = \tilde{T}$, $h = \tilde{h}$ and the remaining assertion follows from Eq.~\eqref{eq:b3-2.12}.
%% --

	(ii) This can be deduced easily from (i) as follows. 
	If\/ $Ah = (\alpha+\im\beta )h$, $A|h| = \alpha|h|$, then we have by Part-I, Corollary~\ref{cor:a3-6.4}
	%% --
	\[
...
Given $\im \alpha \in P\sigma_{b}(A)$ then there is $h \in C_{0}(X)$, $h \neq 0$, such that $Ah = \im\alpha h$ or $T(t)h = \eu^{\im\alpha t}h$ for all $t$ (Part-I, Corollary~\ref{cor:a3-6.4}).
	...
	\end{equation}
	%% --
	Since $\phi$ is strictly positive, Eq.~\eqref{eq:b3-2.14}  and Eq.~\eqref{eq:b3-2.15} imply that $T(t)|h| = |h|$ for $t \geq 0$ or equivalently $A|h| = 0$.
...
More precisely, if\/ $I = \{f \in C_{0}(X) \colon f|_{M} = 0\}$ for a suitable closed subset $M \subset X$, then $I \cong C_{0}(X\backslash M)$ and $C_{0}(X)/I \cong C_{0}(M)$ (\cf Chapter~\ref{chap:b1}).
...
\begin{proof}
	...
	Thus the following relations hold (see Eq.~\eqref{eq:a3-3.6} on p.~\pageref{eq:a3-3.6}),
	%% --
	...
	Since $R(\lambda,A)$ is positive if\/ $\lambda > 0$ (Corollary~\ref{cor:b3-1.3}), it follows from Eq.~\eqref{eq:b3-2.17} that $Q_{k}$ is a positive bounded operator, hence $I_{k-1} = \{f \in E \colon Q_{k}|f| = 0\}$ is a closed ideal.
	Since $Q_{k}$ commutes with $R(\lambda,A)$ (see Eq.~\eqref{eq:b3-2.17}, it follows that $I_{k-1}$ is a $\TT$-invariant ideal.
...
\begin{proof}
	...
	In view of the lemma and Part-I, Corollary~\ref{cor:a3-4.3} (i) we can assume that $s(A)$ is a first order pole with strictly positive residue, we call it $Q$.
	We have $AQ = QA = s(A)A = 0$ (see Eq.~\eqref{eq:a3-3.6} on p.~\pageref{eq:a3-3.6}), hence
	%% --
	...
	By Eq.~\eqref{eq:b3-2.18} we have $Q(T(t)|h| - |h|) = 0$.
...
\end{proof}
%% --
If\/ $A$ has compact resolvent, then every point of\/ $\sigma(A)$ is a pole of the resolvent (see Example~\ref{ex:a3-3.6} hence we have
\index{resolvent!compact|(}
...
%%
    ...
    \]
    %% --
    \eg that $T(t_{0})$ is compact and $r(T(t_{0})) > 0$ (see Eq.~\eqref{eq:a3-3.7} on p.~\pageref{eq:a3-3.7}).
...
\begin{proof}
	...
	From Part-I, Corollary~\ref{cor:a3-6.4} it follows that $\{\lambda \in \sigma(T(t)) \colon |\lambda| > r\}$ contains infinitely many eigenvalues (counted according to their multiplicities) for every $r < \exp(s(A)t) = r(T(t))$.
	This contradicts the assumption $r_{\text{ess}}(T(t)) < r(T(t))$ (see Eq.~\eqref{eq:a3-3.7} on p.~\pageref{eq:a3-3.7}).
\end{proof}
%% --
...
\index{retarded differential equation}
	...
	\end{equation}
	%% --
It follows from Eq.~\eqref{eq:b3-2.25} and Eq.~\eqref{eq:b3-2.26} that for $\lambda > \|\phi\|$, the resolvent $R(\lambda,A_{\phi})$ exists and satisfies $\|R(\lambda,A_{\phi})\| \leq \lambda/(\lambda-\|\phi\|)\cdot 1/\lambda = 1/(\lambda-\|\phi\|)$.
...
Hence, if\/ $\lambda_0 \in \rho_{+}(A)$ then by Part-I, Example~\ref{subsec:a1-3.3} every $R(\lambda_0,A)$-invariant subspace is $\TT$-invariant and vice versa.
%% --
...
\end{enumerate}
%% --
The rotation semigroup (see Part-I, Example~\ref{subsec:a1-2.5}) is irreducible but it does not satisfy condition \ref{item:b3-3.1-vii}.
...
%% --
\begin{proof}
	Since every closed ideal is of the form $\{f \in E \colon f|_{M} = 0\}$ where $M \subset X$ is a closed subset (\cf Section~\ref{sec:b1-1}), it is clear that all closed ideals are invariant under the multiplication operator $M_{h}$ and $M_{-h}$, respectively.
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
	...
	It follows from Eq.~\eqref{eq:b3-2.14} and Eq.~\eqref{eq:b3-2.15} that
	%% --
...
%% --
	...
	By Eq.~\eqref{eq:b3-3.4} $|h|$ is a positive eigenfunction but not strictly positive.
...
\begin{proof}
	...
	\item[(iv)] 
	Supposing $Ah = \im\alpha h$ $(h \neq 0, \alpha \in \R)$ then $A|h| = 0$ by Eq.~\eqref{eq:b3-2.14} and Eq.~\eqref{eq:b3-2.15}.
	By Proposition~\ref{prop:b3-3.5}\,(ii) $|h|$ is strictly positive, thus Theorem~\ref{thm:b3-2.4}\,(ii) implies Eq.~\eqref{eq:b3-3.5}.
	...
	By Eq.~\eqref{eq:b3-3.5} we have $S_{\bar{g}}A S_{g} = A + \im\beta$ and $S_{h}A S_{\bar{h}} = A - \im\alpha$, therefore
	...
	\]
	By Eq.~\eqref{eq:b3-3.7} the linear form $\phi$ vanishes on $\Kern{\lambda - A}$ whenever $\lambda \neq 0$.
...
of period $\tau$ (see Part-I, Example~\ref{subsec:a1-2.5}).
...
Next we apply Theorem~\ref{thm:b3-3.6}\,(iv) in order to show that the spectral bound is a dominant eigenvalue.
	...
	\end{equation}
%% --
If we define $u \coloneq |h|$ and $w \coloneq h/|h|$, then Eq.~\eqref{eq:b3-3.12} reads
...
%% --
		...
		\end{equation}
%% --
	This identity and Eq.~\eqref{eq:b3-3.13} implies that $2u/w\cdot\sum_{ij} a_{ij}w_{i}'w_{j}' = 0$.
	Since $u$ has no zeros and $(a_{ij})$ is positive definite, we have $\text{grad } w = (w_{i}') = 0$ in $\Omega$, hence $w = \text{const}$.
	Then, by Eq.~\eqref{eq:b3-3.13}, we have $\im \alpha uw = L_{0}(uw) = wL_{0}(u) = 0$, a contradiction.
...
A complete proof of this results requires some facts of the theory of Banach lattices, therefore, we postpone it to Part-III, Theorem~\ref{thm:c3-4.2}.
%% --
...
We prove the assertion concerning $A\sigma(A)$ and $P\sigma(A)$.
	Assume that $Ah = (\alpha + \im\beta)h$, $\alpha,\beta \in \R$, $h \neq 0$, then $T(t)h = \eu^{\alpha t}\eu^{\im\beta  t}h$ for all $t \geq 0$ (Part-I, Corollary~\ref{cor:a3-6.4}).
...
Then a semigroup of lattice homomorphisms can be described explicitly by a semi-flow $\phi$, and real-valued functions $h$ and $p$ (see Theorem~\ref{thm:b2-3.5} and Theorem~\ref{thm:b2-3.6}). 
The function $p$ has no influence on spectral properties (\cf  Eq.~\eqref{eq:b2-3.7} on p.~\pageref{eq:b2-3.7}). 
...
In the following we will describe the spectrum of the semigroup given by Eq.~\eqref{eq:b3-4.1} in terms of\/ $\phi$ and $h$.
At first we have to fix some notation.
Let $K$, $\phi$, $h$ be as in Eq.~\eqref{eq:b3-4.1}. 
Then
%% --
...
Studying the spectrum of the semigroup given by Eq.~\eqref{eq:b3-4.1} we divide the complex plane into these three sets
%% --
...
\end{align}
%% --
It is easy to see that $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\|T(t)\|$, hence in the definition of\/ $\overline{c}(h,\phi)$, both the limit and the infimum exist and coincide with the growth bound (see Part-I, Eq.~\eqref{eq:a1-1.1} on p.~\pageref{eq:a1-1.1} ).
...
\begin{theorem}\label{thm:b3-4.4}
	Suppose $\TT$ is a semigroup of lattice homomorphisms given by Eq.~\eqref{eq:b3-4.1} with generator $A$.
	Considering $H \coloneq \{\lambda \in \C \colon \Re(\lambda) < \underline{c}(h,\phi)\}$, where $\underline{c}(h,\phi)$ is given by Eq.~\eqref{eq:b3-4.4}, then we have.
	\begin{enumerate}[\upshape (i)]
		\item 
...
\begin{proof}
	...
	(i) By the definition given in Eq.~\eqref{eq:b3-4.1} there exists a $\tau > 0$ such that $\underline{c}_{t}(h,\phi) \geq \underline{c}(h,\phi) - \epsilon$ for all $t \geq \tau$.
	It follows that
	%% --
	...
	\end{equation}
	%% --
	It follows from Eq.~\eqref{eq:b3-4.8} and Eq.~\eqref{eq:b3-4.7} that $ g_n / \|g_n\|$ is an approximate
	...
	Identifying $C(K)/I_{\infty}$ with $C(K_{\infty})$ (\cf Section~\ref{sec:b1-1}), then $(T(t)_{/I_{\infty}})$ is the semigroup governed by $\phi_{|K_{\infty}}$ and $h_{|K_{\infty}}$.
	...
	We choose $\tau > 0$ such that Eq.~\eqref{eq:b3-4.5} is true.
	...
	\end{equation}
	%% --
	It follows that the disc $D \coloneq \{\lambda \in \C \colon |\lambda| < \exp(\underbar{c}(h,\phi)-2\epsilon)\}$ has an empty intersection with $A{\sigma}(T(\tau))$ and therefore $H_{2\epsilon}\cap A\sigma(A) = \emptyset$ by Part-I, Proposition~\ref{prop:a3-6.2}
	...
	Since we assumed that $\phi$ is surjective, assertions (ii) and (iii) of the theorem are immediate consequences of Eq.~\eqref{eq:b3-4.10}.
\end{proof}
%% --
...
We continue the discussion of the spectrum of semigroups of lattice homomorphisms on $C(K)$ given by Eq.~\eqref{eq:b3-4.1}.
...
\begin{example}\label{ex:b3-4.7}
	\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
		...
		The spectrum of the semigroup corresponding to $\phi$ and $h$ (\cf Eq.~\eqref{eq:b3-4.1}) is given by
		%% --
		\[
...
Eq.~\eqref{eq:b3-4.1} on p.~\pageref{eq:b3-4.1} and let $\underline{c}(h,\phi)$, $\overline{c}(h,\phi)$ be defined as in Eq.~\eqref{eq:b3-4.4} on the same page.
...
%% --
\begin{proof}	
	(i) and (ii): By Eq.~\eqref{eq:b3-4.4} we have $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\|T(t)\|$ hence $\overline{c}(h,\phi) = \omega_{0}(A)$ (\cf Eq.~\eqref{eq:a1-1.1} on p.~\pageref{eq:a1-1.1} ).  
...
\end{equation}
%% --
In view of Eq.~\eqref{eq:b3-4.13} and Eq.~\eqref{eq:b3-4.14} it is not difficult to find a subsequence $k(n)$ of\/ $\N$ such that $(\phi_{n,k(n)})$ is an approximate eigenvector of\/ $A'$ corresponding to $\hat{h} + \im\beta$.
\end{proof}
%% --
...
\item 
    ...
    Then from Eq.~\eqref{eq:b3-4.15} we can conclude that $\|R(\lambda,A)f\| \leq M\left(\int_{0}^{r} \left|\eu^{-\lambda t}\right|\dt\right)\|f\|$ for $\lambda \in U$, $\Re(\lambda) > 0$, therefore $\lambda_{0} = \im\beta \in \rho(A)$.
\end{enumerate}
\end{proof}
%% --
...
\end{remark}
	...
	The main reason is that there is no description as given in Eq.~\eqref{eq:b3-4.1} of the semigroups for locally compact spaces $X$.
...
In this case there is an analogue of Eq.~\eqref{eq:b3-4.1} (\cf Theorem~\ref{thm:b2-3.14}) and the spectrum can be described completely.
...
Lemma~\ref{lem:b3-2.8} in combination with Part-III, Lemma~\ref{lem:c3-3.13}  can be used to characterize semigroups whose spectral bound is a pole of finite algebraic multiplicity (see Eq.~\eqref{eq:c3-3.19} on p.~\pageref{eq:c3-3.19}).
...
Example~\ref{ex:b3-3.4}\,(ii) will be discussed in more detail and more generality in Section~\ref{sec:b4-3}.
