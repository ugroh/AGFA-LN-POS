...
From the explicit representation of\/ $T(t)$,
    ...
    \[
    \langle \phi,f \rangle = \lim_{t \to \infty}\langle T(t)f,\phi \rangle = 0 \text{ for every } f\,,
    \]
    \ie $\phi = 0$.
\end{example}
\index{resolvent!compact|)}
...
\begin{equation}\label{eq:b3-2.9}
	...
	Thus Eq.~\myeqpageref{eq:b3-2.11} implies that $0 \leq T_{1} \leq \tilde{T}$ and therefore from Eq.~\myeqpageref{eq:b3-2.9} and 
    \marginpar{?Eqs. (3.2.9) and (3.2.10)?}Eq.~\myeqpageref{eq:b3-2.10}) we conclude
    \[
...
\item 
    ...
    \[
    Ah = (\alpha+\im\beta )h \text{ amd } A|h| = \alpha|h|\,,
    \] 
...
\item
Suppose $M$ is a closed cyclic subset of\/ $\im \R$, $M = \bigcup_{\alpha \in S} \im\alpha\Z$ for a suitable $S \subset \R$ (\eg $S = M$).
	
The space 
\[
E_{1} \coloneqq \{(f_{\alpha})_{\alpha \in S} \colon f_{\alpha} \in C(\Gamma), \sup_\alpha \|f_{\alpha}\| < \infty\}
\] 
is a Banach space under the norm 
\[
\|(f_{\alpha})\| \coloneqq \sup_\alpha \|f_{\alpha}\|\,.
\]
The closure of the linear subspace 
\[
E_{0} \coloneqq \{(f_{\alpha}) \in E_{1} \colon f_{\alpha} \neq 0 \text{ only for finitely many } \alpha \in S\}
\]
...
\]
for a suitable closed subset $M \subset X$, then $I \cong C_{0}(X\backslash M)$ and $C_{0}(X)/I \cong C_{0}(M)$ (\cf Chapter~\ref{chap:b1}).
...
\begin{proof}
	...
	Since $R(\lambda,A)$ is positive if\/ $\lambda > 0$ (Corollary~\ref{cor:b3-1.3}), it follows from Eq.~\myeqpageref{eq:b3-2.17} that $Q_{k}$ is a positive bounded operator, hence 
    \[
...
\begin{tabular}{p{10mm}p{10mm}p{10mm}|c|p{8mm}ccc} % <--: 1st column left, 2nd middle and 3rd right, with vertical lines in between
		 &&& $ \overset{\text{\small pole}}{\text{\small order}}$ & $\ I_0$ & $I_2$ & $I_2$ & $I_3$ \\[1ex]\hline
...
\index{retarded differential equation}
	...
	\end{equation}
	%% --
It follows from Eq.~\myeqpageref{eq:b3-2.25} and Eq.~\myeqpageref{eq:b3-2.26} that for $\lambda > \|\phi\|$, the resolvent $R(\lambda,A_{\phi})$ exists and satisfies 
\[
...
\[
\Kern{\lambda-A_{m}} = \{\alpha e_{\lambda} \colon a \in \C\}\,,
\]
...
\end{equation}
%% --
since the assumption $\phi + r\delta_{0} \geq 0$ implies that the function $\lambda \mapsto \phi(e_{\lambda})$ is strictly decreasing and $\lim_{\lambda \to \infty}\langle e_{\lambda},\phi \rangle > -\infty$, $\lim_{\lambda \to -\infty}\langle e_{\lambda},\phi \rangle = \infty$ unless $\phi = r_{0}\delta_{0}$ for some $r_{0} \in \R$.)
\end{enumerate}
%% --
\end{example}
%% --
...
%
	...
	Thus given $0 < f \in \Kern{A}$ and any $0 \leq \psi \in E'$ such that $\langle f,\psi \rangle > 0$, we have for $\phi \coloneqq P'\psi \in \Kern{A'}$, and
    \[
...
%% --
    ...
    \end{equation}
%% --
    Actually, its adjoint  $T'$ is dominated by a finite rank operator as well, hence it maps the unit ball in an order interval. Since the dual space has order continuous norm, order intervals are weakly compact (see Part-III, Section~\ref{sec:c1-5}). 
...
\begin{proof}
	...
	It follows that 
    \[\textstyle
    \Kern{A + i(\beta - \alpha)} = S_{h}S_{\bar{g}}(\Kern{A}) \neq \{0\}\,,
    \]
    hence $\im (\beta - \alpha) \in P\sigma(A)$.
	...
	Therefore $\langle h\cdot p_{n}(h),\phi\rangle = 0$ and we have 
    \[
    0 < \langle|h|,\phi\rangle = \lim_{n\to\infty}\langle h\cdot p_{n}(h),\phi\rangle = 0
    \]
...
We prove the assertion concerning $A\sigma(A)$ and $P\sigma(A)$.
	Assume that 
    \[
    Ah = (\alpha + \im\beta)h\,, \alpha,\beta \in \R\,, h \neq 0\,,
    \]
...
where 
\[
\phi = (\phi_{t}): \R_{+} \times K \to K\,,\ \phi_{t+s}
=\phi_{t}\circ\phi_{s}  
\ (t,\,s \geq 0)\,,
\]
