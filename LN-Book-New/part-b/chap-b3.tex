...
To that purpose we recall that the spectral bound $s(A)$ of a generator $A$ is defined as the least upper bound of the real parts of all spectral values (\cf Eq.~\myeqpageref{eq:b3-1.2} ).
%\pagebreak[1]
%
...
Moreover, by Eq.~\myeqpageref{eq:a3-3.6}, we have 
%
\[
...
\begin{proof}
	...
	Then we have $-\1_{K} \leq f - \1_{K} \leq \1_{K}$ hence by Eq.~\myeqpageref{eq:b3-2.2} $\|f - \1_{K} - \im\cdot r\cdot \1_{K}\| \leq \rho_{r}$ for every $r \in \R$.
...
\begin{equation}\label{eq:b3-2.9}
	...
	\end{equation}
	%% --
	Hence we have $\|T_{1}\| \leq \|\tilde{T}\| = 1$ (by Eq.~\myeqpageref{eq:b3-2.11}, Eq.~\myeqpageref{eq:b3-2.9}, Eq.~\myeqpageref{eq:b3-2.1}.
	Then it follows from Lemma~\ref{lem:b3-2.1} that $T_{1}$ is a positive operator.
	Thus Eq.~\myeqpageref{eq:b3-2.11} implies that $0 \leq T_{1} \leq \tilde{T}$ and therefore $\|\tilde{T} - T_{1}\| = \|(\tilde{T} - T_{1})\1\| = 0$ (by Eq.~\myeqpageref{eq:b3-2.10}, Eq.~\myeqpageref{eq:b3-2.9}, Eq.~\myeqpageref{eq:b3-2.1}.
\end{proof}
%% --
...
\begin{proof}
	...
	Thus the following relations hold (see Eq.~\myeqpageref{eq:a3-3.6}),
	%% --
...
\begin{proof}
	...
	We have $AQ = QA = s(A)A = 0$ (see Eq.~\myeqpageref{eq:a3-3.6}), hence
	%% --
...
%%
    ...
    \]
    %% --
    \eg that $T(t_{0})$ is compact and $r(T(t_{0})) > 0$ (see Eq.~\myeqpageref{eq:a3-3.7}).
...
\index{retarded differential equation}
	...
	\end{equation}
	%% --
It follows from Eq.~\myeqpageref{eq:b3-2.25} and Eq.~\myeqpageref{eq:b3-2.26} that for $\lambda > \|\phi\|$, the resolvent $R(\lambda,A_{\phi})$ exists and satisfies $\|R(\lambda,A_{\phi})\| \leq \lambda/(\lambda-\|\phi\|)\cdot 1/\lambda = 1/(\lambda-\|\phi\|)$.
...
\item 
    ...
    Then from Eq.~\myeqpageref{eq:b3-4.15} we can conclude that $\|R(\lambda,A)f\| \leq M\left(\int_{0}^{r} \left|\eu^{-\lambda t}\right|\dt\right)\|f\|$ for $\lambda \in U$, $\Re(\lambda) > 0$, therefore $\lambda_{0} = \im\beta \in \rho(A)$.
\end{enumerate}
\end{proof}
%% --
