...
To that purpose we recall that the spectral bound $s(A)$ of a generator $A$ is defined as the least upper bound of the real parts of all spectral values (\cf Eq.~\eqref{eq:b3-1.2} ).
%\pagebreak[1]
%
...
Moreover, by Eq.~\eqref{eq:a3-3.6}, we have 
%
\[
...
\begin{proof}
	...
	Thus the following relations hold (see Eq.~\eqref{eq:a3-3.6}),
	%% --
...
\begin{proof}
	...
	We have $AQ = QA = s(A)A = 0$ (see Eq.~\eqref{eq:a3-3.6}), hence
	%% --
...
%%
    ...
    \]
    %% --
    \eg that $T(t_{0})$ is compact and $r(T(t_{0})) > 0$ (see Eq.~\eqref{eq:a3-3.7}).
...
\begin{proof}
	...
	This contradicts the assumption $r_{\text{ess}}(T(t)) < r(T(t))$ (see Eq.~\eqref{eq:a3-3.7}).
\end{proof}
%% --
...
The function $p$ has no influence on spectral properties (\\cf Eq.~\eqref{eq:b2-3.7}). 
...
\end{align}
%% --
It is easy to see that $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\|T(t)\|$, hence in the definition of\/ $\overline{c}(h,\phi)$, both the limit and the infimum exist and coincide with the growth bound (see Part-I, Eq.~\eqref{eq:a1-1.1} ).
...
Eq.~\eqref{eq:b3-4.1} and let $\underline{c}(h,\phi)$, $\overline{c}(h,\phi)$ be defined as in Eq.~\eqref{eq:b3-4.4} on the same page.
...
%% --
\begin{proof}	
	(i) and (ii): By Eq.~\eqref{eq:b3-4.4} we have $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\|T(t)\|$ hence $\overline{c}(h,\phi) = \omega_{0}(A)$ (\cf Eq.~\eqref{eq:a1-1.1} ).  
...
Lemma~\ref{lem:b3-2.8} in combination with Part-III, Lemma~\ref{lem:c3-3.13}  can be used to characterize semigroups whose spectral bound is a pole of finite algebraic multiplicity (see Eq.~\eqref{eq:c3-3.19}).
