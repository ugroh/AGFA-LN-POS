...
\begin{proof}
	...
	\begin{equation}\label{eq:b3-1.1}
		[s(A)-\epsilon,\infty) \subseteq \rho(A), \quad \mu_{0} \coloneqq \alpha_{0} + \im\beta_{0} \in \sigma(A) \quad \text{and} \quad \alpha_{0} > s(A)-\epsilon.
	\end{equation}
	%% --
...
\item
Suppose $M$ is a closed cyclic subset of\/ $\im \R$, $M = \bigcup_{\alpha \in S} \im\alpha\Z$ for a suitable $S \subseteq \R$ (\eg $S = M$).
...
\]
for a suitable closed subset $M \subseteq X$, then $I \cong C_{0}(X\backslash M)$ and $C_{0}(X)/I \cong C_{0}(M)$ (\cf Chapter~\ref{chap:b1}).
It follows that given another closed ideal $J = \{f \in C_{0}(X) \colon f|_{N} = 0\}$ such that $I \subseteq J$ \ie $N \subseteq M$, then $J/I$ can be identified with $C_{0}(M\backslash N)$.
...
\begin{equation}\label{eq:b3-2.16}
    I_{-1} \coloneqq \{0\} \subseteq I_{0} \subsetneq I_{1} \subsetneq \ldots \subsetneq I_{k} \coloneqq E
\end{equation}
%% --
