...
The closed principal ideal $\overline{E_{h}}$, which is canonically isomorphic to $C_{0}(X_{1})$ with $X_{1} = \{x \in X \colon h(x) \neq 0\}$, is $T$-invariant.
	...
	Defining $\tilde{R} \coloneqq \bar{\lambda}\tilde{T}$, then $\tilde{T}$, $\tilde{R}$, $\tilde{h}$ satisfy Eq.~\myeqpageref{eq:b3-2.8}, hence we have
	%% --
...
\begin{proof}
	...
	We have $AQ = QA = s(A)A = 0$ (see Part-I, Example~\ref{ex:a3-3.6}), hence
	%% --
...
\item
The concept of irreducibility as defined in~\ref{def:b3-3.1} is closely related to various other notions:
