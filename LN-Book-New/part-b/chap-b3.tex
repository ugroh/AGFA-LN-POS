...
These results were later extended to the infinite dimensional setting, with important contributions from Jentzsch, Karlin, Krein, Krasnoselski'i, Lotz, Rota, Rutman, Schaefer and others (see Chapt.V of \textcite{schaefer:1974}).
...
To that purpose we recall that the spectral bound $s(A)$ of a generator $A$ is defined as the least upper bound of the real parts of all spectral values (cf.\ Part-I, Chapter 3, \,(1.2)).
%\pagebreak[1]
%
...
\begin{proof}
	...
	From Part-I, Chapter 3,  Proposition~2.5 it follows that
	%% --
	...
	\end{equation}
	%% --
	Thus $\sigma(R(\lambda_{0},A))$ contains non-zero spectral values which in view of Part-I, Chapter 3,  Proposition~2.5 is equivalent to $\sigma(A) \neq \emptyset$.
\end{proof}
%% --
...
It follows that $\sigma(T(t)) = \{0\}$ for all $t > 0$ and by Part-I, Chapter 3,  Theorem~6.2 we have $\sigma(A) = \emptyset$.
...
$C_{0}(\left[0,\infty\right[)$ both are topologically (but not isometrically) isomorphic to $C(\left[0,1\right])$ (see \textcite[Section~21.5]{semadeni:1971}), one obtains from (i) and (ii) (non-positive) semigroups on $C(\left[0,1\right])$ whose generators have empty spectrum.
\end{enumerate}
\end{examples}
%% --
...
A direct proof not using this fact is given in Part-III, Chapter 3,  Corollary~1.4.
%% --
...
%% --
\begin{proof}
	The second statement is an immediate consequence of Theorem~\ref{thm:b3-1.1} and Part-I, Chapter 3,  Proposition~2.5.
...
Moreover, by Part-I, Chapter 3,  3.6., we have 
%
\[
...
\end{proof}
% --
The example of the rotation semigroup (Part-I, Chapter 3,  Example~5.6) shows that the assumptions in Corollary~\ref{cor:b3-1.4} do not imply that $s(A)$ is dominant.
...
\end{proof}
%% --
This theorem implies that for positive semigroups on $C(K)$ the growth and spectral bounds coincide (cf.\ Part-I, Chapter 3,  4.4).
Actually, this is true for locally compact spaces as well and can be proved directly (see Part-II, Chapter 4,  Theorem~1.4).
...
%%
	Consider $E = C_{0}(\R^{n})$ and the semigroup $(T(t))_{t \geq 0}$ generated by the Laplacian (cf.\ Part-I, Chapter 1,  2.8).
...
Actually, from Theorem~\ref{thm:b3-1.1} and Part-I, Chapter 3,  Proposition~2.5 it follows that the peripheral spectrum of\/ $R(\lambda,A)$ (\ie the set of spectral values having maximal absolute value) is trivial since it only contains the spectral radius $r(R(\lambda,A)) = (\lambda - s(A))^{-1}$.
...
%% --
	...
	If\/ $Ah = (\alpha+\im\beta )h$, $A|h| = \alpha|h|$, then we have by Part-I, Chapter 3,  Corollary~6.4
	%% --
	\[
...
\index{rotation semigroup} on $C(\Gamma)$ (see Part-I, Chapter 3,  Example~5.6) with period $2\pi/\alpha$ has boundary spectrum $M_{3}$.
...
Given $\im \alpha \in P\sigma_{b}(A)$ then there is $h \in C_{0}(X)$, $h \neq 0$, such that $Ah = \im\alpha h$ or $T(t)h = \eu^{\im\alpha t}h$ for all $t$ (Part-I, Chapter 3,  Corollary~6.4).
...
More precisely, if\/ $I = \{f \in C_{0}(X) \colon f|_{M} = 0\}$ for a suitable closed subset $M \subset X$, then $I \cong C_{0}(X\backslash M)$ and $C_{0}(X)/I \cong C_{0}(M)$ (cf.\ Part-II, Chapter 1).
...
\begin{proof}
	...
	Thus the following relations hold (see Part-I, Chapter 3,  3.6),
	%% --
	...
	By Part-I, Chapter 3,  Corollary~4.3 the generators $A_{|I_{k-1}}$ and $A_{k}$ induced by $A$ on $I_{k-1}$ and $E/I_{k-1}$, respectively, have a pole at $0$.
	...
	From Part-I, Chapter 3,  Corollary~4.3 it follows that the pole order of\/ $A_{k}$ and $A_{|I_{k-1}}$ is precisely $1$ and $k-1$, respectively.
...
\begin{proof}
	...
	In view of the lemma and Part-I, Chapter 3,  Proposition~4.3(i) we can assume that $s(A)$ is a first order pole with strictly positive residue, we call it $Q$.
	We have $AQ = QA = s(A)A = 0$ (see Part-I, Chapter 3,  3.6), hence
	%% --
	\begin{equation}\label{eq:b3-2.18}
		QT(t) = T(t)Q = Q \text{ for all } t \geq 0.
	\end{equation}
	%% --
	If\/ $Ah = \im\alpha h$ for some $\alpha \in \R$, $h \neq 0$, then $T(t)h = \eu^{\im\alpha t}h$ (by Part-I, Chapter 3,  Corollary~6.4).
...
\end{proof}
%% --
If\/ $A$ has compact resolvent, then every point of\/ $\sigma(A)$ is a pole of the resolvent (see Part-I, Chapter 3,  3.6) hence we have
\index{resolvent!compact|(}
...
The assumptions of Corollary~\ref{cor:b3-2.10} do not imply that $s(A)$ is dominant, the rotation semigroup (Part-I, Chapter 3,  Example~5.6) is a counterexample.
%% --
...
%%
    ...
    \]
    %% --
    \eg that $T(t_{0})$ is compact and $r(T(t_{0})) > 0$ (see Part-I, Chapter 3,  3.7).
	Then $s(A)$ is a strictly dominant eigenvalue.
\end{corollary}
%% --
\begin{proof}
	If\/ $s(A)$ is not strictly dominant, then we have by Theorem~\ref{thm:b3-2.9} and Part-I, Chapter 3,  Corollary~6.5 that 
    %% --
    \[
	...
	From Part-I, Chapter 3,  Corollary~6.4 it follows that $\{\lambda \in \sigma(T(t)) \colon |\lambda| > r\}$ contains infinitely many eigenvalues (counted according to their multiplicities) for every $r < \exp(s(A)t) = r(T(t))$.
	This contradicts the assumption $r_{\text{ess}}(T(t)) < r(T(t))$ (see Part-I, Chapter 3,  3.7).
\end{proof}
%% --
...
\begin{proof}
    ...
    \]
    %% --$$ 
    is compact for every $r > 0$ (see Part-I, Chapter 2,  Theorem~1.20) and this set does not have accumulation points because $A$ has compact resolvent.
...
Then the Hille-Yosida Theorem (Part-I, Chapter 2,  Theorem~1.7) implies that $A_{\phi}$ generates a semigroup $(T(t))$ satisfying $\|T(t)\| \leq \exp(\|\phi\|t)$.
Moreover, this semigroup is eventually norm continuous (see Part-II, Chapter 4, Corollary~3.3).

By Part-II, Chapter 2,  Example~1.22 we have the following equivalence.
%% --
...
%
	...
	If\/ $0$ is a first order pole, then the residue $P$ is a positive projection satisfying $PE = \Kern{A}$, $P'E' = \Kern{A'}$ (see Part-I, Chapter 3, \,3.6).
	...
	%% --
	\item 
	If a linear operator $S$ on $C_{0}(X)$ is weakly compact, then $S^{2}$ is compact (see Part-II, Chapter 4,  Proposition~2.4(ii)).
    ...
    \begin{equation}\label{eq:b3-2.31}
    	...
    	If   $T \in \L{C(K)}$, $K$ compact, is positive, then it is weakly compact  	if and only if its biadjoint 	$T''$ maps bounded Borel functions into $C(K)$ (see Part-II, Chapter 4,  Proposition~2.4).
    	\end{minipage}
	\end{equation}
%% --
...
%% --
	...
	\item 
	Stronger results than Theorem~\ref{thm:b3-2.9} will be proved in Part-III, Chapter 3.
	Actually, assuming only that $s(A)$ is a pole of finite algebraic multiplicity one can show that $\sigma_{b}(A)$ contains only poles of finite multiplicity (Part-III, Chapter 3,  Theorem~3.13).
	In Part-III, Chapter 3,  Corollary~2.12 we will show that $\sigma_{b}(A)$ is cyclic whenever $s(A)$ is a pole of the resolvent.
	...
	For details we refer to Section~3 of Part-II, Chapter 4.
\end{enumerate}	
\end{remarks}
\index{spectrum!boundary|)} 
\index{resolvent!compact|)}
...
Hence, if\/ $\lambda_0 \in \rho_{+}(A)$ then by Part-I, Chapter 1,  3.3 every $R(\lambda_0,A)$-invariant subspace is $\TT$-invariant and vice versa.
%% --
...
\end{enumerate}
%% --
The rotation semigroup (see Part-I, Chapter 1,  2.5) is irreducible but it does not satisfy condition \ref{item:b3-3.1-vii}.
	However, assuming that the semigroup $(T(t))$ is holomorphic, then \ref{item:b3-3.1-vii}
	is equivalent to irreducibility.
	We will give a proof of this result in the more general situation of Banach lattices (see Part-III, Chapter 3,  Theorem~3.2\,(ii)).
\end{remark}
%% --
...
%% --
\begin{proof}
	Since every closed ideal is of the form $\{f \in E \colon f|_{M} = 0\}$ where $M \subset X$ is a closed subset (cf.\ Section~1 of Part-II, Chapter 1), it is clear that all closed ideals are invariant under the multiplication operator $M_{h}$ and $M_{-h}$, respectively.
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
	\item 
	(cf.\ Part-II, Chapter 2,  Section~3). Suppose $(T(t))_{t\geq 0}$ is governed by a continuous semiflow
...
%% --
	...
	\item 
	We consider the first derivative $Af = f'$ (cf.\ Part-I, Chapter 1,  2.4). 
	...
	\item 
	(cf.\ Example~\ref{ex:b3-2.14}\,(ii)) Now we consider $Af = f'$ on $E = C[-1,0]$ with $D(A_{\phi}) = \{f \in C^1 \colon f'(0) = \phi(f)\}$ where the linear functional $\phi$ satisfies $\phi + \alpha\delta_{0} \geq 0$ for some $\alpha \in \R$ (see Part-II, Chapter 2,  Example~1.22).
	The corresponding semigroup is irreducible if and only if\/ $-1 \in \supp\phi$.
	
	\item 
	The second derivative $Af = f''$ generates an irreducible semigroup on $C_{0}(\R)$ and on $C_{0}(0,1)$ (cf.\ Part-I, Chapter 1,  2.7).
	With Neumann boundary conditions (or more generally, $f'(0) = \alpha_{0}f(0)$, $f'(1) = \alpha_{1}f(1)$ where $\alpha_{0}, \alpha_{1} \in \R$) the second derivative generates an irreducible semigroup on $C[0,1]$ (cf.\ Part-I, Chapter 1, \,2.7).
	...
	The Laplacian 
    \index{operator!Laplacian} 
    $\Delta$ generates an irreducible semigroup on $C_{0}(\R^n)$ as can be seen easily from Part-I, Chapter 1,  2.8.
...
\begin{enumerate}[\upshape (i), wide, labelindent=.5em]
	...
	The assertion now follows from Part-I, Chapter 3,  Proposition~2.5.	

	\item 
	Suppose $Ah = rh$ where $h \neq 0$ is positive.
	Then $r$ has to be real and we have $T(t)h = \eu^{rt}h$ (Part-I, Chapter 3,  Corollary~6.4).
...
of period $\tau$ (see Part-I, Chapter 1,  2.5).
...
%
	...
	Assuming that $\lambda > \lambda_{0} \coloneqq \sup_x c(x)$, then $h$ is positive, even strictly positive by the maximum principle \textcite[Chapter~2, Theorem~6]{protterweinberger:1967}.
...
A complete proof of this results requires some facts of the theory of Banach lattices, therefore, we postpone it to Part-III (see Part-III, Chapter 3,  Theorem~4.2).
%% --
...
We prove the assertion concerning $A\sigma(A)$ and $P\sigma(A)$.
	Assume that $Ah = (\alpha + \im\beta)h$, $\alpha,\beta \in \R$, $h \neq 0$, then $T(t)h = \eu^{\alpha t}\eu^{\im\beta  t}h$ for all $t \geq 0$ (Part-I, Chapter 3,  Corollary~6.4).
	...
	To prove that $A\sigma(A)$ is cyclic as well, one considers a semigroup $\mathcal{F}$-product $E_{\mathcal{F}}^{\TT}$ of\/ $E$ (see Part-I, Chapter 3, \,4.5).
	...
	The proposition in Part-I, Chapter 3,  3.5 implies $A\sigma(A) = P\sigma(A_{\mathcal{F}})$.
...
Then a semigroup of lattice homomorphisms can be described explicitly by a semi-flow $\phi$, and real-valued functions $h$ and $p$ (see Part-II, Chapter 2,  Thms.3.5 \& 3.6). The function $p$ has no influence on spectral properties (cf.\ Part-II, Chapter 2, \,(3.7)). 
Therefore we will assume that $(T(t))$ has the following form (cf.\ Part-II, Chapter 2,  Theorem~3.5). 
%% --
...
\end{align}
%% --
It is easy to see that $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\|T(t)\|$, hence in the definition of\/ $\overline{c}(h,\phi)$, both the limit and the infimum exist and coincide with the growth bound (see Part-I, Chapter 1, \,(1.1)).
...
\begin{proof}
	...
	Identifying $C(K)/I_{\infty}$ with $C(K_{\infty})$ (cf.\ Part-II, Chapter 1,  Section~1), then $(T(t)_{/I_{\infty}})$ is the semigroup governed by $\phi_{|K_{\infty}}$ and $h_{|K_{\infty}}$.
	...
	Hence we have $\sigma(A_{|I_{\infty}}) = \emptyset$ and therefore $\sigma(A) = \sigma(A_{/I_{\infty}})$ by Part-I, Chapter 3,  Proposition~4.2.
	...
	\end{equation}
	%% --
	It follows that the disc $D \coloneq \{\lambda \in \C \colon |\lambda| < \exp(\underbar{c}(h,\phi)-2\epsilon)\}$ has an empty intersection with $A{\sigma}(T(\tau))$ and therefore $H_{2\epsilon}\cap A_{\sigma}(A) = \emptyset$ by Part-I, Chapter 3, \,6.2.
	Since every boundary point of the spectrum is an approximate eigenvalue (by Part-I, Chapter 3,  Proposition~2.2(i)) we have the following alternative.
	%% --
...
\begin{example}\label{ex:b3-4.6}
	...
	\end{equation}
	%% --
	For details see Sect.18.2 of \textcite{dieudonne:1971}.
	...
	If\/ $F$ is differentiable, its generator is the closure of\/ $A_{1}$ which is defined as follows (cf.\ Part-II, Chapter 2,  Example~3.15).
	%% --
...
%% --
\begin{proof}	
	(i) and (ii): By \eqref{eq:b3-4.4} we have $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\|T(t)\|$ hence $\overline{c}(h,\phi) = \omega_{0}(A)$ (cf.\ Part-I, Chapter 1, \,(1.1)).  
	...
	It is easy to see that $\underline{c}(h,\phi) = \underline{c}(h_{|K_{\infty}},\phi_{|K_{\infty}})$, moreover, we have $\sigma(A_{|I_{\infty}}) = \emptyset$ hence $\sigma(A) = \sigma(A_{/I_{\infty}})$ by Part-I, Chapter 3,  Proposition~4.2.
...
In this case there is an analogue of \eqref{eq:b3-4.1} (cf.\ Part-II, Chapter 2,  Theorem~3.14) and the spectrum can be described completely.
...
We will discuss this fact in more detail in Part-III, Chapter 3.
...
The notion \enquote{(imaginary) additively cyclic} was introduced by \textcite{derndinger:1980} (and {Schaefer (1980)} respectively).
...
Lemma~\ref{lem:b3-2.8} in combination with Part-III, Chapter 3,  Lemma 3.13 can be used to characterize semigroups whose spectral bound is a pole of finite algebraic multiplicity (see Part-III, Chapter 3, \,(3.19)).
The hypothesis of Theorem~\ref{thm:b3-2.9} can be weakened, one only needs that $s(A)$ is a pole of the resolvent (see Part-III, Chapter 3,  Corollary~2.12).
Further results on the cyclicity of the boundary spectrum will be given in Part-III, Chapter 3.
In particular we refer to Part-III, Chapter 3,  Theorems.2.10, 3.11 and 3.13.
...
Example~\ref{ex:b3-3.4}\,(ii) will be discussed in more detail and more generality in Section 3 of Part-II, Chapter 4.
We return to Remark~\ref{rem:b3-2.15}\,(ii) in Section 2 of Part-II, Chapter 4.
...
Proposition~\ref{prop:b3-3.3} will be generalized in Part-III, Chapter 3,  Proposition~3.3.
...
Elliptic operators (more general than Example~\ref{ex:b3-3.4}\,(ii)) as generators on spaces of continuous functions, were investigated by many people, \eg \textcite{bonyetal:1968}, \textcite{kuhn:1984}, \textcite{roth:1976}, \textcite{roth:1978} and \textcite{stewart:1974}.
