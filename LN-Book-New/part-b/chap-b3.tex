...
\chapternopage{Spectral Theory of Positive Semigroups on \texorpdfstring{$C_{0}(X)$}{C_0(X)}}\label{chap:b3}%
\thispagestyle{empty}
%% --
{\Large
\vspace*{-.75cm}
...
\begin{equation}\label{eq:b3-2.16}
    I_{-1} \coloneq \{0\} \subseteq I_{0} \subsetneq I_{1} \subsetneq \ldots \subsetneq I_{k} \coloneq E
\end{equation}
%% --
...
\begin{proof}
	...
	%% --
	\[
	I_{n} \coloneq \{f \in E \colon Q_{n+1}|f| = Q_{n+2}|f| = \ldots = Q_{k}|f| = 0\}
	\]
	%% --
...
\ref{item:b3-3.1-v} $\implies$~\ref{item:b3-3.1-vi}: If\/ $I$ is a $R(\lambda,A)$-invariant ideal and $0 < f \in I$, then $g \coloneq R(\lambda,A)f \in I$. 
