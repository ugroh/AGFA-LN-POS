...
\begin{enumerate}
\item
As we have seen in Chapter Part-I, Chapter 2,  Section 3, strongly continuous semigroups on commutative \WA-algebras, that is, on $L^{\infty}$, are already norm-continuous. 
...
\textcite[Prop. 3.1.13 and Prop. 4.2.1]{zbmath07458830}).
But on $\BH$ (where $H$ is an infinite-dimensional Hilbert space), there always exist $C_{0}$-semigroups that are strongly continuous but not uniformly continuous (see the example in Chapter 2, 1.1). 
It follows from Part-I, Chapter 2,  Theorem 2.5 that $\BH$ with infinite-dimensional $H$ 
does not have the Dunford-Pettis property. This also follows from Example Chapter 2, 1.1 
