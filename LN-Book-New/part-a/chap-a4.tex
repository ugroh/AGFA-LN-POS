...
\]
%% --
Hence, $\omega_{0}(A)$ coincides with the growth bound of the semigroup $(T(t))_{t \geq 0}$ as defined in A-I, 1.3. 
...
To see this, consider $E \coloneqq C_0(\R_{+}) \cap $ $L^{1}(\R_{+},\eu^{x}\dx)$. Then, as shown in A-III, Example~1.3, the translation semigroup satisfies $\|T(t)\| = 1$, and hence $\omega_{0}(A) = 0$. 
...
\item
Rescaling this semigroup (see A-I, 3.2), we obtain a semigroup with $\omega_{1}(A) = -1/2 $ and $\omega_{0}(A) = 1/2 $.
...
As seen in A-I, Proposition~1.11, if\/ $\int_0^\infty \eu^{-\lambda t} T(t) f  \dt$ exists for every $f \in E$, then $\lambda \in \rho(A)$ and $R(\lambda, A) f = \int_0^\infty \eu^{-\lambda t} T(t) f  \dt$. 
...
\end{theorem}
%% --
\begin{proof} The implication ``(a) $ \Rightarrow $ (b)'' follows from the fact that $ r(T(t)) = \eu^{\omega_{0}(A) t} < 1 $ and $ s(A) \leq \omega_{0}(A) < 0 $. For the point and residual spectrum, the spectral mapping theorem is valid (see A-III, Theorem~6.3). Since the approximate point spectrum is closed, the additional assumption in (b)
