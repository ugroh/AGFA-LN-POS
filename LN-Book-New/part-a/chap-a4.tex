...
\end{corollary}
%% --
Example~\ref{ex:a4-1.2}(c) shows that uniform exponential stability is not equivalent to 
%
\[
	\sigma(A) \subset \{\lambda \in \C \colon \Re(\lambda) \leq q < 0 \} .
\]
%
...
\end{align*}
where $\|T(t)\| \leq M$ for all $t\ge 0$. Since the semigroup is uniformly stable, we obtain
...
Then $G$ is $p$-periodic with values in $E$ and $h_{0} \coloneqq \int_{0}^{p} T(p-t)G(t)  \dt$ is the function 
%
\[
