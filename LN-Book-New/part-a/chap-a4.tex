...
In Section~\ref{sec:a4-2}, we consider the long-term behavior of the solutions of Eq.~\eqref{eq:c4-1} for different classes of forcing terms $ F $.
%% --
%% --
...
\]
%% --
which implies  $\|T(t)f\| \to 0$ as $t\to\infty$ for all $f \in C_0(\R^{n})$; see also Example~\ref{ex:b3-1.7}.
%%--
...
From the Eq.~\eqref{eq:a1-3.2} on p. \pageref{eq:a1-3.2}, it follows that
%% --
\[
...
\end{theorem}
%%--
\begin{proof}  The proof of Eq.~\eqref{eq:a4-1.2} is omitted (see \textcite[p.306]{hillephillips:1957}. 
...
For $c < 0$, it follows from Eq.~\eqref{eq:c4-1} that $\int_{0}^{\infty} T(s)Af  \ds$ exists. 
...
\end{proof}
%% --
Finally, from Eq.~\eqref{eq:a4-1.2} and the Uniform Boundedness Principle, it follows that the growth bound 
%
\[
...
%% --
\item
In Equations \eqref{eq:a4-1.4} and \eqref{eq:a4-1.5}, the term \enquote{$\Re(\lambda)$} may be replaced by \enquote{$\lambda \in \R$} (use Eq.~\eqref{eq:c4-1}).
\end{enumerate}
...
\end{remark}
%% --
Unfortunately, the identity Eq.~\eqref{eq:a4-1.10} does not hold on arbitrary Banach spaces. 
...
\ref{ex:a4-1.2}~(ii)), the identity Eq.~\eqref{eq:a4-1.10} is not applicable. 
...
In this case, one may apply Eq.~\eqref{eq:a4-1.7} to obtain the stronger statement Eq.~\eqref{eq:a4-1.8}.
...
%% --
\begin{proof}
The implications $(a)  \Rightarrow  (b)  \Rightarrow  (c) $ are immediate; To prove $(c)  \Rightarrow  (a) $, note that $\omega_{0}(A) = \lim_{t \to \infty} \frac{1}{t} \log \| T(t) \|$ (see Eq.~\eqref{eq:a1-1.1} on p.~\pageref{eq:a1-1.1}). Assume $\|T(t')\| <1$ for some $t'>0$. For $t=nt'+s$ with $s\in [0,t']$, we have $\|T(t)\|\le \|T(t')\|^n\|T(s)\|$, so that
\[
\displaystyle
...
\end{proof}
%% --
By rescaling a given semigroup $ (T(t))_{t \geq 0} $, one obtains the following corollary from Eq.~\eqref{eq:a4-1.1} and statement (c) of the preceding theorem.
%% --
...
It is important to note  that stable semigroups need not be uniformly bounded (see Example \ref{ex:a4-1.2}(c)) and that the equality $ s(A) = \omega_{0}(A) = 0 $ does not imply boundedness or even stability of the semigroup (see also Example~\ref{ex:a1-1.4}\,(i)).
%% --
...
A function $u(\cdot)$ is called a \emph{(strong) solution} of Eq.~\eqref{eq:a4-2.1}, if\/ $u(\cdot) \colon \R_{+} \to D(A)$, 
$u(\cdot) \in C^{1}(\R_{+},E)$, and Eq.~\eqref{eq:a4-2.1} holds for all $t \geq 0$.
The assumption that $A$ generates a semigroup $(T(t))_{t \geq 0}$
guarantees the uniqueness of the solution to Eq.~\eqref{eq:a4-2.1}. 
...
\]
%% --
Thus, the solution $u(t)$ of Eq.~\eqref{eq:a4-2.1} is given by
%% --
...
Whenever the expression Eq.~\eqref{eq:a4-2.2} is well-defined, we refer to it as a  \emph{generalized} (or
\emph{mild}) solution of Eq.~\eqref{eq:a4-2.1}. 
If\/ $F(\cdot)$ is continuous and $f \in D(A)$, then
the generalized solution of Eq.~\eqref{eq:a4-2.1} is a strong solution if and only if the function 
...
is a strong solution of Eq.~\eqref{eq:a4-2.1} (see \textcite{travis:1981}
or  \textcite[Section~4.2]{pazy:1983}).

%%%
Our aim in this section is to study the asymptotic behavior of the solutions of Eq.~\eqref{eq:a4-2.1} as $t \to \infty$. 
...
However, if the
semigroup is positive, these results remain valid even under the weaker assumption of stability (see Chapter~\ref{chap:c4}).
...
Eq.~\eqref{eq:a4-2.1} converges as $t \to \infty$, and $\lim_{t \to \infty} u(t) = -h$, where $h \in D(A)$ satisfies
$Ah = g$.
\end{theorem}

\begin{proof}
If\/ $u(\cdot)$ is a generalized solution of Eq.~\eqref{eq:a4-2.1}, then by Eq.~\eqref{eq:a4-2.2}, we have
\[
...
and $\int_{0}^{p} T(p-s)F(s)  \ds \in \Image{\Id - T(p)}$, then Eq.~\eqref{eq:a4-2.1}
 admits a unique p-periodic mild solution that is \emph{asymptotically stable}; 
 \index{stability!asymptotically stable}\ie for every generalized solution $v(\cdot)$ of Eq.~\eqref{eq:a4-2.1}, 
%% --
\[
...
\begin{proof}
(a) $\Rightarrow$ (b): 
Let $f \coloneqq u(0)$ be the initial value for which Eq.~\eqref{eq:a4-2.1} admits a $p$-periodic mild solution. 
...
(b) $\Rightarrow$ (a):  
Define $u(\cdot)$ as in Eq.~\eqref{eq:a4-2.2}. 
Then
%% --
\begin{align*}
...
Applying Eq.~\eqref{eq:a4-1.14}, we obtain $\Image{A} = \{f\colon\int_{0}^{\infty} f(x) \,\dx \text{ exists}\}$ is dense in $C_{0}(\R_{+})$. 
...
\item 
The exponential growth bounds $\omega(f)$ and $\omega_{0}(A)$ as well as the characterizations Eq.~\eqref{eq:a4-1.2}, Eq.~\eqref{eq:a4-1.6} and Theorem~\ref{thm:a4-1.3} (i) can be found in \textcite{hillephillips:1957}.
...
Statements Eq.~\eqref{eq:a4-1.2}, Eq.~\eqref{eq:a4-1.6} and Theorem~\ref{thm:a4-1.3} (i) are semigroup versions of results in classical Laplace transform theory, see \textcite{hillephillips:1957} and \textcite{widder:1946}. 
...
Statement Eq.~\eqref{eq:a4-1.8} is sometimes called the \emph{spectrum determined growth assumption}, see, for example, \textcite{triggiani:1975b}. 
...
The equivalence of the statements Eq.~\eqref{eq:a4-1.12}, Eq.~\eqref{eq:a4-1.13} and \emph{$\omega_{0}(A) < 0$} were observed by many authors, see for example, \textcite[p.178]{balakrishnan:1976}, 
