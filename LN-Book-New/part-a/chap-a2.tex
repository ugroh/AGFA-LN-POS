...
%
\[
	p(\lambda R(\lambda,A)g) \leq p(g) \text{for all $g \in E$, $\lambda > \omega_{0}(A)$}.
\]
%
...
%% –
\[
T(t) = \lim_{t\to 0} \left( \frac{n}{t} R\left( \frac{n}{t},A \right) \right)^{n} 
	\text{ (strongly) for } t \geq 0. \qedhere
\]
%% --
\end{proof}
%% --
...
\ddp(f) = \{\phi \in E' \colon \Re\dualp{g,\phi} \leq p(g) \text{ for all } g \in E \quad \text{ and } \dualp{f,\phi} = p(f)  \}.
\end{equation}
%% --
...
Then $\lim \|(T(t_n) - \Id)'x'\| = 0$ if and only if\/ $\lim \dualp{x_n, (T(t_n) - \Id)'x'} = 0$ for every bounded sequence $(x_n)$ in $E$.
