...
%% -- Stand: 2026-03-09
%% -- bis zum begin 2.1.6 Norm Continuous
%% --
...
% --
\[ 
\lim_{t \to 0} \frac{}{t} \mleft( T_{1}(t)f - f \mright)
= Af 
\]
%% --$ 
...
Assertion (d) implies that for all $ t \geq 0 $ there exist bounded operators $ T(t) \in \LE $ such that 
%
\[
	u(t,f) = T(t)f 
	\quad \text{if} \quad 
...
Since $D(A)$ is dense, this implies the strong continuity of\/ $(T(t))_{t \geq 0}$.
\end{proof}
%%
...
%
\[
	\|R(\mu,A)\| \leq N/\Re(\mu) \quad \text{and} \quad \|R(\mu,-A)\| \leq N/\Re(\mu) 
\]
%
...
%
\[
	\|R(\lambda,A^{2})\| \leq N^{2}/r^{2}(\cos\beta)^{2} \leq 1/r^{2}[N/\cos\beta]^{2} = M/|\lambda| .
\]
%
\phantom{x}
\end{proof}
...
%
\[
	T(t) \coloneqq T_{1}(t) \circ \cdots \circ T_{n}(t) \quad (t \geq 0)
\]
%
...
\]
%% --
The remaining estimate for a sector symmectric to the imaginary axis is given by the proof of Theorem~\ref{thm:a2-1.14} and shows that $A-w$ generates a holomorphic semigroup.
...
\[
    \Sigma \coloneqq \left\{\lambda \in \C \colon 
        c\eu^{-b\Re(\lambda)} \leq \lvert\Im(\lambda)\rvert
    \right\} \subset \rho(A)
\]
and
\[
...
%
\[
	\lim_{t \to t'} \|T(t) - T(t')\| = 0 ,
\]
%
then it follows from the semigroup property, that the function $t \mapsto T(t)$ is norm continuous on the whole half line $(t',\infty)$.
...
\end{equation}
%% -- 
Moreover, it follows from Eqs.~\eqref{eq:a2-2.1} and \eqref{eq:a2-2.2} on the previous page that
%% --
