...
This will allow us to obtain results on positive contraction semigroups simultaneously by choosing a suitable half-norm (cf. Part-III, Chapter 2,  Section 1).
...
By Part-I, Chapter 1,  Theorem.1.7 there exists a unique solution of (ACP) for all initial values in the domain $D(A)$ whenever $A$ is the generator of a strongly continuous semigroup.
...
%Part-I, Chapter 1, 3.5
Part-I, Chapter 1,  3.6) with domain 
...
see 
%%KGK StandConst schift
%Part-I, Chapter 1,  3.0
Part-I, Chapter 1,  3.1.
...
So by Part-I, Chapter 1,  Proposition 1.9 (ii) $ D(A) $ is a core of\/ $ B $.
...
Hence by 
\textcite[10.2.1]{hillephillips:1957} $\sup_{t \in J}\|T(t)\| < \infty$ for every compact interval $J \subset (0,\infty)$.
...
$A$ is densly defined, $(0,\infty) \subset \rho(A)$ and $\|\lambda R(\lambda,A)\| \leq 1$ for all $\lambda > 0$.
\end{enumerate}
\end{corollary}
%% –
...
%%KGK StandConst schift
%Part-I, Chapter 1,  3.8
Part-I, Chapter 1,  3.9). 
...
%%KGK StandConst schift
%Part-I, Chapter 1,  3.8 
Part-I, Chapter 1,  3.9 the domain of its generator $A$ contains $D(A_{1}^{2}) \cap \cdots \cap D(A_{n}^{2})$ and, in particular
%%--
\[
...
Then the function $t \mapsto T(t)f$ is right sided differentiable at $t_0$ if and only if\/ $T(t_0)f \in D(A)$; and in that case it is differentiable at every $s > t_0$ and the derivative is $AT(s)f$ (this follows from Part-I, Chapter 1,  Proposition 1.6\,(ii)).
...
%%--
So let $\epsilon > 0$.
If\/ $\Re(\lambda) > \omega_{0}(A)$, then by Part-I, Chapter 1,  Proposition 1.11,
\[
...
\begin{align*}
    ...
    & + \frac{1}{n!}\cdot \left\|\int_{t'}^T t^n\eu^{-\iu rt}\eu^{-at}T(t) \dt \right\| \\
...
Eventually norm continuous semigroups are of particular interest in spectral theory (cf. Part-I, Chapter 3,  Theorem 6.6).
Moreover their asymptotic behavior is easy to describe (see Part-I, Chapter 4, \,(1.8)).
...
Consider the nilpotent translation semigroup $(T(t))_{t\geq 0}$ on $F \coloneqq L^{1}([0,1])$ (see Part-I, Chapter 1,  Example 2.6).
...
On the other hand, the generator of the rotation group (Part-I, Chapter 1,  Example 2.5) 
has a compact resolvent.
Hence this condition does not imply any smoothness property of the semigroup.
\end{enumerate}
\end{remark}

Positive eventually compact semigroups have remarkable properties in the setting of the Perron-Frobenius theory (see \eg Part-II, Chapter 3,  Corollary 2.12).
...
The nilpotent shift semigroup (Part-I, Chapter 1,  2.6) is obviously eventually differentiable, eventually compact and eventually norm continuous.
But it is not norm continuous and consequently not differentiable or compact.
\end{example}

\begin{example}\label{ex:a2-1.28}
We consider multiplication semigroups (see Part-I, Chapter 1,  2.3).
...
\]
%% 
If\/ $E = L^{p}$ (see Part-I, Chapter 2,  2.3).
Consequently, $s(A) = \omega_{0}(A) = [\text{ess}]\text{-}\sup_{x \in X} \Re(m(x))$.
...
%%--
\[
\|R(\lambda_{0}+\im\eta,A+B)\| \leq \|(\Id-BR(\lambda_{0} + \iu \eta,A))^{-1}\| \cdot 2M/|\eta| \leq 2M/|\eta|
\]
%%--
for all $\eta \in \R$\,.
...
\end{proof}
%% --
In Part-III, Chapter 4,  Example 2.15 a generator $A$ of an eventually differentiable and eventually compact semigroup and a bounded operator $B$ will be given such that the semigroup generated by $A+B$ is not eventually norm continuous.
...
For later purposes (seePart-II, Chapter 2 and Part-III, Chapter 2) it will be useful not only to consider semigroups which are contractive with respect to the norm but to consider more general sublinear functionals than the norm as well.
...
However in Part-II, Chapter 2 and Part-III, Chapter 2 it will be important to dispose of a variety of sublinear functionals other than $N$.
...
Letting $t \to 0$ we obtain $\langle Af,\phi \rangle \leq 2c\|g - Af\|$ for all $g \in D(A)$.
...
%%
\[
\left\|(T(t_{n}) - \Id)^2 \right\| - \frac{1}{n} \leq \langle (T(t_{n}) - \Id)x_{n}, (T(t_{n}) - \Id)'x_{n}' \rangle\,.
\]
%%
