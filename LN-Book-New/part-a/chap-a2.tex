...
\]
%%-
In fact, then we have by the Lemma~\ref{lem:a2-1.21} 
%%-
\[
...
%% --
\[
N \coloneqq M\cdot\int_{0}^{s'} \eu^{-at}\eu^{wt} \dt.
\]
%% --
...
%% --
\[
\left\lVert \frac{1}{n!}\int_{s'}^s t^{n}\eu^{-\iu rt}\eu^{-at}T(t) \dt \right\lVert  < \epsilon^{n+1}/3
\]
%% --
 whenever $|r| \geq r_{0}$.
...
\begin{align*}
  \norm{R(a + \iu r,A)^{n+1}}
    ...
    &\leq \frac{(s')^{n}}{n!}\int_{0}^{s'} M \eu^{(\omega - a)t}\,\dt
...
%
\[
	\lim \limits_{h \downarrow 0} T(h+t)f = \lim \limits_{h \downarrow 0} T(h)T(t)f 
\]
%
...
and%
\[
	 H \coloneqq \{m(x) \colon \Re(\lambda) \geq b \text{ and for all } \eta > 0,  
...
%
\begin{proof}
The semigroup is right-sided differentiable at a point $t > 0$ if and only if the following equivalent assertions holds.
\begin{enumerate*}[\upshape(a)]
\item
$T(t)E \subset D(A)$,
\item
...
\item
there exists $c > 0$ such that 
$\left\{\lambda \in \C \colon \lvert\Im(\lambda)\rvert > c \cdot \eu^{-t\Re(\lambda)}\right\} \subset \rho(A)$.
\end{enumerate*}
%The semigroup is right-sided differentiable at $t$ if and only if the following equivalent assertions are true.
%\begin{enumerate}[\upshape(a)]
%\item
%$T(t)E \subset D(A)$,
%
...
\end{equation}
%% -- 
Moreover, it follows from Eq.~\myeqpageref{eq:a2-2.1} 
%% --
...
\]
%
Then $\phi \in \dN(f)$ and note that $\dN(f)$ may be an infinite set.
%%--
...
Let $A$ with domain $D(A)$ be an operator on $E$.
...
%
\[
	\pm\langle g,\phi_{t} \rangle \leq p(\pm g) \leq c\|g\|
\]
%
for all $g \in E$, $t > 0$.
...
Since $\langle g,\phi_{t} \rangle \leq p(g)$ for all $t > 0$, 
it follows that $\langle g,\phi \rangle \leq p(g)$ $(g \in E)$.
Moreover, 
%
\[
	\langle f,\phi_{t} \rangle - t\langle Af, \phi_{t} \rangle = p(f - tAf)$ $(t > 0) .
\]
%
