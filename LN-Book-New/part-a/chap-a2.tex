...
\]
%% --
(by Eq.~\eqref{eq:a1-1.3} on p.~\pageref{eq:a1-1.3}).
...
\end{proposition}
%% –
Yosida's proof consists in showing that the limit in Eq.~\eqref{eq:a2-1.2} exists under the hypothesis  \ref{thm:a2-1.7.2} of Theorem~\ref{thm:a2-1.7}  
...
So the same is true for the resolvents of\/ $A_{i}^{2}$ (\cf Eq.~\eqref{eq:a2-1.7} and 
...
Moreover their asymptotic behavior is easy to describe (see Eq.~\eqref{eq:a4-1.8} on p.~\pageref{eq:a4-1.8}).
...
\]
%% 
if\/ $E = L^{p}$ (see Proposition~\ref{prop:a2-2.3}).
...
%% --
\begin{proof}
If\/ $(T(t))_{t \geq 0}$ is norm continuous for $t > 0$, then $S_{n}(t)$ in Eq.~\eqref{eq:a2-1.10} is norm continuous at $t > 0$ for every $n$.
Thus $(S(t))_{t \geq 0}$ is norm continuous at $t > 0$ by Eq.~\eqref{eq:a2-1.11}.
...
Moreover, it follows from Eq.~\eqref{eq:a2-1.12}  that $R(\lambda,A+B)$ is compact whenever $R(\lambda,A)$  is compact.
...
\end{equation}
%%--
Using  Eq.~\eqref{eq:a2-1.9}  we obtain 
\begin{align*}
...
\end{equation}
%% -- 
Moreover, it follows from Eq.~\eqref{eq:a2-2.1} and Eq.~\eqref{eq:a2-2.2} that
%% --
...
Then Eq.~\eqref{eq:a2-2.5}   can be written in the form
...
Hence, by Eq.~\eqref{eq:a2-2.8},  $p(\lambda R(\lambda,A)g) \leq p(g)$ for all $g \in E$, $\lambda > \omega_{0}(A)$.
Hence $\lambda R(\lambda,A)$ is p-contractive for $\lambda > \omega_{0}(A)$.
This implies that $T(t)$ is p-contractive by the formula Eq.~\eqref{eq:a2-1.3}, 
%% –
\[
...
&\leq \frac{1}{t}(p((\Id - tA)(f + tg)) + tp(Af - g) - p(f)) \quad \text{(by Eq.~\eqref{eq:a2-2.7}\,)} \\ %% (2.7))
...
Hence $p(g) + p(-g) \leq 0$ which implies $g = 0$ by Eq.~\eqref{eq:a2-2.11} . 
\end{proof}
...
It follows from Eq.~\eqref{eq:a2-2.14}  
