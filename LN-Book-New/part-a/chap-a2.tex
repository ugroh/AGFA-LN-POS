...
%\[
%    \Sigma \coloneqq \{\lambda \in \C \colon c\eu^{-b\cdot \Re(\lambda)} \leq |\Im(\lambda)|\} \subseteq \rho(A)
%\]
%and
%\[
...
%% --
\begin{proof}
Both assertions follow from Theorem~\ref{thm:a2-1.30}  because $(US(t)U^{-1})_{t\geq 0}$ has $A+C$ as generator.
\end{proof}
%% --
%% -- Stand 2026-03-10
%% –
...
\end{equation}
%% --
If in particular $(w,\infty) \subseteq \rho(A)$ for some $w \in \R$, then $A$ is p-dissipative if and only if
%% --
