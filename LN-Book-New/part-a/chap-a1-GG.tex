...
\begin{align*}
    D(A) &\coloneqq \left\{ f \in E \colon \lim_{h\to0} \frac{1}{h}(T(h)f - f)
             \text{ exists in } E \right\}
\end{align*}
by
\begin{align*}
    Af &\coloneqq \lim_{h\to0} \frac{1}{h}(T(h)f - f) \quad (f \in D(A)).
\end{align*}
\end{definition}
%% --
...
is everywhere defined and therefore bounded (see Proposition~\ref{prop:a1-1.9}\,(ii)). 
...
\begin{equation}\label{eq:a1-1.3}
    A\left(\int_0^t T(s)f \ds\right) = T(t)f - f  .
\end{equation}
%% --
\item
If\/ $f \in D(A)$, then
...
We now collect the fundamental topological properties of semigroup generators, their domains and their resolvents (see also Corollary~\ref{cor:a2-1.34} ).
\index{resolvent}
%% --
...
\begin{align*}
	D(A^{1}) &\coloneqq D(A) \\
	D(A^{n}) &\coloneqq \{f \in D(A^{n-1}) \colon Af \in D(A^{n-1})\} \, \text{ for $ n \geq 2 $, }
\end{align*}
%
Then %
\[
...
\[
\lim_{k \to \infty} f_{n(k)}(x) = f(x) \text{ and } \lim_{k \to \infty} q(x)f_{n(k)}(x) = g(x) \text{ for } \mu\text{-almost every } x \in X\,.
\]
...
Since integration is continuous, we obtain for every $a$ and $b \in \R_{(+)}$ that
%% --
...
%% --
\[
    T(t)f(x) \coloneqq f(y) \quad \text{for $y \in \left[ 0,1 \right], y \equiv x+t \bmod 1$} 
\]
%% --
with generator
%% --
...
%% --
\[
    Bf(x) \coloneqq \frac{\text{d}^{2}}{\dx^{2}}f(x) = f''(x)
\]
%% --
we take the domain
%% --
...
%% --
\[
    e_{n}(x) \coloneqq \cos \pi nx, \quad n \in \Z,\quad x \in [0,1]
\]
%% --
...
\begin{align*}
    T(t)f(x) &= \int_{0}^{1} k_{t}(x,y)f(x)dy \\
     k_{t}(x,y) &= 1 + 2\sum_{n=1}^{\infty} \exp(-\pi^{2}n^{2}t) \cos(\pi nx) \cos(\pi ny)\,.
\end{align*}
%% --
The Jacobi identity
%% --
...
%% --
On $E = L^{p}(\R^{n})$, $1 \leq p < \infty$, the operators
%% --
\begin{align*}
    T(t)f(x) \coloneqq{}& (4\pi t)^{-n/2} \int_{\R^{n}} \exp(-|x-y|^{2}/4t)f(y)\dy \\
    ={}& \mu_{t}*f(x)\quad \text{for }x \in \R^{n}, \  t > 0 
\end{align*}
%% --
%
and 
\[
\mu_{t}(x) \coloneqq (4\pi t)^{-n/2} \exp(-|x|^{2}/4t)
\]
...
\[
    ...
    = (2\pi)^{n/2} \widehat{\mu}_{t}\cdot\widehat{f}
\]
%% --
...
\textbf{Final Remark}.  One-dimensional diffusion as described in Section~\ref{subsec:a1-2.7} are valid in spaces $L^{p}\left[ 0,1 \right]$, and $n$-dimensional diffusion as described in Section~\ref{subsec:a1-2.8} are valid in $C_{0}(\R^{n})$.
...
%% --
Assume $F$ to be a closed subspace of\/ $E$, being $(T(t))$-invariant or, equivalently, $R(\lambda,A)$-invariant for 
$\lambda \in \C$, $ \Re(\lambda) > \omega_{0}$.
Then the semigroup $(T_{|}(t))_{t \geq 0}$ of all restrictions $T_{|}(t) \coloneqq T(t)_{|\,F}$ is strongly continuous on $F$.
...
%% --
\[
    T_{/}(t)q(f) \coloneqq q(T(t)f), \quad f \in E,
\]
%% --
are well defined and form a strongly continuous semigroup $(T_{/}(t))_{t \geq 0}$ on $E_{/}$.
For the generator $(A_{/},D(A_{/}))$ of\/ $(T(t)_{/})_{t \geq 0}$ we obtain.
%% --
\[
    D(A_{/}) = q(D(A)) 
    \quad \text{and} \quad 
...
%% --
for every $f \in D(A)$.
Here we use the fact that every $q(f) \in D(A_{/})$ can be written as
%% --
\[\textstyle
    q(f) = \int_{0}^{\infty}  \eu^{-\lambda s} T_/(s)q(g)\ds 
...
%% --
\[
    (T_{|}(t)f)(x) = \1_{(-\infty,1]}(x) \cdot f(x+t),
\]
%% --
...
%% --
\[
D(A_{1}) = \{f \in E_{1} \colon Af \in E_{1}\} = D(A^{2}) \text{ and } A_{1}f = Af \text{ for every } f \in D(A_{1})\,.
\]
...
\end{center}
%% 
Therefore all Sobolev semigroups $(E_{n}, T_{n}(t))_{t \geq 0}$, $n \in \N$, are equivalent.

\item
For $\lambda \in \rho(A)$ consider for every $f \in E$ the norm
%% --
\[
...
%% --
Real- or complex-valued functions of two variables $x$ and $y$ are often limits of functions of the form $\sum_{i=1}^{n} f_{i}(x)g_{i}(y)$ which, to some extent, allows one to consider the variables $x$ and $y$ separately.
...
These will always be related to one of the following standard methods to define a norm on the tensor product of two Banach spaces $E$ and $F$.
...
%\item
\[
\|u\|_{\pi} \coloneqq \inf\{\sum_{j=1}^{m} \|h_{j}\| \|k_{j}\| \colon u = \sum_{j=1}^{m}h_{j} \otimes k_{j}, h_{j} \in E, k_{j} \in F\}
\]
defines the \emph{greatest cross norm $\pi$} on $E \otimes F$ and
%
%\item
\[
\|u\|_{\epsilon} \coloneqq \sup\{\langle u,\phi \otimes \psi\rangle \colon \phi \in E', \psi \in F', \|\phi\|, \|\psi\| \leq 1\}
\]
defines the \emph{least cross norm $\epsilon$} on $E \times F$. 
...
%item
If\/ $E$ and $F$ are Hilbert spaces, $\|u\|_{h} = (u|u)_{h}^{1/2}$, where the scalar product $(\cdot|\cdot)_{h}$ is given by 
\[\textstyle
\left(\sum_{i=1}^{n} f_{i} \otimes g_{i}\big\vert\sum_{j=1}^{m}\phi_{j} \otimes \psi_{j}\right) = \sum_{i,j} (f_{i}\vert\phi_{j})(g_{i}\vert\psi_{j})
\]
defines the \emph{Hilbert norm $h$} on $E \otimes F$.
%\end{enumerate}
%% --
...
$ E \widetilde{\otimes}_{\alpha} F $ and 
