...
\end{definition}
%% --
Clearly one defines in a similar way \emph{weakly continuous}, \resp \emph{uniformly continuous} (compare Part-I, Chapter 2,  Definition  1.19) semigroups, but since we concentrate on the strongly continuous case we agree on the following terminology.
%% --
\begin{quote}
...
\item 
For every strongly continuous semigroup there exist constants $ M \geq 1 $, $ \omega \in \R $, such that 
...
$ 0 < t \leq \delta $ then it is strongly continuous if and only if\/ $ \lim_{t\to0} T(t)f = f $ for every 
$ f $ in a total subset of\/ $ E $.
...
\end{theorem}
%% --
For more on the relation of semigroups to abstract Cauchy problems we refer to Part-I, Chapter 2,  Section~1.
...
We now collect the fundamental topological properties of semigroup generators, their domains (see also Part-I, Chapter 2,  Corollary~1.34) and their resolvents.
\index{resolvent}
%% --
...
An abstract characterization of generators of multiplication semigroups will be given in Part-III, Chapter 2,  Theorem~5.13.
%% --
...
Examples analogous to \ref{subsec:a1-2.7} are valid in $L^{p}\left[ 0,1 \right]$, \resp to 
...
%% --
\begin{align*}
     \eu^{-\lambda t}T(t)f - f &= (A-\lambda) \int_{0}^{t}  \eu^{-\lambda s}T(s)f \ds \quad\text{ or }  \\
...
%% --
\begin{align*}
     \eu^{-\lambda t}T(t)f - f &= \int_{0}^{t}  \eu^{-\lambda s}T(s)(A-\lambda)f \ds  \quad\text{or}  \\
    \notag \\
...
%% --
quotient semigroup $(T(t)_{|_{/}})$ on $L^{1}(\left[ 0,1 \right])$ is the nilpotent translation semigroup as in Example 2.6.
...
\[
    ...
    = \left\lvert \left\langle f,A'\phi - \frac{1}{s}(T(s)'\phi-\phi) \right\langle \right\rvert ,
\]
%% --
hence
%% --
...
We will show in Part-I, Chapter 3,  Section~4.2 that the above construction preserves and even improves many spectral properties of the semigroup and its generator.
