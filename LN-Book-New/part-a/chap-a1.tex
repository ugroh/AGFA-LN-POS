...
Therefore $(T(t))_{t \geq 0}$ is a strongly continuous semigroup which is bounded, hence has $\omega_{0} = 0$, but the constant $M$ in Eq.~\eqref{eq:a1-1.1} cannot be chosen to be $1$.
...
%
for some $g \in E$.
But these elements belong to $D(A)$ by Eq.~\eqref{eq:a1-1.3}, hence $D(A) = E$.
...
For $\lambda \in \C$ and $S(t) \coloneqq \exp(-\lambda t)T(t)$ the formulas Eq.~\eqref{eq:a1-1.3} and Eq.~\eqref{eq:a1-1.4} yield:
%% --
