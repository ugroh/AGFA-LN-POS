...
It may however be useful to fix our notation, to collect briefly some important definitions and results (ection~\ref{sec:a1-1}), to present a list of \emph{standard examples} in Section~\ref{sec:a1-2} %(Section 2) 
...
%% -- 
We consider a one-parameter semigroup $(T(t))_{t \geq 0}$ on a Banach space $E$ and observe that the domain $\R_{+}$ and the range $\LE$ of the (semigroup) homomorphism $\tau \colon t \mapsto T(t)$ are topological semigroups for the natural topology on $\R_{+}$ and any one of the standard operator topologies on $\LE$.
...
\begin{split}
	\omega_{0} & \coloneq \inf \{ \omega \in \R   \colon \text{there exists $ M \in \R_{+} $   
 		such that $\|T(t)\| \leq M \cdot  \eu^{wt}$  for  $t \geq 0$} \} \\
...
Therefore $(T(t))_{t \geq 0}$ is a strongly continuous semigroup which is bounded, hence has $\omega_{0} = 0$, but the constant $M$ in Eq.~\eqref{eq:a1-1.1} on p.~\pageref{eq:a1-1.1} cannot be chosen to be $1$.
...
\end{proposition}
%% --
The identity Eq.~\eqref{eq:a1-1.2} is of great importance and shows how semigroups are related to certain Cauchy problems.
...
%
for some $g \in E$.
But these elements belong to $D(A)$ by Eq.~\eqref{eq:a1-1.3} on p.~\pageref{eq:a1-1.3}, hence $D(A) = E$.
...
%
\[
	  \eu^{t \cdot \left( A - s/2 \cdot \Id \right)} = \Id + t( A - s/2 \cdot \Id)
\]
%
or
%
\[
	 \eu^{t A } =  \eu^{t\cdot s/2} \, [ ( 1 - \frac{s}{2} \, t )\cdot \Id + t \cdot A] .
\]
%
\item[$ D^{2} \neq 0 $: ]
...
For $\lambda \in \C$ and $S(t) \coloneqq \exp(-\lambda t)T(t)$ the formulas Eq.~\eqref{eq:a1-1.3} and Eq.~\eqref{eq:a1-1.4} on p.~\pageref{eq:a1-1.4} yield:
%% --
\begin{equation}\label{eq:a1-3.1}
\begin{split}
     \eu^{-\lambda t}T(t)f - f &= (A-\lambda) \int_{0}^{t}  \eu^{-\lambda s}T(s)f \ds \\
     \text{ or }  \\
...
\begin{split}
     \eu^{-\lambda t}T(t)f - f &= \int_{0}^{t}  \eu^{-\lambda s}T(s)(A-\lambda)f \ds  \\
     \text{or}  \\
...
%% --
\[
    \hat{f} = \int_{0}^{\infty}  \eu^{-\lambda s} \hat{T}(s)_{/}\hat{g} \ds 
...
%% --
quotient semigroup $(T(t)_{|_{/}})$ on $L^{1}(\left[ 0,1 \right])$ is the nilpotent translation semigroup as in Section~\ref{subsec:a1-2.6}.
...
\]
%% --
on $m(E)$, but the semigroup $(\hat{T}(t))_{t \geq 0}$ is strongly continuous if and only if\/ $\TT$ has a bounded generator.
