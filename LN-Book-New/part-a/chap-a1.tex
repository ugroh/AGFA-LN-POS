...
%% --
\[
    T(t)f(x) \coloneqq f(y) \quad \text{for $y \in \interval{0,1}, y = x+t \bmod 1$} 
\]
%% --
with generator
%% --
...
%% --
\[
    Bf(x) \coloneqq \frac{\mathrm{d}^{2}}{\dx^{2}}f(x) = f''(x)
\]
%% --
we take the domain
%% --
\[
    D(B) \coloneqq \{f \in C^{2}\interval{0,1} \colon f'(0) = f'(1) = 0\}
\]
%% --
...
%% --
\[
    e_{n}(x) \coloneqq \cos \pi nx \quad (n \in \Z, x \in \interval{0,1})
\]
%% --
...
For $\lambda \in \C$ and $S(t) \coloneqq \exp(-\lambda t)T(t)$ the formulas \eqref{eq:a1-1.3} and \eqref{eq:a1-1.4} on p.~\pageref{eq:a1-1.4} yield:
%% --
...
%% --
\[
    T_{\vert}(t) \coloneqq T(t)\big|_{F}, \quad t \geq 0,
\]
%% --
...
%% --
\[
    A_{|} \coloneqq A\big|_{ (D(A)\cap F)} \text{ with domain } D(A_{|}) \coloneqq D(A) \cap F
\]
%% --
...
%% --
Let $F$ be a closed $(T(t))_{t \geq 0}$-invariant subspace of $E$ and consider the quotient space $E_{/} \coloneqq \mfrac{E}{F}$ with quotient map $q \colon E \to E_{/}$.
The quotient operators
%% --
\[
...
%%% --
%\[
%    D(A_{|_{/}}) = \{f \in L^{1}(\interval{0,1}) \colon f \in \text{AC} \text{ with } f' \in L^{1}(\interval{0,1}) \text{ and } f(1) = 0\}.
%\]
We start with the Banach space $E = L^{1}(\R)$ and the translation semigroup $(T(t))_{t \geq 0}$ where $T(t)f(x) \coloneqq f(x+t)$ (see Example~\ref{subsec:a1-2.4}).
Then $L^{1}((-\infty,1])$ can be identified with the closed, 
$(T(t))_{t \geq 0}$-invariant subspace
\[
    F \coloneqq \{f \in E \colon f(x) = 0 \text{ for } x > 1\}.
\]
...
\[
    D(A_{\vert F}) \coloneqq \{f \in F \colon f \in \mathrm{AC},\; 
    f' \in F \text{ and } f(x) = 0 \text{ for } x \geq 1\}.
\]
Next, $L^{1}(\interval{0,1})$ can be identified with the quotient space 
\mfrac{F}{I}, where
\[
    I \coloneqq \{f \in F \colon f(x) = 0 \text{ for } 0 \leq x \leq 1\}.
\]
...
\]
%% --
on $\interval{0,1}$ with derivative $\xi_{f}'(t) = \langle T(t)Af,\phi \rangle = \langle T(t)f,A'\phi \rangle$.
...
Since $E_{1}$ is invariant under $(T(t))_{t\geq 0}$, it makes sense to consider the semigroup $(T_{1}(t))_{t\geq 0}$ of all restrictions $T_{1}(t) \coloneqq T(t)_{\vert E_{1}}$.
