...
%%KGK
%\index{stability!homogeneous abstract Cauchy problem }
%% -- 
...
\omega_{1}(A) &= \inf\{\Re(\lambda) \colon {\int_{0}^{\infty} \eu^{-\lambda t} T(t)f \,\dt} \text{ exists as an improper Riemann} \\
&\phantom{= \inf\{\Re(\lambda) \colon \int_{0}^{\infty} \eu^{-\lambda t} T(t)f} \text{integral for every } f \in E\} \notag \\
...
yields
\[
\omega_1(A) \leq \inf \left\{ \Re(\lambda)\colon\int_0^\infty \eu^{-\lambda t} T(t) f \,\dt \text{ exists for every } f \in \text{im} (\lambda - A) \right\}.
\]
Therefore,
$\omega_1(A) \leq b.$
%--
...
The inequality
\[
    \omega_{0}(A) \geq \inf \left\{ \text{Re } \lambda\colon\int_0^\infty \|\eu^{-\lambda t} T(t) f \| \,\dt 
...
\begin{equation}\label{eq:a4-1.6}
   \omega_{0}(A) = \inf \left\{ \text{Re } \lambda\colon\int_0^\infty \|\eu^{-\lambda t} T(t) f \| \,\dt \text{ exists for every } f \in E \right\}.
\end{equation}
%--
%
...
\[{
T(t) \coloneq  (\eu^{2\pi i n t} \exp(t A_n))_{n \in \N}
},\]
where
\[
A_n =
...
$
x_q \coloneq  (q e_1, 2q^2 e_2, ..., n q^n e_n, ...).
$
Then $x_q \in D(A)$ and
%--
\begin{align*}
...
\[\|AT(t)\| \leq \frac{m}{t}\]
(see \citet[p.33]{goldstein:1985a}), and hence $T(t)f \to 0$ as $t \to \infty$
...
%%KGK nur ohne "semigroup" ?
%%KGK
%\index{stability!holomorphic}
...
%%KGK
%\index{stability!weakly stable}
\end{enumerate}
\end{remark}
%% --
\section{Stability: Inhomogeneous Case}
%%KGK
%\index{stability!inhomogeneous abstract Cauchy problem}
%%KGK 2025-12-16
...
$v(t) \coloneq  \int_{0}^{t} T(t-s)F(s)  \ds$ is differentiable (see \citet[Chapter~4,2.4]{pazy:1983}. 
