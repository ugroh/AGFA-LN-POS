...
\chapter[Basic Results on Semigroups on Banach Spaces]{Basic Results on Semigroups on Banach Spaces\\
\normalsize{\hspace{2cm} R. Nagel \& U. Schlotterbeck}}\label{chap:a1}
\index{Basic Results}

Since the basic theory of one-parameter semigroups can be found in several excellent books (e.g. \cite{davies:1980}, \cite{goldstein:1985a}, \cite{pazy:1983} or \cite{hillephillips:1957}), we do not want to give a self-contained introduction to this subject here.
...
\end{definition}
%% --
Clearly one defines in a similar way \emph{weakly continuous}, resp.\xspace \emph{uniformly continuous} (compare A-II, Def. 1.19) semigroups, but since we concentrate on the strongly continuous case we agree on the following terminology:
%% --
\begin{quote}
...
On the other hand it may happen that $(B,D(B))$ is not closed but has a closed \emph{extension} $(C,D(C))$, \ie $D(B) \subset D(C)$ and $Bf = Cf$ for every $f \in D(B)$.
...
\noindent If $\int_{0}^{\infty} e^{-\lambda t}T(t)f \, dt$ exists for every $f \in E$ and some $\lambda \in \mathbb{C}$, then $\lambda \in \rho(A)$ and $R(\lambda,A)f = \int_{0}^{\infty} e^{-\lambda t}T(t)f \, dt$.
In particular,
%% --
...
\]
%% --
We refer to \cite[Prop.1.14]{davies:1980} for the details.
...
On these function spaces $E = C_{0}(X)$, resp. $E = L^{p}(X,\Sigma,\mu)$, there is a simple way to define \emph{multiplication operators}: Take a continuous, \resp measurable function $q \colon X \to \mathbb{C}$ and define
%% --
\[
...
\end{proposition}
%% --
As a consequence one computes the growth bound of a multiplication semigroup 
%% --
\[
...
\]
%% --
yield a group $(R_{\tau}(t))_{t \in \R}$ having period $\tau$, \ie $R_{\tau}(\tau) = \text{Id}$.
...
\end{align*}
%% --
An isomorphic copy of the group $(R_{\tau}(t))_{t \in \R}$ is obtained if we consider $E = \{f \in C[0,1] \colon f(0) = f(1)\}$, resp. $E = L^{p}([0,1])$ and the group of 'periodic translations'
%% --
\[
...
%% --
\[
    D(B) \coloneqq \{f \in C^{2}[0,1] \colon f'(0) = f'(1) = 0\}
\]
%% --
...
The linear hull $\text{span }\{e_{n} \colon n \in \mathbb{Z}\} =: E_{0}$ forms a subalgebra of $D(B)$ which by the Stone-Weierstrass theorem is dense in $E$.

We now use $e_{n}$ to define bounded linear operators 
%
\[
	e_{n} \otimes e_{n} \colon f \mapsto (\int_{0}^{1} f(x)e_{n}(x) \, \dx)e_{n} 
...
\]
%% --
by Young's inequality, \cite[p.28]{reedsimon:1975}, hence $\|T(t)\| \leq 1$ for every $t > 0$.
...
In other words, $F$ transforms $(T(t)|_{S(\R^{n})})_{t \geq 0}$ into a multiplication semigroup on $S(\R^{n})$ which is pointwise continuous for the usual topology of $S(\R^{n})$.
...
\]
%% --
we refer to Section IX.7 in \cite{reedsimon:1975}. %[Reed-Simon (1975]).

Analogous examples to \ref{subsec:a1-2.7} are valid in $L^{p}[0,1]$, \resp to 
...
%% --
Let $F$ be a closed $(T(t))$-invariant subspace of $E$ and consider the quotient space $E_{/}\coloneqq E/F$ with quotient map $q \colon E \to E_{/}$. 
...
%% --
\[
    D(A_{/}) = q(D(A)) \quad \text{and} \quad A_{/}q(f) = q(Af)
\]
%% --
for every $f \in D(A)$.
...
%% --
\[
    D(A_{|_{/}}) = \{f \in L^{1}([0,1]) \colon f \in \text{AC} \text{ with } f' \in L^{1}([0,1]) \text{ and } f(1) = 0\}.
\]
\end{example}
%% --
%% --
...
It follows from \cite[Thm.III.5.30]{kato:1966} that the resolvent $R(\lambda,A')$ of $A'$ is $R(\lambda,A)'$.
...
%% --
\begin{definition}[\cite{phillips:1954}]\label{def:a1-3.1}
...
Since $\{\xi_{f}'(t) \colon t \in [0,1], f \in D(A), \|f\| \leq 1\}$ is bounded, it follows that the set
%% --
\[
    \{\xi_{f} \colon f \in D(A), \|f\| \leq 1\}
\]
%% --
is equicontinuous at $0$, \ie, for every $\varepsilon > 0$ there exists $0 < t_{0} < 1$ such that
%% --
\[
...
The results of Prop.1.6 imply that $T_{1}(t) \in L(E_{1})$ and $\|T_{1}(t)f-f\|_{1} \to 0$ as $t \to 0$ for every $f \in E_{1}$.
Thus $(T_{1}(t))_{t\geq 0}$ is a strongly continuous semigroup on $E_{1}$ and has a generator denoted by $(A_{1},D(A_{1}))$.

Using Prop.1.6 again we see that $A_{1}$ is the restriction of $A$ to $E_{1}$ with maximal domain, \ie
%% --
...
Occasionally we will need such a construction and refer to Section V.1 of [Schaefer (1974)] for the details or see Chapter-IV for a different approach. 
...
%% --
\[
    m^{T}(E) \coloneqq \{(f_{n}) \in m(E) \colon \lim_{t \to 0} \|T(t)f_{n}-f_{n}\| = 0 \text{ uniformly for } n \in \N\} .
\]
%% --
...
For the generator $\hat{A}$ of $(\hat{T}(t))_{t \geq 0}$ on $m^{T}(E)$ one has the following properties.
%% --
\begin{enumerate}[(i)]

\item 
$D(\hat{A}) = \{(f_{n}) \in m^{T}(E) \colon f_{n} \in D(A) \text{ and } (Af_{n}) \in m^{T}(E)\}$,
...
Now let $\mathcal{F}$ be any filter on $\N$ finer than the Frechét filter (\ie the filter of sets with finite complement).
(In most cases $F$ will be either the Frechét filter or some free ultra filter.)
%% --
space of all $\mathcal{F}$-null sequences in $m(E)$
%% --
\[
...
%% --
\[
    E_{\mathcal{F}} \coloneqq m(E)/c_{\mathcal{F}}(E) \quad \text{and} \quad E_{\mathcal{F}}^{T} \coloneqq m^{T}(E)/c_{\mathcal{F}}(E)\cap m^{T}(E)
\]
%% --
...
\]
%% --
We can apply 3.3 in order to define the \emph{$\mathcal{F}$-product semigroup} $(T_{\mathcal{F}}(t))_{t \geq 0}$ on $E_{\mathcal{F}}^{T}$ by
%% --
\[
    T_{\mathcal{F}}(t)((f_{n}) + c_{\mathcal{F}}(E)) \coloneqq (T(t)f_{n}) 
    	+ c_{\mathcal{F}}(E)\cap m^{T}(E)
\]
%% --
Thus $T_{\mathcal{F}}(t)$ is the restriction of $T(t)_{F}$ where $T(t)_{F}$ denotes the canonical extension of $T(t)$ to the $\mathcal{F}$-product $E_{\mathcal{F}}$. (Note that $(T(t)_{F})_{t \geq 0}$ is not strongly continuous unless $T$ has a bounded generator.)
...
\begin{enumerate}[(i)]
\item 
$D(A_{\mathcal{F}}) = \{(f_{n}) + c_{\mathcal{F}}(E) \colon f_{n} \in D(A); (f_{n}), (Af_{n}) \in m^{T}(E)\}$
...
\end{examples}
%% --
We do not intend to go deeper intothe quite sophisticated problems related to normed tensor products of general Banach spaces, but will rather confine ourselves to the discussion of certain special cases.
...
\item
$\|u\|_{\varepsilon} \coloneqq \sup\{\langle u,\phi \otimes \psi\rangle \colon \phi \in E', \psi \in F', \|\phi\|, \|\psi\| \leq 1\}$ defines the "least cross norm $\varepsilon$" on $E \times F$. Here, $\langle u,\phi \otimes \psi\rangle$ denotes the canonical bilinear form on $(E \otimes F) \times (E' \otimes F')$, \ie $\langle\sum_{i=1}^{n} f_{i} \otimes g_{i},\phi \otimes \psi\rangle = \sum_{i=1}^{n} \langle f_{i},\phi\rangle\langle g_{i},\psi\rangle$.
...
\]
\[
    C(X \otimes Y) = C(X) \widetilde{\otimes}_{\varepsilon} C(Y)
\]
%% --
...
We are now ready to consider semigroups induced on tensor product.
%% --
...
%
\[
	(A \otimes \text{Id} + \text{Id} \otimes B,D(A) \otimes D(B)) .
\]
%
...
For a more complete information on semigroup theory we refer the reader to \cite{hillephillips:1957}, to the monographs by \cite{davies:1980}, 
\cite{goldstein:1985a} and \cite{pazy:1983}, to the survey article by \cite{kreinkhazan:1985}, to the bibliography by 
\cite{goldstein:1985b} and to \cite{engelnagel:2000}.
