...
We claim that for every sequence $(p_{k})$ of mutually orthogonal projections and all bounded sequences $(x_k)$ in $M$
%% --
\[
\lim_n \|(T_n - \Id)(p_{k} x_k p_{k})\| = 0
\]
%% --
uniformly in $k \in \N$.
This follows from an application of the \emph{Lemma of Phillips} and the fact that the sequence $(p_{k} x_{k} p_{k})$ is summable in the $s^{*}(M,M_*)$-topology (compare \citet{elliot:1972}).
...
\begin{enumerate}[(i)]
\item 
$\lim_n \|(T_n - \Id)(p_{k} x_k p_{k})\| = 0$ uniformly in $k \in \N$ for every bounded sequence $(x_k)$ in $M$.
\item 
...
Let $ M $ be a \WA-algebra and let $ H $ be an infinite-dimensional Hilbert-space. 
Then the \WA-tensor produkt $ N \coloneq M \overline{\otimes} \BH $ is a properly infinite \WA-algebra (\citet[Thm. 2.6.6]{sakai:1971}).
...
But in general, a \WA-algebra has not the Dunford-Pettis property (\citet{chu:1990}).
But is known since 1994, that every \WA-algebra is a Grothendick space (\citet{pfitzner:1994}.
\end{remark}
