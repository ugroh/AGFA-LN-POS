...
But as we will show in the example preceding Theorem~\ref{thm:d3-1.11} on page \pageref{thm:d3-1.11} below the boundary spectrum is no longer cyclic.
...
Hence the maps $S \coloneqq (\mu - \im\alpha)R(\mu)'$ and $T \coloneqq \mu R(\mu)'$ fulfil the assumptions of 
...
Hence from Lemma~\ref{lem:d3-1.2}\,\ref{item:d3-1.2-i} on page \pageref{lem:d3-1.2} it follows
%% --
\[
...
From the above proof we obtain the following: If $0 \neq \psi_{\alpha} \in \Kern{\im\alpha - A)$ with polar decomposition $\psi_{\alpha} = u_{\alpha}|\psi_{\alpha}|$ ($\alpha \in \R$} then $A'u_{\alpha} = \im\alpha u_{\alpha}$.

Conversely, if $0 \neq v_{\alpha} \in \Kern{\im\alpha - A')$, then there exists $\psi \in \Psi$ such that $\psi(v_{\alpha}v_{\alpha}^{*}} \neq 0$ and the normal linear form
%% --
\[
...
Since $\Fix{\TT}$ is positively generated (Corollary~\ref{cor:d3-1.5} on page \pageref{cor:d3-1.5}), $\Fix{\TT} = \C.\phi$ for some faithful normal state $\phi$.
...
%% --
\[
	\Fix{\kappa(t)} = \left(\bigcup_{k \in \mathbb{Z}} \Kern{\frac{2\pi ik}{t} - A)\right}^{-}
\]
%% --
...
If $T$ is uniformly ergodic with finite dimensional fixed space, then every $\gamma \in \sigma(A) \cap \im\R$ is a pole of the resolvent $R(.,A)$ and $\dim \Kern{\gamma - A} \leq \dim \Fix{T}$.
\end{theorem}
%% --
\begin{proof}
