...
\end{proof}
%% 
\textbf{Bitte das LN vergleichen und aufschreiben, wie es konkret lauten soll.}
%% --
...
The largest subspace of $L^\infty(\R)$ on which these translations form a semigroup which is strongly continuous with respect to the sup-norm, is the space of all bounded uniformly continuous functions on $\R$, \ie $E^* = C_{bu}(\R)$.
...
Using Prop.1.6 again we see that $A_1$ is the restriction of $A$ to $E_1$ with maximal domain, \ie
%% --
\newpage
%% -- a1-19
...
%  
%  % Horizontale Pfeile
%  \draw[->] (m-1-1) -- node[above] {$T(t)$} (m-1-3);
%  \draw[->] (m-2-1) -- node[above] {$T_{1}(t)$} (m-2-3);
%  \draw[->] (m-3-1) -- node[above] {} (m-3-3);
%  
%  % Vertikale Pfeile
%  \draw[->] (m-1-1) -- node[left] {$i_{1}$} (m-2-1);
%  \draw[->] (m-2-1) -- node[left] {$i_{2}$} (m-3-1);
%  \draw[->] (m-1-3) -- node[right] {$i_{1}$} (m-2-3);
%  \draw[->] (m-2-3) -- node[right] {$i_{2}$} (m-3-3);
%  
%  % Diagonale Pfeile
%  \draw[->] (m-2-1) -- node[above,sloped] {$A$} (m-1-3);
%  \draw[->] (m-3-1) -- node[above,sloped] {$A_{1}$} (m-2-3);
%\end{tikzpicture}
%% --
\begin{center}
\begin{tikzpicture}[
...
\begin{tikzpicture}[
    ...
    \path[->]
        (m-1-1) edge node[above] {$T(t)$} (m-1-3)
        (m-2-1) edge node[above] {$T_1(t)$} (m-2-3)
        (m-1-1) edge node[left] {$\lambda-A$} (m-2-1)
        (m-1-3) edge node[right] {$R(\lambda,A)$} (m-2-3);
...
%% --
\section{The F-Product Semigroup}\label{sec:a1-3.6}
%\index{Semigroups!F-Product}
\index{F-Product!Semigroup}
%% --
...
%% --
\[
    \|(f_{n})_{n \in \mathbb{N}}\| \coloneqq \sup \{\|f_{n}\| \colon n \in \mathbb{N}\}
\]
%% --
...
%% --
\[
    m^{T}(E) \coloneqq \{(f_{n}) \in m(E) \colon \lim_{t \to 0} \|T(t)f_{n}-f_{n}\| = 0 \text{ uniformly for } n \in \mathbb{N}\}
\]
%% --
...
Now let $F$ be any filter on $\mathbb{N}$ finer than the Frechet filter (\ie the filter of sets with finite complement).
...
%% --
\[
    c_{F}(E) \coloneqq \{(f_{n}) \in m(E) \colon F\text{-}\lim\|f_{n}\| = 0\}
\]
%% --
...
With the canonical injection $j \colon f \mapsto (f,f,f,\ldots) + c_{F}(E)$ from $E$ into $E_{F}^{T}$ the operators $T_{F}(t)$ are extensions of $T(t)$ satisfying $\|T_{F}(t)\| = \|T(t)\|$. The basic facts about the generator $(A_{F},D(A_{F}))$ of $(T_{F}(t))_{t \geq 0}$ follow from 3.3 and are collected in the following proposition.
%% --
...
\begin{enumerate}[(i)]
\item 
$D(A_{F}) = \{(f_{n}) + c_{F}(E) \colon f_{n} \in D(A); (f_{n}), (Af_{n}) \in m^{T}(E)\}$
...
\index{Semigroups!Tensor Product}
...
\index{Semigroups!Commuting}
\index{Product!Commuting Semigroups}
...
Using Prop.1.9(ii) 
%Prop.~\ref{prop:a1-1.9}\,(ii) on page\pageref{prop:a1-1.9} 
