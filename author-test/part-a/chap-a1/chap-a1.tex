...
By this we understand a subset $\{T(t) : t \in \mathbb{R}_{+}\}$ of $L(E)$, usually written as $(T(t))_{t\geq0}$, such that
%% --
\begin{align*}
	T(0) &= \text{Id}, \\
...
If not stated otherwise, a semigroup is a strongly continuous one-parameter semigroup of bounded linear operators.
...
Each $T(t)$, $t > 0$, satisfies $\|T(t)\| = 2$ as can be seen by taking $f \coloneqq 1_{[0,t]}$.
Therefore $(T(t))_{t\geq0}$ is a strongly continuous semigroup which is bounded, hence has $\omega_{0} = 0$, but the constant $M$ in (1.1) cannot be chosen to be $1$.
...
%% --
\begin{align*}
D(A) &\coloneqq \{f \in E : \lim_{h\to0} \frac{T(h)f - f}{h} \text{ exists in } E\}
\end{align*}
%% --
by
%% --
\begin{align*}
Af &\coloneqq \lim_{h\to0} \frac{T(h)f - f}{h} \text{ for } f \in D(A).
\end{align*}
%% --
\end{definition}
