...
def convert_citation(text):
    ...
    [Author (YYYY), Note] to \textcite[Note]{author:YYYY}
    [Author-Author (YYYY)] to \textcite{author_author:YYYY}
    """
    ...
    def replace_citation_with_note(match):
        ...
        # Convert author to lowercase and replace spaces/hyphens with underscores
        author_label = author.lower()
        author_label = author_label.replace(' ', '_')
        author_label = author_label.replace('-', '_')

        return f"\\textcite[{note}]{{{author_label}:{year}}}"
    ...
    def replace_simple_citation(match):
        ...
        return f"\\textcite{{{author_label}:{year}}}"
