...
\end{equation}
%%--
$(T(t))_{t\geq 0}$ is nilpotent\index{semigroup!nilpotent} (we have $T(t) = 0$ for $t \geq 1$).
...
A subset $M \subset \C$ is called \emph{imaginary additively cyclic}\index{cyclic!imaginary additively} (or simply \emph{cyclic}), if it satisfies the following condition. 
\begin{quote}
...
\index{example!boundary spectrum}
...
\index{example!rotation semigroup} on $C(\Gamma)$ (see A-III, Example~5.6) with period $2\pi/\alpha$ has boundary spectrum $M_{3}$.
...
\index{boundary spectrum!eigenvalues}
		...
		Then $P\sigma_{b}(A)$ is imaginary additively cyclic\index{cyclic!imaginary additively}.
	\end{proposition}
	%% --	
\begin{proof}
...
A real spectral value $\lambda_{0}$ of a generator $A$ is called a \emph{dominant}\index{spectral value!dominant} provided that $\Re(\lambda) < \lambda_{0}$ for every $\lambda \in \sigma(A)$, it is called \emph{strictly dominant}\index{spectral value!strictly dominant}  if for some $\delta > 0$ one has $\Re(\lambda) \leq \lambda_{0} - \delta$ for every $\lambda \in \sigma(A)$, $\lambda \neq \lambda_{0}$.
...
%%
%%
\index{semigroup!eventually norm continuous}
...
%
\index{example!Schrödinger operator}
...
\index{example!retarded differential equation}
...
\index{example!rotation semigroup} 
...
\index{operator!kernel operator} 
...
\index{semigroup!lattice homomorphism}
%% --
...
We will show that the whole spectrum is an imaginary additively cyclic\index{cyclic!imaginary additively} subset of $\C$ (see Definition~2.5).
...
\begin{example}\label{ex:b3-4.6}
	...
	We denote the maximal flow corresponding to the differential equation\index{example!differential equation} $y' = F(y)$ by $\phi_{0}$.
...
\end{proof}
%% --
We are now going to apply the results obtained so far to the special case where $h = 0$, \ie we consider semigroups of lattice homomorphisms which are Markov operators\index{Markov operator}.
%% --
...
\begin{theorem}\label{thm:b3-4.11}
	Suppose $X$ is a locally compact space and $(T(t))_{t\in\R}$ is a group of lattice homomorphisms\index{lattice homomorphism!group of} governed by the flow $\phi$ and the multiplicator $h$.
	Then we have
	\begin{enumerate}[\upshape (i)]
		\item 
