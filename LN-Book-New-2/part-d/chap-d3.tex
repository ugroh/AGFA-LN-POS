...
&\leq \norm{\psi} <(\mu R(\mu+\im\alpha)'x)(\mu R(\mu+\im\alpha)'x)^{*},\psi>  \\
&\leq \norm{\psi} <\mu R(\mu)'(xx^{*}),|\psi|>
\end{align*}
%% --
...
Let $\RR$ be a pseudo-resolvent on $D = \{\lambda \in \C \colon \Re(\lambda) > 0\}$ with values in a Banach space $E$ such that $\norm{R(\mu + \im\alpha)} \leq 1$ for all $(\mu,\alpha) \in \R_{+} \times \R$.
Then
%% --
\[
...
%% --
\[
\mu R(\mu,A) = V_{\beta} \circ \mu R(\mu+ \iu \beta,A) \circ V_{\beta}^{-1}. \quad (2) 
\]
%% --
Hence
%% --
\[
\mu R(\mu,A) = V_{\alpha\beta} \circ \mu R(\mu+ \iu (\alpha+\beta),A) \circ V_{\alpha\beta}^{-1} \quad (3)
\]
%% --
...
$\Fix{\TT} = \C.\phi$ and let $H \coloneqq P\sigma(A) \cap \im\R$.
...
\item 
$G \coloneqq (\TT_{0})^{-} \subseteq \mathcal{L}_{s}(N_{*})$ is a compact, Abelian group in the strong operator topology.
...
%% --
\begin{proof}
For $\eta \in \Omega \coloneqq P\sigma(A) \cap \im\R$ let
%% --
\[
U(\eta) \coloneqq \{\psi \in D(A): A\psi = \eta\psi\}
\]
%% --
and $U = \{U(\eta) \colon \eta \in \Omega\}$.
Then $(U)^{-} = N_{*}$.
...
%% --
\[
\sup\{\norm{\psi_{i} - T_{0}(t)\psi_{i}} \colon 1 \leq i \leq n\} < \delta
\]
%% --
or 
\[
...
Then the induced lattice semigroup $\TT = (\kappa(t))_{t \geq 0}$ fulfils the assertions of Theorem 1.10.
...
%% --
\[
c_{\mathfrak{U}}(E) \coloneqq \{(x_{n}) \in \ell^{\infty}(E) \colon \lim_{\mathfrak{U}}\norm{x_{n}} = 0\}
\]
%% --
is a closed subspace of\/ $\ell^{\infty}(E)$ and equal to the kernel of the seminorm
%% --
\[
\norm{(x_{n})} \coloneqq \lim_{\mathfrak{U}}\norm{x_{n}} \, , \, (x_{n}) \in \ell^{\infty}(E).
\]
%% --
By the \emph{ultrapower} $\widehat{E}$ we understand the quotient space $\ell^{\infty}(E)/c_{\mathfrak{U}}(E)$ with norm
%% --
\[
\norm{\widehat{x}} = \lim_{\mathfrak{U}}\norm{x_{n}} \, , \, (x_{n}) \in \widehat{x} \in \widehat{E}.
\]
%% --
Moreover, for a bounded linear operator $T \in \LE$, we denote by $\widehat{T}$ the well defined operator $\widehat{T}\widehat{x} \coloneqq (Tx_{n}) + c_{\mathfrak{U}}(E)$, $(x_{n}) \in \widehat{x}$.

It is clear by virtue of\/ $(x \mapsto (x, x, ..) + c_{\mathfrak{U}}(E))$ that each $x \in E$ defines an element $\widehat{x} \in \widehat{E}$.
...
%% --
\[
\widehat{\RR} \coloneqq (\lambda \mapsto R(\lambda)^{\wedge}): D \to \mathcal{L}(\widehat{E})
\]
%% --
...
Let $\RR$ be a pseudo-resolvent on $D = \{\lambda \in \C \colon \Re(\lambda) > 0\}$ such that $\norm{\mu R(\mu+\im\alpha)} \leq 1$ for all $(\mu,\alpha) \in \R_{+} \times \R$ and suppose
%% --
\[
...
%% --
\[
\lim_{n}\norm{(\lambda - \im\alpha)R(\lambda)x_{n} - x_{n}} = 0 .
\]
%% --
...
%% --
\[
	\norm{x_{n} - x_{m}} > \delta \quad \text{for} \quad n \neq m .
\]
%% --
Take $k \in \N $ and let $\widehat{x}_{k}$ be the image of\/ $(x_{n+k})$ in $\widehat{E}$.
Since
%% --
\[
	\lim_{n}\norm{(\lambda - \im\alpha)R(\lambda)x_{n+k} - x_{n+k}} = 0 ,
\]
%% --
the so defined $\widehat{x}_{k}$'s belong to $\Fix{(\lambda - \im\alpha)\widehat{R}(\lambda)}$.
Since this space is finite dimensional there exist $j < \ell$,  such that
%% --
\[
	\norm{\widehat{x}_{j} - \widehat{x}_{\ell}} \leq \frac{\delta}{2} .
\]
%% --
From the definition of the norm in $\widehat{E}$ it follows that there are natural numbers $n < m$ such that
%% --
\[
\norm{x_{n} - x_{m}} \leq \frac{\delta}{2} ,
\]
%% --
...
$\widehat{x} \in \Fix  {(\lambda - \im\alpha)\widehat{R}(\lambda)}$ such that
%% --
\[
\gamma \leq \norm{\widehat{y} - \widehat{x}}
\]
%% --
for all $y \in \Fix  {(\lambda - \im\alpha)R(\lambda)}$.

Take a normalized sequence $(x_{n}) \in \widehat{x}$.
...
%% --
\[
\lim_{n} \norm{(\lambda - \im\alpha)R(\lambda)y_{n} - y_{n}} = 0 .
\]
%% --
...
\]
%% --
in $L(\widehat{E})$ and $\rank(P_{\alpha}) = \rank(\widehat{P}_{\alpha})$.
Because of
%% --
\[
...
\]
%% --
one part of the corollary is proved. The other follows from Lemma~\ref{lem:d3-2.2}.
\end{proof}
%% --
...
For $\widehat{x} \in \widehat{M}$ and $\widehat{\phi} \in (M_{*})^{\wedge}$ let $J: (M_{*})^{\wedge} \to \widehat{M}'$ be defined by
%% --
\[
\langle x,J(\widehat{\phi}) \rangle \coloneqq \lim_{\mathfrak{U}}\phi_{n}(x_{n}) \, , \, (x_{n}) \in \widehat{x} \, , \, (\phi_{n}) \in \widehat{\phi} .
\]
%% --
...
Since $ P(E) = \Fix{T} $, we obtain $ \widehat{P}(\widehat{E}) = \Fix{\widehat{T}} $ and 
$\dim \Fix{T} = \dim \Fix{\widehat{T}} < \infty$, where $\widehat{P}$ is the canonical extension of\/ $P$ onto $(M_{*})^{\wedge}$.
Since $\widehat{P} = \lim_{\mu \downarrow 0} \mu R(\mu)^{\wedge}$ it follows that
%% --
\[
\dim \Fix  {(\lambda - \im\alpha)\widehat{R}(\lambda)} \leq \dim \Fix{\widehat{T}} < \infty
\]
%% -- 
...
Our main result Theorem~\ref{thm:d3-1.11} on extends Part-II, Theorem~\ref{thm:b3-3.6} to the noncommutative setting.
