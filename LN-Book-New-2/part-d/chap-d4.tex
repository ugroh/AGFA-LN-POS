...
Since $\norm{\phi} = \phi(\1)$ for every $\phi \in M_{*}^{+}$ and since the norm is additive on the positive cone of\/ $M_{*}$, the integral
%% --
\[
	\int_{0}^{\infty} \eu^{\lambda t}\norm{T(s)\phi} \ds
\]
%% --
...
\item 
\emph{uniformly exponentially stable} if\/ $\norm{T(t)} \leq M\eu^{-\omega t}$ for some $\omega$, $M > 0$ and all $t \geq 0$.
...
%% --
\begin{proof}
Since $s(A) = \omega_{0}$ by Theorem~\ref{thm:d4-1.3}, it suffices to show that (d) implies (a).
...
Then {$\norm{T(t)} = 1$}, hence $s(A) = 1$, but $(T(t))_{t \geq 0}$ is strongly stable.
...
$\int_{0}^{\infty} \norm{U(s)\xi}^2 \ds$ exists for all $\xi \in H$.
...
Then $x \leq \norm{x} \1$ and therefore
%% --
\[
	\phi(T(s)x) \leq \norm{x}\phi(T(s) \1) \quad (s \in \R_{+}).
\]
%% --
...
%% --
\[
\int_{0}^{\infty}\norm{U(s)}^2ds = \int_{0}^{\infty}(T(s)\1\xi \,|\, \xi) \ds
\]
%% --
exists.
...
\item
$\Kern{ \im\gamma - A } \subset p_{r}M_{*}p_{r}$ for all $\gamma \in \R$.
...
Let $0 \leq \psi \in M_{*}$, then $\psi_{0} \coloneqq \lim_{s \to \infty} C(s)\psi \in \Fix{\TT}$ and $s(\psi_{0}) \leq p_{r}$.
Therefore
%% --
\begin{align*}
...
The identity $Q$ of\/ $\mathcal{K}$ is a projection onto the closed linear span of all eigenvectors of\/ $ A $ pertaining to the eigenvalues in $ \iu  \R $. 
...
%% --
\begin{proof}
Consider $\Omega \coloneqq P\sigma(A) \cap \iu  \R$ which is not empty by assumptions.
...
%% --
\[
\int_{K} \langle S\psi,x \rangle \, \overline{\langle S,\eta \rangle} \dm (S) = 0, \eta \in K.
\]
%% --
...
$(S \mapsto S'): K \to \mathcal{L}( (N,\sigma(N,N_{*}) )$ is a representation of the compact, Abelian group $K$ as group of *-automorphism such that the fixed space is one dimensional.
...
Choose $ 0 \leq \phi \in \BH_{*} $, $ \norm{\phi} \leq 1 $  and let 
%
\[
	\phi_{t} \coloneqq 
	\begin{cases}
...
%% --
\[
\norm{(\1 - p_{i})\psi(\1 - p_{i})} \leq \delta
\]
%% --
...
%% --
\[
	\norm{(\phi_{t} - \phi_{0})\vert_{p \BH p} } \leq \delta .
\]
%% --
for all $ t \in U $.
Consequently
%% --
\[
\norm{(\phi_{t} - \phi_{0})} \leq (\delta + 4\sqrt{\delta})
\]
%% --
for all $ t \in U $.
...
But if\/ $ 0 \neq \im\gamma \in P\sigma(A) \cap \im\R $  then $ \im\gamma\Z \subset P\sigma(A) $  by Theorem~\ref{thm:d3-1.11}, whence $ N $  must be infinite dimensional.
...
%% --
If\/ $ H $  is the unit circle, $ \dm $  the normalized Haar measure on $ H $  and $ 0 < \tau \in \R $, then we define the maps $ T_{\tau}(t) $, $ t \in \R_{+} $, on $ L^{1}(H,m) $  by
%% --
\[
(T_{\tau}(t)f)(\xi) = f\mleft( \exp\mleft(\frac{2\pi \iu  \, t }{\tau} \mright) \cdot \xi \mright) \quad (f \in L^{1}(H,\dm), \xi \in H).
\]
%% --
%
%\[
...
\]
%
Thus $ \TT $  is irreducible and uniformly ergodic on $ L^{1}(H,\dm) $  
(see Part-I, Section~\ref{subsec:a1-2.5}).

Now let $ \TT $  be a semigroup on a predual $ M_{*} $ of a von Neumann-algebra $ M $.
It is called \emph{partially periodic}, if there exists a projection $ Q \in \mathcal{L}(M_{*}) $  reducing $ \TT $  such that $ Q(M_{*}) \cong L^{1}(H,\dm) $  and $ \TT_{| \Image{Q}} $  is conjugate to a periodic semigroup on $ L^{1}(H,\dm) $.
...
If\/ $ \TT $  is uniformly ergodic, then $ \sigma(A) \cap \im\R = P\sigma(A) \cap \im\R = \im\gamma\Z $  for some $ \gamma \in \R $.
...
\begin{enumerate}[\upshape (i)]
\item
$ Q $  reduces $ \TT $  and $ Q(M_{*}) \cong L^{1}(H) $, $ H $  being the one dimensional torus.

\item
The restriction $ \TT_{0} $  of\/ $ \TT $  to $ \Image{Q} $  is irreducible and conjugate to a rotation semigroup of period $ \tau = \frac{2\pi}{\gamma} $  on $ H $.
...
it follows that $ \TT_{\vert \Image{Q}} $  is conjugate to the rotation semigroup of period $ \tau $  on $ L^{1}(H,m) $.
\end{proof}
%% --
...
%% --
\begin{equation}
\norm{R(\eta + \im\beta A_{0})} \leq c \tag{*} ,
\end{equation}
%% --
...
by Lemma~\ref{lem:d3-1.6}, $ \dim \Fix{\RR} \leq \dim \Fix{\RR_{1}'} $.
...
$ \lim_{n} \norm{(\Id - \lambda R(\lambda))\phi_{n}} = 0 $  for all $ \lambda \in D $,
...
%% --
\[
\sigma(\widehat{M}'',\widehat{M}')\text{-}\lim_{\eta} \widehat{z}_{n,\eta} = s(\widehat{\psi}_{n}).
\]
%% --
From \textcite[1.7.8]{sakai:1971} and the above considerations, we obtain that the net $ (p\widehat{z}_{n,\eta}\widehat{p}) $  converges to $ s(\widehat{\psi}_{n}) $  in the $ \sigma(\widehat{M}'',\widehat{M}') $-topology.
Therefore we may assume $ \widehat{z}_{n,\eta} \in (\widehat{M}_{p}')_{1}^{+} $.
...
\item
$ \norm{(\Id - \mu R(\mu))\psi_{n,k}} \leq r_{n} $,
...
%% --
\[
\lim_{n} \norm{(\Id - \lambda R(\lambda))\phi_{n}} = 0
\]
%% --
...
We can decompose $ \phi \in M'_{+} $  into its normal and singular part $ \phi = \phi^{(n)} + \phi^{(s)} $, $ 0 \leq \phi^{(n)} \in M_{*} $, $ 0 \leq \phi^{(s)} \in M_*^{\perp} $  and $ \norm{\phi} = \norm{\phi^{(n)}} + \norm{\phi^{(s)}} $  (\textcite[Theorem III.2.14]{takesaki:1979}).
