...
Then there exists a sequence $(T_n)$  in  $ \TT $ and $\epsilon > 0$ such that $\norm{T_n - \Id} \geq \epsilon$, 
...
%% --
\[
\lim_n \norm{(T_n - \Id)(p_{k} x_{k} p_{k})} = 0
\]
%% --
uniformly in $k \in \N$.
This follows from the {Lemma of Phillips} (\textcite[Lemma II.10.3]{schaefer:1974}) and the fact that the sequence $(p_{k} x_{k} p_{k})$ is summable in the $s^{*}(M,M_*)$-topology (compare \textcite{elliot:1972}, Lemma 2.).
...
Without loss of generality we may assume $\norm{(T_n - \Id)(u_n)} \leq n^{-1}$ since the semigroup $\TT$ is strongly continuous.
Thus we obtained the following.
%% --
\begin{enumerate}[\upshape (i)]
\item 
$\lim_n \norm{(T_n - \Id)(p_{k} x_{k} p_{k})} = 0$ uniformly in $k \in \N$ for every bounded sequence $(x_{k})$ in $M$.
\item 
Every projection $p_{k}$ is equivalent to $\1$ via some $u_{k} \in M$.
\item 
$\norm{(T_n - \Id)u_n} \leq n^{-1}$ for all $n \in \N$.
\end{enumerate}
%% --
