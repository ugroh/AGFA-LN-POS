...
By definition, a Banach lattice is a Banach space $ (E,\norm{\cdot}) $ which is endowed with an order relation, usually written $ \leq $, such that $ (E,\leq) $ is a lattice and 
%% --
\begin{align*}
...
\begin{equation}\label{eq:c1-1}
	\text{(LN)} \quad \text{ $|f| \leq |g|$ implies $ \norm{f} \leq \norm{g} $.}   
\end{equation}
%% --
...
\begin{equation*}\label{eq:c1-2}
	\mathrm{(M)} \quad \lVert \sup(f,g) \rVert = \sup(\lVert f \rVert,\norm{g}) \text{ for $ f, g \in E_{+}$, }  
\end{equation*}
%% --
...
%% --
\[
	|h| = \sup\{ \mathrm{Re} \left(  \eu^{\im\theta}h \right) \colon 0 \leq \theta \leq 2\pi \}.
\]
%% --
...
%% --
\[
M = \{\Re( \eu^{\im\theta}h) \colon 0 \leq \theta \leq 2\pi\}
\]
%% --
...
%% --
\[ 
    \rvert f + \iu g \lvert = \rvert f - \iu g \lvert 
        \leq \rvert f \lvert + \rvert g \lvert
\]
%% --
