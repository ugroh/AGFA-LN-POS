...
It follows that
\begin{align*}
	\norm*{\lambda \int_{0}^{r} \eu^{-\lambda s}T(s)f \ds} &\leq \left(r\lambda \eu^{-r} + \lambda^{2}\int_{0}^{r} s\eu^{-\lambda s} \ds\right)M\|f\| \\
	&= (1 - \eu^{-\lambda r})M\|f\|\,.
\end{align*}
%% --
...
% 	\]
	...
	\begin{equation}\label{eq:c3-4.4}
		\begin{split}
		\|(\lambda-A_{1})f\|_{\psi} &= \lim_{t \to 0}\norm*{\frac{1}{t}(\eu^{-\lambda t}T_{1}(t)f-f)}_{\psi} \geq \lim_{t \to 0}\frac{1}{t}(\eu^{-t\Re(\lambda)}-1)\|f\|_{\psi}\\
