...
%% --
	...
	It follows from Eq.~\myeqpageref{eq:c3-1.3} at the net $(\int_{0}^t \eu^{-\mu s}T(s) f \ds )_{t\geq 0}$ converges weakly to $R(\mu,A)f$ for $t \to \infty$.
...
\begin{align*}
	...
	\text{ hence } \norm*{\int_{r}^t \eu^{-\lambda s}T(s)f \ds } &\leq \left\|\int_{r}^t \eu^{-\mu s}T(s)|f| \ds \right\| \
\end{align*}
%% --
...
\begin{equation}\label{eq:c3-1.4}
	\int_{0}^{\infty} \eu^{-\mu s}|\langle T(s)f,\phi\rangle| \ds  = \norm{b(f,\phi)} \leq M\norm{f}\norm{\phi} \quad (f \in E, \phi \in E')\,.
\end{equation}
%% --
...
\begin{align*}
	...
	&\leq \eu^{-\epsilon t}\int_{t}^r \eu^{-\lambda s}|\langle T(s)f,\phi\rangle| \ds  \leq \eu^{-\epsilon t} M\norm{f}\norm{\phi}.
\end{align*}
%% --
It follows that $\norm*{\int_{t}^r \eu^{-\lambda s}T(s) \ds } \leq M\eu^{-\epsilon t}$, hence $\left(\int_{0}^t \eu^{-\lambda s}T(s) \ds \right)_{t\geq 0}$ is a Cauchy net with respect to the operator norm.
\end{proof}
%% --
...
\norm{R(z,A)} \leq M \text{ for } z  \text{ with } \Re(z) = s(A)\,.
\]
...
By Eq.~\myeqpageref{eq:c3-1.5} we have $\norm{R(\epsilon+\im\beta ,A)} \leq \norm{R(\epsilon,A)}$ for every $\beta \in \R $, $\epsilon > 0$.
Therefore 
\[
\lim_{\epsilon\to 0}\norm{\epsilon^k R(\epsilon+\im\beta ,A)} \leq \lim_{\epsilon\to 0}\norm{\epsilon^k R(\epsilon,A)} = 0 \text{ for } k > m\,.
\]
\end{proof}
%% --
...
\index{spectrum!point spectrum $P\sigma$}
	Suppose $E$ is a Banach lattice such that the norm is strictly monotone on $E_{+}$ (\ie  $0 \leq f < g \Rightarrow \norm{f} < \norm{g}$).
...
\begin{equation}\label{eq:c3-2.11}
	\begin{split}
	&\text{Given  $\lambda_{0} \in D$,  then for  $\mu \in D$  with  $|\mu-\lambda_{0}| < \norm{R(\lambda_{0})}^{-1}$}\\ 
...
The identity stated in Eq.~\myeqpageref{eq:c3-2.10} can be rewritten as follows
%% --
\[
...
It follows from Eq.~\myeqpageref{eq:c3-2.11} and Eq.~\myeqpageref{eq:c3-2.12} that every pseudo-resolvent has a unique maximal extension.
...
\begin{proposition}\label{prop:c3-2.6}
	...
	\begin{enumerate}[\upshape (i)]
		...
		\item Suppose $\mu \in \overline{D}\backslash D$. Then $R$ can be extended to an open set containing $\mu$ if and only if there exists a sequence $(\lambda_{n}) \subset D$ converging to $\mu$ such that $\norm{R(\lambda_{n})}$ is bounded.
	\end{enumerate}
\end{proposition}
%% --
\begin{proof}
	\begin{enumerate}[\upshape (i), wide, labelindent=.25em]
		...
		\item 
        If there exists an extension, then $\norm{R(\lambda_{n})}$ is bounded for every sequence $(\lambda_{n})$ converging to $\mu$ by Eq.~\myeqpageref{eq:c3-2.12}.
		On the other hand assuming that $\norm{R(\lambda_{n})}$ is bounded by $M$ for a fixed sequence $(\lambda_{n}) \subset D$ with $\lambda_{n} \to \mu$ $(M \geq 0)$, we have
		\[
		\norm{R(\lambda_{n}) - R(\lambda_{m})} = |\lambda_{n}-\lambda_{m}| \norm{R(\lambda_{n})R(\lambda_{m})} \leq M^2|\lambda_{n}-\lambda_{m}|
        \]
...
\begin{enumerate}[\upshape (i), wide, labelindent=0.5em]
    ...
    On the other hand, the set of operators $\{\frac{1}{t} \int_{0}^{t} \exp(-\tau s(A))T(\tau) \diff{\tau} \colon 0 \leq t \leq t_{0} \}$ is bounded for every $t_{0} \geq 0$, hence in Eq.~\myeqpageref{eq:c3-2.15}(b) the boundedness for $t \to \infty$ is important.

    \item 
    By Corollary~\ref{cor:c3-1.4} we have $\norm{R(\lambda,A)} \geq r(R(\lambda,A)) = (\lambda-s(A))^{-1}$. 
    Hence $\norm{R(\lambda,A)}$ grows at least as fast as $(\lambda-s(A))^{-1}$. 
...
%% --
\begin{proof} 
To prove equivalence of \upshape(a) and \upshape(b) of Eq.~\myeqpageref{eq:c3-2.15}, we assume $s(A) = 0$ and write $C(t) \coloneqq \frac{1}{t} \int_{0}^{t} T(\tau) \diff{\tau}$.
...
It follows that
\begin{align*}
	\norm*{\lambda \int_{0}^{r} \eu^{-\lambda s}T(s)f \ds} &\leq \left(r\lambda \eu^{-r} + \lambda^{2}\int_{0}^{r} s\eu^{-\lambda s} \ds\right)M\norm{f} \\
	&= (1 - \eu^{-\lambda r})M\norm{f}\,.
\end{align*}
%% --

Letting $r \to \infty$, we obtain by Eq.~\myeqpageref{eq:c3-1.1} $\norm{\lambda R(\lambda,A)f} \leq M\norm{f}$ $(f \in E, \lambda > 0)$ hence $\norm{\lambda R(\lambda,A)} \leq M$
\end{proof}
%% -- 
...
Since $\norm{R_{1}(\lambda)'} = \norm{R_{1}(\lambda)} = \norm{R(\lambda,A)}$, the assumption of slow growth implies that $\{\lambda R_{1}(\lambda)'\phi \colon \lambda > 0\}$ is bounded in $E_{1}'$, hence $\sigma(E_{1}',E_{1})$-relatively compact by \emph{Alaoglu's Theorem}.
...
In particular, we have $\norm{R_{2}(r+\im n\beta)} \geq \frac{1}{r}$, thus 
\[
\norm{R(r+\im n\beta,A)} = \norm{R_{1}(r+\im n\beta)} \geq \norm{R_{2}(r+\im n\beta)} \geq \frac{1}{r} \ \text{ for } r > 0\,.
\]
...
\begin{enumerate}[\upshape (i), wide, labelindent=.25em]
	...
	We consider a null sequence $(t_{n})$, $0 < t_{n} < t_{0}$, such that $\norm{T(t_{n})f - f} \leq 2^{-n}$ and define $f_{n} \coloneqq T(t_{n})f$, $g_{n} \coloneqq f - \sum_{k=n}^{\infty}(f-f_{k})^{+}$.
...
If one chooses $m$ such that $m(z,w)$ tends to $+\infty$ sufficiently fast as $(z,w) \to (1,1)$, one obtains $\norm{T(t)} = \norm{m_{t}}_{\infty} \leq \exp(-t^{2})$ for all $t \geq 0$.
Obviously such an estimate of\/ $\norm{T(t)}$ implies $\omega_0(A) = -\infty$, hence $\sigma(A) = \emptyset$.
\end{example}
%% --
...
\begin{proof}
    ...
    \begin{equation}\label{eq:c3-3.14}
        \norm{g_{n} - g_{m}} \geq \epsilon \text{ whenever } n \neq m.
    \end{equation}
    %% --
    ...
    $\norm{\hat{g}^{k} - \hat{g}^{m}} = \mathcal{F}\text{-}\lim\sup\norm{g_{n+k} - g_{n+m}} \geq \epsilon$ provided that $k \neq m$.
...
\begin{proof}
	...
	By Eq.~\myeqpageref{eq:c3-3.16} and Eq.~\myeqpageref{eq:c3-3.17} the absolute value of an element $\tilde{f} \in M_/$ is a scalar multiple of\/ $\tilde{u} \coloneqq \hat{u} + I$. 
...
\begin{split}
    ...
    R_{m}(t) &\coloneqq \textit{\small $\int_{0}^{t}\int_{0}^{t_{1}} .. \int_{0}^{t_{m-1}} T(t-t_{1})KT(t_{1}-t_{2})K \ldots T(t_{m-1}-t_m)KS(t_{m})\dt _{m}..\dt _{1}$}\,.
\end{split}
\end{equation}
%% --
...
Thus the integrand appearing in the the representation Eq.~\myeqpageref{eq:c3-3.21} of\/ $\langle R_{m}(t)f,\phi\rangle$ is non-zero at $t_{1}=t_{2}= .. =t_{m-1}=t$, $t_{m}= t$.
...
\]
%% --
where the kernel $k$ is given by $k(x,v,x',v') \coloneqq  \kappa(x,v,v')r_{\lambda}(x,x',v')$ (\cf Eq.~\myeqpageref{eq:c3-3.5}, Eq.~\myeqpageref{eq:c3-3.7}). 
...
% 	\]
	...
	\norm{T_{1}(t)f}_{\psi} = \langle|T_{1}(t)f|,\psi\rangle = \langle|f|,T_{1}(t)'\psi\rangle \geq \langle|f|,\psi\rangle = \norm{f}_{\psi}\,.
	\end{equation}
	%% --
	Then for $\lambda \in \C $ with $\Re(\lambda) < 0$ we have 
	\[ 
	\norm{(\eu^{\lambda t}-T_{1}(t))f}_{\psi} \geq \norm{T_{1}(t)f}_{\psi}-\norm{\eu^{\lambda t}f}_{\psi} \geq (1-|\eu^{\lambda t}|)\norm{f}_{\psi} \ (f \in (E,\psi))
	\]
	and we obtain for the corresponding generator
	%% --
	\begin{equation}\label{eq:c3-4.4}
		\begin{split}
		\norm{(\lambda-A_{1})f}_{\psi} &= \lim_{t \to 0}\norm*{\frac{1}{t}(\eu^{-\lambda t}T_{1}(t)f-f)}_{\psi} \geq \lim_{t \to 0}\frac{1}{t}(\eu^{-t\Re(\lambda)}-1)\norm{f}_{\psi}\\
		&= -\Re(\lambda)\cdot\norm{f}_{\psi} \ \text{ for } \ \Re(\lambda) < 0 \ \text{ and } \ f \in (E,\psi)\,.
		\end{split}
	\end{equation}
	%% --
...
\begin{proof}
	...
	By Eq.~\myeqpageref{eq:c3-4.5} $|f|_{o} \coloneqq  \lim_{n\to \infty} T^{n}|f|$ exists and we have $T|f|_{o} = \lim_{n\to \infty} T^{n+1}|f| = |f|_{o}$, \ie $|f|_{o} \in F_{\R }$.
	And, for $g \in F_{\R }$ satisfying $\pm f \leq g$, we have $|f|_{o} \leq g$ thus $|f|_{o} = \sup_{F}\{f,-f\}$. 
Moreover, $\norm{f}_{o} \coloneqq \norm{\abs{f}_{o}}$ $(f \in F)$ is an equivalent norm on $F$ such that $(F,\norm{\cdot}_{o})$ is a Banach lattice.
...
(The explicit representation Eq.~\myeqpageref{eq:c3-4.6} only allows the estimate $\norm{T(t)} \leq \sqrt{2} + t \cdot \norm{k}_2$ $(t\geq 0)$.)
\end{example}
...
$(f_n) \subset E$ such that $\lim_{n \to \infty}\norm{T(t)f_n - rf_n} = 0$.
Since we have
%% --
\[
...
\]
Therefore $\lim_{n \to \infty}\norm{(A - \alpha) g_n} = 0$\,. It remains to prove 
$\liminf_{n \to \infty}\norm{g_n} > 0$\,.
...
\begin{enumerate}[\upshape (i),wide, labelindent=.25em]
    ...
    \]
    \[
    \text{for } \ 0 \leq s \leq t\,, f \in E \ \text{ and } \ M \coloneqq \sup_{0 \leq s \leq t}\norm{T(s) }\,.
    \]
...
$\sigma(A_{\vert I_{\mu}}) \subset \{\lambda \in \C  \colon \Re(\lambda) < \mu\}$. The same argument applied to $-A$ and
...
$L^p(\mu)$ with $1 \leq p < \infty$. For $t_0 > 0$ we define $M \coloneqq  (\sup\{\norm{T(t)} \colon 0 \leq t \leq t_0\})^{1/p}$.
Given a Borel set $C \subset X$ we write $C(t) \coloneqq  \phi_t^{-1}(C)$.
Then we have $T(t)\1_C = \1_{C(t)}$, hence
$\mu(\phi_t^{-1}(C)) = \norm{\1_{C(t)}}_p^p = \norm{T(t)\1_C}_p^p \leq M \cdot \norm{\1_C}_p^p = M \cdot \mu(C)$.
...
$\mu(C(t)) \leq M \cdot \mu(C)$, it is easy to see that $\norm{h_n \circ \phi_t}_p^p \leq M \cdot \norm{h_n}_p^p \leq M \cdot \norm{f}_p^p$.
Thus by the Monotone Convergence Theorem we have $f \circ \phi_t \in L^p(\mu)$ and
$\norm{f \circ \phi_t}_p \leq M^{1/p}\norm{f}_p$. It follows that $T(t)$ is a bounded linear
operator on $L^p(\mu)$ and $\norm{T(t)} \leq M^{1/p}$ for $0 \leq t \leq t_0$. Since $\phi$ is a
...
$\lim_{t \to 0}\norm{T(t)f - f}_p = 0$ for $f \in C_c(X)$. Since $C_c(X)$ is dense in
$L^p(\mu)$ and $\norm{T(t)} \leq M^{1/p}$ for $0 \leq t \leq t_0$, the semigroup is strongly
continuous.
...
field. Note that for continuous flows condition Eq.~\myeqpageref{eq:c3-4.12} is automatically satisfied because for a compact $K \subset X$ the set $\phi_t^{-1}(K) = \phi_{-t}(K)$
...
$\norm{D\phi_t(x)} \leq \eu^{M|t|}$  for all  $x \in \R ^n$, $t \in \R$  where   
%
\[
	M \coloneqq  \sup \{\norm{DF(x)} \colon x \in \R ^n\}.
\]
% 
\end{minipage}
\end{equation}
