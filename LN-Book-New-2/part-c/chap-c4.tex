...
\begin{enumerate}[\upshape (i)] 
    ...
    There exist $\tau > 0$, $\phi \in E'_{+}$ such that $\norm{T(\tau)f} \leq \langle f,\phi\rangle$ for all $f \in E_{+}$.
\end{enumerate}
\end{theorem}
%% --
\begin{proof}
...
Since for positive semigroups we always have $\norm{R(\lambda,A)} \leq \norm{R(\Re(\lambda), A)}$ ($\Re(\lambda) > s(A)$) (see   Corollar~\ref{cor:c3-1.3}), the assertion for $L^{2}$-spaces follows from Part-I, Corollary~\ref{cor:a3-7.11}.
...
%% --
\begin{align*}
\int_{0}^{r} \eu^{-\alpha s}\norm{T(s)f} \, \ds &= \int_{0}^{\tau} \eu^{-\alpha s}\norm{T(s)f} \, \ds + \eu^{-\alpha \tau}\int_{0}^{r-\tau} \eu^{-\alpha s}\norm{T(\tau+s)f} \, \ds \\
&\leq \int_{0}^{\tau} \eu^{-\alpha s}\norm{T(s)f} \, \ds + \eu^{-\alpha \tau}\int_{0}^{r-\tau} \eu^{-\alpha s}\langle T(s)f,\phi \rangle \, \ds \\
&\leq \int_{0}^{\tau} \eu^{-\alpha s}\norm{T(s)f} \, \ds + \norm{R(\alpha,A)f}
\end{align*}
%% --
(the last inequality follows from Theorem~\ref{thm:c3-1.2}).
\index{Datko's theorem}
Now Datko's theorem (Part-I, Theorem~\ref{thm:a4-1.11}) implies $\omega_{0}(A) < \alpha$.
\end{enumerate}
\end{proof}
...
%% --
\[
\norm{T_{0}f}_{p} \leq C_{p}\norm{f}_{p} \ \text{ and } \ \norm{T_{0}f}_{q} \leq C_{q}\norm{f}_{q} 
...
u \mapsto \log\norm*{T_{1/u}} \text{ is a convex function on the interval } \left[\frac{1}{q},\frac{1}{p}\right].
\end{equation}
%% --
...
u \mapsto \log(r(T_{1/u})) = \lim_{n\to\infty} \frac{1}{n}\log\norm*{T_{1/u}^{n}} \text{ is convex on }  \left[\frac{1}{q},\frac{1}{p}\right].
\end{equation}
%% --
...
\textit{Let $R$ be a linear operator on a Banach space such that $\sup\{ \norm{R^{n}} \colon n \in \N \} < \infty$. 
...
%% --
\begin{proof}
\upshape{a} $\Rightarrow$ \upshape{b} was proven in Part-I, Theorem~\ref{thm:a4-1.13} in a more general setting.

\upshape{b} $\Rightarrow$ \upshape{a}:  In case $\omega_{0}(A) < 0$ one trivially has~\ref{thm:c4-1.5-1}.
...
\end{equation}
%% --
From \myeqpageref{eq:c4-1.4} it follows that $\lim_{n \to \infty} \norm{T(n) - T(n+1)} = 0$ and therefore $\lim_{t \to \infty} \norm{T(t) - T(t+1)} = 0$.
...
Since the semigroup is bounded, we have $\lim_{t \to \infty} \norm{T(t)f} = 0$ for every $f \in \overline{\Image{\Id - T(1)}} = E$, \ie  $(T(t))$ is uniformly stable.
...
\end{proof}
Next we show that weak uniform stability implies uniform stability provided that $E$ is weakly sequentially complete (see Section~\ref{sec:c1-5}) and $(\image{A})_{+} \coloneqq A(D(A)) \cap E_{+}$ is a total subset of\/ $E$.
...
Since $f_{0} \in \Image{A}$, the second term converges to $h \coloneqq  \int_{0}^{\infty} T(s)f_{0} \, \ds \in \Image{A}$ (Part-I, Theorem~\ref{thm:a4-1.16}) and $Ah = -f_{0}$.
...
\begin{enumerate}[\upshape (i), resume]
    \item $ \norm*{\int_{t'}^{\infty} T(s)g_{0} \, \ds} \leq \frac{\epsilon}{2M}$ and  
$G(s) \leq \frac{\epsilon}{2} \,  \norm*{\int_{0}^{\infty} T(s)g_{0} \, \ds}$ for every $s \geq t'$.
\end{enumerate}
%% --
Then, for $t > 2t'$,
%% --
...
&\leq M \int_{t-t'}^{\infty} T(t)g_{0} \, \ds + \epsilon/2 \,  \norm*{\int_{0}^{\infty} T(t)g_{0} \, \ds\, }^{-1} \int_{0}^{t-t'} T(t)g_{0} \, \ds\\
&\leq M \int_{t'}^{\infty} T(t)g_{0} \, \ds + \epsilon/2 \,  \norm*{\int_{0}^{\infty} T(t)g_{0} \, \ds\, }^{-1} \int_{0}^{\infty} T(t)g_{0} \, \ds.
\end{align*}
%% --

Hence $\norm*{\int_{0}^{t} T(t)H_{\pm}(s) \, \ds} \leq \epsilon$ for every $t > 2t'$.
\end{proof}
...
Assume that the forcing term $F$ has values in $D(A)$, that it is continuous with respect to the graph norm and that $f_{0} \coloneq  \norm{\cdot}_{A}$-$\lim_{t \to \infty} F(t)$ exists.
...
By assumption we have $\lim_{s \to \infty}\norm{A(F(s)-f_{0})} = 0$ and from Part-I,
Theorem~\ref{thm:a4-1.3} and Eq.~\myeqpageref{eq:a4-1.3} we deduce that $\norm{T(s)R(0,A)} \leq M\eu^{-\epsilon s}$ for $s \geq 0$ and suitable constants $M \geq 1$, $\epsilon > 0$.
Thus for the third term we have
%% --
\begin{align*}
 \norm*{\int_{0}^{t} T(s)(F(t-s)-f_{0}) \, \ds\,} &\leq \int_{0}^{t} \norm{T(s)R(0,A)} \norm{A(F(t-s)-f_{0})} \, \ds \\
...
For example, the norm convergence to zero, \ie  $\lim_{t \to \infty} \norm{T(t)} = 0$ for a positive semigroup on certain Banach lattices, is characterized by the condition $s(A) < 0$ (see Theorem~\ref{thm:c4-1.1}).
...
\norm{T(t) - P} \leq M \cdot \eu^{-\delta t} \text{ for all } t \geq 0.
\end{equation}
%% --
\end{theorem}
%% --
...
\end{equation*}
%% --
where $P$ is the residue of\/ $R(\cdot,A)$ at $0$, $k$ is the pole order and $\norm{R(t)} \leq M \cdot \eu^{-\delta t}$ for suitable constants $\delta > 0$, $M \geq 1$.
...
%% --
\[
\widehat{T}(t) \coloneqq \eu^{-\omega_0 \, t }T(t)
\]
%% --
...
\norm{T(t_{0})f} \leq \langle f,\phi \rangle + b\norm{f} \quad \text{for all} \quad f \geq 0.
\end{equation}
%% --
...
Then (\refeq{eq:c4-1.4}) implies $\lim_{n \to \infty} \norm{T(n) - T(n+1)} = 0$, hence $\lim_{t \to \infty}\norm{T(t) - T(t+1)} = 0$.
Take $f = g - T(1)g$.
Then 
%
\[
	\norm{T(t)f} = \norm{T(t)g - T(t+1)g} \leq \norm{T(t) - T(t+1)} \cdot \norm{g}
\]
%
...
Since $\Kern{A} = \bigcap_{t \geq 0} \Kern{\Id - T(t)} = \Kern{\Id - T(1)}$ (cf.\ Part-I, Corollary~\ref{cor:a3-6.4}), we have $\Image{\Id - T(1)} + \Kern{\Id - T(1)} \subset F$.
...
\[
\norm{T(t)f} = \eu^{-t} \norm*{\sum_{n=0}^{\infty} \frac{t^n}{n!}R^{n}f} = 
\eu^{-t}\sum_{n=0}^{\infty} \frac{t^n}{n!}\norm{R^{n}f}= \norm{f} > 0
\] 
...
Moreover, there exist $t_{0} > 0$ and $\tau > 0$ such that $\norm{T(t_{0}) - T(t_{0}+\tau)} < 1$.

For bounded linear operators $S \in \L{L^{1}}$ 
one has $\norm{S} = \norm{\lvert S \rvert}$ 
(see \textcite[IV, Theorem 1.5]{schaefer:1974}), 
hence 
hence $ \norm{ \lvert T(t_{0}) - T(t_{0}+\tau) \rvert f} < \|f\|$ for every $f \in L^{1}(\mu)$, $f \neq 0$.
...
\rho(A) = \rho(A) + \im\alpha\Z \quad \text{and} \quad \norm{R(\lambda,A)} = \norm{R(\lambda+\im\alpha k,A)}
\end{equation}
%% --
...
Moreover, for small $\epsilon > 0$, $\norm{R(-\epsilon+\im\lambda,A)}$ is uniformly bounded for $\lambda \in \R$.
...
$s(A_{2}) < 0$ and $\norm{R(\lambda,A_{2})}$ is uniformly bounded in each semiplane\\ 
...
\norm{T(t)f - R_{\tau}(t)g} \leq M\eu^{-\epsilon t}\norm{f} \text{ for every } t \geq 0 \text{ where } g \coloneqq Qf.
\end{equation}
\end{proposition}
%% --
...
In fact, the assumptions imply that $T(t)h = h$ for some $h \gg 0$, $\norm{h}_{p} = 1$.
...
The \emph{Riesz Convexity Theorem} (see \textcite[VI.10.11]{dunfordschwartz:1958}) then implies that by restricting the semigroup $(T(t))$ to $L^{q}(\mu)$ $(p < q < \infty)$ we obtain a strongly continuous semigroup $(T_{q}(t))_{t \geq 0}$ on $L^{q}(\mu)$ such that $\norm{T_{q}(t)} \leq \norm{T(t)}^{p/q}$ for $t \geq 0$, $q \geq p$.
...
%% --
\begin{align*}
\norm{R(1,A_{2})^{n} - P_{2}} &\leq \norm{R(1,A)^{n} - P}^{2/p}\norm{R(1,A)_{\infty}^{n} - P_{\infty}}^{1-2/p} \\
&\leq \norm{R(1,A)^{n} - P}^{2/p}(1 + \norm{P_{\infty}})^{1-2/p}.
\end{align*}
%% --
...
This semigroup satisfies $\norm{T_{\phi}(t)} \leq \exp(t \cdot s(A))$ and has spectral bound $s(A)$.
...
Clearly 
%\linebreak[4] 
$\norm{\epsilon_{\lambda}} = 1/\lambda \cdot (1 - \eu^{-\lambda}) \to 0$ as $\lambda \to \infty$ and we have
%% --
\[
\norm{S_{\lambda}} = \norm{\epsilon_{\lambda}} \cdot \norm{\Phi} = 1/\lambda \cdot (1 - \eu^{-\lambda}) \cdot \norm{\Phi} \leq 1/\lambda \cdot \norm{\Phi}\,.
\]
%% --
For every $\lambda > \norm{\Phi}$, the operator $(\Id - S_{\lambda})$ is an isomorphism of\/ $E$ and it induces a bijection from $D(A)$ onto $D(A_{0})$ such that
%% --
...
This yields the invertibility of\/ $\lambda - A$ for each $\lambda \geq \norm{\Phi}$.

In order to obtain an estimate on $\norm{R(\lambda,A)}$ we use Formula Eq.~\myeqpageref{eq:c4-3.2}.
Since 
\[ 
\norm{R(1,S_{\lambda})} = \norm{\sum_{n=0}^{\infty} S_{\lambda}^n} \leq \sum_{n=0}^{\infty} \norm{\epsilon_{\lambda}}^n \cdot \norm{\Phi}^n = (1-\norm{\epsilon_{\lambda}} \cdot \norm{\Phi})^{-1}
\]
and $\norm{R(\lambda,A_0)} \leq 1/\lambda$ for $\lambda > 0$, we obtain for $\lambda \geq \norm{\Phi}$ that
%% --
\begin{align*}
\norm{R(\lambda,A)} &\leq (1 - \norm{\epsilon_{\lambda}} \cdot \norm{\Phi})^{-1} \cdot 1/\lambda = (\lambda - \lambda \cdot \norm{\epsilon_{\lambda}} \cdot \norm{\Phi})^{-1} \\
&= (\lambda - (1 - \eu^{-\lambda}) \cdot \norm{\Phi})^{-1} \leq (\lambda - \norm{\Phi})^{-1}.
\end{align*}
%% --
...
Then
\begin{align*}
\norm{w(t)}_F & = \norm{\Phi(w_{t})} \leq \norm{\Phi} \cdot  \norm{w_{t}}_{E} =  \norm{\Phi} \cdot \int_{-1}^{0} \norm{w_{t}(s)\_{F}} \, \ds \\
&=  \norm{\Phi} \int_{-1}^0 \norm{w(t+s)}_{F} \ds = \norm{\Phi} \int_{t-1}^{t} \norm{w(s)}_{F} \ds \\
&\leq \norm{\Phi} \int_{-1}^{t} \norm{w(s)}_{F} \ds \ \text{ for } \ t \geq 0\,.
\end{align*}
By Gronwall's lemma $\norm{w(t)}_F \leq 0$, thus $w(t) = 0$.
\end{proof}
%% --
...
\index{stability}
\index{spectral bound $s(A)$}
Let $A \coloneqq \frac{\diff{}}{\dt}, D(A) \coloneqq \{f \in AC([-1,0],F) \colon f' \in L^{1}([-1,0],F)$ and $f(0) = \Phi(f)\}$ be the generator of the solution semigroup on $E \coloneqq L^{1}([-1,0],F)$ corresponding to \myeqpageref{eq:c4-re}. 
...
Let $s(\Phi_{\lambda}) > 1$. 
Since $s(\Phi_{\mu}) \leq \norm{\Phi_{\mu}} \leq \norm{\Phi} \cdot \norm{\epsilon_{\mu}}$ we see that $s(\Phi_{\mu})$ tends to zero as $\mu \to \infty$. 
...
If we define $\Phi f \coloneqq \int_{-1}^{0} K(s)f(s) \, \ds$ for all $f \in E$, then 
...
%% --
\[
\norm{(\Phi_{\mu})^n} > \eu^{n(\mu-\lambda)\epsilon}\norm{(\Phi_{\lambda})^n}
\]
%% --
and consequently 
%% --
...
As in Example~\ref{ex:c4-3.13} the first equation in Eq.~\myeqpageref{eq:c4-3.14} corresponds to the aging process.
