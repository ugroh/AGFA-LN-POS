...
% 		such that $\norm{T(t)} \leq M \cdot  \eu^{\omega t}$  for  $t \geq 0$} \} \\
%	& = \lim_{t\to\infty} \frac{1}{t}\log\norm{T(t)} = \inf_{t>0} \frac{1}{t}\log\norm{T(t)} .
%\end{split}
%\end{equation}
\begin{equation}\label{eq:a1-1.1}
\begin{aligned}
    \omega_{0} & \coloneqq \inf \bigl\{ \omega \in \R \colon \text{there exists } %\exists\,
         M \in \R_{+} \text{ such that }
        \norm{T(t)} \leq M \eu^{\omega t} \ (t \geq 0) \bigr\} \\
    & = \lim_{t\to\infty} \frac{1}{t}\log\norm{T(t)}
      = \inf_{t>0} \frac{1}{t}\log\norm{T(t)} .
\end{aligned}
\end{equation}
%% --
\end{definition}
%% --
Particularly important are semigroups such that for every $t \geq 0$ we have $\norm{T(t)} \leq M$ (\emph{bounded semigroups}) or $\norm{T(t)} \leq 1$ (\emph{contraction semigroups}).
...
\]
%
Then for the $\ell^{1}$-norm on $E$ we obtain $\norm{T(t)} = 1 + t$, hence $(T(t))_{t\geq0}$ is an unbounded semigroup having growth bound $\omega_{0} = 0$.
...
\end{align*}
%% --
Each $T(t)$, $t > 0$, satisfies $\norm{T(t)} = 2$ as can be seen by taking $f \coloneq  \1_{[0,t]}$.
...
%% --
\[
    \{f_{n} \colon n \in \N \} \subset D(B), f_{n} \to 0 \text{ and } Bf_{n} \to g \text{ implies } g = 0.
\]
%% --
...
Finally, we call a subset $D_{0}$ of\/ $D(B)$ a \emph{core} for $B$ if\/ $D_{0}$ is $\norm{\cdot}_{B}$-dense in $D(B)$.
\index{core}
This means that a closed operator is determined (via closure) by its restriction to a core in its domain.

We now collect the fundamental topological properties of semigroup generators, their domains (see also Corollary~\ref{cor:a2-1.34} ) and their resolvents.
\index{resolvent}
%% --
...
Moreover, the growth bound $\omega_0$ satisfies $|\omega_0| \leq \norm{A}$ in this situation.
...
\end{enumerate}
%% --
One has $\norm{M_{q}} = \norm{q}_{\infty}$ in this situation.
...
%% --
\[
    R_{\tau}(t)f(z) \coloneqq f( \eu^{2\pi \im t/\tau}\cdot z), \quad z \in \Gamma
\]
%% --
...
\]
%% -- 
satisfying $\norm{e_{n} \otimes e_{n}} \leq 1$ and
...
%% --
\begin{align*}
    w_{t}(x) &\coloneqq \left( \frac{1}{4\pi t}\right)^{\frac{1}{2}} \sum_{m \in \Z} \exp(-(x+2m)^{2}/4t) \\
...
%% --
\[
    \norm{T(t)} = \norm{T(t)\1} = \sup_{x \in \left[ 0,1 \right]} \int_{0}^{1} k_{t}(x,y)dy = 1 .
\]
%% --
...
\]
%% --
by Young's inequality (\textcite[p.28]{reedsimon:1975}). Hence $\norm{T(t)} \leq 1$ for every $t > 0$.
...
Examples analogous to Section~\ref{subsec:a1-2.7} are valid in $L^{p}\left[ 0,1 \right]$, \resp to Section~\ref{subsec:a1-2.8} in $C_{0}(\R^{n})$.
%% --
...
For $\lambda \in \C$ and $S(t) \coloneqq \exp(-\lambda t)T(t)$ the formulas Eq.~\myeqpageref{eq:a1-1.3} and Eq.~\myeqpageref{eq:a1-1.4} yield:
%% --
...
\index{standard example!translation semigroup}
%%
We start with the Banach space $E = L^{1}(\R)$ and the translation semigroup $(T(t))_{t \geq 0}$ where $T(t)f(x) \coloneqq f(x+t)$ (see Example~\ref{subsec:a1-2.4}).
...
\]
%% --
for $f \in D(A_{|}) \coloneqq \{f \in E \colon f \in \text{AC} \text{ with } f' \in E \text{ and } f(x) = 0 \text{ for } x \geq 1\}$.

Next we identify $L^{1}(\left[ 0,1 \right])$ with the quotient space \sfrac{$L^{1}((-\infty,1])$}{I} where
...
%% --
quotient semigroup $(T(t)_{\vert _{/}})$ on $L^{1}(\left[ 0,1 \right])$ is the nilpotent translation semigroup as in Section~\ref{subsec:a1-2.6}.
...
%% --
\[
    E^{*} \coloneqq \{\phi \in E' \colon \norm{\cdot}\text{-}\lim_{t \to 0} T(t)'\phi = \phi\}.
\]
%% --
The adjoint semigroup on $E^{*}$ is given by the operators
%% --
\[
    T(t)^{*} \coloneqq T(t)'_{ \vert E^{*}}, \quad t \geq 0.
\]
%% --
...
%% --
\[
    \sup\{\langle f,\phi \rangle \colon \phi \in E^{*}, \norm{\phi} \leq 1\} \leq \norm{f} \leq M\cdot\sup\{\langle f,\phi \rangle \colon \phi \in E^{*}, \norm{\phi} \leq1\}
\]
%% --
where $M \coloneqq \lim\sup_{t \to 0} \norm{T(t)}$.
In particular, $E^{*}$ separates $E$, \ie $E^{*}$ is $\sigma(E',E)$-dense in $E'$.
In addition, the norm estimate given above yields
%% --
\[
    \norm{T(t)^{*}} \leq \norm{T(t)} \leq M\norm{T(t)^{*}} \quad \text{for all } t \geq 0.
\]
%% --
...
For every $f \in D(A)$ with $\norm{f} \leq 1$ we define a continuously differentiable function
%% --
\[
...
Since $\{\xi_{f}'(t) \colon t \in \left[ 0,1 \right], f \in D(A), \norm{f} \leq 1\}$ is bounded, it follows that the set
%% --
\[
    \{\xi_{f} \colon f \in D(A), \norm{f} \leq 1\}
\]
%% --
...
\]
%% --
for every $0 \leq s \leq t_{0}$ and $f \in D(A)$, $\norm{f} \leq 1$.
But this implies $\norm{T(s)'\phi - \phi} < \epsilon$ and hence $\phi \in E^{*}$.
...
This means that for every $\epsilon > 0$ there exists $0 < t_{0} < 1$ such that $|f_f'(0) - f_f'(s)| < \epsilon$ for every $f \in D(A)$, $\norm{f} \leq 1$ and $0 < s < t_{0}$.
%% --
In particular,
%% --
\[
...
%% --
\[
    \epsilon > \norm*{A'\phi - \frac{1}{s}(T(s)'\phi-\phi) }
\]
%% --
...
Since $E_{1}$ is invariant under $(T(t))_{t\geq 0}$, apply Proposition~\ref{prop:a1-1.6}\,(i), it makes sense to consider the semigroup $(T_{1}(t))_{t\geq 0}$ of all restrictions $T_{1}(t) \coloneqq T(t)_{\vert E_{1}}$.
The results of Proposition~\ref{prop:a1-1.6} imply that $T_{1}(t) \in \L{E_{1}}$ and $\norm{T_{1}(t)f-f}_{1} \to 0$ as $t \to 0$ for every $f \in E_{1}$.
...
\item
\index{resolvent}
For $\lambda \in \rho(A)$ the operator $(\lambda - A)$ and the resolvent $R(\lambda,A)$ are isomorphisms from $E_{1}$ onto $E$, \resp from $E$ onto $E_{1}$ (show that $\norm{\cdot}_{1}$ and $\norm{\cdot}_{\lambda}$ with $\norm{\cdot}_{\lambda} \coloneqq \norm{(\lambda - A)\cdot}$ are equivalent).
%% --
...
\]
%% --
for every $f \in E$ and define $E_{-1}$ as the completion of\/ $E$ for $\norm{\cdot}_{-1}$.
...
%% --
\[
    \norm{(f_{n})_{n \in \N}} \coloneqq \sup \{\norm{f_{n}} \colon n \in \N\} .
\]
%% --
It is clear that every $T(t)$ extends canonically to a bounded linear operator
%% --
\[
    \widehat{T}(t)(f_{n}) \coloneqq (T(t)f_{n})
\]
%% --
...
%% --
\[
    m^{\mathcal{T}}(E) \coloneqq \{(f_{n}) \in m(E) \colon \lim_{t \to 0} \norm{T(t)f_{n}-f_{n}} = 0 \text{ uniformly for } n \in \N\} .
\]
%% --
Then the restricted semigroup
%% --
\[
    \widehat{T}(t)(f_{n}) = (T(t)f_{n}), \quad (f_{n}) \in m^{\mathcal{T}}(E)
\]
%% --
...
For the generator $\widehat{A}$ of\/ $(\widehat{T}(t))_{t \geq 0}$ on $m^{\mathcal{T}}(E)$ one has the following properties.
%% --
\begin{enumerate}[\upshape(i)]

\item 
$D(\widehat{A}) = \{(f_{n}) \in m^{\mathcal{T}}(E) \colon f_{n} \in D(A) \text{ and } (Af_{n}) \in m^{\mathcal{T}}(E)\}$,
...
%% --
\[
    c_{\mathcal{F}}(E) \coloneqq \{(f_{n}) \in m(E) \colon \mathcal{F}\text{-}\lim\norm{f_{n}} = 0\}
\]
%% --
...
%% --
\[
    \norm{(f_{n}) + c_{\mathcal{F}}(E)} = \mathcal{F}\text{-}\lim \sup \norm{f_{n}} .
\]
%% --
...
With the canonical injection $j \colon f \mapsto (f,f,f,\ldots) + c_{\mathcal{F}}(E)$ from $E$ into $E_{\mathcal{F}}^{T}$ the operators $T_{\mathcal{F}}(t)$ are extensions of\/ $T(t)$ satisfying $\norm{T_{\mathcal{F}}(t)} = \norm{T(t)}$. The basic facts about the generator $(A_{\mathcal{F}},D(A_{\mathcal{F}}))$ of\/ $(T_{\mathcal{F}}(t))_{t \geq 0}$ follow from 3.3 and are collected in the following proposition.
%% --
...
\item
$\norm{u}_{\pi} \coloneqq \inf\{\sum_{j=1}^{m} \norm{h_{j}} \norm{k_{j}} \colon u = \sum_{j=1}^{m}h_{j} \otimes k_{j}, h_{j} \in E, k_{j} \in F\}$ defines the \emph{greatest cross norm $\pi$} on $E \otimes F$.

\item
$\norm{u}_{\epsilon} \coloneqq \sup\{\langle u,\phi \otimes \psi\rangle \colon \phi \in E', \psi \in F', \norm{\phi}, \norm{\psi} \leq 1\}$ defines the 
...
\item
If\/ $E$ and $F$ are Hilbert spaces, $\norm{u}_{h} = (u|u)_{h}^{1/2}$, where the scalar product $(\cdot|\cdot)_{h}$ is defined as in (ii), defines the \emph{Hilbert norm $h$} on $E \otimes F$.
\end{enumerate}
%% --
...
In each case one has $\norm{f \otimes g} = \norm{f} \norm{g}$ for $f \in E$, $g \in F$.
...
This operator, as well as its continuous extension, will be denoted by $S \otimes T$ and satisfies $\norm{S \otimes T} = \norm{S} \norm{T}$. 
...
Proposition~\ref{prop:a1-1.9}\,(ii) one easily shows that $D_{0} \coloneqq D(A) \cap D(B)$ is a core for its generator $C$ and $Cf = Af + Bf$ for all $f \in D_{0}$.
%% --
%\newpage
\section*{Notes}
...
\textcite{goldstein:1985a} and \textcite{pazy:1983}, to the survey article by \textcite{kreinkhazan:1985}, to the bibliography by 
