...
In Section~\ref{sec:a4-2}, we consider the long-term behavior of the solutions of Eq.~\myeqpageref{eq:c4-1} for different classes of forcing terms $ F $.
%% --
%% --
...
\item 
$ \norm{T(t)f} \to 0 $ for every $ f \in \C^{n} $\,,

\item 
$ \norm{T(t)} \leq M \eu^{-\omega t} $ for some $M \geq 1$ and  $\omega > 0 $\,.
\end{enumerate}
%% --
...
\item 
$\omega(f) \coloneqq \inf\{\omega \colon \norm{T(t)f} \leq M\eu^{\omega t} \text{ for some } M \text{ and every } t \geq 0\}$ is called the \emph{exponential growth bound} of\/ $T(\cdot)f$,
...
\item 
\emph{uniformly stable} if\/ $\norm{T(t)f} \to 0$ as $t \to \infty$ for every $f \in E$,
\index{stability!uniformly stable}
\item 
\emph{stable} if\/ $\norm{T(t)f} \to 0$ as $t \to \infty$ for every $f \in D(A)$.
\index{stability!stable}
\end{enumerate}
\end{definition}
%% --
...
\]
%% --
It is easy to see that $\norm{T(t)}=1$ and that $\norm{T(t)f}\to 0$ for every $f \in c_{0}$.
...
\]
%
defines a probability density with $\int_{\R^n} k_t(x,y) \dy\, = 1$. Hence, $\norm{T(t)} \le 1$; in fact, $\norm{T(t)} = 1$ since $k_t$ also forms an approximate identity. Let $f \in C_0(\R^{n})$ and $\epsilon > 0$. Then there exists a compactly supported $g \in C_0(\R^{n})$ such that $\norm{f-g} \le \epsilon$. Therefore,
%% --
\[
\norm{T(t)f} \le  \norm{T(t)}\norm{f-g} + \norm{T(t)g} \le \epsilon + (4\pi t)^{-n/2} \int_{\R^{n}} |g(y)|  \dy\, ,
\]
%% --
which implies  $\norm{T(t)f} \to 0$ as $t\to\infty$ for all $f \in C_0(\R^{n})$; see also Example~\ref{ex:b3-1.7}.
%%--
...
%% --
\[
0 \leq \omega_{0}(A) = \inf\{\omega\colon\norm{T(t)} \leq M\eu^{\omega t} \text{ for all } t \geq 0\}
\]
%% --
does not exclude the possibility that the semigroup is exponentially stable.

To see this, consider $E \coloneqq C_0(\R_{+}) \cap $ $L^{1}(\R_{+},\eu^{x}\dx)$. 
Then, as shown in Example~\ref{ex:a3-1.3} 
the translation semigroup satisfies $\norm{T(t)} = 1$, and hence $\omega_{0}(A) = 0$. 
...
From the Eq.~\myeqpageref{eq:a1-3.2}, it follows that
%% --
\[
...
%% --
it follows that 
$\norm{T(t)f} \leq M\eu^{\lambda t}$ for every $f \in D(A)$ and some constant $M$ depending on $f$. This yields $\omega_{1}(A) \le -1 < 0 = \omega_{0}(A)$. 
...
establishing that, for certain semigroups beyond the class of eventually norm-continuous semigroups (see Theorem~\ref{thm:a3-6.6}), the growth bounds $\omega_{0}(A)$ and/or $\omega_{1}(A)$ coincide with the spectral bound $s(A) = \sup\{\Re(\lambda)\colon\lambda \in \sigma(A)\}$. For results of this type, see, for example, Part-II, Chapter~\ref{chap:b4},  Part-III, Chapter~\ref{chap:c4},  and Part-IV, Chapter~\ref{chap:d4}.
...
In the following theorem, we show that the bound of the half-plane for which the Laplace transform of\/ $T(\cdot)f$ ($f \in E$) exists absolutely, and the bound of the half-plane for which the Laplace transform of\/ $T(\cdot)Af$ ($f \in D(A)$) exists strongly, coincide with the growth bound $\omega(f) = \inf\{\omega\colon\norm{T(t)f} \leq M\eu^{\omega t} \text{ for all } t \geq 0\}$.
...
\omega(f) = \limsup_{t \to \infty} \frac{1}{t}\log\norm{T(t)f}, 
\end{equation}
%% --
and
%% --
\begin{enumerate}[(i)]
\item 
$\,\omega(f) = \inf\{\Re(\lambda)\colon\int_{0}^{\infty} \norm{\eu^{-\lambda t} T(t)f}  \dt \text{ exists}\}.$
\end{enumerate}
%% --
...
%
\[
	b \coloneqq \inf \left\{\Re(\lambda)\colon\int_{0}^{\infty} \norm{^{-\lambda t} T(t)f}  \dt \text{ exists} \right\}.
\]
%
...
For $c < 0$, it follows from Eq.~\myeqpageref{eq:c4-1} that $\int_{0}^{\infty} T(s)Af  \ds$ exists. 
...
\end{align*}
%% --
From $\norm*{\int_{0}^{t} \eu^{-r s} T(s)Af  \ds} \leq M$ for some $M$ and every $t \geq 0$ we conclude that $\norm{T(t)f} \leq \tilde{M}\eu^{rt}$ for all $t \geq 0$ and some constant $\tilde{M}$.
Hence, $\omega(f) \leq r$ for every $c < r < 0$, \ie $\omega(f) \leq c$.

If\/ $c \geq 0$ and $w > c$, then $\norm*{\int_{0}^{t} \eu^{-w s} T(s)Af \ds} \leq M$ for all $t \geq 0$. 
By 
%% --
\begin{align*}
...
\end{align*}
%% --
we obtain $\norm{T(t)f-f} \leq M\eu^{w t} + M(\eu^{w t} - 1)
\le 2M\eu^{wt}$. 
...
& \phantom{xxxxxxxx} \text{ $\norm{T(t)f} \leq M\eu^{\omega t}$  for every $t \geq 0$} \} \\
&={}
\limsup_{t \to \infty} \frac{1}{t} \log\norm{T(t)R(\lambda,A)} \quad (\lambda \in \rho(A)). 
\end{split}
\end{equation}
%% --
...
= \inf\{\Re(\lambda) &\colon {\int_{0}^{\infty} \norm{\eu^{-\lambda t} T(t)f}  \dt} \text{ exists for every } f \in D(A)\}.  
\end{split}
\end{equation}
\end{theorem}
%--
...
%% --
\item
In Equations \eqref{eq:a4-1.4} and \eqref{eq:a4-1.5}, the term \enquote{$\Re(\lambda)$} may be replaced by \enquote{$\lambda \in \R$} (use Eq.~\myeqpageref{eq:c4-1}).
\end{enumerate}
...
The equality 
 \[ 
 \omega_1(A) = \inf \left\{ \Re(\lambda)\colon\int_0^\infty \norm{\eu^{-\lambda t} T(t) f }  \dt \text{ exists for all } f \in D(A) \right\} 
 \] 
...
The inequality
\[
    \omega_{0}(A) \geq \inf \left\{ \Re(\lambda)\colon\int_0^\infty \norm{\eu^{-\lambda t} T(t) f }  \dt 
...
\begin{equation}\label{eq:a4-1.6}
   \omega_{0}(A) = \inf \left\{ \Re  (\lambda)\colon\int_0^\infty \norm{\eu^{-\lambda t} T(t) f }  \dt \text{ exists for every } f \in E \right\}.
\end{equation}
%--
%
...
As in Example~\ref{ex:a3-1.4}, consider the semigroup $ (T(t))_{t \geq 0} $ on the Hilbert space $ E = \{(x^1, x^2, ...) \colon x^n \in \C^n\colon\sum_{j=1}^{\infty} \norm{x^j}^2 < \infty\} $, defined by 
\[{
...
\]
This semigroup satisfies $ \norm{T(t)} = \eu^{t} $ for all $t\ge 0$. Hence, $ \omega_{0}(A) = 1 $, while the generator $ A = (2\pi i n + A_n)_{n \in \N} $ has spectral bound $ s(A) = 0 $. 
...
%% --  
\begin{align*}
	\norm{\exp(t A_n) \cdot e_n}^2 &= \\
	&= \frac{1}{n} \norm*{ (1 + t + \dots + \frac{t^{n-1}}{(n-1)!}, 1 + t + \dots + \frac{t^{n-2}}{(n-2)!}, \dots, 1+t, 1) }^2 \\
	&=
...
%--
\begin{align*}
	\norm{T(t)x_q}^2 & = \sum_{n=1}^{\infty} n^2 q^{2n} \norm{ \exp(t A) e_n }^2
...
\begin{split}
%% --
\omega_{0}(A) = \inf \{ w\colon\norm{R(\lambda, A)^n} \leq &M ( \Re(\lambda) - w)^{-n} \text{ for some } M \text{ and}\\
...
\begin{equation}\label{eq:a4-1.10}
 \omega_{0}(A) = \inf \{ w\colon\norm{ R(\lambda, A) } \leq M_w \quad \text{for } \Re(\lambda) > w \}.
\end{equation}
\end{remark}
%% --
Unfortunately, the identity Eq.~\myeqpageref{eq:a4-1.10} does not hold on arbitrary Banach spaces. 
However, as we will see in Part-III, Section~\ref{sec:c4-1},  the identity 
%--
\begin{equation}\label{eq:a4-1.11}
 s(A)=\omega_1(A) = \inf \{ w\colon\norm{ R(\lambda, A) } \leq M_w \quad \text{for } \Re(\lambda) > w \}
\end{equation}
%% --
holds for every positive semigroup on a Banach lattice. 
Consequently, for positive semigroups with $s(A) = \omega_1(A) < \omega_{0}(A)$ (see Example~\ref{ex:a4-1.2}\,(ii)), the identity Eq.~\myeqpageref{eq:a4-1.10} is not applicable. 
...
%% --
\[
    \norm{ R(\lambda, A) } \leq C | \lambda |^{n-2}
\]
%% --
...
%% -- 
\begin{proof} 
The hypothesis $\norm{ R(\lambda, A) } \leq C | \lambda |^{n-2}$ is invariant under rescaling. 
That is, the resolvent $R(\lambda, -b+A)$ of the generator $-b+A$ of the rescaled semigroup $\eu^{-bt} T(t)$ satisfies
$\norm{ R(\lambda, -b+A) } \leq \tilde{C} | \lambda |^{n-2}$ for every $\lambda \in \C$ with $\Re(\lambda) > q-b$ and $| \Im(\lambda) | > a+2b$, for a suitable constant $\tilde{C}$. 
...
$ \frac {1}{2\pi i} \int_{p' - i\infty}^{p' + i\infty} \eu^{\lambda t} \, \lambda^{-k} \, d\lambda = 0 $ for $ k \geq 1, p' < 0 $ and $ t > 0$, it follows that
 ...
 If\/ $q < p'$, then by Cauchy's Integral Theorem and the growth bound $\norm{ R(\lambda, A) } \leq C | \lambda |^{n-2}$, we can shift the path of integration to 
...
Thus, we estimate  
\[
\norm{ T(t) R(0,A)^n f } \leq c' \eu^{q t} \norm{ f } \int_{-\infty}^{\infty} (q^2 + s^2)^{-1}\ds = M \eu^{q t} \norm{ f }.
\]
%% --
Equivalently,
%% --
\[
\norm{ T(t) f } \leq M \eu^{q t} \norm{ A^n f } \text{for $f \in D(A^n)$.}
\]
\end{proof}
%% --
...
In this case, one may apply Eq.~\myeqpageref{eq:a4-1.7} to obtain the stronger statement Eq.~\myeqpageref{eq:a4-1.8}.
...
$ \lim_{t \to \infty} \norm{ T(t) } = 0 $.
\item  
$ \norm{ T(t') } < 1 $ for some $ t' > 0 $.
\end{enumerate}
\end{proposition}
%% --
\begin{proof}
The implications $(a)  \Rightarrow  (b)  \Rightarrow  (c) $ are immediate; To prove $(c)  \Rightarrow  (a) $, note that $\omega_{0}(A) = \lim_{t \to \infty} \frac{1}{t} \log \norm{ T(t) }$ (see Eq.~\myeqpageref{eq:a1-1.1}). Assume $\norm{T(t')} <1$ for some $t'>0$. For $t=nt'+s$ with $s\in [0,t']$, we have $\norm{T(t)}\le \norm{T(t')}^n\norm{T(s)}$, so that
\[
\displaystyle
\frac{\log \norm{T(t)}}{t} \leq \frac{n\log \norm{T(t')}}{nt'+s} + \frac{\log \norm{T(s)}}{nt' + s}.
\]
Since $\norm{T(s)}$ is bounded on compact intervals, the second term tends to zero as $n\to\infty$. The first term tends to $\frac{\log \norm{T(t')}}{t'} < 0$. Thus, $\omega_0(A) < 0$, which proves  $(c)  \Rightarrow  (a) $.
\end{proof}
%% --
...
\item
For some (equivalently, every) $ p \geq 1 $ the integral $ \int_{0}^{\infty} \norm{ T(t) f }^p\dt $ exists for every $ f \in E $.
\end{enumerate}
\end{theorem}
%% --
...
\end{theorem}
%% --
\begin{proof} (i) If\/ $ (T(t))_{t \geq 0} $ is stable, then $ \norm{T(t) f} \leq M_f $ for every $ f \in D(A) $. Therefore, $ s(A) \leq \omega_1(A) \leq 0 $. 
...
Since $\norm{T(t)} \leq M$, it follows that   $T(t)f \to 0$ for every $f \in E$.
\end{proof}
...
guarantees the uniqueness of the solution to Eq.~\myeqpageref{eq:a4-2.1}. 
...
$\int_{0}^{\infty} \norm{F(s) - g}  \ds < \infty$, then every generalized solution $u(\cdot)$ of
...
Then for any $r>0$,
\begin{align*}
\norm*{\int_{0}^{t} T(t-s)G(s)  \ds} & 
{\leq \norm*{\int_{0}^{r} T(t-r+r-s)G(s)  \ds} + \norm*{\int_{r}^{t} T(t-s)G(s)  \ds}}\\
& 
{\leq \norm*{T(t-r)\int_{0}^{r} T(r-s)G(s)  \ds} + M \int_{r}^{\infty} \norm{G(s)}  \ds\,},
\end{align*}
where $\norm{T(t)}\le M$ for all $t\ge 0$. Since the semigroup is uniformly stable, we obtain
$T(t-r)\int_{0}^{r} T(r-s)G(s)  \ds \to 0$ as $t \to \infty$ for every $r \geq 0$.
Therefore, $\norm{\int_{0}^{t} T(t-s)G(s)  \ds} \leq \epsilon$ for all sufficiently large $t$.
\end{proof}
...
\begin{proof}
(a) $\Rightarrow$ (b): 
Let $f \coloneqq u(0)$ be the initial value for which Eq.~\myeqpageref{eq:a4-2.1} admits a $p$-periodic mild solution. 
...
Applying Eq.~\myeqpageref{eq:a4-1.14}, we obtain $\Image{A} = \{f\colon\int_{0}^{\infty} f(x) \,\dx \text{ exists}\}$ is dense in $C_{0}(\R_{+})$. 
...
The equivalence of the statements Eq.~\myeqpageref{eq:a4-1.12}, Eq.~\myeqpageref{eq:a4-1.13} and \emph{$\omega_{0}(A) < 0$} were observed by many authors, see for example, \textcite[p.178]{balakrishnan:1976}, 
