...
\end{equation*}
%% –
By a solution of \eqref{eq:a2-acp} for the initial value $f \in D(A)$ we understand a continuously differentiable function $u \colon \left[0,\infty\right) \to E$ satisfying $u(0) = f$ and $u(t) \in D(A)$ for all $t \geq 0$ such that $u'(t) = Au(t)$ for $t \geq 0$.
...
Assume that (a) holds, \ie for every $f \in D(A)$ there exists a unique solution $u(\cdot,f) \in C^{1}(\left[0,\infty\right),E)$ of \label{eq:a2.acp}.
...
Thus $u$ is a solution of \eqref{eq:a2-acp}.
...
\end{proof}
%% --
If \eqref{eq:a2-acp} has a unique solution for every initial value in $D(A)$, then $A$ is the generator of a strongly continuous semigroup only if some additional assumptions on the solutions (continuous dependence from the initial value) or on $A$ $(\rho(A) \neq \emptyset)$ are made.
%% –
...
So by Theorem~\ref{prop:a1-1.9}\,(ii) $ D(A) $ is a core of\/ $ B $.
...
Hence by 
\textcite[10.2.1]{hillephillips:1957} $\sup_{t \in J}\norm{T(t)} < \infty$ for every compact interval $J \subset (0,\infty)$.
...
%% –
\[
    \norm{ {(\lambda - w)^{n}} (\lambda-A)^{-n}} \leq M 
\]
%% –
...
$A$ is densly defined, $(0,\infty) \subset \rho(A)$ and $\norm{\lambda R(\lambda,A)} \leq 1$ for all $\lambda > 0$.
\end{enumerate}
\end{corollary}
%% –
...
(see \textcite{davies:1980}, \textcite{goldstein:1985b} or \textcite{pazy:1983}).
...
\begin{equation}\label{eq:a2-1.6}
    \norm{R(\lambda,A)} \leq M/|\lambda| \quad (\lambda \in S(\alpha_{1} + \pi/2). 
\end{equation}
%% –
\end{theorem}
...
$\norm{R(\mu,A)} \leq N/\Re(\mu)$, $\norm{R(\mu,-A)} \leq N/\Re(\mu)$ for all $\mu \in S(\pi/2)$.
Consequently, $\norm{R(\lambda,A^{2})} \leq N^{2}/r^{2}(\cos\beta)^{2} \leq 1/r^{2}[N/\cos\beta]^{2} = M/|\lambda|$.
\end{proof}
%%
...
According to  
Section~\ref{subsec:a1-3.9} the domain of its generator $A$ contains $D(A_{1}^{2}) \cap \cdots \cap D(A_{n}^{2})$ and, in particular
%%--
\[
...
$\alpha_{1}$ (\cf \textcite[p.252]{reedsimon:1978}).
...
and
\[
\norm{R(\lambda,A)} \leq M/|\lambda|  \quad \text{for all}  \quad \lambda \in S(\alpha_{1}+\pi/2) \setminus B(r).
\]
%%-
This shows that the condition of the theorem is necessary.
Conversely, assume that the condition holds.
Since $\norm{R(\lambda,A)} \to \infty$ when $\lambda$ approaches $\sigma(A)$ (\cf Lemma ~\ref{lem:a2-1.21}   below), 
it follows that $\lambda \in \rho(A)$ and $\norm{R(\lambda,A)} \leq M/|\lambda|$ if\/ $\Re(\lambda) = 0$ and $|\lambda| > r$ as well.
...
then 
\[
\norm{\xi R(\im\eta,A)} \leq |\xi| \cdot M/|\eta| \leq c \cdot M = 1/2.
\]
Hence $R(\xi+\im\eta,A) = \sum_{n=0}^\infty (-\xi)^{n}R(\im\eta,A)^{n+1}$ exists and 
%% –
\begin{align*}
    \norm{R(\xi+\im\eta,A)} &\leq 
...
\end{align*}
%% –
This together with the assumption implies that there exist $N' \geq 0$ and $\alpha \in \left]0,\pi/2\right]$ such that $\lambda \in \rho(A)$ and $\norm{R(\lambda,A)} \leq N'/|\lambda|$ for all $\lambda \in S(\alpha+\pi/2)$.
\end{proof}
%% --
...
\[
    \norm{R(w+\im\eta,A)} \leq \frac{M}{|\eta|}  \ \text{ for all }\ \eta \in \R.
\]
\end{corollary*}
%% –
...
$\norm{R(\lambda,A-w)} \leq N/\Re(\lambda)$ for some $N > 0$ and all $\lambda \in \C$ 
satisfying $\Re(\lambda) > 0$.
Consequently, for every $\alpha \in (0,\pi/2)$, 
\[
\norm{R(\lambda,A-w)} \leq (|\lambda|/\Re(\lambda))N/|\lambda| \leq N(\cos\alpha)^{-1}/|\lambda| 
...
Consequently, there exists $M \geq 0$ such that $\norm{R(\mu,\pm A - w)} \leq M/\Re(\mu)$ whenever $\Re(\mu) > 0$.
...
%%
\[
|\lambda| \cdot \norm{R(\lambda,A^2)} \leq |\lambda| \cdot M^2/(\cos\theta)^2r^2 \leq (|\lambda|/r^2)\cdot M^2/(\cos\beta)^2.
\]
%%
Thus $|\lambda| \cdot \norm{R(\lambda,A^2)}$ is uniformly bounded for $\lambda \in S(\alpha+\pi/2) \setminus B(r_0)$.
\end{proof}
\index{semigroup!holomorphic semigroup|)}
%%
%% –
...
$\norm{R(\lambda,\pm A - w)} \leq M/\Re(\lambda)$ for some $M \geq 0$, $w \geq 0$, then $A^2$ generates a holomorphic semigroup of angle $\pi/2$\,.
\end{remark*}
%% –
...
Then the function $t \mapsto T(t)f$ is right sided differentiable at $t_0$ if and only if\/ $T(t_0)f \in D(A)$; and in that case it is differentiable at every $s > t_0$ and the derivative is $AT(s)f$ (this follows from Theorem~\ref{prop:a1-1.6}\,(ii)).
...
and
\[
    \norm{R(\lambda,A)} \leq M\cdot|\Im(\lambda)| \text{ for all } \lambda \in \Sigma \text{ satisfying } \Re(\lambda) \leq \omega_{0}(A).
\]
\end{theorem}
%% –
...
and
\[
    \norm{R(\lambda,A)} \leq M\cdot \lvert \Im(\lambda) \rvert \text{ for all } \lambda \in \Sigma \text{ satisfying } \Re(\lambda) \leq \omega_{0}(A).
\]
\end{theorem}
%% –

For the proofs of these two theorems we refer to \textcite[Chapter 3, Theorem 4.7 and 4.8]{pazy:1983}.
...
If\/ $\lim_{t \to t'} \norm{T(t) - T(t')} = 0$, then it follows from the semigroup property, that the function $t \mapsto T(t)$ is norm continuous on the whole half line $(t',\infty)$.
...
%%-
\[
\norm{R(a+ \iu r,A)^n}^{1/n} < \epsilon \quad  \text{ for all } r \in \R \text{ satisfying $|r| \geq r_{0}$.}
\]
%%-
...
There exists $M \geq 1$ such that $\norm{T(t)} \leq M\eu^{wt}$ for all $t \geq 0$.
Let 
%% --
\[
...
%
\[
	N\cdot \frac{(t')^n}{n!} < \frac{1}{3}\,\epsilon^{n+1}.
\]
%
Choose $T \geq t'$ such that $\frac{1}{n!}\int_{T}^{\infty} t^n\eu^{-at}\norm{T(t)} \dt < \epsilon^{n+1}/3$.
...
%% --
\[
\left\lVert \frac{1}{n!}\int_{t'}^T t^{n}\eu^{-\iu rt}\eu^{-at}T(t) \dt \right\lVert  < \epsilon^{n+1}/3
\]
%% --
 whenever $|r| \geq r_{0}$.
...
\begin{align*}
    \norm{R(a + \iu r,A)^{n+1}} & = \frac{1}{n!}\cdot 
    	\norm*{\int_0^\infty \eu^{-(a + \iu r)t}t^nT(t) \dt } \\
    & \leq \frac{1}{n!}\cdot\int_{0}^{t'} \eu^{-at}t^n\norm{T(t)} \dt \\
    & + \frac{1}{n!}\cdot \norm*{\int_{t'}^T t^n\eu^{-\iu rt}\eu^{-at}T(t) \dt } \\
    & + \frac{1}{n!}\cdot\int_T^\infty \eu^{-at}t^n\norm{T(t)} \dt \\
    & \leq \frac{1}{n!}\cdot(t')^{n}\int_{0}^{t'} \eu^{-at}M\eu^{wt} \dt + \frac{2}{3}\cdot\epsilon^{n+1} \\
    & \leq N\cdot \frac{(t')^n}{n!} + \frac{2}{3}\cdot\epsilon^{n+1} \\
    & \leq \epsilon^{n+1}.
\end{align*}
\end{proof}
%% --
...
Consider the nilpotent translation semigroup $(T(t))_{t\geq 0}$ on $F \coloneqq L^{1}([0,1])$ (see Section~\ref{subsec:a1-2.6}).
Let $E = F \widetilde{\otimes}_{\pi} F = L^{1}([0,1] \times [0,1])$ and $S(t) = T(t) \otimes \Id $ $(t \geq 0)$.
Then $(S(t))_{t\geq 0}$ is a strongly continuous semigroup (see Section~\ref{subsec:a1-3.8}), 
with $B$ as its generator.
Clearly $(S(t))_{t\geq 0}$ is a nilpotent semigroup (so it is eventually compact), but $R(\lambda,B) = R(\lambda,A) \otimes \Id $ is not compact.
...
%%--
\[
    \left|\eu^{tm(x)} - \eu^{t'm(x)}\right| \leq \eu^{t\Re(m(x))} + \eu^{t'\Re(m(x))} \leq 2\eu^{(t'+1)b} < \epsilon
\]
%%--
whenever $|t-t'| \leq 1$.
...
\[
[\text{ess}]\text{-}\esssup\left\{\left|\eu^{tm(x)} - \eu^{t'm(x)}\right| \colon x \in X, \Re(m(x)) > b\right\} < \epsilon
\] 
whenever $|t-t'| < \delta$.
Together with the inequality above, we obtain that 
%%
\[
\norm{T(t) - T(t')} = [\text{ess}]\text{-}\sup\left\{\left|\eu^{tm(x)} - \eu^{t'm(x)}\right| \colon x \in X\right\} < \epsilon
\]
%% 
whenever $|t-t'| < \delta$.
...
if and only if there exists $c > 0$ such that 
$[\text{ess}]\text{-}\Image{m} \subset 
\left\{\lambda \in \C \colon \eu^{t\Re(\lambda)}|\Im(\lambda)| \leq c\right\}$ 
if and only if there exists $c > 0$ such that 
$\left\{\lambda \in \C \colon |\Im(\lambda)| > c \cdot \eu^{-t\Re(\lambda)}\right\} \subset \rho(A)$.
\end{proof}
...
Conversely, if\/ $S(\theta + \pi/2) \subset~\rho(A)$, then one verifies directly that 
%%--
\[
...
If\/ $m(n) = -n +\iu \cdot n^2$, then the semigroup is differentiable but not holomorphic.
\end{enumerate}
\end{example}	
...
$\norm{R(\lambda,A)} \leq (2\norm{B})^{-1}$ for $\Re(\lambda) \geq \lambda_{0}$.
...
Now if\/ $A$ generates a holomorphic semigroup, there exists $M \geq 0$ such that $\norm{R(\lambda_{0}+\im\eta,A)} \leq M/|\eta|$ for all $\eta \in \R$.
Consequently, 
%%--
\[
\norm{R(\lambda_{0}+\im\eta,A+B)} \leq \norm{(\Id-BR(\lambda_{0} + \iu \eta,A))^{-1}} \cdot 2M/|\eta| \leq 2M/|\eta|
\]
%%--
for all $\eta \in \R$\,.
...
$\lim_{h \to 0} \norm{(T(t+s-h) - T(t-s))BS(s)} = 0$.
It follows from the dominated convergence theorem that
%%--
\begin{equation}\label{eq:a2-1.13}
    \lim_{h \to 0} \int_{0}^{t} \norm{(T(t+s-h) - T(t-s))BS(s)}\, \ds = 0.
\end{equation}
%%--
...
\begin{align*}
	...
	\leq \norm{T(t+h) - T(t)} &+ \int_{t}^{t+h} \norm{T(t+h-s)BS(s)} \ds    \\ 
	&+ \int_{0}^{t} \norm{(T(t+h-s) - T(t-s))BS(s)}\, \ds \to 0 
\end{align*}
%% --
when  $h \to 0$. 	
\end{proof}
...
Let $\lambda > s(A)$ be so large such that $\norm{SR(\lambda,A)} < 1$.
Then 
%%
\[
...
\end{equation}
%% -- 
Moreover, it follows from Eq.~\myeqpageref{eq:a2-2.1} and Eq.~\myeqpageref{eq:a2-2.2} that
%% --
...
Of course, the most important case in this section is when $p$ is the norm function $N$ given by $N(f) = \norm{f}$ $(f \in E)$.
...
Then a bounded operator $T$ is $N^{+}$-contractive if and only if\/ $T$ is positive and $\norm{T} \leq 1$.
\end{remark*}
...
So we denote by $N \colon E \to \R$ the function given by $N(f) = \norm{f}$ throughout.
Then Eq.~\myeqpageref{eq:a2-2.5}   can be written in the form

\begin{equation}\label{eq:a2-2.6}
\dN(f) = \{\phi \in E' \colon \norm{\phi}\leq 1, \langle f,\phi \rangle = \norm{f}\}.
\end{equation}
...
Let $E = C[0,1]$, $f \in E$.
Then there exists $x \in [0,1]$ such that $|f(x)| = \norm{f}_{\infty}$.
...
$\langle g,\phi_{f} \rangle = 1/\norm{f}\,(g|f)$.
%%--
\item \label{ex:a2-2.2.3}
$A - \norm{A}\Id$ is strictly dissipative for every bounded operator $A$.
\end{enumerate}
\end{examples}
%% –
...
Then $\pm\langle g,\phi_{t} \rangle \leq p(\pm g) \leq c\norm{g}$ for all $g \in E$, $t > 0$.
...
%% –
\begin{align*}
D^{+}_{g} p(f) &= \lim_{t \downarrow 0} \frac{1}{t} (p(f+tg) - p(f)) \quad \text{and} \\
...
Then 
\begin{align*}
	\langle Af,\phi \rangle &= \lim_{t\to 0} \frac{1}{t} (\langle T(t)f,\phi \rangle - \langle f,\phi \rangle) \\
	&= \lim_{t\to 0} \frac{1}{t} (\langle T(t)f,\phi \rangle - p(f)) \\
...
Hence, by Eq.~\myeqpageref{eq:a2-2.8},  $p(\lambda R(\lambda,A)g) \leq p(g)$ for all $g \in E$, $\lambda > \omega_{0}(A)$.
Hence $\lambda R(\lambda,A)$ is p-contractive for $\lambda > \omega_{0}(A)$.
This implies that $T(t)$ is p-contractive by the formula Eq.~\myeqpageref{eq:a2-1.3}, 
%% –
\[
T(t) = \lim_{t\to 0} \left( \frac{n}{t} R\left( \frac{n}{t},A \right) \right)^{n} \text{ (strongly) for } t \geq 0.
\]
%% –
\end{proof}
%% --
...
&\leq \frac{1}{t}(2tc\norm{g - Af} + t^2c\norm{Ag}) \quad \text{(by (2.3))} \\
&= 2c\norm{g - Af} + tc\norm{Ag}.
\end{align*}

Letting $t \to 0$ we obtain $\langle Af,\phi \rangle \leq 2c\norm{g - Af}$ for all $g \in D(A)$.
Since $D(A)$ is dense in $E$, this implies that $\langle Af,\phi \rangle \leq 0$.
\end{proof}
...
%$
\[
p(f) \coloneqq \text{dist}(-f,E_{+}) = \inf\{\norm{f + g} \colon g \in E_{+}\}
\]
%$ 
...
\lambda\norm{f}_{p} \leq \norm{(\lambda - A)f}_{p} \text{ for all } f \in D(A), \lambda > 0.
\end{equation}
...
%%
\[
\lambda\norm{f}_{p} = \lambda p(f) + \lambda p(-f) \leq p((\lambda - A)f) + p(-(\lambda-A)f) = \norm{(\lambda-A)f}_{p}.
%%
\]
Thus Eq.~\myeqpageref{eq:a2-2.14}   
is proved.
Now suppose that $p$ is strict.
Then $\norm{\cdot}_{p}$ is equivalent to the given norm.
...
It follows from Eq.~\myeqpageref{eq:a2-2.14}  
...
%%
\begin{proof}
Since $p$ is a strict half-norm, we can assume that $\norm{f} = \norm{f}_{p}$ for all $f \in E$.
...
that $\mu \in \rho(A)$ and $\norm{\mu R(\mu,A)} \leq 1$ whenever $\mu > 0$ such that $\Image{\mu-A} = E$.
So by hypothesis $\lambda \in \rho(A)$ and 
$\text{dist}(\lambda,\sigma(A)) \geq \norm{R(\lambda,A)}^{-1} \geq \lambda$.
...
Of course, the norm function $N$ given by $N(f) = \norm{f}$ is a strict half-norm.
...
A linear operator $A$ is simply called \emph{dissipative} if it is N-dissipative where $N(f) = \norm{f}$ $(f \in E)$.
...
Suppose that\, $s = \lim\sup_{t\to 0} \norm{T(t) - \Id}$\, is finite.
Then $\TT$ is uniformly continuous, if\/ $\lim_{t\to 0} \norm{(T(t) - \Id)^2} = 0$.
\end{lemma}
%% –
\begin{proof}
...
\]
%% -- 
Hence $2s \leq \lim \sup_{t \downarrow 0} \norm{T(2t) - \Id}$.
Obviously, $\lim \sup_{t \downarrow 0} \norm{T(2t) - \Id} = s$ and so, $2s \leq s$.
Consequently, $s = 0$.
\end{proof}
%% --
...
$\lim_{t \to 0} \norm{(T(t) - \Id)^2} = 0$ by the weaker condition 
%%--
\[
\lim \sup_{t \to 0} r(T(t) - \Id) < 1\,, \]
%%--see \textcite[Lemma 2]{lotz:1985}, where $r$ denotes the spectral radius.
%%--
see \textcite[Lemma 2]{lotz:1985}, where $r$ denotes the spectral radius.
%%--
...
Let $f \colon \R \to \R$ be non-continuous with $f(s+t) = f(s) + f(t)$ for all $s, t \in \R$ (see \textcite{hamel:1905}).
...
Then $\lim \norm{(T(t_n) - \Id)'x'} = 0$ if and only if\/ $\lim \langle x_n, (T(t_n) - \Id)'x' \rangle = 0$ for every bounded sequence $(x_n)$ in $E$.
...
Then $\lim \norm{V_{n}x} = 0$ and therefore $\lim \langle x, V_{n}'x_{n}' \rangle = 0$ for every $x \in E$.
...
Hence by the uniform boundedness principle, $\lim \sup_{t \to 0} \norm{(T(t) - \Id)}$ is finite.
By Lemma~\ref{lem:a2-3.1}   it suffices to show that $\lim_{t \to 0} \norm{(T(t) - \Id)^2} = 0$.
...
%%
\[
\norm*{(T(t_{n}) - \Id)^2 } - \frac{1}{n} \leq \langle (T(t_{n}) - \Id)x_{n}, (T(t_{n}) - \Id)'x_{n}' \rangle\,.
\]
%%
...
Since $E$ has the Dunford-Pettis property, $\lim \norm{(T(t_{n}) - \Id)^2} = 0$.
Consequently, $\lim_{t \to 0} \norm{(T(t) - \Id)^2} = 0$.
\end{proof}
...
Differentiable semigroups are treated in detail in the book of \textcite{pazy:1983} who discovered Theorem~\ref{thm:a2-1.17}   and~\ref{thm:a2-1.18}. 
...
A complete information on the general theory can be obtained by consulting the books of \textcite{davies:1980}, \textcite{goldstein:1985a} and \textcite{pazy:1983}.
...
Related previous results were obtained by \textcite{calvert:1971a}, \textcite{hasegawa:1966}, \textcite{sato:1968}, \textcite{benilanpicard:1979} and \textcite{picard:1972}, where the two last consider non-linear semigroups.
