...
\begin{proof}
	...
	\end{equation}
	%% --
	This and Eq.~\myeqpageref{eq:b3-1.2} implies that $\lambda_{0} - r(R(\lambda_{0},A))^{-1}$ is a real spectral value which is greater than $s(A) - \epsilon$.
	...
	\end{equation}
	%% --
	It follows that $\norm{R(\lambda_{0},A)^{n}} \geq (2\lambda_{0})^{-n}$ and therefore
	%% --
	\begin{equation}\label{eq:b3-1.6}
		r(R(\lambda_{0},A)) = \lim_{n \to \infty}\norm{R(\lambda_{0},A)^{n}}^{1/n} \geq (2\lambda_{0})^{-1} > 0.
	\end{equation}
	%% --
...
\begin{proof}
	...
	Note that for $g \geq 0$ we have $f \coloneqq R(\lambda_0,A)g \geq 0$ hence 
	%
\[
...
Moreover, by Example~\ref{ex:a3-3.6}, we have 
%
\[
...
\begin{proof}
	...
	\]
	%% --
	It follows that $\norm{R(\lambda,T)} \leq \|R(|\lambda|,T)\|$ and therefore
	%% --
	\begin{equation}\label{eq:b3-1.15}
		\lim_{\lambda \downarrow r}\norm{R(\lambda,T)} = \infty.
	\end{equation}
	%% --
	By the uniform boundedness principle there exist a sequence $(\lambda_{n})$, $\lambda_{n} \downarrow r$ and a positive $\psi \in M(K)$ such that $\norm{R(\lambda_{n},T)'\psi} \to \infty$.
	Defining 
    \[
    \psi_{n} \coloneqq \norm{R(\lambda_{n},T)'\psi}^{-1}R(\lambda_{n},T)'\psi
    \] we have
%% --
	\begin{equation}\label{eq:b3-1.16}
		\begin{split}
		(r - T')\psi_{n} & = \norm{R(\lambda_{n},T)'\psi}^{-1}\cdot((r-\lambda_{n}) + (\lambda_{n} - T'))R(\lambda_{n},T)'\psi\\ 
		& = (r-\lambda_{n})\psi_{n} + \norm{R(\lambda_{n},T)'\psi}^{-1}\psi \to 0.
		\end{split}
	\end{equation}
	%% --
	Since $(r - T')$ is $\sigma(M(K),C(K))$-continuous, equation Eq.~\myeqpageref{eq:b3-1.16} implies that every $\sigma(M(K),C(K))$ cluster point of\/ $(\psi_{n})$ is a positive eigenvector, provided that it is non-zero.
	Because $K$ is compact we have 
%
\[
	\{\phi \in M(K) \colon \phi \geq 0, \norm{\phi} = 1\} 
...
From the explicit representation of\/ $T(t)$,
	...
	\end{equation}
	%% --
	it follows that $\lim_{t \to \infty}T(t)f = 0$ for every $f \in C_{0}(\R^{n})$. Note that $\norm{T(t)} = 1$ for all $t \geq 0$ and, for $f$ having compact support,      
    $\norm{T(t)f} \leq (4\pi t)^{-n/2}\int_{\R^{n}} |f(y)| \dy \to 0$. 
...
\begin{lemma}\label{lem:b3-2.1}
	...
	Then we have $T \geq 0$ if and only if\/ $\norm{T} \leq 1$.
\end{lemma}
%% --
\begin{proof}
	...
	|Tf| \leq T|f| \leq T(\norm{f}\cdot \1_{K}) = \norm{f}\cdot T(\1_{K}), \quad f \in C(K)\,,
	\end{equation}
	%% --
	hence $\norm{T} = \norm{T\1_{K}}$ whenever $T$ is positive.
	This shows that $T \geq 0$ implies $\norm{T} \leq 1$ whenever $T\1_{K} = \1_{L}$.
	...
	-1 \leq f \leq 1 \text{ if and only if } \norm{f - \im\cdot r\cdot 1} \leq \rho_{r} \coloneqq (1+r^{2})^{1/2} \text{ for every } r \in \R.
	\end{equation}
	%% --
	Now suppose $f \in C(K)$, $0 \leq f \leq 2\cdot \1_{K}$.
	Then we have $-\1_{K} \leq f - \1_{K} \leq \1_{K}$ hence by Eq.~\myeqpageref{eq:b3-2.2} $\norm{f - \1_{K} - \im\cdot r\cdot \1_{K}} \leq \rho_{r}$ for every $r \in \R$.
	From $T\1_{K} = \1_{L}$ and $\norm{T} \leq 1$ it follows that $\norm{Tf - \1_{L} - \im\cdot r\cdot \1_{L}} \leq \rho_{r}$ for every $r \in \R$.
	Using Eq.~\myeqpageref{eq:b3-2.2} once again, we obtain $-\1_{L} \leq Tf - \1_{L} \leq \1_{L}$ or $0 \leq Tf \leq 2\cdot \1_{L}$.
\end{proof}
%% --
...
\begin{equation}\label{eq:b3-2.9}
	...
	Defining $T_{1} \coloneqq M_{\tilde{h}}^{-1}\tilde{R}M_{\tilde{h}}$ we have by Eq.~\myeqpageref{eq:b3-2.9}
	%% --
	...
	\end{equation}
	%% --
	Hence we have $\norm{T_{1}} \leq \norm{\tilde{T}} = 1$ (by Eq.~\myeqpageref{eq:b3-2.11}, Eq.~\myeqpageref{eq:b3-2.9}, Eq.~\myeqpageref{eq:b3-2.1}).
	Then it follows from Lemma~\ref{lem:b3-2.1} that $T_{1}$ is a positive operator.
	Thus Eq.~\myeqpageref{eq:b3-2.11} implies that $0 \leq T_{1} \leq \tilde{T}$ and therefore $\norm{\tilde{T} - T_{1}} = \norm{(\tilde{T} - T_{1})\1} = 0$ (by Eq.~\myeqpageref{eq:b3-2.10}, Eq.~\myeqpageref{eq:b3-2.9}, Eq.~\myeqpageref{eq:b3-2.1}).
\end{proof}
%% --
...
Let $T$ be a positive linear operator on $C_{0}(X)$, let $h \in C_{0}(X)$ and $\lambda \in \C$, $|\lambda| = 1$.
	If\/ $Th = \lambda h$ and $T|h| = |h|$, then we have $Th^{[n]} = \lambda^{n}h^{[n]}$ for every $n \in \Z$ (\cf Eq.~\myeqpageref{eq:b3-2.4}).
...
\begin{proof}
	...
	Defining $\tilde{R} \coloneqq \lambda\tilde{T}$, then $\tilde{T}$, $\tilde{R}$, $\tilde{h}$ satisfy Eq.~\myeqpageref{eq:b3-2.8}, hence we have
	%% --
	...
	\end{equation*}
	%% --
	(see Eq.~\myeqpageref{eq:b3-2.7} and Eq.~\myeqpageref{eq:b3-2.12}), which is precisely $Th^{[n]} = \lambda^{n}h^{[n]}$ for all $n \in \Z$.
...
Suppose $M$ is a closed cyclic subset of\/ $\im \R$, $M = \cup_{\alpha \in S} \im\alpha\Z$ for a suitable $S \subset \R$ (\eg $S = M$).
	...
	The space $E_{1} \coloneqq \{(f_{\alpha})_{\alpha \in S} \colon f_{\alpha} \in C(\Gamma), \sup_\alpha \norm{f_{\alpha}} < \infty\}$ is a Banach space under the norm $\norm{(f_{\alpha})} \coloneqq \sup_\alpha \norm{f_{\alpha}}$.
...
\begin{proof}
	...
	Thus the following relations hold (see Part-I, Example~\ref{eq:a3-3.6}),
	%% --
	...
	Since $R(\lambda,A)$ is positive if\/ $\lambda > 0$ (Corollary~\ref{cor:b3-1.3}), it follows from Eq.~\myeqpageref{eq:b3-2.17} that $Q_{k}$ is a positive bounded operator, hence $I_{k-1} = \{f \in E \colon Q_{k}|f| = 0\}$ is a closed ideal.
...
\begin{proof}
	...
	We have $AQ = QA = s(A)A = 0$ (see Part-I, Example~\ref{eq:a3-3.6}), hence
	%% --
...
\begin{proof}
	...
	This contradicts the assumption $r_{\text{ess}}(T(t)) < r(T(t))$ (see Eq.~\myeqpageref{eq:a3-3.7}).
\end{proof}
%% --
...
%\end{empheq}
	...
	\end{equation}
	%% --
	Then $(T(t))_{t \geq 0}$ is a positive semigroup and $\norm{T(t)} \leq (1 + \norm{k}_{1})$.
...
\index{retarded differential equation}
	...
	\end{equation}
	%% --
	Denoting the exponential function $x \mapsto \eu^{\lambda x}$ by $e_{\lambda}$, we have for real $\lambda$ and $\lambda > \norm{\phi}$
	%% --
	...
	\begin{equation}\label{eq:b3-2.28}
		\norm{(\Id  - 1/\lambda \cdot \phi \otimes e_{\lambda})^{-1}} \leq \lambda/(\lambda - \norm{\phi}) \quad \text{if } \lambda > \norm{\phi}.
	\end{equation}
	%% --
It follows from Eq.~\myeqpageref{eq:b3-2.25} and Eq.~\myeqpageref{eq:b3-2.26} that for $\lambda > \norm{\phi}$, the resolvent $R(\lambda,A_{\phi})$ exists and satisfies $\norm{R(\lambda,A_{\phi})} \leq \lambda/(\lambda-\norm{\phi})\cdot 1/\lambda = 1/(\lambda-\norm{\phi})$.
Then the Hille-Yosida Theorem (Part-I, Theorem~\ref{thm:a2-1.7}) implies that $A_{\phi}$ generates a semigroup $(T(t))$ satisfying $\norm{T(t)} \leq \exp(\norm{\phi}t)$.
...
\begin{enumerate}[\upshape (i), wide, labelindent=.25em]
	...
	$r(R(\lambda,A)) = \lim_{n \to \infty}\norm{R(\lambda,A)^{n}}^{1/n} \geq \epsilon > 0 $.
	...
	It follows from Eq.~\myeqpageref{eq:b3-2.14} and Eq.~\myeqpageref{eq:b3-2.15} that
	%% --
...
\begin{proof}
	...
	\item[(iv)] 
	Supposing $Ah = \im\alpha h$ $(h \neq 0, \alpha \in \R)$ then $A|h| = 0$ by Eq.~\myeqpageref{eq:b3-2.14} and Eq.~\myeqpageref{eq:b3-2.15}.
...
\begin{proof}
	...
	If\/ $X$ is compact, then $\tilde{h}(X)$ is closed, hence $\tilde{h}$ is onto, moreover, $|h| \geq \epsilon$ for some $\epsilon > 0$, thus the definition of\/ $j$ implies that $\norm{j(f)} > \epsilon\norm{f}$ for every $f \in C(\Gamma)$.
\end{proof}
%% --
...
%% --
		...
		\end{equation}
%% --
	This identity and Eq.~\myeqpageref{eq:b3-3.13} implies that $2u/w\cdot\sum_{ij} a_{ij}w_{i}'w_{j}' = 0$.
...
\end{align}
%% --
It is easy to see that $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\norm{T(t)}$, hence in the definition of\/ $\overline{c}(h,\phi)$, both the limit and the infimum exist and coincide with the growth bound (see Part-I, Eq.~\myeqpageref{eq:a1-1.1}).
...
\begin{proof}
	...
	(i) By the definition given in Eq.~\myeqpageref{eq:b3-4.1} there exists a $\tau > 0$ such that $\underline{c}_{t}(h,\phi) \geq \underline{c}(h,\phi) - \epsilon$ for all $t \geq \tau$.
	It follows that
	%% --
	...
	\begin{equation}\label{eq:b3-4.6}
		\begin{aligned}
			\norm{f_{n}} \leq 1 &, \quad {f_{n}|}_{K_{n+1}} = 0\,, \\
	...
	Moreover,
	%% --
	\[\norm{g_{n}} \geq |g_{n}(y_{n})| = \left|\int_{0}^{n+1} \eu^{-\lambda t} h_{t}(y_{n})f_{n}(\phi(t,y_{n})) \dt \right| \geq\]
	%% --
	...
	\begin{equation}\label{eq:b3-4.8}
		\norm{g_{n}} \geq 1/2\epsilon \cdot\left(\eu^{\epsilon n} - \eu^{\epsilon(n-1)})\right) \to \infty \text{ for } n \to \infty.
	\end{equation}
	%% --
	It follows from Eq.~\myeqpageref{eq:b3-4.8} and Eq.~\myeqpageref{eq:b3-4.7} that $ g_n / \norm{g_n}$ is an approximate
	...
	Since $\phi$ is surjective, for every $f \in C(K)$ there is a $x_{f} \in K$ such that $\norm{f} = \norm{f(\phi(\tau,x_{f}))}$ and we obtain for $\lambda \in H_{2\epsilon}$, $\lambda = a + \im\beta$, $\alpha,\beta \in \R$
	%% --
	\begin{equation}\label{eq:b3-4.9}
		\begin{aligned}
			\norm{(\eu^{\lambda\tau} - T(\tau))f} &\geq |h_{\tau}(x_{f})f(\phi(\tau,x_{f})) - \eu^{\lambda\tau}f(x_{f})| \\
			& \geq h_{\tau}(x_{f})\norm{f} - \eu^{a\tau}|f(x_{f})| 
			 \geq \eu^{(a+\epsilon)\tau}\norm{f} - \eu^{a\tau}\norm{f} \\
			&= \eu^{a\tau}(\eu^{\epsilon\tau} - 1)\norm{f}.
		\end{aligned}
	\end{equation}
	%% --
...
\begin{example}\label{ex:b3-4.6}
	...
	\begin{enumerate}[\upshape (a), wide, labelindent=.25em]
	\item
	...
	$\phi_{0}$ and $\tilde{\phi}_{0}$ satisfy condition Theorem~\ref{thm:b3-4.4}\,(iii).
...
\begin{example}\label{ex:b3-4.7}
	\begin{enumerate}[\upshape (i), wide, labelindent=.25em]
		...
		The spectrum of the semigroup corresponding to $\phi$ and $h$ (\cf Eq.~\myeqpageref{eq:b3-4.1}) is given by
		%% --
		\[
...
Eq.~\myeqpageref{eq:b3-4.1} and let $\underline{c}(h,\phi)$, $\overline{c}(h,\phi)$ be defined as in Eq.~\myeqpageref{eq:b3-4.4} on the same page.
...
%% --
\begin{proof}	
	(i) and (ii): By Eq.~\myeqpageref{eq:b3-4.4} we have $\overline{c}_{t}(h,\phi) = 1/t\cdot\log\norm{T(t)}$ hence $\overline{c}(h,\phi) = \omega_{0}(A)$ (\cf Eq.~\myeqpageref{eq:a1-1.1}).  
...
\begin{equation}\label{eq:b3-4.13}
	\lim \sup_{m\to\infty} \norm{(\hat{h}+\im\beta -A')\phi_{nm}} \leq 1/n \text{ for every } n \in \N.
\end{equation}
%% --
Because the orbit is infinite we have
%% --
	\begin{align*}
	\norm{\phi_{nm}} & {= \frac{1}{n}\int_{0}^{n} \left|\eu^{-(\hat{h}+\im\beta )t} h_{t}(\phi(m,x_{0}))\right| \dt} \\
...
\begin{equation}\label{eq:b3-4.14}
	\lim_{m\to\infty} \norm{\phi_{nm}} = 1 \text{ for every } n \in \N.
\end{equation}
%% --
...
\item 
    ...
    Then from Eq.~\myeqpageref{eq:b3-4.15} we can conclude that $\norm{R(\lambda,A)f} \leq M\left(\int_{0}^{r} \left|\eu^{-\lambda t}\right|\dt\right)\norm{f}$ for $\lambda \in U$, $\Re(\lambda) > 0$, therefore $\lambda_{0} = \im\beta \in \rho(A)$.
\end{enumerate}
\end{proof}
%% --
...
Example~\ref{ex:b3-3.4}\,(iii) will be discussed in more detail and more generality in Section~\ref{sec:b4-3}.
