...
If\/ $\langle T(t)f,\mu \rangle \to 0$ for every $\mu \in C(K)'$, then, by the Uniform Boundedness Principle, $\norm{T(t)f} \leq M_f$ for every $f \in D(A)$. 
\index{growth bound!omega1@$\omega_1(A)$}
Using 
%% -- 
\[
...
\]
%% --
Obviously, $T(t)f$ tends pointwise to $0$ as $t \to \infty$ and is dominated by the $\mu$-integrable function $\norm{f}_\infty \cdot 1$. 
...
\item
At first we point out that for $f \in C_{0}(X)$ such that $f$ vanishes on the unit circle $\Gamma$, we have $\lim_{t\to\infty} \norm{T(t)f} = 0$.
...
For positive functions $f_1$, $f_2 \in C_{0}(X)$ we have $\sup(\norm{f_1},\norm{f_2}) = \norm{\sup(f_1,f_2)}$. 
Let $\mu_1$, $\mu_2 \in C_{0}(X)'_+$ and $\epsilon  >  0$. 
Then there are positive elements $f$, $g$ in the unit ball of\/ $C_{0}(X)$ such that 
%
\[
	\langle f,\mu_1 \rangle \geq \norm{\mu_1} - \epsilon 
	\quad \text{and} \quad
	\langle g,\mu_2 \rangle \geq \norm{\mu_2} - \epsilon.
\]
%
For $h \coloneq \sup(f,g)$ we obtain $\norm{h} \leq 1$ and 
%
\[
	 \norm{\mu_1 + \mu_2} \geq \langle h,\mu_1 + \mu_2 \rangle \geq \langle f,\mu_1 \rangle + \langle g,\mu_2 \rangle \geq \norm{\mu_1} + \norm{\mu_2} - 2\epsilon .
\]
%
Hence $\norm{\mu_1 + \mu_2} = \norm{\mu_1} + \norm{\mu_2}$ for $\mu_1$, $\mu_2 \in C_{0}(X)'_+$ (see also Part-III, Chapter~\ref{chap:c1}).

Approximating the integral by Riemann sums one obtains 
%% -- 
\[
\norm*{\int_0^r T(t)^*\mu \dt} = \int_0^r \norm{T(t)^*\mu} \dt \text{ for } \mu \in C_{0}(X)^*_{+} , r  >  0 .
\]
%% -- 
and therefore, for $r \to \infty$, 
\[
\norm{R(0,A^*)\mu} = \norm*{\int_0^{\infty} T(t)^*\mu \dt} 
 = \int_0^{\infty} \norm{T(t)\mu} \dt\  (\mu \in C_{0}(X)_{+}^{*}) .
\]
%% -- 
...
%
\[
	 |\langle f,T(t)'\mu \rangle| \leq \liminf_{n\to\infty}\norm{f} \cdot \norm{T(t)^*\mu_n} 
\]
%
and 
%
\[
	 \norm{T(t)'\mu} \leq \liminf_{n\to\infty}\norm{T(t)^*\mu_n} \quad (t \geq 0) .
\]
%
...
\[
\begin{aligned}
	\int_0^{\infty} \norm{T(t)'\mu} \dt &\leq \int_0^{\infty} \left(\liminf_{n\to\infty} \norm{T(t)'\mu_n}\right) \dt \leq  
	\liminf_{n\to\infty} \int_0^{\infty} \norm{T(t)'\mu_n} \dt = \\ 
	& = \liminf_{n\to\infty} \norm{R(0,A^{*})\mu_n} \leq \norm{R(0,A^{*})}\cdot\liminf_{n\to\infty} \norm{\mu_n} < \infty .
\end{aligned}
\]
(observe that $t \to \norm{T(t)'\mu} = \sup \{\langle T(t)f,\mu \rangle \colon \norm{f} \leq 1\}$ is lower semi-continuous and hence measurable). 
...
\begin{aligned}
	...
	T_n(t) &= \exp(\lambda_n t) \cdot \sum_{j=0}^{k(n)-1} \frac{1}{j!}t^j(A - \lambda_n)^j \circ P_n 
		\quad (t \geq 0) ,\\
	\norm{R_m(t)} & \leq C \cdot \exp((\epsilon +\Re(\lambda_m))t) \text{ for }  t \geq 0, \epsilon >0\\
...
\begin{proof}
	...
	%% --
	\[
	\sigma(T(t_{0})_{\vert \Kern{P}} \subset \{z \in \C \colon |z| < \exp(\Re(\lambda_m) \cdot t_{0})\} .
\]
%
	Hence the type of
...
%% -- 
\[
\norm{T(t)(\Id -P)} \leq \norm{T(t)_{\vert \Kern{P}}}\cdot\norm{\Id -P} \leq \norm{\Id -P}\cdot C_{0} \cdot \exp(\Re(\lambda_m) \cdot t) .
\]
%% --
...
\begin{corollary}\label{cor:b4-2.2}
	...
	\begin{equation}\label{eq:b4-2.3} 
		\left(\left|\int f \diff{\nu}\right| - M\cdot \eu^{-\delta t}\norm{f}\right) \leq \eu^{-rt}\norm{T(t)f} \leq \left(\left|\int f \diff{\nu}\right| + M\cdot \eu^{-\delta t}\norm{f}\right).
	\end{equation}
\end{corollary}
%% --
\begin{proof}
...
\colon y' \in G'$, $\norm{y'} \leq 1\}$ is relatively weakly compact, \ie  $R'$ is
weakly compact.
...
the adjoint $R'$, \ie $\{R'y' \colon y' \in G'$, $\norm{y'} \leq 1\}$ is relatively weakly compact in $M(K)$.
From Proposition~\ref{prop:b4-2.3} $(a)\Rightarrow(b)$ we obtain that
$\langle Rf_n,y'\rangle = \langle f_n,R'y'\rangle \to 0$ as $n \to \infty$ uniformly for $y' \in G'$, $\norm{y'}\leq1$.
That is $\lim_{n\to\infty}\norm{Rf_n} = 0$.
...
\begin{theorem}\label{thm:b4-2.5}
	...
	such that $\norm{T(t) - P} \leq M\cdot \eu^{-\delta t}$ for suitable constants $\delta>0$, $M \geq 1$.
\end{theorem}
...
\begin{align*} 
    ...
    &\leq \epsilon\cdot P_t(x,U) + 2\norm{f}_{\infty}\cdot P_t(x,K\backslash U) .
\end{align*}
%% --
...
%
\[
	\text{dist}(T,\mathcal{K}(G))  \coloneq  \inf\{\norm{T - K} \colon K \in \mathcal{K}(G)\} .
\]
%% --
...
\begin{proposition}\label{prop:b4-2.8}
	...
	\begin{enumerate}[\upshape (a)]
		...
		\item 
		There exist $t_{0} > 0$, $K \in \mathcal{K}(G)$ such that $\norm{T(t_{0}) - K} < 1$.
	\end{enumerate}
\end{proposition}
%% --
\begin{proof}
...
assertion $(c)$ implies 
$r_{\text{ess}}(T(t_0)) \leq \norm{T(t_0)}_{\text{ess}} < 1 .$ 
...
\[
\text{
$\lim_{n \to \infty}\norm{T(n)}_{\text{ess}}^{1/n} < 1$,  where $\norm{T}_{\text{ess}} = \mathrm{dist}(T,\mathcal{K}(G))$ .}
\]
%% --
Thus for suitable $n_{0} \in \N$, $a < 1$ we have $\norm{T(n)}_{\text{ess}} < a^n$ for $n \geq n_{0}$.

Choosing a sequence $K_n \in \mathcal{K}(G)$ such that $\norm{T(n) - K_n} < a^n$ for $n \geq n_{0}$ and defining $M \coloneqq \sup_{0 \leq s \leq 1}\norm{T(s)}$ we obtain for $t \in [n,n+1]$ $(n \geq n_{0})$ 
%% -- 
\[
\norm{T(t) - T(t-n)K_n} \leq \norm{T(t-n)}\norm{T(n) - K_n} \leq M \cdot a^n .
\] 
...
\begin{proof}
	...
	Since the mapping $(t,x) \mapsto T(t)x$ is jointly continuous on $\R_+ \times G$ and since $K$ is compact, the set $M_t \coloneq \{T(s)Kx \colon 0 \leq s \leq t, \norm{x} \leq 1\}$ is relatively compact in $G$. Having in mind that $\int_0^t T(t-s)KS(s)x  \ds $ $(x \in G)$ is the norm limit of Riemann sums, one observes that 
	$(ct)^{-1}\int_0^t T(t-s)KS(s)x  \ds $ is an element of the closed convex hull
	 $\overline{\mathrm{co}(M_t)}$ of\/ $M_t$, where $c \coloneq \sup \{\norm{S(s)} \colon 0 \leq s \leq t\}$ and $\norm{x} \leq 1$. Since $\overline{\mathrm{co}(M_t)}$ is compact (see II.4.3 in \textcite{schaefer:1966}) the assertion follows.
\end{proof}
%% --
...
\index{resolvent!pole}
	...
	\begin{equation}\label{eq:b4-2.5}
		...
		\norm{R(t)} &\leq C \cdot \eu^{-\epsilon t} \quad \text{for suitable constants} \quad \epsilon > 0, C \geq 1 .		
		\end{aligned}
	\end{equation}
\end{theorem}
%% --
\begin{proof} We have $\omega_{ess}(\TT) < 0$, hence $r_{ess}(T(1)) < 1$ (see  Eq.~\myeqpageref{eq:a3-3.10}).

Therefore $\{z \in \sigma(T(1)) \colon |z| \geq 1\}$ is a finite set and contains only poles of finite algebraic multiplicity 
(\cf Eq~\myeqpageref{eq:a3-3.8}). 
...
\begin{enumerate}[\upshape (i)]
	...
	\begin{equation}\label{eq:b4-2.6}
		\norm{\eu^{-rt}T(t) - P} \leq M \cdot \eu^{-\delta t} \quad \text{for all} \quad t \geq 0 \text{.}
	\end{equation}
	...
	\begin{equation}\label{eq:b4-2.7}
		\norm{\exp(-\omega_{0}(\TT)t) \cdot T(t) - \nu \otimes h} \leq M \cdot \eu^{-\delta t} \quad \text{for all} \quad t \geq 0 \text{.}
	\end{equation}
\end{enumerate}
In both cases $(i)$ and $(ii)$ the estimates Eq.~\myeqpageref{eq:b4-2.3} for $\norm{T(t)f}$ hold true (in case $(i)$ one has to replace $|\int f \diff{\nu}|$ by $\norm{Pf}$).
\end{corollary}
%% --
\begin{proof}
	\begin{enumerate}[\upshape (i), wide, labelindent=.25em]
...
Since $\norm{R(\lambda,B)} \leq (\lambda-\omega_0)^{-1}$ it follows that $S_{\lambda}$ is invertible for $\lambda  >  \norm{\Phi} + \omega_0$ and that $\norm{S_{\lambda}^{-1}} \leq (\lambda-\omega_0)\cdot(\lambda-\norm{\Phi}-\omega_0)^{-1}$.
...
%% --
\[\norm{R(\lambda,A)} \leq (\lambda - \omega_0)\cdot(\lambda - \norm{\Phi} - \omega_0)^{-1}\cdot(\lambda - \omega_0)^{-1} \leq (\lambda - \norm{\Phi} - \omega_0)^{-1}.\]
...
Notice that the map $x \mapsto x + \Phi H_\lambda(e_\mu \otimes x)$ $(x \in F)$ is surjective on $F$ if\/ $\mu$ is chosen so large that $\norm{\Phi H_\lambda(e_\mu \otimes x)} \leq 1/2 \cdot \norm{x}$ for all $x \in F$. 
...
Indeed, if\/ $B$ generates a bounded positive semigroup on $F$, then $\norm{x}  \coloneq  \sup_{t \geq 0}\|S(t)|x|\|$ for $x \in F$ defines an equivalent lattice norm on $F$ for which $(S(t))_{t \geq 0}$ is contractive.
%% --
...
\end{proposition}
%% --
\begin{proof} By Formula Eq.~\myeqpageref{eq:b4-3.2} we have $R(\lambda,A) = S_\lambda^{-1}R(\lambda,A_{0})$ for $\lambda > \norm{\Phi}+w$, (where $S_\lambda f = f - \epsilon_\lambda \otimes R(\lambda,B)\Phi f$ for $f \in E$). 
Thus the fact that $R(\lambda,A_{0})$ is positive (Part-III, Proposition~\ref{prop:c2-4.1}) reduces the problem to showing that $S_\lambda^{-1}$ is a positive operator for $\lambda > \norm{\Phi}+w$.

Since $S_\lambda = \Id  - \epsilon_\lambda \otimes R(\lambda,B)\Phi$ and $\norm{\epsilon_\lambda \otimes R(\lambda,B)\Phi} \leq (\lambda-w)^{-1} \cdot \norm{\Phi} < 1$ we see that $S_\lambda^{-1} = \sum_{n=0}^{\infty}(\epsilon_\lambda \otimes R(\lambda,B)\Phi)^n$ is positive. 
Hence $(T(t))_{t \geq 0}$ is a positive semigroup again by Part-III, Proposition~\ref{prop:c2-4.1}.
\end{proof}
%% --
\begin{remark*}
Suppose that $\Phi$ has no mass in zero (\ie  for every $\epsilon > 0$ there exists $\delta > 0$ such that $\norm{\Phi f} \leq \epsilon \norm{f}$ for all $f \in E$, supp$(f) \subset [-\delta,0])$. 
...
Moreover $\Phi_{0}$ is the multiplication by $ \langle \epsilon_{0}, \Phi  \rangle = \norm{\Phi}$ (notice 
$\Phi\geq 0$) and $s(B + \Phi_{0}) = \alpha + \norm{\Phi}$. 
Since $\omega_{0}(A) = s(A)$ by Eq.~\myeqpageref{eq:b4-1.1}, we obtain from Corollary~\ref{cor:b4-3.8} that $A$ generates a uniformly exponentially stable semigroup if and only if\/ $\alpha + \norm{\Phi} < 0$.
\end{example}
%% --
