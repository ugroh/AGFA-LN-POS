...
Let $x \in X$.
We have to show that $(Af)(x) + \norm{A}f(x) \geq 0$ for all $f \in C_{0}(X)$\,.
...
%%--
\[
(Af)(x) + \norm{A}f(x) = \langle f, A'\delta_{x} + \norm{A}\delta_{x} \rangle = \langle f, \mu + (c+\norm{A})\delta_{x} \rangle \geq 0.
\]
%%--
%%$.
...
%
\[
	\vert R(0,A)f \vert \leq R(0,A)|f| \leq \norm{f}R(0,A)\1 = \norm{f}u.
\]
%
...
%%$
%%-
\[\norm{f}_{0} = \inf \{\lambda > 0 \colon |f| \leq \lambda u\} = \norm{f \cdot u^{-1} }_{\infty}.
\]
%%--
%%$.
Then $\norm{\cdot}_{0}$ is an equivalent norm on $C(K)$.
Moreover, $\norm{f}_{0} \leq 1$ if and only if\/ $f \in [-u,u]$.
...
This implies that $\lambda R(\lambda,A)$ is contractive for the norm $\norm{\cdot}_{0}$.
Thus by the Hille-Yosida Theorem $A$ is the generator of a semigroup which is contractive with respect to the norm $\norm{\cdot}_{0}$ and so is bounded with respect to the supremum norm on $C(K)$.
...
If\/ $A$ satisfies (P) then $A$ is bounded and $A + \norm{A}\Id \geq 0$.
\end{corollary}
%% –
...
Hence $A$ is bounded.
Since $A$ satisfies (P), it follows from Theorem~\ref{thm:b2-1.3} that $A + \norm{A}\Id \geq 0$.
\end{proof}
...
The proof of Theorem~\ref{thm:b2-1.13} yields estimates for the growth bound of a positive semigroup (see Eq.~\myeqpageref{eq:a3-1.3}) which we state explicitly in the next corollary.
...
\norm{m^{-1}}_{\infty}^{-1}\, \omega_{0}(A) \leq \omega_{0}(m \cdot A) \leq \norm{m}_{\infty}\, \omega_{0}(A).
\end{equation}
\end{theorem}
%% –
\begin{proof}
We can assume that $\norm{m}_{\infty} \leq 1$ (in fact, if\/ $B \coloneqq (m/\norm{m}_{\infty}) \cdot A$ is the generator of a positive semigroup, then by 
Part-I, Section~\ref{subsec:a1-3.2}, the operator $m \cdot A = \norm{m}_{\infty}B$ also generates a positive semigroup).
...
&1 \geq q \in C(K)_{+} \text{ is such that } \norm{1 - q}_{\infty} < 1/2,  \\
&\text{ then } q \cdot B \text{ generates a } p_{u}\text{-contraction semigroup.}
\end{split}
\end{equation}
%% –
Because of Eq.~\myeqpageref{eq:b2-1.11} we only have to show that $(I - q \cdot B)$ is surjective. 
...
%%--
\[
\norm{f}_{u} = \max\{p_{u}(f), p_{u}(-f)\} = \sup_{x \in K} |f(x)|/u(x)
\]
%%--
%%$ 
is equivalent to the supremum norm.
Denote by $\norm{T}_{u}$ the operator norm corresponding to $\norm{\cdot}_{u}$ $(T \in \L{C(K)}$.
Then it is enough to show that $\norm{(q-1)BR(1,B)}_{u} = \norm{(q-1)(R(1,B) - I)}_{u} < 1$.
...
%% --
\begin{align*}
	    \norm{M_{r}}_{u} &= \sup\{\norm{r \cdot f}_{u} \colon \norm{f}_{u} \leq 1\} \\
	              &= \sup\nolimits_{\norm{f}_{u} \leq 1}\{ \, 
	              \sup \{ r(x) \cdot |f(x)| \cdot u(x)^{-1}  \colon {x \in K} \}  \\
	              &\leq \norm{r}_{\infty}.
\end{align*}
%% --
Since $B$ is $p_{u}$-dissipative we have $\norm{R(1,B)}_{u} \leq 1$ (by Part-I, Lemma~\ref{lem:a2-2.10}).
This gives $\norm{(q-1)(R(1,B) - I)}_{u} \leq \norm{M_{(1-q)}}_{u}(\norm{R(1,B)}_{u} + 1) \leq 2\norm{1 - q}_{\infty} < 1$.

The proof of Eq.~\myeqpageref{eq:b2-1.12} is complete.
\index{dissipative operator|)}

There exists $k \in \N$ such that $\norm{1 - m^{1/k}}_{\infty} < 1/2$.
...
Finally we show Eq.~\myeqpageref{eq:b2-1.10} 
to hold.
Let $0 \ll u \in D(A) = D(m \cdot A)$ and $Au \leq \lambda u$.
Then $m \cdot Au \leq \norm{m}_{\infty}\lambda u$.
So Eq.~\myeqpageref{eq:b2-1.8} 
implies that $\omega_{0}(m \cdot A) \leq \norm{m}_{\infty}\omega_{0}(A)$.
This is one part of Eq.~\myeqpageref{eq:b2-1.10}. 
The other part follows from this since $\omega_{0}(A) = \omega_{0}(m^{-1} \cdot m \cdot A) \leq \norm{m^{-1}}_{\infty} \omega_{0}(m \cdot A)$.
\end{proof}
...
c) We show that $(\lambda-A)$ is bijective for $\lambda > \alpha + \norm{\mu}$.
Let $g \in C[-1,0]$.
...
\end{equation}
%% –
If\/ $\lambda > \alpha + \norm{\mu}$, then $\lambda - \alpha - \int_{-1}^{0} \eu^{\lambda x} d\mu(x) \neq 0$ and there exists exactly one $c \in \R$ satisfying Eq.~\myeqpageref{eq:b2-1.13}.
We have shown that $(\lambda- A)$ is bijective for $\lambda > \alpha + \norm{\mu}$.

By Theorem~\ref{thm:b2-1.13}, it follows from a), b) and c) that $A$ generates a positive semigroup.
\end{proof}

Let us mention in addition that it follows from a) in the proof that $(\alpha + \norm{\mu}, \infty) \subset \rho(A)$, since $A$ is closed.
...
%% --
\begin{proof}
There exists $L \geq 0$ such that $\norm{\Theta(x) - \Theta(y)} \leq L\norm{x - y}$ for all $x,y \in G$.
Then
%% --
\[
\begin{array}{lll}
\lim_{t \downarrow 0} &\norm{\frac{1}{t} (\Theta(k(a+t)) - \Theta(k(a))) - D_{k'(a)}\Theta(k(a))} \\
&\leq \limsup_{t \downarrow 0} \norm{\frac{1}{t} (\Theta(k(a+t)) - \Theta(k(a)+tk'(a)))} \\
&\quad + \limsup_{t \downarrow 0} \norm{\frac{1}{t} [\Theta(k(a)+tk'(a)) - \Theta(k(a)) - D_{k'(a)}\Theta(k(a))]} \\
&\leq \limsup_{t \downarrow 0} L \cdot \norm{\frac{1}{t} (k(a+t) - k(a)-tk'(a))} + 0 \\
&= 0.
\end{array}
\]
\phantom{x}
...
%% --
\begin{proof}
If\/ $z = 0$, relation Eq.~\myeqpageref{eq:b2-2.6} is obvious from the definiton.
Let $z = (x_{0} + iy_{0}) \neq 0$.
...
%% --
\begin{equation*}
\overline{\lim}_{h \downarrow 0}\, \frac{1}{h}\, \norm{(T(t-(s+h)) - T(t-s))'\phi} < \infty
\end{equation*}
%% --
...
Hence by Eq.~\myeqpageref{eq:b2-2.9}, $\langle \Re[(\sign \bar{f})(Af)],\phi \rangle = \langle |f|,A'\phi \rangle$ for all $\phi \in D(A')$.
...
&= \langle \Re[\widehat{(\sign} f)(Af)],\delta_{0} \rangle = \langle |f|,A'\delta_{0} \rangle \quad \text{(by Eq.~\myeqpageref{eq:b2-2.9})} \\
&= \langle |f|,\mu \rangle.
\end{align*}
%% --
...
Eq.~\myeqpageref{eq:a2-1.8} % we have 
\[
...
%%
\begin{proof}
We first note that $\norm{T(t)}$ is bounded on compact intervals of\/ $\R$.
This follows from \textcite[7.4.1]{hillephillips:1957} since $q(t) = \log\norm{T(t)}$ defines a subadditive, measurable function from $\R$ into $\R$ [In fact, $\norm{T(t)} = \sup_{x \in X}|h_{t}(x)|$ for $t \in \R$.
So it follows from the assumption that $t \mapsto \norm{T(t)}$ is lower semicontinuous and hence measurable].
...
The group defined by Eq.~\myeqpageref{eq:b2-3.12} is positive whenever $(h_t)_{t \in \R} \subset C^{b} (X)_+$.
...
There exists $t_0 > 0$ such that $\norm{T(t)f - f} < \frac{1}{2}$ for $|t| \leq t_0$.
...
However every positive cocyle is a product of cocycles of the form Eq.~\myeqpageref{eq:b2-3.13} and Eq.~\myeqpageref{eq:b2-3.14}.
...
\norm{T(t)} \leq M\eu^{(\omega-1)|t|}
\end{equation}
%% --
for all $t \in \R$.
Since $\norm{T(t)} = \sup_{x \in X} k_{t}(x)$ the right inequality of
Eq.~\myeqpageref{eq:b2-3.16} follows.
Moreover, $k_{-t} = 1/(k_{t} \circ \phi_{-t})$.
Hence $\norm{T(-t)} = \sup_{x \in X} 1/k_{t}(x) = [\inf_{x \in X} k_{t}(x)]^{-1}$.
...
\end{theorem}
%% --
And note that  Eq.~\myeqpageref{eq:b2-3.18} means that $D(A) = \{ f \colon V^{-1}f \in D(\delta) \}$ and $Af = V\delta V^{-1}f + hf$.
%% –
\begin{proof}
...
By Proposition~\ref{prop:b2-3.9} and Lemma~\ref{lem:b2-3.12} we know that there exist a continuous flow $\phi$, $0 \ll p \in C^{b}(X)$ and $h \in C^{b}(X)$ such that Eq.~\myeqpageref{eq:b2-3.19} holds.
...
%%
Let $X = \R^{n}$.
Consider a function $F \in C^{1}(\R^{n},\R^{n})$ such that $\sup_{x \in \R^{n}} \norm{DF(x)} < \infty$ where $DF(x) \in \L{\R^{n}}$ denotes the derivative of\/ $F$ at $x$.
...
Then 
\[D_{0} = \{ f \in C_{0}(\R^{n}) \cap C^{1}(\R^{n}) \colon \lim_{\norm{x} \to \infty} \norm{(\mathrm{grad}\: f)(x)} = 0 \}
\]
...
Consequently $\norm{D\phi_{t}(x)} \leq 1 + \int_{0}^{t}  M \cdot  \norm{D\phi_{s}(x)} \ds$ for all $t \geq 0$ and $x \in \R^{n}$; where $M \coloneqq \sup_{x \in \R^{n}} \norm{DF(x)} < \infty$ by hypothesis.
Hence by Gronwall's inequality, $\norm{D\phi_{t}(x)} \leq \eu^{Mt}$ $(t \geq 0)$ for all $x \in \R^{n}$.
Now let $f \in D_{0}$, $t \geq 0$.
Then $[\mathrm{grad}\:(f \circ \phi_{t})](x) = [(\mathrm{grad}\:f)(\phi_{t}(x))] \cdot D\phi_{t}(x)$.
Hence $\norm{[\mathrm{grad}\:(f \circ \phi_{t})](x)} \leq \eu^{Mt}\norm{(\mathrm{grad}\:f)(\phi_{t}(x))}$, and so 
%%--
\[
\lim_{\norm{x} \to \infty} \norm{[\mathrm{grad}\:(f \circ \phi_{t})](x)} \leq \eu^{Mt} \lim_{\norm{x} \to \infty} \norm{(\mathrm{grad}\:f)(\phi_{t}(x))} = 0.
\]
%%--
