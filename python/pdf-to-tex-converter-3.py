...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        for page_num, page in enumerate(doc, 1):
            ...
            # Text mit verschiedenen Strategien extrahieren
            text = page.get_text("text")  # Plain text
            if not text.strip():
                print(f"Versuche alternative Extraktionsmethode für Seite {page_num}")
                text = page.get_text("words")  # Wortweise Extraktion
                if text:
...
def convert_to_tex_commands(text):
    ...
    # Mathematische Ausdrücke in $ ... $ einschließen
    # Subscripts
    tex_text = re.sub(r'(\w+)_(\w+)', r'$\1_\2$', tex_text)
    # Superscripts
    tex_text = re.sub(r'(\w+)\^(\w+)', r'$\1^\2$', tex_text)
    # Brüche erkennen und formatieren
    tex_text = re.sub(r'(\d+)/(\d+)', r'$\\frac{\1}{\2}$', tex_text)
