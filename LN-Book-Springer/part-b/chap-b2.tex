...
%A-I, 3.1
A-I, 3.2 $m \cdot A = \|m\|_{\infty}B$ also generates a positive semigroup).
...
Thus, since \\
$\ker(\lambda - A_{\max}) = \C e_{\lambda}$ $(e_{\lambda}(x) = \eu^{\lambda x})$, Kato's equality does not have as strong consequences as the positive minimum principle (which by Theorem 1.13 would imply that $A_{\max}$ is a generator).
\end{example}
\index{Kato's equality|)} 
...
\end{definition}
%% –
Note: If $m$ is admissible and $a > -\infty$, then $m(a) = 0$; similarly, if $b < \infty$, then $m(b) = 0$.
...
But one can obtain every such generator by a similarity transformation (see 
%%KGK StandConst schift
%A-I,3.1
A-I, 3.2) from some $\delta_{m}$.
%% –
