...
As shown in A-IV, Example~1.2\,(ii), the equality $s(A) = \omega_{0}(A)$ may not hold for positive semigroups on arbitrary Banach lattices.
...
If \ref{cor:c4-1.4-2} is satisfied then $R(0,A) \geq 0$, hence $s(A) < 0$ by C-III, Theorem 1.1\,(ii)).
...
Thus irreducibility of the semigroup (see A-I, 2.8 and C-III, Example~3.4\,(i)) implies that $\dim \Kern\Delta \leq 1$.
...
Using the weak version of A-IV, Theorem~1.14, 
...
\end{proof}
%% --
In A-IV, Theorem 1.13 we have seen that a generator $A$ of a stable semigroup satisfies necessarily $s(A) \leq 0$, $\Re(\lambda) < 0$ for all $\lambda \in P{\sigma}(A) \cup R{\sigma}(A)$ and from $\lambda R(\lambda,A)f = R(\lambda,A)Af + f$ we conclude that $\lim_{\lambda \to 0+} R(\lambda,A)g$ exists for all $g \in \Image{A}$. 
...
Since $f_{0} \in \Image{A}$, the second term converges to $h \coloneqq  \int_{0}^{\infty} T(s)f_{0} \, \ds \in \Image{A}$ (A-IV, Theorem 1.16) and $Ah = -f_{0}$.
...
By assumption we have $\lim_{s \to \infty}\|A(F(s)-f_{0})\| = 0$ and from Theorem~\ref{thm:c4-1.3} and A-IV,\,(1.3) we deduce that $\|T(s)R(0,A)\| \leq M\eu^{-\epsilon s}$ for $s \geq 0$ and suitable constants $M \geq 1$, $\epsilon > 0$.
...
\index{asymptotic behavior}
\begin{quote} 
The asymptotic behavior of a strongly continuous semigroup $(T(t))_{t \geq 0}$ is determined by the structure
and location of the spectrum $\sigma(A)$ of the generator $A$.
\end{quote}
%% --
...
To prove results in this direction, we must assume additional hypotheses on the semigroup.
Positivity may serve to this purpose.
For instance, the norm convergence to zero, \ie  $\lim_{t \to \infty} \|T(t)\| = 0$ for a positive semigroup on certain Banach lattices, is characterized by the condition $s(A) < 0$ (see Theorem~\ref{thm:c4-1.1}).
Thus in this case the location of the spectrum determines the norm convergence of the semigroup.

Here we focus on the case $s(A) = 0$.
First, we note that
...
In this section we mainly consider the case $s(A) = 0 \in P{\sigma}(A)$ and demonstrate that the symmetric structure of the boundary spectrum of the generator of a positive semigroup yields interesting results.
...
Since $\Kern{A} = \bigcap_{t \geq 0} \Kern{\Id - T(t)} = \Kern{\Id - T(1)}$ (cf.\ A-III, Corollary~6.4), we have $\Image{\Id - T(1)} + \Kern{\Id - T(1)} \subset F$.
Since power bounded operators on a reflexive Banach space are mean ergodic (\eg  see \citet[Chapter~2, Theorem~1.2]{krengel:1985}), we obtain that $\Image{\Id - T(1)} + \Kern{\Id - T(1)}$ is dense in $E$, hence $F = E$.
\end{proof}
%% --
...
%
Assume that --- additionally to the assumptions made in Theorem~\ref{thm:c4-2.6} --- $1 < p < \infty$  and  $P{\sigma}(A) \cap \im\R = \{0\}$.
...
\end{equation}
%% --
The assumption $P{\sigma}(A) \cap \im\R = \{0\}$ implies $\Kern{\Id - T(\tau)} = \Kern{A}$ (cf.\ A-III, Corollary 6.4), hence we have convergence on $\Kern{\Id -T(\tau)}$.
Since $T(\tau)$ is a contraction on a reflexive Banach space, we have
\[
L^{p}(\mu) = \Kern{\Id - T(\tau)} \oplus \overline{\Image{\Id - T(\tau)}} \quad \text{(see \citet [p.74]{krengel:1985})}
\]  
...
Now C-III, Proposition 3.5(i)-(iii) implies that $P$ has the form $P = h \otimes e$ for a strictly positive function $h \in L^{q}(\mu) = (L^{p}(\mu))'$.
\end{proof}
%%% --
...
Since $0$ is an eigenvalue of\/ $A$, it follows that $\TT$ has a quasi-interior fixed point $h \in E_{+}$ (use C-III, Proposition 3.5(i)).
...
By (A-I, 3.6), $A_1$ generates the semigroup $(T(t)_{|D(A)})_{t \geq 0}$.
...
The positivity of\/ $(T(t))_{t \geq 0}$ implies $r \in \sigma(A)$ (see C-III, Theorem 1.1.(i)) and by Proposition~\ref{prop:c4-3.6}\,\ref{prop:c4-3.6-1}  this implies $1 \in \sigma(\Phi_r)$, so that $s(\Phi_{r)} \geq 1$. 
Since $r \geq \lambda$, this yields $s(\Phi_{\lambda}) \geq s(\Phi_{r}) \geq 1$.

\item 
%\ref{thm:c4-3.7-2} 
Let $s(\Phi_{\lambda}) = 1$. 
Since $1 \in \sigma(\Phi_{\lambda})$ (see C-III, Theorem 1.1(i)), $\lambda \in \sigma(A)$ by Proposition~\ref{prop:c4-3.6}\,\ref{prop:c4-3.6-3} whence $s(A) \geq \lambda$. 
...
These similarly apply to Volterra equations, see \citet{miller:1974}, \citet{webb:1977} and to 
\index{equation!population}
\index{population!equation}
age-dependent population equations, see \citet{Pruess:1981}, \citet{webb:1984}, \citet{webb:1985a}.
