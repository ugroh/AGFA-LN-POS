...
such that $\lambda = r^{2}\eu^{i2\beta}$.
Then 
\[
(\lambda - A^{2}) = (r\eu^{\im\beta} - A)(r\eu^{\im\beta} + A) \, .
\] 
...
Choose $T \geq t'$ such that $\frac{1}{n!}\int_T^\infty t^n\eu^{-at}\|T(t)\| \dt < \epsilon^{n+1}/3$.
...
\begin{align*}
    \|R(a+ir,A)^{n+1}\| & = \frac{1}{n!}\cdot\|\int_0^\infty \eu^{-(a+ir)t}t^nT(t) \dt\| \\
    & \leq \frac{1}{n!}\cdot\int_{0}^{t'} \eu^{-at}t^n\|T(t)\| \dt \\
    & + \frac{1}{n!}\cdot\|\int_{t'}^T t^n\eu^{-irt}\eu^{-at}T(t) \dt\| \\
...
\[
[\text{ess}]\text{-}\sup\{|\eu^{tm(x)} - \eu^{t'm(x)}| \colon x \in X, \Re(m(x)) > b\} < \epsilon
\] 
whenever $|t-t'| < \delta$.
