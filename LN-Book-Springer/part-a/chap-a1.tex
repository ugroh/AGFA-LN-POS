...
%\index{Standard Definitions and Results}
%% -- 
...
%\index{One-parameter semigroups|textbf}
%\index{One-parameter semigroups!Basic facts}
...
%%KGK
%\index{One-parameter semigroup!Growth Bound $\omega_{0}$}
...
%%KGK
%\index{One-parameter semigroup!Generator}
...
\begin{align*}
    ...
    ( \eu^{\lambda t}-T(t))f &= (\lambda-A) \int_{0}^{t}  \eu^{\lambda(t-s)}T(s)f \ds \quad \text{for } f \in E, %\tag{3.1}
\end{align*}
%% --
and
%% --
\begin{align*}
    ...
    \notag \\
    ( \eu^{\lambda t} - T(t))f &= \int_{0}^{t}  \eu^{\lambda(t-s)}T(s)(\lambda-A)f \ds \quad \text{for } f \in D(A). %\tag{3.2}
\end{align*}
%% --
%% --
...
%%KGK kleinschreiben + kuerzen
%\index{standard example!translation semigroup on $L^{1}$}
\index{standard example!translation semigroup}
%%
...
%%KGK ????
%\index{Standard Constructions!fprod@$\mathcal{F}$-Product!Semigroup}
