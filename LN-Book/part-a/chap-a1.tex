...
\]
%
define $ \delta \coloneq ad - bc = \det(A)$, $ \tau = a + b $ and $ p_{A} $ the characteristic polynomial of $ A $, \ie 
%
\[
	p_{A}(t) = \det(t - A) = t^{2} - \tau \, t + \delta .
\]
%
$A = (a_{ij})_{2 \times 2}$, one can derive an explicit formula for $e^{tA}$. 

Let\quad
$\tau \coloneqq \text{trace } A$\,,
\quad 
$\delta \coloneqq \text{det } A$\,,
\quad
$D \coloneqq (\tau^{2}/4 - \delta)^{1/2}$ \quad and 
\[
p_{A}(t) \coloneqq \det(t - A) = t^{2} - \tau t + \delta .
\]
the characteristic polynomial of $A$. 

By the \emph{Cayley-Hamilton theorem} we have 
\quad$p_A(A) = A^2-sA +d\Id = 0$\quad  or, equivalently, 
\[
(A-s/2\Id)^2 = D^2\Id \,.
\]
\begin{itemize}[\textbullet, wide]
\item 
If $D^2=0$, then $(A-s/2\Id)^k = 0$ for $ k \geq 2$, hence 
$e^{t(A-s/2\Id)} = \Id + t(A-s/2\Id)$.
\end{itemize}
Otherwise, by iteration we obtain for every $m\in\N$
\[
(A-s/2\Id)^{2m} = D^{2m}\Id  
\quad \text{and} \quad 
...
Thus  
\begin{align*}
e^{-t\frac{s}{2}}e^{tA} = e^{t(A-s/2\Id)} &= \sum_{k=0}^\infty \frac{t^k}{k!}(A-s/2\cdot\Id)^k = \sum_{k\ \mathrm{even}} \cdots + 
...
\begin{itemize}[\textbullet, wide]
\item  
If $D^2 > 0$, and $D$ its positive or negative root, then $\sinh(tD)$ and $\cosh(tD)$ are linear combinations of $e^{tD}$ and $e^{-tD}$.
