...
If the semigroup $(T(t))_{t \geq 0}$ is uniformly continuous, then $\frac{1}{t}\int_{0}^{t} T(s) \, ds$ uniformly converges to $T(0) = \mathrm{Id}$ as $t \to 0$.
...
\[
    e^{tA} = 
        e^{ts/2} \cdot [D^{-1}2\sinh(tD/2) \cdot A + (\cosh(tD/2) - sD^{-1}\sinh(tD/2)) \cdot \mathrm{Id}] 
\]
%% --
and if $D=0$
%% --
\[
	e^{ts/2} \cdot [tA + (1 - ts/2) \cdot \mathrm{Id}] .
\]
%% --
...
\]
%% --
yield a group $(R_{\tau}(t))_{t \in \R}$ having period $\tau$, i.e.\xspace $R_{\tau}(\tau) = \mathrm{Id}$.
...
\]
%% --
From the behavior of $T(t)$ on the dense subspace $E_{0}$ it follows that $(T(t))_{t \geq 0}$ with $T(0) = \mathrm{Id}$ is a strongly continuous semigroup on $E$ and its generator $A$ coincides with $B$ on $E_{0}$.
...
%% --
\[
    B \coloneqq \alpha A + \lambda \mathrm{Id} \text{ with } D(B) = D(A) .
\]
%% --
...
\]
%% --
is equicontinuous at $0$, i.e.\xspace, for every $\varepsilon > 0$ there exists $0 < t_{0} < 1$ such that
%% --
\[
...
\item
$\|u\|_{\varepsilon} \coloneqq \sup\{\langle u,\phi \otimes \psi\rangle \colon \phi \in E', \psi \in F', \|\phi\|, \|\psi\| \leq 1\}$ defines the 
\emph{least cross norm $\varepsilon$} on $E \times F$. 
...
The closure of $A \otimes \mathrm{Id} + \mathrm{Id} \otimes B$, defined on the core $D(A) \otimes D(B)$, is its generator.
\end{proposition}
%% --
\begin{proof}
...
%
\[
	(A \otimes \mathrm{Id} + \mathrm{Id} \otimes B,D(A) \otimes D(B)) .
\]
%
