...
%% -- Stand 2025-05-08 ulgr
%% -- Korrekturen bitte belassen
%% --
...
%% --
\begin{enumerate}[\upshape (i), wide, labelsep=1em, itemsep=.5ex]
...
By the \emph{growth bound} (or \emph{type}) of the semigroup $(T(t))_{t\geq0}$ we understand the number
%% --
...
is everywhere defined and therefore bounded (use Proposition~\ref{prop:a1-1.9}\,(ii)) on p.\,\pageref{prop:a1-1.9}). %Prop.1.9(i)).
...
R(\lambda,A)^{n+1}f = \frac{(-1)^{n}}{n!}\left(\frac{\diff{ }}{\diff\lambda}\right)^{n} R(\lambda,A)f = \int_{0}^{\infty} e^{-\lambda t}\frac{t^{n}}{n!}T(t)f \, \dt
\end{equation}
%% --
...
\index{Basic Results!Standard Examples!Uniformly Continuous Semigroups}
%% --
...
\index{Basic Results!Standard Examples!Matrix Semigroups}
%% --
...
\item
$d \coloneqq \text{det } A$ and 

\item
$D \coloneqq (s^{2} - 4d)^{1/2}$.
\end{enumerate*}
%% --
Then if  $D \neq 0$
%% -- 
...
%If $n = 2$ and $A = (a_{ij})_{2 \times 2}$ the following explicit formulas for $e^{tA}$ might be of interest.
If
$n = 2$ and $A = (a_{ij})_{2 \times 2}$ one can derive an explicit formula for $e^{tA}$.
...
\quad
$D \coloneqq (s^{2}/4 - d)^{1/2}$
\quad and $p_A$ the characteristic polynomial of $A$
\[
p_A(\lambda) \coloneqq\lambda^2-s\cdot \lambda + d \,.
\]

By the \emph{Cayley-Hamilton theorem} we have $p_A(A) = A^2-sA +d\Id = 0$  or,
equivalently,
\[
(A-s/2\Id)^2 = D^2\Id \,.
\]
%If $D=0$, then $(A-s/2\Id)^k = 0$ for $k \geq 2$, hence $\exp(t(A-s/2\Id) = \Id+(A-s/2\Id)$.
By iteration we obtain for every $m\in\N$
\[
(A-s/2\Id)^{2m} = D^{2m}\Id
\quad \text{and} \quad
(A-s/2\cdot\Id)^{2m+1} =\frac{1}{D} D^{2m+1}(A-s/2\cdot\Id)\,.
\]
Thus
\begin{align*}
\exp(t(A-s/2\Id) &= \sum_{k=0}^\infty \frac{t^k}{k!}(A-s/2\cdot\Id)^k = \sum_{k\ \mathrm{even}} \cdots +
\sum_{k\ \mathrm{odd}} \cdots = \\
& = \cosh(tD)\Id + \frac{1}{D}\sinh(tD)(A-s/2\Id)\,.
\end{align*}
In case $A$ is a real $2 \times2$-matrix, then $D^2 = s^2/4-d$ can be positive or negative.
\begin{itemize}[\textbullet, wide]
\item
If $D^2 > 0$, ($D$ its positive or negative root), then $\sinh(tD)$ and $\cosh(t)$ are linear combinations of $\exp(tD)$ and $\exp(-tD)$.

\item
If $D^2 < 0$, then $D = \im|D|$, and
$\cosh(tD) = \cos(t|D|)$ and $\sinh(tD) = \sin(t|D|)$\,.

\item
If $D^2=0$, then $\cosh(tD) = 1$ and $\frac{1}{D}\sinh(tD) = t $
\end{itemize}

%% --
\subsection{Multiplication Semigroups}\label{subsec:a1-2.3}
\index{Basic Results!Standard Examples!Multiplication Semigroups}
%% --
...
\index{Basic Results!Standard Examples!Translation Semigroups}
%% --
...
\index{Basic Results!Standard Examples!Rotation Groups}
%% --
...
\index{Basic Results!Standard Examples!Nilpotent Translation Semigroups}
%% --
...
\index{Basic Results!Standard Examples!One-dimensional Diffusion Semigroup}
%% --
...
\marginpar{GG: besser wäre  $\langle f, e_n\rangle$}
\marginpar{UG: nein, da es ein Hilbertraum ist }
%
\[
...
\index{Basic Results!Standard Examples!$n$-dimensionalDiffusion Semigroup}
%% --
...
\index{Basic Results!Standard Constructions!Similar Semigroups}
%% --
...
\index{Basic Results!Standard Constructions!Rescaled Semigroup}
%% --
...
\index{Basic Results!Standard Constructions!Subspace Semigroup}
%% --
...
\index{Basic Results!Standard Constructions!Quotient Semigroup}
%% --
...
\index{Basic Results!Standard Examples!Translation Semigroup on $L^{1}$}
%%
...
\index{Basic Results!Standard Constructions!Adjoint Semigroup}
%% --
...
Since the semigroup $(T(t)')_{t \geq 0}$ is obviously \emph{weak*-continuous} in the sense that 
%% --
\[
\lim_{t \to s} \langle f,(T(t)'-T(s)')\phi \rangle = 0
\]
%% --
for every $f \in E$, $\phi \in E'$ and $s$, $t \geq 0$, it is natural to associate $(T(t)')_{t \geq 0}$ its a \emph{weak*-generator}
%% --
\begin{align*}
...
(\citet[Theorem~III.5.30]{kato:1966}).
...
\index{Basic Results!Standard Constructions!Associated Sobolev Semigroups}
%% --
...
\end{remarks*}
%% --
\subsection{The \texorpdfstring{$\mathcal{F}$}{F}-Product Semigroup}\label{sec:a1-3.6}
\index{Basic Results!Standard Constructions!$\mathcal{F}$-Product!Semigroup}
%% --
...
\]
%% --
We can apply Subsection~\ref{subsec:a1-3.3} in order to define the \emph{$\mathcal{F}$-product semigroup} $(T_{\mathcal{F}}(t))_{t \geq 0}$ on $E_{\mathcal{F}}^{T}$ by
%% --
\[
...
%% --
\subsection{The Tensor Product Semigroup}\label{sec:a1-3.7}
\index{Basic Results!Standard Constructions!Tensor Product Semigroup}
...
\end{proof}
%% --
\subsection{The Product of Commuting Semigroups}\label{sec:a1-3.8}
\index{Basic Results!Standard Constructions!Commuting Semigroups}
%% --
