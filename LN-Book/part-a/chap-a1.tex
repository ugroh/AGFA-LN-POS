...
\item[$ D^{2} = 0 $: ] 
Since $\left( \lambda - s/2 \right)^{k} = D^{k} $  for all $ (k \geq 2)$ this implies 
%
\[
...
$ e^{-tD} $ simplifying our formula (see below).
...
%
\[
	\sinh(\im z) = \frac{1}{2}( e^{\im z} - e^{-\im z}) = \im \sin(z)
	\quad \text{and} \quad
	\cosh(\im z) = \frac{1}{2}( e^{ \im z} + e^{- \im z}) = \cos(z). 
\]
%
...
\[
  \begin{array}{lllll}
    A= & \ABC & \implies & e^{tA}= & \BBC \\\\
    A= & \CBC & \implies & e^{tA}= & \DBC \\\\ 
    A= & \EBC & \implies & e^{tA}= & \FBC  
  \end{array}
\]
%% --
