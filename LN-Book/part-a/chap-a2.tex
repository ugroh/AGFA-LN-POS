...
%% KGK 2025-05-13 Korrekturen von Rainer und Wolfgang eingeaarbeitet
%% KGK 2025-05-12 Gesperrt. entlden für Überarbeitung
%% Stand 2025-05-08 
%% --
...
%% --
\begin{enumerate}[\upshape (a), parsep=0.0em]
...
Hence by
\marginpar{!!!! Im Buch 1975 ???} \citet[10.2.1]{hillephillips:1957} $\sup_{t \in J}\|T(t)\| < \infty$ for every compact interval $J \subset (0,\infty)$.
...
\marginpar{Im Buch Pazy 1982 in Bib 1983 ?????}
...
Let $\alpha \in (0,\pi/2]$.
A strongly continuous semigroup $(T(t))_{t \geq 0}$ is called a \emph{bounded holomorphic semigroup of angle $\alpha$} if $T(\cdot)$ is the restriction of a holomorphic function
%% –
\[
...
%%
\RaggedRight
and for each $\alpha_{1} \in (0,\alpha)$ the set $\{T(z) \colon z \in S(\alpha_{1})\}$ 
is uniformly bounded and  $\lim_{n \to \infty} T(z_{n})f = f $
for every null-sequence $(z_{n}) \in S(\alpha_{1}) $ and every $ f \in E$.
\end{minipage}     
\end{equation}
\end{definition}
%% 
...
\marginpar{Blau wenn referenziert, Später da Chap.  ext}
...
\index{Semigroups!Contraction Semigroups}
\index{Operators!Dissipative Operators}
%% --
\hspace{1cm}{\Large by Wolfgang Arendt}
\vspace{.5cm}
\newline
...
\begin{description}[style=standard, wide]
...
%%--
\item[\ref{thm:a2-3.6-6}] In \citet{bourgain:1982} it is shown, that $H^{\infty}(D)$ is a Grothendieck space, where $D$ is the open unit disc $\{z \colon |z| < 1\}$, and in \citet{bourgain:1984}, that this Banach space has the Dunford-Pettis property (see also the summary of Bouurgain in \citet{blei:1983}).
...
\begin{enumerate}[label=\emph{Section \arabic*:}, wide, itemsep=1ex]
	\marginpar{":" Uli fragen wegen abgleich}
%% --
\item
