...
This was shown by \mycite{zbmath00537321}.
An alternative approach to this can be found in \mycite{zbmath00125315}.

In contrast to this one can easily see that \eg $\LH$ with $H$ infinite dimensional does not have the Dunford-Pettis property, using example D-II-1.1.
\mycite{zbmath00125315} showed that a \WA-algebra $M$ has the Dunford-Pettis property if and only if it is of finite type I and has a representation 
$M = \oplus_{j} ( \L{H_{j}} \overline{\otimes} M_{j} )$ such that $\sup_{j} \dim H_{j} < \infty$ (see also \mycite[Thm. V.1.27]{zbmath01692441} for more details about this representation).
...
Surprisingly, if every strongly continuous $C_{0}$-semigroup on a \CA-algebra $M$ has a bounded generator, then $M$ is a Grothendieck space, which is shown for the commutative case in \mycite{zbmath00097715}.
...
Note that on $\L{H}$ with $H$ an infinite-dimensional Hilbert space, there always exist strongly but not uniformly continuous $C_{0}$-semigroups (see the example in D-II-1.1).
However, this \WA-algebra is a Grothendieck space. 

In contrast to all of the above, a strongly continuous $C_{0}$-semigroup of completely positive operators on a \WA-algebra is norm-continuous and thus has a bounded generator.
To prove this, one can use a result of \mycite{zbmath01495754}: for sequences of completely positive maps, strong and norm convergence to the identity operator are equivalent on \WA-algebras (even on the larger class of \AW-algebras).
