...
But on $\BH$, $H$ an infinite-dimensional Hilbert space, there always exist strongly but not uniformly continuous $C_{0}$-semigroups (see the example in D-II-1.1).

In contrast to this, one can easily see that \eg $\BH$ with $H$ infinite-dimensional does not have the Dunford-Pettis property, using example D-II-1.1, since the Hilbert space $H$ is a direct factor of $\LH$.

To understand what happens one needs the representation of finite type I \WA-algebras.
By \mycite[Thm. V.1.27]{zbmath01692441} such a \WA-algebra has a representation $\oplus_{j} ( \L{H_{j}} \overline{\otimes} M_{j} )$, where the $M_{j}$ are commutative \WA-algebras.
...
This follows from \mycite{zbmath01495754}: for sequences of completely positive maps, strong and norm convergence to the identity operator is equivalent on \WA-algebras (even on the larger class of \AW-algebras).
...
\begin{enumerate}
\item 
Using \mycite{zbmath03883065} or \mycite{zbmath03736445}, one can derive properties such as $s(A) \in \sigma(A)$ or $s(A) = \omega_{0}$ from the theory of semigroups of positive operators on ordered Banach spaces since the positive cone of a \CA-algebra has the  required properties. 
