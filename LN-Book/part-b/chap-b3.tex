%% Stand 2025-05-094 ulgr
%% -- Index
%% -- Korrektur von 2.31 und 2.32 split weg!!
%% -- Noch nicht fertig
...
\begin{enumerate}[\upshape (a), wide]
\item
...
\begin{remarks}\label{rem:b3-2.15}
	...
	If   $T \in \L{(C(K))}$, $K$ compact, is positive, then it is weakly compact  
	if and only if its biadjoint $T''$ maps bounded
    Borel functions into $C(K)$ (see B-IV, Proposition~2.4).
		\end{minipage}
	\end{equation}
%% --
	\begin{equation}\label{eq:b3-2.32}
    \begin{minipage}{.75\textwidth}
		A positive operator   $T$ on $C_{0}(X)$ 
        which is dominated by a finite rank 
...
(a) $\implies$ (b): Suppose there are $0 < f \in E$, $0 < \phi \in E'$ such that $\langle T(t)f,\phi \rangle = 0$ for every $t \geq 0$.
